...
FmuComponent::FmuComponent(fmi2String instanceName, fmi2Type fmuType,
                           fmi2String fmuGUID, fmi2String fmuResourceLocation,
                           const fmi2CallbackFunctions *functions,
                           fmi2Boolean visible, fmi2Boolean loggingOn)
    : FmuChronoComponentBase(instanceName, fmuType, fmuGUID,
  ...
  auto x_t_funpair = MAKE_GETSET_PAIR(
      fmi2Real, { return this->sys.SearchBody("cart")->GetPosDt().x(); }, {});
  auto x_funpair = MAKE_GETSET_PAIR(
      fmi2Real, { return this->sys.SearchBody("cart")->GetPos().x(); }, {});

  auto theta_tt_funpair =
      MAKE_GETSET_PAIR(fmi2Real,
                       {
                         return std::dynamic_pointer_cast<ChLinkMateRevolute>(
                                    this->sys.SearchLink("pendulum_rev"))
                             ->GetRelativeAngleDt2();
                       },
                       {});
  ...
  AddFmuVariable(x_tt_funpair, "x_tt", FmuVariable::Type::Real, "m/s2",
                 "cart acceleration");
  AddFmuVariable(x_t_funpair, "x_t", FmuVariable::Type::Real, "m/s",
                 "cart velocity");
  AddFmuVariable(x_funpair, "x", FmuVariable::Type::Real, "m", "cart position");
  AddFmuVariable(theta_tt_funpair, "theta_tt", FmuVariable::Type::Real,
                 "rad/s2", "pendulum ang acceleration");
  AddFmuVariable(theta_t_funpair, "theta_t", FmuVariable::Type::Real, "rad/s",
                 "pendulum ang velocity");
  AddFmuVariable(theta_funpair, "theta", FmuVariable::Type::Real, "rad",
                 "pendulum angle");
  AddFmuVariable(&pendulum_length, "pendulum_length", FmuVariable::Type::Real,
                 "m", "pendulum length", FmuVariable::CausalityType::parameter,
                 FmuVariable::VariabilityType::fixed);
  AddFmuVariable(&cart_mass, "cart_mass", FmuVariable::Type::Real, "kg",
                 "pendulum mass", FmuVariable::CausalityType::parameter,
                 FmuVariable::VariabilityType::fixed);
  AddFmuVariable(&pendulum_mass, "pendulum_mass", FmuVariable::Type::Real, "kg",
                 "cart mass", FmuVariable::CausalityType::parameter,
                 FmuVariable::VariabilityType::fixed);
...
void FmuComponent::_exitInitializationMode() {
  ...
  auto cart =
      chrono_types::make_shared<ChBodyEasyBox>(0.2, 0.1, 0.1, 750, true, false);
  cart->SetName("cart");
  cart->SetPos(VNULL);
  cart->SetRot(QUNIT);
  ...
  auto pendulum = chrono_types::make_shared<ChBodyEasyBox>(
      0.025, pendulum_length, 0.01, 750, true, false);
...
fmi2Status FmuComponent::_doStep(fmi2Real currentCommunicationPoint,
  ...
  while (m_time < currentCommunicationPoint + communicationStepSize) {
    ...
    sendToLog("Step at time: " + std::to_string(m_time) + " with timestep: " +
                  std::to_string(step_size) + "ms succeeded.\n",
