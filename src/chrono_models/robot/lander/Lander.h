...
/// @addtogroup robot_models_lander
/// @{
...
class CH_MODELS_API Lander {
  public:
    ...
    Lander(ChSystem* system,  ///< containing system
           std::shared_ptr<ChContactMaterial> contact_material  ///< contact material for all lander parts
    );
    ~Lander();
    ...
    void Initialize(const ChFrame<>& pos,  ///< position and orientation of the lander
    ...
    void SetInitialVelocity(const ChVector3d& velocity);
    ...
    /// Set visual colors.
    void SetBodyColor(const ChColor& color) { m_body_color = color; }
    void SetLegColor(const ChColor& color) { m_leg_color = color; }
    void SetFootpadColor(const ChColor& color) { m_footpad_color = color; }
    ...
    std::shared_ptr<ChBody> GetBody() const { return m_lander_body; }

    /// Get the specified leg body.
    std::shared_ptr<ChBody> GetLeg(int index) const {
        if (index >= 0 && index < 4) {
            return m_legs[index];
        }
        return nullptr;
    ...
    /// Get all leg bodies.
    std::array<std::shared_ptr<ChBody>, 4> GetLegs() const { return m_legs; }

    /// Get the specified footpad body (if footpads are enabled).
    std::shared_ptr<ChBody> GetFootpad(int index) const {
    ...
    std::array<std::shared_ptr<ChBody>, 4> GetFootpads() const { return m_footpads; }

    /// Get all leg joints.
    std::array<std::shared_ptr<ChLinkLockLock>, 4> GetLegJoints() const { return m_leg_joints; }

    /// Get the specified footpad joint (if footpads are enabled).
    std::shared_ptr<ChLinkLock> GetFootpadJoint(int index) const {
        if (m_use_footpads && index >= 0 && index < 4) {
    ...
    std::array<std::shared_ptr<ChLinkLock>, 4> GetFootpadJoints() const { return m_footpad_joints; }
  ...
  private:
    ...
    std::shared_ptr<ChContactMaterial> m_contact_material;  ///< contact material

    bool m_initialized;  ///< flag indicating whether the lander was initialized

    // Lander components
    std::shared_ptr<ChBody> m_lander_body;  ///< main lander body (cylinder)
    std::array<std::shared_ptr<ChBody>, 4> m_legs;  ///< leg bodies
    std::array<std::shared_ptr<ChBody>, 4> m_footpads;  ///< footpad bodies (if enabled)

    // Joints
    std::array<std::shared_ptr<ChLinkLockLock>, 4> m_leg_joints;  ///< joints connecting legs to body
    std::array<std::shared_ptr<ChLinkLock>, 4> m_footpad_joints;  ///< joints connecting footpads to legs
    ...
    // Visual colors
    ChColor m_body_color;     ///< body visual color
    ChColor m_leg_color;      ///< leg visual color
    ChColor m_footpad_color;  ///< footpad visual color
};
