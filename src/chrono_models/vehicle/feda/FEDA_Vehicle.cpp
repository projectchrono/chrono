...
void FEDA_Vehicle::Initialize(const ChCoordsys<>& chassisPos, double chassisFwdVel) {
    ...
    // frame).
    ChVector3d offset = ChVector3d(0.0, 0, 0.0);
    ChQuaternion<> rotation = ChQuaternion<>(1, 0, 0, 0);
    m_steerings[0]->Initialize(m_chassis, offset, rotation);

    // Initialize the axle subsystems.
    m_axles[0]->Initialize(m_chassis, nullptr, m_steerings[0], ChVector3d(0, 0, 0), ChVector3d(0.3, 0, 0.05), 0.0,
                           m_omega[0], m_omega[1]);
    m_axles[1]->Initialize(m_chassis, nullptr, nullptr, ChVector3d(-3.302, 0, 0), ChVector3d(0), 0.0, m_omega[2],
                           m_omega[3]);
