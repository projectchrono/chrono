...
// =============================================================================
//
// Wrapper classes for modeling an entire SedanL vehicle assembly
...
#include "chrono_models/vehicle/sedan/SedanL.h"
...
namespace vehicle
{
    namespace sedan
    {
        ...
        // -----------------------------------------------------------------------------
        void SedanL::SetAerodynamicDrag(double Cd, double area, double air_density)
        {
            m_Cd = Cd;
            m_area = area;
        ...
        // -----------------------------------------------------------------------------
        void SedanL::Initialize()
        {
            // Create and initialize the SedanL vehicle
            m_vehicle = m_system ? new SedanL_Vehicle(m_system, m_fixed, m_chassisCollisionType) :
                                   new SedanL_Vehicle(m_fixed, m_contactMethod, m_chassisCollisionType);
        ...
        // -----------------------------------------------------------------------------
        void SedanL::SetTireVisualizationType(VisualizationType vis)
        {
        ...
        // -----------------------------------------------------------------------------
        void SedanL::Synchronize(double time,
        ...
        // -----------------------------------------------------------------------------
        void SedanL::Advance(double step)
        {
        ...
        void SedanL::SetLockingTorqueLimit(double torqueLimit)
        {
            std::static_pointer_cast<ChShaftsDriveline2WDL>(m_vehicle->GetDriveline())
                ->SetLockingTorqueLimit(fabs(torqueLimit));
        }

        void SedanL::LockDiffBox(bool lock)
        {
            std::static_pointer_cast<ChShaftsDriveline2WDL>(m_vehicle->GetDriveline())->LockDifferential(lock);
        }

        // -----------------------------------------------------------------------------
        double SedanL::GetTotalMass() const
        {
