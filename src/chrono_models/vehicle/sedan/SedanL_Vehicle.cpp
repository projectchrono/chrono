...
#include "chrono_models/vehicle/sedan/SedanL_Vehicle.h"
...
namespace vehicle
{
    namespace sedan
    {
        ...
        // -----------------------------------------------------------------------------
        // -----------------------------------------------------------------------------
        SedanL_Vehicle::SedanL_Vehicle(const bool fixed,
            ChMaterialSurface::ContactMethod contact_method,
            ChassisCollisionType chassis_collision_type)
            : ChWheeledVehicle("Sedan", contact_method)
            , m_omega({ 0, 0, 0, 0 })
        {
            Create(fixed, chassis_collision_type);
        }

        SedanL_Vehicle::SedanL_Vehicle(ChSystem* system, const bool fixed, ChassisCollisionType chassis_collision_type)
            : ChWheeledVehicle("Sedan", system)
            , m_omega({ 0, 0, 0, 0 })
        {
            Create(fixed, chassis_collision_type);
        }

        void SedanL_Vehicle::Create(bool fixed, ChassisCollisionType chassis_collision_type)
        {
            // -------------------------------------------
            // Create the chassis subsystem
            // -------------------------------------------
            m_chassis = std::make_shared<Sedan_Chassis>("Chassis", fixed, chassis_collision_type);
            ...
            // --------------------
            m_driveline = std::make_shared<Sedan_Driveline2WDL>("Driveline");

            // -----------------
            // Create the brakes
            // -----------------
            m_brakes.resize(4);
            m_brakes[0] = std::make_shared<Sedan_BrakeSimple>("Brake_FL");
            m_brakes[1] = std::make_shared<Sedan_BrakeSimple>("Brake_FR");
            m_brakes[2] = std::make_shared<Sedan_BrakeSimple>("Brake_RL");
            m_brakes[3] = std::make_shared<Sedan_BrakeSimple>("Brake_RR");
        }

        SedanL_Vehicle::~SedanL_Vehicle()
        {
        }

        // -----------------------------------------------------------------------------
        // -----------------------------------------------------------------------------
        void SedanL_Vehicle::Initialize(const ChCoordsys<>& chassisPos, double chassisFwdVel)
        {
            ...
            m_suspensions[0]->Initialize(m_chassis->GetBody(), ChVector<>(1.388, 0, .25),
                m_steerings[0]->GetSteeringLink(), 0, m_omega[0], m_omega[1]);
            m_suspensions[1]->Initialize(
                m_chassis->GetBody(), ChVector<>(-1.388, 0, .125), m_chassis->GetBody(), -1, m_omega[2], m_omega[3]);
            ...
            std::static_pointer_cast<ChShaftsDriveline2WDL>(m_driveline)->SetLockingTorqueLimit(0);
            std::static_pointer_cast<ChShaftsDriveline2WDL>(m_driveline)->LockDifferential(false);
        ...
        // -----------------------------------------------------------------------------
        // -----------------------------------------------------------------------------
        double SedanL_Vehicle::GetSpringForce(const WheelID& wheel_id) const
        {
            return std::static_pointer_cast<ChDoubleWishbone>(m_suspensions[wheel_id.axle()])
                ->GetSpringForce(wheel_id.side());
        }

        double SedanL_Vehicle::GetSpringLength(const WheelID& wheel_id) const
        {
            return std::static_pointer_cast<ChDoubleWishbone>(m_suspensions[wheel_id.axle()])
                ->GetSpringLength(wheel_id.side());
        }

        double SedanL_Vehicle::GetSpringDeformation(const WheelID& wheel_id) const
        {
            return std::static_pointer_cast<ChDoubleWishbone>(m_suspensions[wheel_id.axle()])
        ...
        // -----------------------------------------------------------------------------
        // -----------------------------------------------------------------------------
        double SedanL_Vehicle::GetShockForce(const WheelID& wheel_id) const
        {
            return std::static_pointer_cast<ChDoubleWishbone>(m_suspensions[wheel_id.axle()])
                ->GetShockForce(wheel_id.side());
        }

        double SedanL_Vehicle::GetShockLength(const WheelID& wheel_id) const
        {
            return std::static_pointer_cast<ChDoubleWishbone>(m_suspensions[wheel_id.axle()])
                ->GetShockLength(wheel_id.side());
        }

        double SedanL_Vehicle::GetShockVelocity(const WheelID& wheel_id) const
        {
            return std::static_pointer_cast<ChDoubleWishbone>(m_suspensions[wheel_id.axle()])
        ...
        // -----------------------------------------------------------------------------
        void SedanL_Vehicle::LogHardpointLocations()
        {
        ...
        // -----------------------------------------------------------------------------
        void SedanL_Vehicle::DebugLog(int what)
        {
            ...
            if(what & OUT_SPRINGS) {
                ...
                GetLog() << "Deformation [m]  " << GetSpringDeformation(FRONT_LEFT) << "  "
