...
M113a_TrackAssemblySinglePin::M113a_TrackAssemblySinglePin(VehicleSide side) : ChTrackAssemblySinglePin("", side) {
    ...
    switch (side) {
        case LEFT:
            ...
            m_idler = chrono_types::make_shared<M113a_IdlerLeft>();
            m_brake = chrono_types::make_shared<M113a_BrakeSimple>("M113_BrakeLeft");
            m_sprocket = chrono_types::make_shared<M113a_SprocketSinglePinLeft>();
            num_shoes = 63;
            suspName += "Left_";
            shoeName += "Left_";
            break;
        case RIGHT:
            ...
            m_idler = chrono_types::make_shared<M113a_IdlerRight>();
            m_brake = chrono_types::make_shared<M113a_BrakeSimple>("M113_BrakeRight");
            m_sprocket = chrono_types::make_shared<M113a_SprocketSinglePinRight>();
            num_shoes = 64;
            suspName += "Right_";
            shoeName += "Right_";
            break;
    ...
    m_suspensions[0] = chrono_types::make_shared<M113a_Suspension>(suspName + "0", side, 0, true);
    m_suspensions[1] = chrono_types::make_shared<M113a_Suspension>(suspName + "1", side, 1, false);
    m_suspensions[2] = chrono_types::make_shared<M113a_Suspension>(suspName + "2", side, 2, false);
    m_suspensions[3] = chrono_types::make_shared<M113a_Suspension>(suspName + "3", side, 3, false);
    m_suspensions[4] = chrono_types::make_shared<M113a_Suspension>(suspName + "4", side, 4, true);

    for (size_t it = 0; it < num_shoes; it++) {
        m_shoes.push_back(chrono_types::make_shared<M113a_TrackShoeSinglePin>(shoeName + std::to_string(it)));
    }
}
