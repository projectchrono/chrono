...
const double Generic_PitmanArm::m_maxAngle = 30.0 * (CH_PI / 180);

const ChVector3d Generic_PitmanArm::m_steeringLinkInertiaMoments(0.252, 0.00233, 0.254);
const ChVector3d Generic_PitmanArm::m_steeringLinkInertiaProducts(0.0, 0.0, 0.0);
const ChVector3d Generic_PitmanArm::m_pitmanArmInertiaMoments(0.00638, 0.00756, 0.00150);
const ChVector3d Generic_PitmanArm::m_pitmanArmInertiaProducts(0.0, 0.0, 0.0);

// -----------------------------------------------------------------------------

Generic_PitmanArm::Generic_PitmanArm(const std::string& name) : ChPitmanArm(name) {}

const ChVector3d Generic_PitmanArm::getLocation(PointId which) {
    switch (which) {
        case STEERINGLINK:
...
const ChVector3d Generic_PitmanArm::getDirection(DirectionId which) {
    switch (which) {
        case REV_AXIS:
