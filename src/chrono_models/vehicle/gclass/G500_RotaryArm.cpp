...
const double G500_RotaryArm::m_maxAngle = 12.5 * (CH_C_PI / 180);

const ChVector<> G500_RotaryArm::m_pitmanArmInertiaMoments(0.00638, 0.00756, 0.00150);
const ChVector<> G500_RotaryArm::m_pitmanArmInertiaProducts(0.0, 0.0, 0.0);
...
// -----------------------------------------------------------------------------
const ChVector<> G500_RotaryArm::getLocation(PointId which) {
    switch (which) {
        case ARM_L:
...
const ChVector<> G500_RotaryArm::getDirection(DirectionId which) {
    switch (which) {
        case REV_AXIS:
