...
ChContactNode::ChContactNode(collision::ChCollisionModel* mmodA,  ///< model A (body)
                             ...
                             float* mreaction_cache,  ///< pass the pointer to array of N,U,V reactions: a cache in
                             /// contact manifold. If not available=0.
...
void ChContactNode::ContIntStateGatherReactions(const unsigned int off_L, ChVectorDynamic<>& L) {
    L(off_L) = react_force.x;
    L(off_L + 1) = react_force.y;
    L(off_L + 2) = react_force.z;
}

void ChContactNode::ContIntStateScatterReactions(const unsigned int off_L, const ChVectorDynamic<>& L) {
    react_force.x = L(off_L);
    react_force.y = L(off_L + 1);
    react_force.z = L(off_L + 2);
}

void ChContactNode::ContIntLoadResidual_CqL(const unsigned int off_L,    ///< offset in L multipliers
...
void ChContactNode::ContIntLoadConstraint_C(const unsigned int off_L,  ///< offset in Qc residual
...
void ChContactNode::ContIntToLCP(const unsigned int off_L,  ///< offset in L, Qc
...
void ChContactNode::ContIntFromLCP(const unsigned int off_L,  ///< offset in L
                                   ChVectorDynamic<>& L) {
    L(off_L) = Nx.Get_l_i();
