...
int ChIntegrator::ODEintegrate_step(
    ...
    int getdy(ChMatrix<>* m_Y,
              double m_t,
              ChMatrix<>* m_dYdt,
              void* m_userdata),  // function which computes dy/dt=f(y,t), returning TRUE if OK, FALSE if error
    ...
    void* userdata)               // additional user data
{
    ...
    double a2 = 0.2, a3 = 0.3, a4 = 0.6, a5 = 1.0, a6 = 0.875, b21 = 0.2, b31 = 3.0 / 40.0, b32 = 9.0 / 40.0, b41 = 0.3,
           b42 = -0.9, b43 = 1.2, b51 = -11.0 / 54.0, b52 = 2.5, b53 = -70.0 / 27.0, b54 = 35.0 / 27.0,
    ...
    switch (this->method) {
        ...
        case INTEG_EULEROMOD_EXP:
            ...
            for (i = 0; i < nequations; i++)
                Ytemp.SetElement(i, 0, Y->GetElement(i, 0) + h * 0.5 * Ydt.GetElement(i, 0));

            m_ok = (*getdy)(&Ytemp, t + h / 2, &Yk2, userdata);  // >>>>>>>>>>>>>>>>>>>>>>> Yk2

            // 3- Ynew = Y + (Yk2 * dt)
            for (i = 0; i < nequations; i++)
                Ynew.SetElement(i, 0, Y->GetElement(i, 0) + h * Yk2.GetElement(i, 0));
        ...
        case INTEG_HEUN_EXP:
            ...
            for (i = 0; i < nequations; i++)
                Ytemp.SetElement(i, 0, Y->GetElement(i, 0) + h * Yk1.GetElement(i, 0));
            ...
            for (i = 0; i < nequations; i++)
                Ynew.SetElement(i, 0,
                                Y->GetElement(i, 0) + h * 0.5 * Yk1.GetElement(i, 0) + h * 0.5 * Yk2.GetElement(i, 0));
        ...
        case INTEG_KUTTA_EXP:
            ...
            for (i = 0; i < nequations; i++)
                Ytemp.SetElement(i, 0, Y->GetElement(i, 0) + b21 * h * Yk1.GetElement(i, 0));
            ...
            for (i = 0; i < nequations; i++)
                Ytemp.SetElement(i, 0,
                                 Y->GetElement(i, 0) + h * (b31 * Yk1.GetElement(i, 0) + b32 * Yk2.GetElement(i, 0)));

            m_ok = (*getdy)(&Ytemp, t + a3 * h, &Yk3, userdata);  // >>>>>>>>>>>>>>>>>>>>>>> Yk3

            // 4-- dY/dt   at time: t0 +a4*h
            for (i = 0; i < nequations; i++)
                Ytemp.SetElement(i, 0, Y->GetElement(i, 0) +
                                           h * (b41 * Yk1.GetElement(i, 0) + b42 * Yk2.GetElement(i, 0) +
            ...
            for (i = 0; i < nequations; i++)
                Ytemp.SetElement(i, 0, Y->GetElement(i, 0) +
                                           h * (b51 * Yk1.GetElement(i, 0) + b52 * Yk2.GetElement(i, 0) +
                                                b53 * Yk3.GetElement(i, 0) + b54 * Yk4.GetElement(i, 0)));
            ...
            for (i = 0; i < nequations; i++)
                Ytemp.SetElement(i, 0, Y->GetElement(i, 0) +
                                           h * (b61 * Yk1.GetElement(i, 0) + b62 * Yk2.GetElement(i, 0) +
                                                b63 * Yk3.GetElement(i, 0) + b64 * Yk4.GetElement(i, 0) +
            ...
            for (i = 0; i < nequations; i++)
                Ynew.SetElement(i, 0, Y->GetElement(i, 0) +
                                          h * (c1 * Yk1.GetElement(i, 0) + c3 * Yk3.GetElement(i, 0) +
                                               c4 * Yk4.GetElement(i, 0) + c6 * Yk6.GetElement(i, 0)));

            // -- compute local error
            for (i = 0; i < nequations; i++)
                int_error += ChMax(int_error, fabs(h * (dc1 * Yk1.GetElement(i, 0) + dc3 * Yk3.GetElement(i, 0) +
                                                        dc4 * Yk4.GetElement(i, 0) + dc5 * Yk5.GetElement(i, 0) +
        ...
        case INTEG_RK_EXP:
            ...
            for (i = 0; i < nequations; i++)
                Ytemp.SetElement(i, 0, Y->GetElement(i, 0) + 0.5 * h * Yk1.GetElement(i, 0));

            m_ok = (*getdy)(&Ytemp, t + h / 2, &Yk2, userdata);  // >>>>>>>>>>>>>>>>>>>>>>> Yk2

            // 3-- dY/dt   at time: t0 + h/2
            for (i = 0; i < nequations; i++)
                Ytemp.SetElement(i, 0, Y->GetElement(i, 0) + 0.5 * h * Yk2.GetElement(i, 0));

            m_ok = (*getdy)(&Ytemp, t + h / 2, &Yk3, userdata);  // >>>>>>>>>>>>>>>>>>>>>>> Yk3

            // 4-- dY/dt   at time: t0 + h
            for (i = 0; i < nequations; i++)
                Ytemp.SetElement(i, 0, Y->GetElement(i, 0) + h * Yk3.GetElement(i, 0));
            ...
            for (i = 0; i < nequations; i++)
                Ynew.SetElement(i, 0,
                                Y->GetElement(i, 0) +
                                    h * ((1.0 / 6.0) * Yk1.GetElement(i, 0) + (1.0 / 3.0) * Yk2.GetElement(i, 0) +
                                         (1.0 / 3.0) * Yk3.GetElement(i, 0) + (1.0 / 6.0) * Yk4.GetElement(i, 0)));
