...
void ChContactContainerNodes::AddContact(const collision::ChCollisionInfo& mcontact) {
    ...
    if (lastcontact != contactlist.end()) {
        // reuse old contacts
        (*lastcontact)
            ->Reset(mmboA, mmnoB, varA, varB, frameA, posB, mvpA, mvpB, mvN, mcontact.distance, mcontact.reaction_cache,
                    mat.static_friction);
        lastcontact++;
    } else {
        // add new contact
        ChContactNode* mc = new ChContactNode(mmboA, mmnoB, varA, varB, frameA, posB, mvpA, mvpB, mvN,
                                              mcontact.distance, mcontact.reaction_cache, mat.static_friction);
...
void ChContactContainerNodes::ReportAllContacts(ChReportContactCallback* mcallback) {
    ...
    while (itercontact != contactlist.end()) {
        bool proceed = mcallback->ReportContactCallback(
            ...
            (*itercontact)->GetContactDistance(), (*itercontact)->GetFriction(), (*itercontact)->GetContactForce(),
...
void ChContactContainerNodes::IntLoadResidual_CqL(const unsigned int off_L,    ///< offset in L multipliers
...
void ChContactContainerNodes::IntLoadConstraint_C(const unsigned int off,  ///< offset in Qc residual
...
void ChContactContainerNodes::IntToLCP(const unsigned int off_v,  ///< offset in v, R
...
void ChContactContainerNodes::IntFromLCP(const unsigned int off_v,  ///< offset in v
                                         ChStateDelta& v,
