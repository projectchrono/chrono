...
void ChContactContainer::AddContact(const collision::ChCollisionInfo& mcontact) {
    ...
    if ((mat.rolling_friction == 0) && (mat.spinning_friction == 0)) {
        if (lastcontact != contactlist.end()) {
            // reuse old contacts
            (*lastcontact)
                ->Reset(mcontact.modelA, mcontact.modelB, varA, varB, frameA, frameB, mcontact.vpA, mcontact.vpB,
                        mcontact.vN, mcontact.distance, mcontact.reaction_cache, mat);
            lastcontact++;
        } else {
            // add new contact
            ChContact* mc = new ChContact(mcontact.modelA, mcontact.modelB, varA, varB, frameA, frameB, mcontact.vpA,
                                          mcontact.vpB, mcontact.vN, mcontact.distance, mcontact.reaction_cache, mat);
        ...
        if (lastcontact_roll != contactlist_roll.end()) {
            ...
            // add new contact
            ChContactRolling* mc =
                new ChContactRolling(mcontact.modelA, mcontact.modelB, varA, varB, frameA, frameB, mcontact.vpA,
                                     mcontact.vpB, mcontact.vN, mcontact.distance, mcontact.reaction_cache, mat);
...
void ChContactContainer::ReportAllContacts(ChReportContactCallback* mcallback) {
    ...
    while (itercontact != contactlist.end()) {
        bool proceed =
            mcallback->ReportContactCallback((*itercontact)->GetContactP1(), (*itercontact)->GetContactP2(),
                                             *(*itercontact)->GetContactPlane(), (*itercontact)->GetContactDistance(),
                                             (float)(*itercontact)->GetFriction(), (*itercontact)->GetContactForce(),
                                             VNULL,  // no react torques
                                             (*itercontact)->GetModelA(), (*itercontact)->GetModelB());
        if (!proceed)
            break;
        ++itercontact;
    }
    ...
    while (itercontact_roll != contactlist_roll.end()) {
        bool proceed = mcallback->ReportContactCallback(
            (*itercontact_roll)->GetContactP1(), (*itercontact_roll)->GetContactP2(),
            *(*itercontact_roll)->GetContactPlane(), (*itercontact_roll)->GetContactDistance(),
            (float)(*itercontact_roll)->GetFriction(), (*itercontact_roll)->GetContactForce(),
            (*itercontact_roll)->GetContactTorque(), (*itercontact_roll)->GetModelA(),
