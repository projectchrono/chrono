...
ChFile_ps::ChFile_ps(char m_name[],
					 double x,
					 double y,
					 double w,
					 double h,
...
{
	if ((mfrom.x > 1.e+20)||(mfrom.x < -1.e+20)) mfrom.x = 0;
	if ((mfrom.y > 1.e+20)||(mfrom.y < -1.e+20)) mfrom.y = 0;
...
void ChFile_ps::DrawRectangle(ChPageVect mfrom, ChPageVect mwh, int space, int filled)
{
	...
	mp1.x = mfrom.x + mwh.x;	mp1.y= mfrom.y;
	mp2.x = mfrom.x + mwh.x;	mp2.y= mfrom.y + mwh.y;
	mp3.x = mfrom.x;			mp3.y= mfrom.y + mwh.y;
	GrSave();
	StartLine();
	MoveTo		(TransPt(mfrom, space));
...
void ChFile_ps::DrawGraphAxes(ChFile_ps_graph_setting* msetting)
{
	...
	cpt.x = G_p.x + Gs_p.x; cpt.y = G_p.y + Gs_p.y;
	...
	{
		if ((upri_g.x-lole_g.x)/ msetting->Xaxis.ticks_step > 200.0) break;
		cpt.x = w; cpt.y = 0;
		if (msetting->gridx)
		{
			cp1 = cpt;
			cp2 = cp1;
			cp1.y= lole_g.y; cp2.y = upri_g.y;
		...
		if (msetting->Xaxis.numbers)
		{
			...
			cp1 = To_page_from_graph(cpt);
			cp1.y = G_p.y + 0.16; cp1.x = cp1.x + 0.1;
			double tw = w; if (fabs(w)< 1.e-15) tw = 0;
	...
	{
		if ((upri_g.y-lole_g.y)/msetting->Yaxis.ticks_step > 200.0) break;
		cpt.x = 0; cpt.y = h;
		if (msetting->gridy)
		{
			cp1 = cpt;
			cp2 = cp1;
			cp1.x= lole_g.x; cp2.x = upri_g.x;
			cp1 = To_page_from_graph(cp1);
			cp2 = To_page_from_graph(cp2);
			SetWidth(msetting->grid_width);
			SetRGB(msetting->grid_color);
		...
		if (msetting->Yaxis.numbers)
		{
			...
			cp1 = To_page_from_graph(cpt);
			cp1.x = G_p.x + 0.16; cp1.y = cp1.y + 0.1;
			double th = h; if (fabs(th)< 1.e-15) th = 0;
...
void ChFile_ps::DrawGraphXY (ChMatrix<>* Yvalues, ChMatrix<>* Xvalues)
{
	...
	if (Xvalues->GetRows()<points) points = Xvalues->GetRows();
	if (!points) return;
...
void ChFile_ps::DrawGraphLabel(double dx, double dy, double fontsize, char* label, int dolinesample,
...
ChPageVect pv_set(double x, double y)
	{ChPageVect mpv; mpv.x = x; mpv.y = y; return mpv;}
ChPageVect pv_set(Vector mv)
	{ChPageVect mpv; mpv.x = mv.x; mpv.y = mv.y; return mpv;}
