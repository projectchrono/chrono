...
ChOgreGUIText::ChOgreGUIText(const ChVector2<>& Position, const ChVector2<>& Size, MyGUI::Gui* GUI) {
    m_pGUI = GUI;
...
void ChOgreGUIText::setPosition(const ChVector2<>& Position) {
    m_pTextBox->setRealPosition(Position.x, Position.y);
    //m_pTextBox->setDepth(int(Position.z)); //setDepth is not a function in mygui::button
}

void ChOgreGUIText::setSize(const ChVector2<>& Size) {
