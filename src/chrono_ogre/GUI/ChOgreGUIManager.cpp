...
namespace ChOgre {
	...
	ChOgreGUIManager::ChOgreGUIManager(Ogre::RenderWindow* RenderWindow, Ogre::SceneManager* SceneManager, ChOgre_SDLInputHandler* InputManager) {
		...
		m_keyboard.call = [](scancode_t s, keycode_t k, const ChOgreKeyState& ks) {
			if (ks.down) {
				MyGUI::InputManager::getInstance().injectKeyPress(MyGUI::KeyCode((MyGUI::KeyCode::Enum)s), SDL_GetKeyName(k)[0]);
