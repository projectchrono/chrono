    ...
    namespace vehicle {
        ...
        void ChTrackShoeRigidCB::AddWebContact() {
            ...
            for (size_t web = 0; web < GetNumWebSegments(); web++){
                ...
                m_web_segments[web]->GetCollisionModel()->AddBox(web_lengths[web] / 2, GetBeltWidth() / 2, GetWebThickness() / 2*10, web_location);
        ...
        void ChTrackShoeRigidCB::Connect(std::shared_ptr<ChTrackShoe> next) {
            ...
            if (m_index == 0) {
                ChVector<> loc = m_shoe->TransformPointLocalToParent(ChVector<>(GetToothBaseLength() / 2, -GetBushingDepth(), 0));
