...
ChTrackAssemblyDoublePin::ChTrackAssemblyDoublePin(const std::string& name, VehicleSide side)
...
bool ChTrackAssemblyDoublePin::Assemble(std::shared_ptr<ChBodyAuxRef> chassis) {
    ...
    while (sign * (p2.x() - idler_pos.x()) > shoe_pitch && index < num_shoes) {
        ...
        CreateTrackShoe(index, ps, pc, angle, angle);
    ...
    dy = (wheel_idler_pos.y() - wheel_radius) - (idler_pos.y() - idler_radius);
    dx = sign * (wheel_idler_pos.x() - idler_pos.x());
    angle = ccw ? std::atan2(dy, dx) : -std::atan2(dy, dx);
    ...
    double alpha = sign * std::atan2(p0.y() - p2.y(), sign * (p0.x() - p2.x()));
