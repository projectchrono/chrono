        ...
        bool ChTrackAssemblyRigidCB::Assemble(std::shared_ptr<ChBodyAuxRef> chassis) {
            ...
            for (size_t s = 0; s < num_shoes; s++){
                ...
                for (size_t i = 0; i < num_shoe_elements; i++) {
                    ChVector<> loc((ShoePoints(i + s * num_shoe_elements, 0) + ShoePoints(i + 1 + s * num_shoe_elements, 0)) / 2, 
                        m_sprocket_offset, 
                        (ShoePoints(i + s * num_shoe_elements, 1) + ShoePoints(i + 1 + s * num_shoe_elements, 1)) / 2);

                    double ang = std::atan2(ShoePoints(i + 1 + s * num_shoe_elements, 1) - ShoePoints(i + s * num_shoe_elements, 1), 
