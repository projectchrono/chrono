...
class SprocketDoublePinContactCB : public ChSystem::CustomCollisionCallback {
  ...
  private:
    ...
    void CheckCircleProfile(std::shared_ptr<ChBody> connector,                 // connector body
                            std::shared_ptr<ChContactMaterial> mat_connector,  // connector contact material
                            const ChVector3d& loc,                             // center of circular connector end
                            const ChVector3d& p1L,
                            const ChVector3d& p2L,
                            const ChVector3d& p3L,
                            const ChVector3d& p4L,
                            const ChVector3d& p1R,
                            const ChVector3d& p2R,
                            const ChVector3d& p3R,
                            const ChVector3d& p4R);
    ...
    void CheckCircleArc(std::shared_ptr<ChBody> connector,                 // connector body
                        ...
                        const ChVector3d& p1,                              // arc end point 1
                        const ChVector3d& p2                               // arc end point 2
    );
...
void SprocketDoublePinContactCB::CheckConnectorSprocket(std::shared_ptr<ChBody> connector,
    ...
    ChVector3d loc = m_sprocket->GetGearBody()->TransformPointParentToLocal(shape_frame.GetPos());
    ...
    ChVector3d p0L(-(m_gear_RT - m_gear_D) * m_sbeta, loc.y(), (m_gear_RT - m_gear_D) * m_cbeta);
    ChVector3d p0R(+(m_gear_RT - m_gear_D) * m_sbeta, loc.y(), (m_gear_RT - m_gear_D) * m_cbeta);

    ChVector3d p1L = p0L + ChVector3d(+m_gear_D * m_cbeta, 0, m_gear_D * m_sbeta);
    ChVector3d p1R = p0R + ChVector3d(-m_gear_D * m_cbeta, 0, m_gear_D * m_sbeta);

    ChVector3d p2L = ChVector3d(-m_gear_C * m_sbeta, loc.y(), m_gear_C * m_cbeta) +
                     ChVector3d(+m_gear_D * m_cbeta, 0, m_gear_D * m_sbeta);
    ChVector3d p2R = ChVector3d(+m_gear_C * m_sbeta, loc.y(), m_gear_C * m_cbeta) +
                     ChVector3d(-m_gear_D * m_cbeta, 0, m_gear_D * m_sbeta);

    ChVector3d p3L = p2L + ChVector3d(+m_gear_R * m_cbeta, 0, m_gear_R * m_sbeta);
    ChVector3d p3R = p2R + ChVector3d(-m_gear_R * m_cbeta, 0, m_gear_R * m_sbeta);

    ChVector3d p4L = p3L - ChVector3d(0, 0, m_gear_R);
    ChVector3d p4R = p3R - ChVector3d(0, 0, m_gear_R);

    ChMatrix33<> rot(alpha, ChVector3d(0, 1, 0));
    ...
    ChVector3d P1_abs = shape_frame.TransformPointLocalToParent(ChVector3d(m_shoe_len / 2, 0, 0));
    ChVector3d P2_abs = shape_frame.TransformPointLocalToParent(ChVector3d(-m_shoe_len / 2, 0, 0));
    ChVector3d P1 = m_sprocket->GetGearBody()->TransformPointParentToLocal(P1_abs);
    ChVector3d P2 = m_sprocket->GetGearBody()->TransformPointParentToLocal(P2_abs);
...
void SprocketDoublePinContactCB::CheckCircleArc(std::shared_ptr<ChBody> connector,                 // connector body
    ...
    ChVector3d a = p1 - ac;
    ChVector3d b = p2 - ac;
    ChVector3d axb = Vcross(a, b);
    ChVector3d axd = Vcross(a, delta);
    ChVector3d dxb = Vcross(delta, b);
    ...
    ChVector3d pt_gear = ac - m_gear_R * normal;
    ChVector3d pt_shoe = cc - m_shoe_R * normal;
...
void SprocketDoublePinContactCB::CheckCircleSegment(
    ...
    ChVector3d pt_gear = p1 + t * s;

    // No contact if circle center is too far from segment.
    ChVector3d delta = cc - pt_gear;
    double dist2 = delta.Length2();
    if (dist2 >= cr * cr)
        return;
    ...
    double dist = std::sqrt(dist2);
    ChVector3d normal = delta / dist;
    ChVector3d pt_shoe = cc - cr * normal;
...
// -----------------------------------------------------------------------------
std::shared_ptr<ChLinePath> ChSprocketDoublePin::GetProfile() const {
    ...
    for (int i = 0; i < num_teeth; ++i) {
        ChVector3d p0L(-(R_T - D) * sbeta, (R_T - D) * cbeta, 0);
        ChVector3d p0R(+(R_T - D) * sbeta, (R_T - D) * cbeta, 0);

        ChVector3d p1L = p0L + ChVector3d(+D * cbeta, D * sbeta, 0);
        ChVector3d p1R = p0R + ChVector3d(-D * cbeta, D * sbeta, 0);

        ChVector3d p2L = ChVector3d(-C * sbeta, C * cbeta, 0) + ChVector3d(+D * cbeta, D * sbeta, 0);
        ChVector3d p2R = ChVector3d(+C * sbeta, C * cbeta, 0) + ChVector3d(-D * cbeta, D * sbeta, 0);

        ChVector3d p3L = p2L + ChVector3d(+R * cbeta, R * sbeta, 0);
        ChVector3d p3R = p2R + ChVector3d(-R * cbeta, R * sbeta, 0);

        ChVector3d p4L = p3L - ChVector3d(0, R, 0);
        ChVector3d p4R = p3R - ChVector3d(0, R, 0);

        double alpha = -i * beta;
        ChMatrix33<> rot(alpha, ChVector3d(0, 0, 1));
