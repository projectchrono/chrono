...
class SprocketBandBushingContactCB : public ChSystem::CustomCollisionCallback {
  public:
    //// TODO Add in a collision envelope to the contact algorithm for NSC
    SprocketBandBushingContactCB(ChTrackAssembly* track,  ///< containing track assembly
        ...
        m_sprocket = std::dynamic_pointer_cast<ChSprocketBandBushing>(m_track->GetSprocket());
        auto shoe = std::dynamic_pointer_cast<ChTrackShoeBandBushing>(track->GetTrackShoe(0));
  ...
  private:
    ...
    // Test collision between a web segment body and the sprocket's gear profile
    void CheckWebSegmentSprocket(std::shared_ptr<ChBody> websegment,  // web segment body
                                 const ChVector<>& locS_abs,          // center of sprocket (global frame)
    ...
    std::shared_ptr<ChSprocketBandBushing> m_sprocket;  // handle to the sprocket
...
void SprocketBandBushingContactCB::OnCustomCollision(ChSystem* system) {
    ...
    if (m_update_tread) {
        ...
        auto shoe = std::dynamic_pointer_cast<ChTrackShoeBandBushing>(m_track->GetTrackShoe(0));
    ...
    for (size_t is = 0; is < m_track->GetNumTrackShoes(); ++is) {
        auto shoe = std::static_pointer_cast<ChTrackShoeBandBushing>(m_track->GetTrackShoe(is));
...
void SprocketBandBushingContactCB::CheckTreadSegmentSprocket(
    std::shared_ptr<ChBody> treadsegment,  // tread segment body
    const ChVector<>& locS_abs             // center of sprocket (global frame)
) {
...
void SprocketBandBushingContactCB::CheckTreadTipSprocketTip(std::shared_ptr<ChBody> treadsegment) {
...
void SprocketBandBushingContactCB::CheckTreadArcSprocketArc(std::shared_ptr<ChBody> treadsegment,
...
void SprocketBandBushingContactCB::CheckWebSegmentSprocket(std::shared_ptr<ChBody> websegment,
    ...
    // center of the gear (test performed in the sprocket's x-z plane).
    if (loc.x() * loc.x() + loc.z() * loc.z() > m_gear_web_broadphase_dist_squared)
        return;

    //(3) check for box to circle collision for the web
    ChVector<> xaxis = m_sprocket->GetGearBody()->TransformDirectionParentToLocal(websegment->GetRot().GetXaxis());
    ChVector<> zaxis = m_sprocket->GetGearBody()->TransformDirectionParentToLocal(websegment->GetRot().GetZaxis());

    ChVector<> p1 = loc + (web_segment_length / 2) * xaxis + (web_thickness / 2) * zaxis;
    ChVector<> p2 = loc - (web_segment_length / 2) * xaxis + (web_thickness / 2) * zaxis;
    p1.y() = 0;
    p2.y() = 0;
    CheckSegmentCircle(websegment, m_sprocket->GetOuterRadius(), p1, p2);
}
...
void SprocketBandBushingContactCB::CheckSegmentCircle(std::shared_ptr<ChBody> BeltSegment,  // connector body
                                                      double cr,                            // circle radius
                                                      const ChVector<>& p1,                 // segment end point 1
                                                      const ChVector<>& p2                  // segment end point 2
) {
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
ChSystem::CustomCollisionCallback* ChSprocketBandBushing::GetCollisionCallback(ChTrackAssembly* track) {
    ...
    return new SprocketBandBushingContactCB(track, 0.005, GetNumTeeth(), GetSeparation());
}

// -----------------------------------------------------------------------------
// Create and return the sprocket gear profile.
// -----------------------------------------------------------------------------
std::shared_ptr<geometry::ChLinePath> ChSprocketBandBushing::GetProfile() {
