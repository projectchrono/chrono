...
class CH_VEHICLE_API ChTrackedVehicle : public ChVehicle {
  public:
    ...
    /// By default, contact forces between wheels and track shoes are generated by the underlying Chrono contact
    /// processing. If enabled, no contact forces are applied automatically. Instead, collisions between wheels (idlers
    /// and/or road-wheels) and track shoes are cached and passed to the user-supplied callback which must compute the
    /// contact force for each individual collision.
    void EnableCustomContact(std::shared_ptr<ChTrackCustomContact> callback,  ///< user-supplied callback interface
                             bool idler_shoe = true,  ///< intercept collisions between idlers and track shoes?
                             bool wheel_shoe = true   ///< intercept collisions between road-wheels and track shoes?
    );
