...
std::shared_ptr<RigidTerrain::Patch> RigidTerrain::AddPatch(std::shared_ptr<ChMaterialSurface> material,
    ...
    // Check if Y-Up world orientation. In a Y-Up worldframe, the width is mapped across the z-axis and thickness along
    // the y-axis
    ChVector<> v_check = ChWorldFrame::Vertical();  // store upward world direction
    bool is_y_up = false;                           // boolean for whether a YUp worldframe is enabled. default false.
    if (v_check.Equals(ChVector<>(0, 1, 0))) {
        is_y_up = true;
    }  // Y-Up worldframe detected. Adjust Terrain Box Patches accordingly.
    ...
    if (tiled && !is_y_up) {
        ...
        auto ct_shape = chrono_types::make_shared<ChCollisionShapeBox>(material,  // collision material
                                                                       length,    // x
                                                                       (is_y_up ? thickness : width),   // y
                                                                       (is_y_up ? width : thickness));  // z
        ChVector<> loc(0, (is_y_up ? (-0.5 * thickness) : 0),
    ...
    patch->m_normal =
        (is_y_up ? position.rot.GetYaxis()
                 : position.rot.GetZaxis());  // Normal along the Y-axis for Y-up orientation, Z for a Z-Up
    ...
    patch->m_visual_offset = ChVector<>(0,
                                        (is_y_up ? (-0.5 * thickness) : 0),   // Offset in Y for Y-up world
                                        (is_y_up ? 0 : (-0.5 * thickness)));  // Offset in Z for Z-up world
    patch->m_dim = ChVector<>(length, (is_y_up ? thickness : width),
                              (is_y_up ? width : thickness));  // dimensions for visual asset, adjusted for worldframe
    return patch;
}
// -----------------------------------------------------------------------------
...
std::shared_ptr<RigidTerrain::Patch> RigidTerrain::AddPatch(std::shared_ptr<ChMaterialSurface> material,
                                                            const ChCoordsys<>& position,
                                                            const std::vector<ChVector<double>>& point_cloud,
                                                            double length,
                                                            double width,
    ...
    std::vector<std::vector<double>> max_height_map(heightmap_resolution, std::vector<double>(heightmap_resolution, std::numeric_limits<double>::lowest()));
    std::vector<std::vector<double>> min_height_map(heightmap_resolution, std::vector<double>(heightmap_resolution, std::numeric_limits<double>::max()));
    ...
    double vertical_flat_angle_threshold = 4;        // Dot product close to 0 to the vertical/horizontal world frame for nearly vertical triangle (for exclusion from refinement)
    double flat_threshold = std::cos(vertical_flat_angle_threshold * CH_C_DEG_TO_RAD);     // Convert angle thresholds to cos rad
    double vertical_threshold = std::cos((90 - vertical_flat_angle_threshold) * CH_C_DEG_TO_RAD);  // Convert angle thresholds to cos rad
    ChVector<> vertical_vector(0, 0, 1);  // vertical vector for Z-up world frame (mesh processing is done within a z-up world and rotated at the end)
        ...
        for (int dx = -1; dx <= 1; ++dx) {
            for (int dy = -1; dy <= 1; ++dy) {
                ...
                if (nx >= 0 && nx < heightmap_resolution && ny >= 0 && ny < heightmap_resolution) {
                    ...
                    double dist_sq = (cell_central_x - point.x()) * (cell_central_x - point.x()) + (cell_central_y - point.y()) * (cell_central_y - point.y());
                    // check the distance and influence, add the count and record the max and min of this cell for clamping later
    ...
    // Normalise, assign and clamp heights (ensure the averaged heights aren't excessively influencing magnitude beyond what's already max from point cloud)
    ...
    for (int i = 0; i < n_verts_across_down; ++i) {
        for (int j = 0; j < n_verts_across_down; ++j) {
            ...
            patch->m_trimesh->getCoordsVertices()[i * n_verts_across_down + j] = ChVector<>(x, y, z);
        }
    }
    ...
    for (int i = 0; i < coarse_grid_resolution; ++i) {
        for (int j = 0; j < coarse_grid_resolution; ++j) {
            ...
            if ((i + j) % 2 == 0) {
                patch->m_trimesh->getIndicesVertexes().push_back(ChVector<int>(v0, v2, v1));
                patch->m_trimesh->getIndicesVertexes().push_back(ChVector<int>(v1, v2, v3));
            } else {
                patch->m_trimesh->getIndicesVertexes().push_back(ChVector<int>(v0, v2, v3));
                patch->m_trimesh->getIndicesVertexes().push_back(ChVector<int>(v0, v3, v1));
            }
        }
    }
    ...
    while (current_iteration < max_refinements) {
        ...
        for (const auto& edge : winged_edges) {
            ...
            ChVector<> normal1 = Vcross(vertices[tri1.y()] - vertices[tri1.x()], vertices[tri1.z()] - vertices[tri1.x()]).GetNormalized();
            ChVector<> normal2 = Vcross(vertices[tri2.y()] - vertices[tri2.x()], vertices[tri2.z()] - vertices[tri2.x()]).GetNormalized();

            /// Triangle Exclusion checks:
            // Check the dot product of the normals against the vertical and horizontal frame and skip if nearly vertical or nearly flat triangles
            // look further up to note they are set for a parallel and perpendicular check against the vertical
        ...
        if (execute_refine) {  // Refine marked triangles
            ...
            patch->m_trimesh->RefineMeshEdges(marked_tris, max_edge_length, nullptr, nullptr, aux_data_double, aux_data_int, aux_data_bool, aux_data_vect);
            ...
            for (auto& vertex : patch->m_trimesh->getCoordsVertices()) {
                ...
                vertex.z() = height_map[cell_x][cell_y] * (1 - fx) * (1 - fy) +
                                ...
                                height_map[cell_x][nextCellY] * (1 - fx) * fy +
                                height_map[nextCellX][nextCellY] * fx * fy;
            }
        } // End refinement
    ...
    if (smoothing_factor != 0) { // skip if smoothing is zero
        ...
        // push-pull smoothing
        std::vector<ChVector<double>> temp_smoothing_vertices(n_verts); //up to date vector
        ...
        for (int iter = 0; iter < 10; ++iter) {  // number of iterations. 10 is usually enough
            ...
            for (int i = 0; i < n_verts; ++i) {
                ...
                bool is_edge_vertex = vertices[i].x() <= boundary_min_x + edge_threshold || vertices[i].x() >= boundary_max_x - edge_threshold ||
                                                        vertices[i].y() <= boundary_min_y + edge_threshold || vertices[i].y() >= boundary_max_y - edge_threshold;

                ChVector<double> laplacian_vector(0, 0, 0);
            ...
            for (int i = 0; i < n_verts; ++i) {
                bool is_edge_vertex = // check and mark the boundary vertices
                    vertices[i].x() <= boundary_min_x + edge_threshold || vertices[i].x() >= boundary_max_x - edge_threshold ||
                    vertices[i].y() <= boundary_min_y + edge_threshold || vertices[i].y() >= boundary_max_y - edge_threshold;

                ChVector<double> laplacian_vector(0, 0, 0);
                for (int adj_vertex : adjacency_list[i]) {
                    laplacian_vector += (temp_smoothing_vertices[adj_vertex] - temp_smoothing_vertices[i]); // consider original
                }
    ...
    // ----------------------------------------------------------------------------------------
    // Final setup, collision body and cachin
    // ----------------------------------------------------------------------------------------
    ...
    if (ChWorldFrame::Vertical().Equals(ChVector<>(0, 1, 0))) {
        ChQuaternion<> mquat = Q_from_AngAxis(-CH_C_PI_2, ChVector<>(1, 0, 0));  // Rotation from Z-up to Y-up
        patch->m_trimesh->Transform(ChVector<>(0, 0, 0), mquat);                 // Apply transformation
    }
    ...
    patch->m_radius = ChVector<>(length, width, (height_max - height_min)).Length() / 2;
    patch->m_type = PatchType::MESH;
