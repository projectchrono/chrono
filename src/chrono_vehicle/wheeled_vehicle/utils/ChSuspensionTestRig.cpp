...
void ChSuspensionTestRig::LoadWheel(const std::string& filename, int side) {
    ...
    if (subtype.compare("Wheel") == 0) {
        m_wheels[side] = ChSharedPtr<ChWheel>(new Wheel(d));
    }
...
void ChSuspensionTestRig::Initialize(const ChCoordsys<>& chassisPos) {
    ...
    m_post_L = ChSharedPtr<ChBody>(new ChBody(m_system->GetContactMethod()));
    ...
    m_post_R = ChSharedPtr<ChBody>(new ChBody(m_system->GetContactMethod()));
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
void ChSuspensionTestRig::AddVisualize_post(ChSharedPtr<ChBody> post_body,
