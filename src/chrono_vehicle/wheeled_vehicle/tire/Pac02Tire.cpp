...
// =============================================================================
//
// Pac02 tire constructed with data from file (JSON format).
//
// =============================================================================
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
Pac02Tire::Pac02Tire(const std::string& filename) : ChPac02Tire(""), m_mass(0), m_normalDamping(0), m_has_mesh(false) {
...
Pac02Tire::Pac02Tire(const rapidjson::Document& d) : ChPac02Tire(""), m_mass(0), m_normalDamping(0), m_has_mesh(false) {
    Create(d);
}
...
void Pac02Tire::Create(const rapidjson::Document& d) {  // Invoke base class method.
    ...
    if (d.HasMember("Dimension")) {
        ...
        if (d["Dimension"].HasMember("Rim Ratio")) {
            m_PacCoeff.rim_radius = d["Dimension"]["Rim Ratio"].GetDouble();
        }
        if (d["Dimension"].HasMember("Rim Width")) {
            m_PacCoeff.rim_width = d["Dimension"]["Rim Width"].GetDouble();
        }
    }
    if (d.HasMember("Scaling Coefficients")) {
        if (d["Scaling Coefficients"].HasMember("lfz0")) {
            m_PacScal.lfz0 = d["Scaling Coefficients"]["lfz0"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lcx")) {
            m_PacScal.lcx = d["Scaling Coefficients"]["lcx"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lmux")) {
            m_PacScal.lmux = d["Scaling Coefficients"]["lmux"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lex")) {
            m_PacScal.lex = d["Scaling Coefficients"]["lex"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lkx")) {
            m_PacScal.lkx = d["Scaling Coefficients"]["lkx"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lhx")) {
            m_PacScal.lhx = d["Scaling Coefficients"]["lhx"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lvx")) {
            m_PacScal.lvx = d["Scaling Coefficients"]["lvx"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lgax")) {
            m_PacScal.lgax = d["Scaling Coefficients"]["lgax"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lcy")) {
            m_PacScal.lcy = d["Scaling Coefficients"]["lcy"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lmuy")) {
            m_PacScal.lmuy = d["Scaling Coefficients"]["lmuy"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("ley")) {
            m_PacScal.ley = d["Scaling Coefficients"]["ley"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lky")) {
            m_PacScal.lky = d["Scaling Coefficients"]["lky"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lhy")) {
            m_PacScal.lhy = d["Scaling Coefficients"]["lhy"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lvy")) {
            m_PacScal.lvy = d["Scaling Coefficients"]["lvy"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lgay")) {
            m_PacScal.lgay = d["Scaling Coefficients"]["lgay"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("ltr")) {
            m_PacScal.ltr = d["Scaling Coefficients"]["ltr"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lres")) {
            m_PacScal.lres = d["Scaling Coefficients"]["lres"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lgaz")) {
            m_PacScal.lgaz = d["Scaling Coefficients"]["lgaz"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lxal")) {
            m_PacScal.lxal = d["Scaling Coefficients"]["lxal"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lyka")) {
            m_PacScal.lyka = d["Scaling Coefficients"]["lyka"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lvyka")) {
            m_PacScal.lvyka = d["Scaling Coefficients"]["lvyka"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("ls")) {
            m_PacScal.ls = d["Scaling Coefficients"]["ls"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lsgkp")) {
            m_PacScal.lsgkp = d["Scaling Coefficients"]["lsgkp"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lsgal")) {
            m_PacScal.lsgal = d["Scaling Coefficients"]["lsgal"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lgyr")) {
            m_PacScal.lgyr = d["Scaling Coefficients"]["lgyr"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lmx")) {
            m_PacScal.lmx = d["Scaling Coefficients"]["lmx"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lvmx")) {
            m_PacScal.lvmx = d["Scaling Coefficients"]["lvmx"].GetDouble();
        }
        if (d["Scaling Coefficients"].HasMember("lmy")) {
            m_PacScal.lmy = d["Scaling Coefficients"]["lmy"].GetDouble();
        }
    }
    if (d.HasMember("Vertical")) {
        ...
        if (d["Vertical"].HasMember("Fnomin")) {
            m_PacCoeff.FzNomin = d["Vertical"]["Fnomin"].GetDouble();
        }
    }
...
void Pac02Tire::RemoveVisualizationAssets() {
    ...
    // Make sure we only remove the assets added by Pac02Tire::AddVisualizationAssets.
