...
//
// This implementation does not include transient slip state modifications at 
//  this time.
//
// =============================================================================
// =============================================================================
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
ChPac89Tire::ChPac89Tire(const std::string& name) : ChTire(name), m_stepsize(1e-6) {
...
void ChPac89Tire::Synchronize(double time, const WheelState& wheel_state, const ChTerrain& terrain) {
    ...
    if (m_data.in_contact) {
        ...
        m_states.vsy = -m_data.vel.y(); //PAC89 is defined in a modified SAE coordinate system, so flip Y to get it into SAE
...
void ChPac89Tire::Advance(double step) {
    if (m_data.in_contact) {
        ...
        m_states.cp_side_slip = std::max<>(-CH_C_PI_2 + .001, std::min<>(CH_C_PI_2 - .001, m_states.cp_side_slip));
        ...
        double Fx = 0;
        double Fy = 0;
        double Fz = m_data.normal_force / 1000; //N -> kN (Note that all other forces & moments besides Fz are in N and Nm)
        double Mx = 0;
        double My = 0;
        double Mz = 0;
        double gamma = (90.0-(std::acos(m_states.disc_normal.z()) * 180.0 / CH_C_PI)); // rad -> deg
        double alpha = -(m_states.cp_side_slip * 180.0 / CH_C_PI); // rad -> deg -> Flip to convert to PAC89 modified SAE coordinates
        double kappa = (m_states.cp_long_slip * 100.0); // decimal -> perecent
        ...
        // Lateral Force
        {
            ...
            double BCD = m_PacCoeff.A3*std::sin(std::atan(Fz / m_PacCoeff.A4)*2.0)*(1.0 - m_PacCoeff.A5*std::abs(gamma));
            double B = BCD / (C*D);
            ...
            m_states.cp_side_slip = std::max<>(-89.5, std::min<>(89.5, X1));
        ...
        // Self-Aligning Torque
        {
            ...
            double BCD = (m_PacCoeff.C3 * std::pow(Fz, 2) + m_PacCoeff.C4*Fz)*(1 - m_PacCoeff.C6*std::abs(gamma))*std::exp(-m_PacCoeff.C5*Fz);
            double B = BCD / (C*D);
            double Sh = m_PacCoeff.C11*gamma + m_PacCoeff.C12*Fz + m_PacCoeff.C13;
            double Sv = (m_PacCoeff.C14 * std::pow(Fz, 2) + m_PacCoeff.C15*Fz)*gamma + m_PacCoeff.C16*Fz + m_PacCoeff.C17;
            double X1 = (alpha + Sh);
            double E = (m_PacCoeff.C7 * std::pow(Fz, 2) + m_PacCoeff.C8*Fz + m_PacCoeff.C9)*(1.0 - m_PacCoeff.C10*std::abs(gamma));
        ...
        // Rolling Resistance
        {
            ...
            My = m_rolling_resistance * m_data.normal_force * Lrad * sgn(m_states.omega);
        }
