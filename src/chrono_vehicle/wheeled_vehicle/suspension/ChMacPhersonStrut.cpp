...
// -----------------------------------------------------------------------------
ChVector<> ChMacPhersonStrut::GetCOMPos() const {
    ChVector<> com(0, 0, 0);

    com += getSpindleMass() * m_spindle[LEFT]->GetPos();
    com += getSpindleMass() * m_spindle[RIGHT]->GetPos();

    com += getStrutMass() * m_strut[LEFT]->GetPos();
    com += getStrutMass() * m_strut[RIGHT]->GetPos();
...
void ChMacPhersonStrut::Output(ChVehicleOutput& database) const {
    ...
    database.WriteBody(m_strut[0]);
    database.WriteBody(m_strut[1]);
    ...
    database.WriteJoint(m_cylindricalStrut[0]);
    database.WriteJoint(m_cylindricalStrut[1]);
    database.WriteJoint(m_universalStrut[0]);
    database.WriteJoint(m_universalStrut[1]);
