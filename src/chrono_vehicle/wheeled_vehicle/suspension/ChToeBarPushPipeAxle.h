...
#ifndef CH_TOEBAR_PUSHPIPEAXLE_H
#define CH_TOEBAR_PUSHPIPEAXLE_H
...
/// suspension and will be mirrored (reflecting the y coordinates) to construct
/// the right side.
class CH_VEHICLE_API ChToeBarPushPipeAxle : public ChSuspension {
  public:
    ChToeBarPushPipeAxle(const std::string& name  ///< [in] name of the subsystem
    );

    virtual ~ChToeBarPushPipeAxle();

    /// Get the name of the vehicle subsystem template.
    virtual std::string GetTemplateName() const override { return "ToeBarPushPipeAxle"; }
    ...
    /// aligned with the chassis reference frame. If a steering subsystem is
    /// provided, the suspension tierods are to be attached to the steering's
    /// central link body (steered suspension); otherwise they are to be attached
    /// to the chassis (non-steered suspension).
  ...
  protected:
    ...
    std::shared_ptr<ChLinkLockSpherical> m_axleTubeGuideLong;  ///< longitudinal guidance, forbids translations,
                                                               ///< allows rotations
  ...
  private:
    ...
    ChVector3d m_panhardOuterC;
    ChVector3d m_panhardOuterA;

    // Points for pushpipe visualization
    ChVector3d m_pushPipeOuterC;
    ChVector3d m_pushPipeOuterA;
