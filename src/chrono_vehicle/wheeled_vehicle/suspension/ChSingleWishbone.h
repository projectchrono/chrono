...
class CH_VEHICLE_API ChSingleWishbone : public ChSuspension {
  ...
  protected:
    ...
    /// Indicate whether or not tierod bodies are modeled (default: false).
    ...
    virtual const ChVector3d& getCAInertiaMoments() const = 0;
    /// Return the products of inertia of the control arm body.
    virtual const ChVector3d& getCAInertiaProducts() const = 0;
    ...
    std::shared_ptr<ChJoint> m_revoluteCA[2];  ///< chassis-CA revolute joints (left/right)
    std::shared_ptr<ChJoint> m_revoluteUA[2];  ///< upright-CA revolute joints (left/right)
