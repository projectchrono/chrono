...
// =============================================================================
//
// Implementation of the DEM granular TERRAIN NODE (using Chrono::Dem).
//
...
//// TODO: eliminate when Chrono::Dem can use a domain that is *not* centered at origin!
...
ChVehicleCosimTerrainNodeGranularDEM::ChVehicleCosimTerrainNodeGranularDEM(double length, double width)
    : ChVehicleCosimTerrainNodeChrono(Type::GRANULAR_DEM, length, width, ChContactMethod::SMC),
    ...
    m_integrator_type = dem::CHDEM_TIME_INTEGRATOR::CENTERED_DIFFERENCE;
    m_tangential_model = dem::CHDEM_FRICTION_MODE::MULTI_STEP;
...
ChVehicleCosimTerrainNodeGranularDEM::ChVehicleCosimTerrainNodeGranularDEM(const std::string& specfile)
    : ChVehicleCosimTerrainNodeChrono(Type::GRANULAR_DEM, 0, 0, ChContactMethod::SMC),
      m_constructed(false),
      m_use_checkpoint(false),
      m_settling_output(false),
      m_settling_fps(100),
    ...
    // Read DEM granular terrain parameters from provided specfile
    SetFromSpecfile(specfile);
}

ChVehicleCosimTerrainNodeGranularDEM ::~ChVehicleCosimTerrainNodeGranularDEM() {}

// -----------------------------------------------------------------------------

//// TODO: error checking
void ChVehicleCosimTerrainNodeGranularDEM::SetFromSpecfile(const std::string& specfile) {
    Document d;
    ...
    if (model.compare("MULTI_STEP") == 0)
        ...
        m_tangential_model = dem::CHDEM_FRICTION_MODE::SINGLE_STEP;
    else if (model.compare("FRCTIONLESS") == 0)
        m_tangential_model = dem::CHDEM_FRICTION_MODE::FRICTIONLESS;

    m_fixed_proxies = d["Simulation settings"]["Fix proxies"].GetBool();
}

void ChVehicleCosimTerrainNodeGranularDEM::SetGranularMaterial(double radius, double density) {
    m_radius_g = radius;
    m_rho_g = density;
}

////void ChVehicleCosimTerrainNodeGranularDEM::SetContactForceModel(ChSystemSMC::ContactForceModel model) {
////}

void ChVehicleCosimTerrainNodeGranularDEM::SetIntegratorType(dem::CHDEM_TIME_INTEGRATOR type) {
    m_integrator_type = type;
}

void ChVehicleCosimTerrainNodeGranularDEM::SetTangentialDisplacementModel(dem::CHDEM_FRICTION_MODE model) {
...
void ChVehicleCosimTerrainNodeGranularDEM::SetMaterialSurface(const std::shared_ptr<ChContactMaterialSMC>& mat) {
    m_material_terrain = mat;
}

void ChVehicleCosimTerrainNodeGranularDEM::SetInputFromCheckpoint(const std::string& filename) {
    m_use_checkpoint = true;
    m_checkpoint_filename = filename;
}

void ChVehicleCosimTerrainNodeGranularDEM::SetSettlingTime(double time) {
    m_time_settling = time;
    m_fixed_settling_duration = true;
}

void ChVehicleCosimTerrainNodeGranularDEM::SetSettlingKineticEneryThreshold(double threshold) {
    m_KE_settling = threshold;
    m_fixed_settling_duration = false;
}

void ChVehicleCosimTerrainNodeGranularDEM::EnableSettlingOutput(bool output, double output_fps) {
...
// -----------------------------------------------------------------------------
void ChVehicleCosimTerrainNodeGranularDEM::Construct() {
    if (m_constructed)
        return;

    if (m_verbose)
        cout << "[Terrain node] GRANULAR_DEM " << endl;
    ...
    m_systemDEM = new dem::ChSystemDemMesh((float)m_radius_g, (float)m_rho_g, box);
    m_systemDEM->SetGravitationalAcceleration(ChVector3f(0, 0, (float)m_gacc));
    m_systemDEM->SetTimeIntegrator(m_integrator_type);
    m_systemDEM->SetFrictionMode(m_tangential_model);
    m_systemDEM->SetParticleOutputMode(dem::CHDEM_OUTPUT_MODE::CSV);
    m_systemDEM->SetVerbosity(dem::CHDEM_VERBOSITY::QUIET);
    m_systemDEM->SetMeshVerbosity(dem::CHDEM_MESH_VERBOSITY::QUIET);
    ...
    m_systemDEM->SetFixedStepSize((float)m_step_size);
    ...
    for (auto& b : m_obstacles) {
        ...
        /*auto imesh =*/m_systemDEM->AddMesh(trimesh, mass);
    }
...
// -----------------------------------------------------------------------------
void ChVehicleCosimTerrainNodeGranularDEM::Settle() {
    Construct();
    ...
    while (true) {
        ...
        m_systemDEM->AdvanceSimulation((float)m_step_size);
        m_timer.stop();
        ...
        if (m_fixed_settling_duration) {
            ...
            if (time >= m_time_settling) {
                KE = m_systemDEM->GetParticlesKineticEnergy();
                break;
            }
        } else if (time > 0.1) {
            KE = m_systemDEM->GetParticlesKineticEnergy();
    ...
    m_init_height = m_systemDEM->GetMaxParticleZ() + m_radius_g;
    if (m_verbose)
        cout << "[Terrain node] initial height = " << m_init_height << endl;
...
double ChVehicleCosimTerrainNodeGranularDEM::CalculatePackingDensity(double& depth) {
    // Find height of granular material
    double z_max = m_systemDEM->GetMaxParticleZ();
...
void ChVehicleCosimTerrainNodeGranularDEM::SetMatPropertiesInternal() {
    auto material_terrain = std::static_pointer_cast<ChContactMaterialSMC>(m_material_terrain);

    m_systemDEM->SetKn_SPH2SPH(material_terrain->GetKn());
    m_systemDEM->SetKn_SPH2WALL(material_terrain->GetKn());

    m_systemDEM->SetGn_SPH2SPH(material_terrain->GetGn());
    m_systemDEM->SetGn_SPH2WALL(material_terrain->GetGn());

    m_systemDEM->SetKt_SPH2SPH(material_terrain->GetKt());
    m_systemDEM->SetKt_SPH2WALL(material_terrain->GetKt());

    m_systemDEM->SetGt_SPH2SPH(material_terrain->GetGt());
    m_systemDEM->SetGt_SPH2WALL(material_terrain->GetGt());

    m_systemDEM->SetStaticFrictionCoeff_SPH2SPH(material_terrain->GetStaticFriction());
    m_systemDEM->SetStaticFrictionCoeff_SPH2WALL(material_terrain->GetStaticFriction());

    //// TODO: adhesion/cohesion
    //// TODO: why are cohesion and adhesion defined as ratios to sphere weight?!?
    m_systemDEM->SetCohesionRatio(0);
    m_systemDEM->SetAdhesionRatio_SPH2WALL(0);
}

// Set composite material properties for external contacts (granular-object)
void ChVehicleCosimTerrainNodeGranularDEM::SetMatPropertiesExternal(unsigned int i_shape) {
    //// RADU TODO
    //// Chrono::DEM is currently limited to a single material for an interacting object?!?
    ...
    m_systemDEM->SetKn_SPH2MESH(Kn);
    m_systemDEM->SetGn_SPH2MESH(Gn);
    m_systemDEM->SetKt_SPH2MESH(Kt);
    m_systemDEM->SetGt_SPH2MESH(Gt);
    m_systemDEM->SetStaticFrictionCoeff_SPH2MESH(mu);

    //// TODO: adhesion/cohesion
    //// TODO: why are cohesion and adhesion defined as ratios to sphere weight?!?
    m_systemDEM->SetAdhesionRatio_SPH2MESH(0);
}

// -----------------------------------------------------------------------------

void ChVehicleCosimTerrainNodeGranularDEM::CreateRigidProxy(unsigned int i) {
    ...
    for (auto& mesh : m_geometry[i_shape]->coll_meshes) {
        auto imesh = m_systemDEM->AddMesh(mesh.trimesh, (float)m_load_mass[i]);
        if (imesh != i + num_obstacles) {
            throw std::runtime_error("Error adding DEM mesh for object " + std::to_string(i));
        }
    }
    ...
    // Complete construction of the granular system
    m_systemDEM->InitializeMeshes();
}

// Once all proxy bodies are created, complete construction of the underlying system.
void ChVehicleCosimTerrainNodeGranularDEM::OnInitialize(unsigned int num_objects) {
...
void ChVehicleCosimTerrainNodeGranularDEM::UpdateRigidProxy(unsigned int i, BodyState& rigid_state) {
    ...
    m_systemDEM->ApplyMeshMotion(i, rigid_state.pos, rigid_state.rot, rigid_state.lin_vel, rigid_state.ang_vel);
}

// Collect resultant contact force and torque on rigid proxy body.
void ChVehicleCosimTerrainNodeGranularDEM::GetForceRigidProxy(unsigned int i, TerrainForce& rigid_contact) {
    ChVector3d force;
    ChVector3d torque;
    m_systemDEM->CollectMeshContactForces(i, force, torque);
...
void ChVehicleCosimTerrainNodeGranularDEM::OnAdvance(double step_size) {
    double t = 0;
    while (t < step_size) {
        ...
        m_systemDEM->AdvanceSimulation((float)h);
        t += h;
    }
}
...
void ChVehicleCosimTerrainNodeGranularDEM::UpdateVisualizationParticles() {
    ...
    for (unsigned int i = 0; i < m_num_particles; i++) {
        auto pos = m_systemDEM->GetParticlePosition(i);
        blist[i]->SetPos(pos);
    }
}
...
void ChVehicleCosimTerrainNodeGranularDEM::WriteCheckpoint(const std::string& filename) const {
    ...
    for (unsigned int i = 0; i < m_num_particles; i++) {
        auto pos = m_systemDEM->GetParticlePosition(i);
        auto vel = m_systemDEM->GetParticleVelocity(i);
        auto omg = m_systemDEM->GetParticleAngVelocity(i);
...
void ChVehicleCosimTerrainNodeGranularDEM::OnOutputData(int frame) {
...
void ChVehicleCosimTerrainNodeGranularDEM::OutputVisualizationData(int frame) {
