...
// =============================================================================
//
// Definition of the DEM granular TERRAIN NODE (using Chrono::Dem).
//
...
#ifndef TESTRIG_TERRAIN_NODE_GRANULAR_DEM_H
#define TESTRIG_TERRAIN_NODE_GRANULAR_DEM_H
...
/// Definition of the DEM granular terrain node (using Chrono::Dem).
class CH_VEHICLE_API ChVehicleCosimTerrainNodeGranularDEM : public ChVehicleCosimTerrainNodeChrono {
  public:
    /// Create a Chrono::Granular terrain node.
    ChVehicleCosimTerrainNodeGranularDEM(double length, double width);

    /// Create a Chrono::Granular terrain node and set parameters from the provided JSON specfile.
    ChVehicleCosimTerrainNodeGranularDEM(const std::string& specfile);

    ~ChVehicleCosimTerrainNodeGranularDEM();
    ...
    void SetMaterialSurface(const std::shared_ptr<ChContactMaterialSMC>& mat);
    ...
    void SetTangentialDisplacementModel(dem::CHDEM_FRICTION_MODE model);
    ...
    void SetIntegratorType(dem::CHDEM_TIME_INTEGRATOR type);
  ...
  private:
    ...
    virtual unsigned int GetNumContacts() const override { return m_systemDEM->GetNumContacts(); }
    ...
    /// Note that this requires memory transfer from DEM.
