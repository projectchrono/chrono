...
__host__ void ChSystemGpu_impl::runSphereBroadphase() {
    ...
    // Copy the offesets in the scratch pad; they will be soon stepped on
    ...
    nBlocks = (MAX_SDs_TOUCHED_BY_SPHERE * nSpheres + CUDA_THREADS_PER_BLOCK - 1) / (CUDA_THREADS_PER_BLOCK);
