...
class ChApiModal ChEigenvalueSolverSettings {
  public:
    ChEigenvalueSolverSettings(
        int m_nmodes,               ///< number of desired lower eigenvalues. If =0, return all eigenvalues.
        ...
        std::complex<double> msigma = 1e-5,  ///< for shift&invert. Too small gives ill conditioning (no convergence).
                                             ///< Too large misses rigid body modes.
    ...
    std::complex<double> sigma = 1e-5;  ///< for shift&invert. Too small gives ill conditioning (no convergence). Too
                                        ///< large misses rigid body modes.
...
class ChApiModal ChModalSolveUndamped {
  public:
    ...
    ChModalSolveUndamped(
        ...
        double mtolerance = 1e-10,  ///< tolerance for the iterative solver.
        bool mverbose = false,      ///< turn to true to see some diagnostic.
        const ChGeneralizedEigenvalueSolver& asolver =
    ...
    ChModalSolveUndamped(
        std::vector<ChFreqSpan> mfreq_spans,  ///< vector of {nmodes,freq}_i , will provide first nmodes_i starting at
...
/// Note: since the method is direct, all eigenvalues are computed, regardless of n_modes, but only lower n_modes are
/// returned.
...
class ChApiModal ChModalSolveDamped {
  public:
    ...
    /// finds first 7 lowest damped modes, using default settings (i.e. the ChQuadraticEigenvalueSolverNullspaceDirect
    /// solver). Ex.
    ...
    ChModalSolveDamped(
        ...
        double mtolerance = 1e-10,  ///< tolerance for the iterative solver.
        bool mverbose = false,      ///< turn to true to see some diagnostic.
        const ChQuadraticEigenvalueSolver& asolver =
            ChQuadraticEigenvalueSolverNullspaceDirect()  /// solver to use (default: direct, null-space based)
        )
        : freq_spans({{n_lower_modes, base_freq}}),
          max_iterations(max_iters),
          tolerance(mtolerance),
