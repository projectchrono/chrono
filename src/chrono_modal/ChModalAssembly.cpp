...
ChModalAssembly::ChModalAssembly()
    : modal_variables(nullptr), m_num_modes_coords_vel(0), is_modal(false), internal_nodes_update(true) {}
...
void ChModalAssembly::SwitchModalReductionON(ChSparseMatrix& full_M,
    ...
    int bou_int_coords_w = this->m_num_coords_vel_boundary + this->m_num_coords_vel_internal;
    double fooT;
    ...
    ChSparseMatrix K_II = full_K.block(this->m_num_coords_vel_boundary, this->m_num_coords_vel_boundary,
                                       this->m_num_coords_vel_internal, this->m_num_coords_vel_internal);
    ChSparseMatrix K_IB = full_K.block(this->m_num_coords_vel_boundary, 0, this->m_num_coords_vel_internal,
                                       this->m_num_coords_vel_boundary);

    ChSparseMatrix M_II = full_M.block(this->m_num_coords_vel_boundary, this->m_num_coords_vel_boundary,
                                       this->m_num_coords_vel_internal, this->m_num_coords_vel_internal);
    ChSparseMatrix M_IB = full_M.block(this->m_num_coords_vel_boundary, 0, this->m_num_coords_vel_internal,
                                       this->m_num_coords_vel_boundary);

    ChSparseMatrix Cq_B = full_Cq.block(0, 0, full_Cq.rows(), this->m_num_coords_vel_boundary);
    ChSparseMatrix Cq_I =
        full_Cq.block(0, this->m_num_coords_vel_boundary, full_Cq.rows(), this->m_num_coords_vel_internal);

    ChMatrixDynamic<> V_B =
        this->modes_V.block(0, 0, this->m_num_coords_vel_boundary, this->m_num_modes_coords_vel).real();
    ChMatrixDynamic<> V_I =
        this->modes_V
            .block(this->m_num_coords_vel_boundary, 0, this->m_num_coords_vel_internal, this->m_num_modes_coords_vel)
            .real();
    ...
    ChMatrixDynamic<> Psi_S(this->m_num_coords_vel_internal, this->m_num_coords_vel_boundary);
    ...
    for (int i = 0; i < K_IB.cols(); ++i) {
        ChVectorDynamic<> rhs(this->m_num_coords_vel_internal + full_Cq.rows());
        if (Cq_B.rows())
            rhs << K_IB.col(i).toDense(), Cq_B.col(i).toDense();
        else
            rhs << K_IB.col(i).toDense();
        ChVectorDynamic<> x = solver.solve(rhs);
        Psi_S.block(0, i, this->m_num_coords_vel_internal, 1) = -x.head(this->m_num_coords_vel_internal);
    }
    ...
    ChMatrixDynamic<> Psi_D(this->m_num_coords_vel_internal, this->m_num_modes_coords_vel);

    for (int i = 0; i < this->m_num_modes_coords_vel; ++i) {
        ChVectorDynamic<> rhs(this->m_num_coords_vel_internal + full_Cq.rows());
        rhs << (M_IB * V_B + M_II * V_I).col(i), Eigen::VectorXd::Zero(full_Cq.rows());
        ChVectorDynamic<> x = solver.solve(rhs);
        Psi_D.block(0, i, this->m_num_coords_vel_internal, 1) = -x.head(this->m_num_coords_vel_internal);
    }
...
void ChModalAssembly::SetupModalData(int nmodes_reduction) {
    this->m_num_modes_coords_vel = nmodes_reduction;

    if (!modal_variables || (modal_variables->Get_ndof() != this->m_num_modes_coords_vel)) {
        // Initialize ChVariable object used for modal variables
        if (modal_variables)
            delete modal_variables;
        modal_variables = new ChVariablesGenericDiagonalMass(this->m_num_modes_coords_vel);
        ...
        this->modal_q.setZero(this->m_num_modes_coords_vel);
        this->modal_q_dt.setZero(this->m_num_modes_coords_vel);
        this->modal_q_dtdt.setZero(this->m_num_modes_coords_vel);
        this->custom_F_modal.setZero(this->m_num_modes_coords_vel);
        this->custom_F_full.setZero(this->m_num_coords_vel_boundary + this->m_num_coords_vel_internal);
    }
}
...
bool ChModalAssembly::ComputeModesExternalData(ChSparseMatrix& full_M,
    ...
    modes_assembly_x0.setZero(this->m_num_coords_pos, nullptr);
    modes_assembly_v0.setZero(this->m_num_coords_vel, nullptr);
    this->IntStateGather(0, modes_assembly_x0, 0, modes_assembly_v0, fooT);

    // cannot use more modes than n. of tot coords, if so, clamp
    // int nmodes_clamped = std::min(nmodes, this->m_num_coords_vel);

    this->Setup();

    assert(full_M.rows() == this->m_num_coords_vel);
    assert(full_K.rows() == this->m_num_coords_vel);
    assert(full_Cq.cols() == this->m_num_coords_vel);
...
void ChModalAssembly::SetFullStateWithModeOverlay(int n_mode, double phase, double amplitude) {
    if (n_mode >= this->modes_V.cols()) {
        this->Update();
        throw std::invalid_argument("Error: mode " + std::to_string(n_mode) + " is beyond the " +
                                    std::to_string(this->modes_V.cols()) + " computed eigenvectors.");
    }

    if (this->modes_V.rows() != this->m_num_coords_vel) {
        this->Update();
        return;
    }
    ...
    assembly_x_new.setZero(this->m_num_coords_pos, nullptr);
    assembly_v.setZero(this->m_num_coords_vel, nullptr);
    assembly_Dx.setZero(this->m_num_coords_vel, nullptr);

    // pick the nth eigenvector
    assembly_Dx = sin(phase) * amplitude * this->modes_V.col(n_mode).real() +
...
void ChModalAssembly::SetInternalStateWithModes(bool full_update) {
    ...
    int bou_int_coords = this->m_num_coords_pos_boundary + this->m_num_coords_pos_internal;
    int bou_int_coords_w = this->m_num_coords_vel_boundary + this->m_num_coords_vel_internal;
    int bou_mod_coords_w = this->m_num_coords_vel_boundary + this->m_num_modes_coords_vel;
    ...
    if (needs_temporary_bou_int)
        this->is_modal =
            false;  // to have IntStateIncrement IntStateScatter referencing both boundary AND INTERNAL items
...
void ChModalAssembly::SetFullStateReset() {
    if (this->modes_assembly_x0.rows() != this->m_num_coords_pos)
        return;
...
void ChModalAssembly::Setup() {
    ...
    if (this->is_modal == false) {
        ...
        m_num_coords_vel = m_num_coords_vel_boundary + m_num_modes_coords_vel;
...
void ChModalAssembly::GetStateLocal(ChStateDelta& Dx, ChStateDelta& v_local) {
    if (is_modal == false) {
        ...
        Dx.setZero(this->m_num_coords_vel_boundary + this->m_num_modes_coords_vel, nullptr);

        // fetch the state snapshot (modal reduced)
        int bou_mod_coords = this->m_num_coords_pos_boundary + this->m_num_modes_coords_vel;
        int bou_mod_coords_w = this->m_num_coords_vel_boundary + this->m_num_modes_coords_vel;
        double fooT;
        ChState x_mod;
        ChStateDelta v_mod;
        ...
        x0_mod.segment(0, this->m_num_coords_pos_boundary) =
            this->assembly_x0.segment(0, this->m_num_coords_pos_boundary);
...
void ChModalAssembly::IntStateGather(const unsigned int off_x,
    ...
    if (is_modal == false) {
        ...
        x.segment(off_x + this->m_num_coords_pos_boundary, this->m_num_modes_coords_vel) = this->modal_q;
        v.segment(off_v + this->m_num_coords_vel_boundary, this->m_num_modes_coords_vel) = this->modal_q_dt;
    }
}
...
void ChModalAssembly::IntStateScatter(const unsigned int off_x,
    ...
    if (is_modal == false) {
        ...
        this->modal_q = x.segment(off_x + this->m_num_coords_pos_boundary, this->m_num_modes_coords_vel);
        this->modal_q_dt = v.segment(off_v + this->m_num_coords_vel_boundary, this->m_num_modes_coords_vel);
...
void ChModalAssembly::IntStateGatherAcceleration(const unsigned int off_a, ChStateDelta& a) {
    ...
    if (is_modal == false) {
        ...
        a.segment(off_a + this->m_num_coords_vel_boundary, this->m_num_modes_coords_vel) = this->modal_q_dtdt;
    }
}
...
void ChModalAssembly::IntStateScatterAcceleration(const unsigned int off_a, const ChStateDelta& a) {
    ...
    if (is_modal == false) {
        ...
        this->modal_q_dtdt = a.segment(off_a + this->m_num_coords_vel_boundary, this->m_num_modes_coords_vel);
    }
}
...
void ChModalAssembly::IntStateIncrement(const unsigned int off_x,
    ...
    if (is_modal == false) {
        ...
        x_new.segment(off_x + this->m_num_coords_pos_boundary, this->m_num_modes_coords_vel) =
            x.segment(off_x + this->m_num_coords_pos_boundary, this->m_num_modes_coords_vel) +
            Dv.segment(off_v + this->m_num_coords_vel_boundary, this->m_num_modes_coords_vel);
    }
}
                                        ...
                                        const double c)          ///< a scaling factor
{
    ...
    if (is_modal == false) {
        ...
        if (!this->custom_F_full.isZero()) {
            R.segment(displ_v, this->m_num_coords_vel_boundary + this->m_num_coords_vel_internal) +=
                c * this->custom_F_full;
        }
    } else {
        // 1-
        // Add elastic forces from current modal deformations
        ChStateDelta Dx_local(this->m_num_coords_vel_boundary + this->m_num_modes_coords_vel, nullptr);
        ChStateDelta v_local(this->m_num_coords_vel_boundary + this->m_num_modes_coords_vel, nullptr);
        this->GetStateLocal(Dx_local, v_local);

        R.segment(off, this->m_num_coords_vel_boundary + this->m_num_modes_coords_vel) -=
            c * (this->modal_K * Dx_local + this->modal_R * v_local);  //  note -= sign

        // 2-
        // Add custom forces (in modal coordinates)
        if (!this->custom_F_modal.isZero())
            R.segment(off + this->m_num_coords_vel_boundary, this->m_num_modes_coords_vel) += c * this->custom_F_modal;

        // 3-
        // Add custom forces (applied to the original non reduced system, and transformed into reduced)
        if (!this->custom_F_full.isZero())
            R.segment(off, this->m_num_coords_vel_boundary + this->m_num_modes_coords_vel) +=
                c * this->Psi.transpose() * this->custom_F_full;
    }
}
...
void ChModalAssembly::IntLoadResidual_Mv(const unsigned int off,      ///< offset in R residual
    ...
    if (is_modal == false) {
        ...
        ChVectorDynamic<> w_modal = w.segment(off, this->m_num_coords_vel_boundary + this->m_num_modes_coords_vel);
        R.segment(off, this->m_num_coords_vel_boundary + this->m_num_modes_coords_vel) += c * (this->modal_M * w_modal);
    }
}
...
void ChModalAssembly::IntLoadLumpedMass_Md(const unsigned int off, ChVectorDynamic<>& Md, double& err, const double c) {
    ...
    if (is_modal == false) {
        ...
        Md.segment(off, this->m_num_coords_vel_boundary + this->m_num_modes_coords_vel) += c * this->modal_M.diagonal();
        err += (Md.sum() -
                Md.diagonal()
...
void ChModalAssembly::IntToDescriptor(const unsigned int off_v,
    ...
    if (is_modal == false) {
        ...
        this->modal_variables->Get_qb() =
            v.segment(off_v + this->m_num_coords_vel_boundary, this->m_num_modes_coords_vel);
        this->modal_variables->Get_fb() =
            R.segment(off_v + this->m_num_coords_vel_boundary, this->m_num_modes_coords_vel);
    }
}
