...
int main(int argc,
   ...
   chrono::ChMatrixDynamic<> result(ma1 + ma2);  // Note: size of result is automatically set because of copy constructor
...
// (that is, integrates 2x1 matrix)
   ...
   class MySine2dM : public ChIntegrable2D<ChMatrixNM<double, 2, 1> > {
    public:
      void Evaluate(ChMatrixNM<double, 2, 1>& result,
                    const double x,
                    const double y) {
         result(0) = x * y;
         result(1) = 0.5 * y * y;
