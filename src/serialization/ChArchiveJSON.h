...
class  ChArchiveOutJSON : public ChArchiveOut {
  public:
   ...
   GetLog() << "json out_ref_abstract : pos " << position << "already inserted:" << already_inserted << "\n";
          comma_cr();
          indent();
          if (is_array.top()==false)
            (*ostream) << "\"" << bVal.name() << "\"" << "\t: \n";
...
GetLog() << "json out_ref : pos " << position << "already inserted:" << already_inserted << "\n";
          comma_cr();
          indent();
          if (is_array.top()==false)
            (*ostream) << "\"" << bVal.name() << "\"" << "\t: \n";
  ...
  public:
      ...
      rapidjson::Value* GetValueFromNameOrArray(const char* mname)
      {
          ...
          if (this->is_array.top() == true) {
              if (!level->IsArray()) {throw (ChException( "Cannot retrieve from ID num in non-array object."));}
      ...
      virtual void in     (ChNameValue<bool> bVal) {
		    ...
		    if (!mval->IsBool()) {throw (ChException( "Invalid true/false flag after '"+std::string(bVal.name())+"'"));}
			bVal.value() = mval->GetBool();
      }
      virtual void in     (ChNameValue<int> bVal) {
            rapidjson::Value* mval = GetValueFromNameOrArray(bVal.name());
			if (!mval->IsInt()) {throw (ChException( "Invalid integer number after '"+std::string(bVal.name())+"'"));}
			bVal.value() = mval->GetInt();
      }
      virtual void in     (ChNameValue<double> bVal) {
            rapidjson::Value* mval = GetValueFromNameOrArray(bVal.name());
		    if (!mval->IsNumber()) {throw (ChException( "Invalid number after '"+std::string(bVal.name())+"'"));}
      ...
      virtual void in     (ChNameValue<char> bVal){
            rapidjson::Value* mval = GetValueFromNameOrArray(bVal.name());
			if (!mval->IsInt()) {throw (ChException( "Invalid char code after '"+std::string(bVal.name())+"'"));}
			bVal.value() = (char)mval->GetInt();
      }
      virtual void in     (ChNameValue<unsigned int> bVal){
            rapidjson::Value* mval = GetValueFromNameOrArray(bVal.name());
			if (!mval->IsUint()) {throw (ChException( "Invalid unsigned integer number after '"+std::string(bVal.name())+"'"));}
			bVal.value() = mval->GetUint();
      }
      virtual void in     (ChNameValue<std::string> bVal){
            rapidjson::Value* mval = GetValueFromNameOrArray(bVal.name());
            if (!mval->IsString()) {throw (ChException( "Invalid string after '"+std::string(bVal.name())+"'"));}
			bVal.value() = mval->GetString();
      }
      virtual void in     (ChNameValue<unsigned long> bVal){
            rapidjson::Value* mval = GetValueFromNameOrArray(bVal.name());
            if (!mval->IsUint64()) {throw (ChException( "Invalid unsigned long number after '"+std::string(bVal.name())+"'"));}
			bVal.value() = (unsigned long)mval->GetUint64();
      }
      virtual void in     (ChNameValue<unsigned long long> bVal){
            rapidjson::Value* mval = GetValueFromNameOrArray(bVal.name());
            if (!mval->IsUint64()) {throw (ChException( "Invalid unsigned long long number after '"+std::string(bVal.name())+"'"));}
      ...
      virtual void in_array_pre (const char* name, size_t& msize) {
            ...
            if (!mval->IsArray()) {throw (ChException( "Invalid array [...] after '"+std::string(name)+"'"));}
            msize = mval->Size();
      ...
      virtual void in     (ChNameValue<ChFunctorArchiveIn> bVal) {
            rapidjson::Value* mval = GetValueFromNameOrArray(bVal.name());
            if (!mval->IsObject()) {throw (ChException( "Invalid object {...} after '"+std::string(bVal.name())+"'"));}
      ...
      virtual void in_ref_abstract (ChNameValue<ChFunctorArchiveIn> bVal) 
      {
            ...
            if (level->HasMember("type")) {
                if (!(*level)["type"].IsString()) {throw (ChException( "Invalid string after '"+std::string(bVal.name())+"'"));}
                cls_name = (*level)["type"].GetString();
            }
            size_t ref_ID = 0;
            if (level->HasMember("_reference_ID")) {
                if (!(*level)["_reference_ID"].IsUint64()) {throw (ChException( "Invalid number after '"+std::string(bVal.name())+"'"));}
            ...
            if (ref_ID ==0) {
                ...
                if (bVal.value().CallGetRawPtr(*this)) {
                    ...
                    throw(ChException("Archive cannot create abstract object of class '" + cls_name + "'"));
            }

            } else {
                if (ref_ID >= objects_pointers.size()) {throw (ChException( "Object _reference_ID is larger than pointer array"));}
  ...
  protected:
      ...
      void token_notfound(const char* mname) {
          if (!tolerate_missing_tokens)
            throw (ChException( "Cannot find '"+std::string(mname)+"'"));
      }
