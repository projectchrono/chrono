...
public:
	...
	ChMatrix<Real>& operator*=(const Real factor) { MatrScale(factor); return *this; }
	
		/// Increments this matrix by another matrix, in place
	template <class RealB>
	ChMatrix<Real>& operator+=(const ChMatrix<RealB>& matbis) { MatrInc(matbis); return *this; }
	
		/// Decrements this matrix by another matrix, in place
	template <class RealB>
	ChMatrix<Real>& operator-=(const ChMatrix<RealB>& matbis) { MatrDec(matbis); return *this; }
	...
	ChVector<Real> Matr34_x_Quat(const ChQuaternion<RealB>& qua)
		{
			...
			return ChVector<Real>(  ((Get34Element(0,0)) * (Real)qua.e0)+
			                        ((Get34Element(0,1)) * (Real)qua.e1)+
			                        ((Get34Element(0,2)) * (Real)qua.e2)+
			                        ((Get34Element(0,3)) * (Real)qua.e3)  ,
			                        ((Get34Element(1,0)) * (Real)qua.e0)+
			                        ((Get34Element(1,1)) * (Real)qua.e1)+
			                        ((Get34Element(1,2)) * (Real)qua.e2)+
			                        ((Get34Element(1,3)) * (Real)qua.e3)  ,
			                        ((Get34Element(2,0)) * (Real)qua.e0)+
			                        ((Get34Element(2,1)) * (Real)qua.e1)+
			                        ((Get34Element(2,2)) * (Real)qua.e2)+
			                        ((Get34Element(2,3)) * (Real)qua.e3)  );
		}
	...
	ChQuaternion<Real> Matr34T_x_Vect(const ChVector<RealB>& va)
		{
			...
			return ChQuaternion<Real>(  ((Get34Element(0,0)) * (Real)va.x)+
			                            ((Get34Element(1,0)) * (Real)va.y)+
			                            ((Get34Element(2,0)) * (Real)va.z)  ,
			                            ((Get34Element(0,1)) * (Real)va.x)+
			                            ((Get34Element(1,1)) * (Real)va.y)+
			                            ((Get34Element(2,1)) * (Real)va.z)  ,
			                            ((Get34Element(0,2)) * (Real)va.x)+
			                            ((Get34Element(1,2)) * (Real)va.y)+
			                            ((Get34Element(2,2)) * (Real)va.z)  ,
			                            ((Get34Element(0,3)) * (Real)va.x)+
			                            ((Get34Element(1,3)) * (Real)va.y)+
	...
	ChQuaternion<Real> Matr44_x_Quat(const ChQuaternion<RealB>& qua)
		{
			...
			return ChQuaternion<Real>(  ((Get44Element(0,0)) * (Real)qua.e0)+
			                            ((Get44Element(0,1)) * (Real)qua.e1)+
			                            ((Get44Element(0,2)) * (Real)qua.e2)+
			                            ((Get44Element(0,3)) * (Real)qua.e3)  ,
			                            ((Get44Element(1,0)) * (Real)qua.e0)+
			                            ((Get44Element(1,1)) * (Real)qua.e1)+
			                            ((Get44Element(1,2)) * (Real)qua.e2)+
			                            ((Get44Element(1,3)) * (Real)qua.e3)  ,
			                            ((Get44Element(2,0)) * (Real)qua.e0)+
			                            ((Get44Element(2,1)) * (Real)qua.e1)+
			                            ((Get44Element(2,2)) * (Real)qua.e2)+
			                            ((Get44Element(2,3)) * (Real)qua.e3)  ,
			                            ((Get44Element(3,0)) * (Real)qua.e0)+
			                            ((Get44Element(3,1)) * (Real)qua.e1)+
			                            ((Get44Element(3,2)) * (Real)qua.e2)+
			                            ((Get44Element(3,3)) * (Real)qua.e3)  );
		}
	...
	void RowColInterp(Real vmin, Real vmax, Real umin, Real umax)
		{
			for (int iu = 0; iu <GetColumns(); iu++)
				...
				{
					if (GetRows()>1)    Element(iv,iu) =  vmin+(vmax-vmin)*((Real)iv/((Real)(GetRows()-1)));
					if (GetColumns()>1) Element(iv,iu) += umin+(umax-umin)*((Real)iu/((Real)(GetColumns()-1)));
				}
		}
...
public:
	...
	ChVector<Real> operator*(const ChVector<Real>& myvect) { return (this->Matr_x_Vect (myvect)); }
	...
	inline ChVector<Real> Matr_x_Vect(const ChVector<RealB>& va) const
		{
			return ChVector<Real>(
			  ...
			  this->Get33Element(1,0) * (Real)va.x + this->Get33Element(1,1) * (Real)va.y + this->Get33Element(1,2) * (Real)va.z ,
			  this->Get33Element(2,0) * (Real)va.x + this->Get33Element(2,1) * (Real)va.y + this->Get33Element(2,2) * (Real)va.z );
		}
	...
	inline ChVector<Real> MatrT_x_Vect(const ChVector<RealB>& va) const
		{
			return ChVector<Real>(
			  ...
			  this->Get33Element(0,1) * (Real)va.x + this->Get33Element(1,1) * (Real)va.y + this->Get33Element(2,1) * (Real)va.z ,
			  this->Get33Element(0,2) * (Real)va.x + this->Get33Element(1,2) * (Real)va.y + this->Get33Element(2,2) * (Real)va.z );
		}
