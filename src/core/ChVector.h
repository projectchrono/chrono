...
public:
	...
	ChVector<Real>& operator*=(const Real s)     { x*=s; y*=s; z*=s; return *this; }
	...
	bool Equals(const ChVector<Real>& other, Real tol) const
		...
		/// this=A+B
	void Add(const ChVector<Real>& A, const ChVector<Real>& B) { x=A.x+B.x; y=A.y+B.y; z=A.z+B.z; }

		/// The vector becomes the difference of the two vectors A and B:
		/// this=A-B
	void Sub(const ChVector<Real>& A, const ChVector<Real>& B) { x=A.x-B.x; y=A.y-B.y; z=A.z-B.z; }
	...
	void Mul(const ChVector<Real>& A, const Real v) { x=A.x*v; y=A.y*v; z=A.z*v; }
