...
//------------------------------------------------------------------------------
__device__ __host__ inline void MapBeamDataFrom_1D_Array(real3 * beamData1, real3 * beamData2, const real_* e, int2 nodesPortion, real_ mult) { //beamData1: postion, beamData2: slope
	int numNodes = nodesPortion.y - nodesPortion.x;
	for (int j = 0; j < numNodes; j++) {
		int nodeIdx = nodesPortion.x + j;
		real3 data1 = mult * R3(e[6 * j + 0], e[6 * j + 1], e[6 * j + 2]);
		real3 data2 = mult * R3(e[6 * j + 3], e[6 * j + 4], e[6 * j + 5]);
		beamData1[nodeIdx] = data1;
		beamData2[nodeIdx] = data2;
	}
}
//------------------------------------------------------------------------------
		...
		bool isCantilever)
{
	...
	real_* v = new real_ [numNodesAdjusted * 6];
	MapBeamDataTo_1D_Array(v, flex_FSI_NodesForcesD1, flex_FSI_NodesForcesD2, nodesPortionAdjusted2);
	real_* edd = new real_ [numNodesAdjusted * 6];
	//--------------------------------------- Calculation Block
	if (isCantilever) {
		switch(nE) {
	...
	MapBeamDataFrom_1D_Array(ANCF_NodesAccD, ANCF_SlopesAccD, edd, nodesPortionAdjusted2, mult);
}
