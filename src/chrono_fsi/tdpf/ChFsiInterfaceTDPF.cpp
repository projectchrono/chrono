...
void ChFsiInterfaceTDPF::ExchangeSolidStates() {
    ...
    for (size_t i = 0; i < num_bodies; i++) {
        ...
        m_sysTDPF->m_impl->m_ss_state.bodies[i].position = p.eigen();
        m_sysTDPF->m_impl->m_ss_state.bodies[i].orientation_rpy = r.eigen();
        m_sysTDPF->m_impl->m_ss_state.bodies[i].linear_velocity = pd.eigen();
        m_sysTDPF->m_impl->m_ss_state.bodies[i].angular_velocity = rd.eigen();
        if (m_verbose) {
...
void ChFsiInterfaceTDPF::ExchangeSolidForces() {
    ...
    for (size_t i = 0; i < num_bodies; i++) {
        ...
        auto force = ChVector3d(m_sysTDPF->m_impl->m_ss_forces[i].force);
        auto torque = ChVector3d(m_sysTDPF->m_impl->m_ss_forces[i].moment);
