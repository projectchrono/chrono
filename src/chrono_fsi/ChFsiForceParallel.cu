...
__device__ inline Real4 DifVelocityRho(Real3& dist3,
  ...
  //	Real rAB_Dot_GradW = dot(dist3, gradW);
  //	Real3 derivV = -paramsD.markerMass * (rhoPresMuA.y / (rhoPresMuA.x * rhoPresMuA.x) + rhoPresMuB.y /
  //(rhoPresMuB.x *
  // rhoPresMuB.x)) * gradW
...
__global__ void collideD(Real4* sortedDerivVelRho_fsi_D,  // output: new velocity
  ...
  // *** method since it would cause large forces. ADAMI method is used only to calculate forces on the fluid markers
  // (A)
...
void ChFsiForceParallel::collide(thrust::device_vector<Real4>& sortedDerivVelRho_fsi_D,
  ...
  *isErrorH = false;
  ...
  collideD << <numBlocks, numThreads>>>
      (mR4CAST(sortedDerivVelRho_fsi_D), mR3CAST(sortedPosRad), mR3CAST(sortedVelMas), mR3CAST(vel_XSPH_Sorted_D),
  ...
  //					// unroll sorted index to have the location of original particles in the sorted
  //arrays
...
void ChFsiForceParallel::CollideWrapper() {
  thrust::device_vector<Real4> m_dSortedDerivVelRho_fsi_D(
      numObjectsH->numAllMarkers);  // Store Rho, Pressure, Mu of each particle in the device memory
...
void ChFsiForceParallel::ForceSPH(SphMarkerDataD* otherSphMarkersD, FsiBodiesDataD* otherFsiBodiesD) {
  // Arman: Change this function by getting in the arrays of the current stage: useful for RK2. array pointers need to
  // be private members
