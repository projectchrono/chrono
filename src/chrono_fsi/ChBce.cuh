...
public:
  thrust::device_vector<Real3>
      velMas_ModifiedBCE; //(numRigidAndBoundaryMarkers);
  thrust::device_vector<Real4>
      rhoPreMu_ModifiedBCE; //(numRigidAndBoundaryMarkers);

  ChBce(SphMarkerDataD *sortedSphMarkersD, ProximityDataD *markersProximityD,
        FsiGeneralData *otherFsiGeneralData, SimParams *otherParamsH,
        NumberOfObjects *otherNumObjects);

  ~ChBce();

  virtual void UpdateRigidMarkersPositionVelocity(SphMarkerDataD *sphMarkersD,
                                                  FsiBodiesDataD *fsiBodiesD);

  virtual void Rigid_Forces_Torques(SphMarkerDataD *sphMarkersD,
                                    FsiBodiesDataD *fsiBodiesD);

  void ModifyBceVelocity(SphMarkerDataD *sphMarkersD,
                         FsiBodiesDataD *fsiBodiesD);

  virtual void Populate_RigidSPH_MeshPos_LRF(SphMarkerDataD *sphMarkersD,
                                             FsiBodiesDataD *fsiBodiesD);
