...
ChWheeledVehicleAssembly* mVehicle;
opengl::ChOpenGLWindow& gl_window = opengl::ChOpenGLWindow::getInstance();



// -----------------------------------------------------------------------------
// Specification of the mVehicle model
// -----------------------------------------------------------------------------
...
// Initial mVehicle position and orientation
...
//					utils::LoadConvexMesh(mVehicle::GetDataFile(lugged_file), lugged_mesh, lugged_convex);
...
int main(int argc, char* argv[]) {
    ...
    // -----------------------------------------
    // Create and initialize the mVehicle system.
    // -----------------------------------------

//    ChWheeledVehicleAssembly* mVehicle;
    ChTireContactCallback* tire_cb;

    // Create the mVehicle assembly and the callback object for tire contact
...
//                system, time_step, time);  // Keep only this if you are just interested in the rigid sys
        ...
        if (monitor_bilaterals && sim_frame % bilateral_frame_interval == 0) {
            ...
            ////mVehicle->GetVehicle()->LogConstraintViolations();
