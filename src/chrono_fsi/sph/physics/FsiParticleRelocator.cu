...
FsiParticleRelocator::FsiParticleRelocator(FsiDataManager& data_mgr, const DefaultProperties& props)
...
struct shift_op {
    shift_op(const Real3& shift, const FsiParticleRelocator::DefaultProperties& props) : s(shift), p(props) {}
    ...
    Real3 s;
    FsiParticleRelocator::DefaultProperties p;
};

void FsiParticleRelocator::Shift(MarkerType type, const Real3& shift) {
...
struct togrid_op {
    togrid_op(const IntAABB& aabb_dest, Real spacing, const FsiParticleRelocator::DefaultProperties& props)
...
void FsiParticleRelocator::MoveAABB2AABB(MarkerType type,
