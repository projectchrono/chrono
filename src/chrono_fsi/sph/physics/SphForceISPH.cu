...
__global__ void Velocity_Correction_and_update(Real4* sortedPosRad,
    ...
    //        printf("Error! particle %d q_i is NAN: thrown from SphForceISPH.cu  %f\n", i_idx, q_i[i_idx]);
    //    }
    ...
    //        printf("Error! particle %d position is NAN: thrown from SphForceISPH.cu  %f,%f,%f,%f\n", i_idx,
    //               sortedPosRad[i_idx].x, sortedPosRad[i_idx].y, sortedPosRad[i_idx].z, sortedPosRad[i_idx].w);
    //    }
    //    if (!IsFinite(sortedRhoPreMu[i_idx])) {
    //        printf("Error! particle %d rhoPreMu is NAN: thrown from SphForceISPH.cu %f,%f,%f,%f\n", i_idx,
    //               sortedRhoPreMu[i_idx].x, sortedRhoPreMu[i_idx].y, sortedRhoPreMu[i_idx].z,
    //               sortedRhoPreMu[i_idx].w);
    //    }
    //
    //    if (!IsFinite(sortedVelMas[i_idx])) {
    //        printf("Error! particle %d velocity is NAN: thrown from SphForceISPH.cu %f,%f,%f\n", i_idx,
...
SphForceISPH::~SphForceISPH() {}

void SphForceISPH::Initialize() {
    SphForce::Initialize();
