...
static inline void TimingOutput(chrono::ChSystem* mSys,
	...
	if (chrono::ChSystemParallel* parallel_sys =
			dynamic_cast<chrono::ChSystemParallel*>(mSys)) {
		RESID =
				((chrono::ChLcpSolverParallel*) (mSys->GetLcpSolverSpeed()))->GetResidual();
		REQ_ITS =
				((chrono::ChLcpSolverParallel*) (mSys->GetLcpSolverSpeed()))->GetTotalIterations();
