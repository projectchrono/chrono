...
public:
  ...
  ///
  /// It uses a Runge-Kutta 2nd order algorithm to update both the fluid and
  /// multibody
  ...
  /// Function to fill out the dependent data based on the independent one. For
  /// instance
  /// it copies the position of the rigid bodies from the multibody dynamics
  /// system
  ...
  /// function to return the pointer to the fsi bodies. The pointer to all fsi
  /// bodies, i.e. the bodies
...
private:
  ...
  ChFsiDataManager
      *fsiData; ///< pointer to data manager which holds all the data
  std::vector<std::shared_ptr<ChBody>>
      fsiBodeisPtr; ///< vector of a pointers to fsi bodies. fsi bodies
  ...
  NumberOfObjects *numObjectsH; ///< pointer to the number of objects, fluid,
                                ///bce, and boundary markers

  double mTime;   ///< current real time of the simulation
  bool haveFluid; ///< boolean to check if the system have fluid or not

#ifdef CHRONO_OPENGL
  chrono::opengl::ChOpenGLWindow
      *gl_window; ///< opengl graphics window if opengl exist
#endif
};
} // end namespace fsi
