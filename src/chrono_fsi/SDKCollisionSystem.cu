 ...
 * @details  See SDKCollisionSystem.cuh
 */
__device__ inline Real4 DifVelocityRho(const Real3& dist3,
                                       const Real& d,
                                       const Real4& velMasA,
...
//--------------------------------------------------------------------------------------------------------------------------------
__device__ inline Real3 DifVelocity_SSI_DEM(const Real3& dist3,
                                            const Real& d,
                                            const Real4& velMasA,
                                            const Real4& velMasB) {
  ...
  Real3 force = (/*pow(paramsD.sizeScale, Real(3)) * */ kS * l - kD * m_eff * dot(mR3(velMasA - velMasB), n)) *
                n;  // relative velocity at contact is simply assumed as the relative vel of the centers. If you are
...
//--------------------------------------------------------------------------------------------------------------------------------
// collide a particle against all other particles in a given cell
__device__ void BCE_modification_Share(Real4& deltaVDenom,  // in and out
                                       Real4& deltaRP,
                                       int& isAffected,
                                       int3 gridPos,
                                       uint index,
    ...
    for (uint j = startIndex; j < endIndex; j++) {
      if (j != index) {  // check not colliding with self
        ...
        if (rhoPresMuA.w < 0 || rhoPresMuB.w < 0) {
          ...
          derivVelRho =
              DifVelocityRho(dist3, d, velMasA, vel_XSPH_A, velMasB, vel_XSPH_B, rhoPresMuA, rhoPresMuB, multViscosit);
...
// collide a particle against all other particles in a given cell
__device__ __inline__ void stressCell(Real3& devS3,
                                      Real3& volS3,
                                      int3 gridPos,
                                      uint index,
                                      Real3 posRadA,
  ...
  if (startIndex != 0xffffffff) {  // cell is not empty
    ...
    for (uint j = startIndex; j < endIndex; j++) {
      if (j != index) {  // check not colliding with self
        ...
        volS3 +=
            -paramsD.mu0 * velMasB.w / rhoPresMuB.x * 4.0 / 3.0 * mR3(vr.x * gradW.x, vr.y * gradW.y, vr.z * gradW.z);
      }
    }
  }
}
...
//--------------------------------------------------------------------------------------------------------------------------------
// collide a particle against all other particles in a given cell
__device__ void projectTheClosestFluidMarker(Real3& distRhoPress,
                                             int3 gridPos,
                                             uint index,
                                             Real3 posRadA,
                                             Real3* sortedPosRad,
...
//--------------------------------------------------------------------------------------------------------------------------------
// collide a particle against all other particles in a given cell
__device__ void calcOnCartesianShare(Real3& v_share,
                                     Real4& rp_share,
                                     int3 gridPos,
                                     Real4 gridNodePos4,
                                     Real3* sortedPosRad,
  ...
  for (int z = -1; z <= 1; z++) {
    for (int y = -1; y <= 1; y++) {
      for (int x = -1; x <= 1; x++) {
        ...
        deltaV += deltaVShare(neighbourPos,
                              index,
                              posRadA,
                              velMasA,
                              rhoPreMuA,
...
//--------------------------------------------------------------------------------------------------------------------------------
__global__ void new_BCE_VelocityPressure(Real4* sortedVelMas_ModifiedBCE,    // input: sorted velocities
  ...
  for (int z = -1; z <= 1; z++) {
    for (int y = -1; y <= 1; y++) {
      for (int x = -1; x <= 1; x++) {
        int3 neighbourPos = gridPos + mI3(x, y, z);
        BCE_modification_Share(deltaVDenom,
                               deltaRP,
                               isAffected,
                               neighbourPos,
                               index,
  ...
  if (isAffected) {
    ...
    Real pressure = (deltaRP.w + dot(paramsD.gravity, mR3(deltaRP))) / deltaVDenom.w;  //(in fact:  (paramsD.gravity -
...
__global__ void collideD(Real4* derivVelRhoD,  // output: new velocity
  ...
  for (int x = -1; x <= 1; x++) {
    for (int y = -1; y <= 1; y++) {
      for (int z = -1; z <= 1; z++) {
        derivVelRho += collideCell(gridPos + mI3(x, y, z),
                                   index,
                                   posRadA,
                                   velMasA,
                                   vel_XSPH_A,
...
__global__ void CalcBCE_Stresses_kernel(Real3* devStressD,
                                        Real3* volStressD,
                                        Real3* sortedPosRad,
                                        Real4* sortedVelMas,
                                        Real4* sortedRhoPreMu,
  ...
  uint BCE_Index =
      index + min(numObjectsD.startRigidMarkers,
                  numObjectsD.startRigidMarkers);     // updatePortionD = [start, end] index of the update portion
...
//--------------------------------------------------------------------------------------------------------------------------------
// calculate particles stresses
__global__ void CalcBCE_MainStresses_kernel(Real4* mainStressD, Real3* devStressD, Real3* volStressD, int numBCE) {
  uint index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index >= numBCE) {
    return;
  }
  ...
  Real4 mainS3 = mR4(0);
  mainS3.w =
      sqrt(.5 * (pow(volS3.x - volS3.y, Real(2)) + pow(volS3.x - volS3.z, Real(2)) + pow(volS3.y - volS3.z, Real(2)) +
                 6 * (devS3.x * devS3.x + devS3.y * devS3.y + devS3.z * devS3.z)));

  mainStressD[index] = mainS3;
}
//--------------------------------------------------------------------------------------------------------------------------------
// without normalization
__global__ void ReCalcDensityD_F1(Real3* oldPosRad,
                                  Real4* oldVelMas,
                                  Real4* oldRhoPreMu,
                                  Real3* sortedPosRad,
                                  Real4* sortedVelMas,
  ...
  for (int z = -1; z <= 1; z++) {
    for (int y = -1; y <= 1; y++) {
      for (int x = -1; x <= 1; x++) {
        int3 neighbourPos = gridPos + mI3(x, y, z);
        collideCellDensityReInit(densityShare,
                                 denominator,
                                 neighbourPos,
                                 index,
                                 posRadA,
...
__global__ void ProjectDensityPressureToBCandBCE_D(Real4* oldRhoPreMu,
  ...
  for (int z = -1; z <= 1; z++) {
    for (int y = -1; y <= 1; y++) {
      for (int x = -1; x <= 1; x++) {
        int3 neighbourPos = gridPos + mI3(x, y, z);
        projectTheClosestFluidMarker(
            distRhoPress, neighbourPos, index, posRadA, sortedPosRad, sortedRhoPreMu, cellStart, cellEnd);
      }
    }
  }
  // write new velocity back to original unsorted location
...
__global__ void CalcCartesianDataD(Real4* rho_Pres_CartD,
                                   ...
                                   uint* cellEnd) {
  uint index = __mul24(blockIdx.x, blockDim.x) + threadIdx.x;
  if (index >= cartesianGridDimsD.x * cartesianGridDimsD.y * cartesianGridDimsD.z)
    return;
  ...
  for (int z = -1; z <= 1; z++) {
    for (int y = -1; y <= 1; y++) {
      for (int x = -1; x <= 1; x++) {
        int3 neighbourPos = gridPos + mI3(x, y, z);
        calcOnCartesianShare(vel_share,
                             rho_pres_share,
                             neighbourPos,
                             mR4(gridNodePos3),
                             sortedPosRad,
...
__global__ void UpdateFluidD_init_LF(Real3* posRadD, Real4* velMasD_half, Real4* rhoPresMuD_half, Real4* derivVelRhoD) {
  ...
  Real rho2 =
      rhoPresMu.x + derivVelRho.w * (0.5 * dTD);  // rho update. (i.e. rhoPresMu.x), still not wriiten to global matrix
  rhoPresMu.y = Eos(rho2, rhoPresMu.w);
  rhoPresMu.x = rho2;
...
//--------------------------------------------------------------------------------------------------------------------------------
// updates the fluid particles' properties, i.e. velocity, density, pressure, position
__global__ void UpdateFluidD_rho_vel_LF(Real4* velMasD,
                                        Real4* rhoPresMuD,
                                        Real4* velMasD_old,
                                        Real4* rhoPresMuD_old,
                                        Real4* derivVelRhoD) {
  uint index = blockIdx.x * blockDim.x + threadIdx.x;
  index += updatePortionD.x;  // updatePortionD = [start, end] index of the update portion
  if (index >= updatePortionD.y) {
...
//--------------------------------------------------------------------------------------------------------------------------------
// updates the fluid particles' properties, i.e. velocity, density, pressure, position
__global__ void UpdateFluidD_EveryThing_LF(Real3* posRadD,
                                           Real4* velMasD_half,
                                           Real4* rhoPresMuD_half,
                                           Real4* derivVelRhoD) {
  uint index = blockIdx.x * blockDim.x + threadIdx.x;
  index += updatePortionD.x;  // updatePortionD = [start, end] index of the update portion
  if (index >= updatePortionD.y) {
...
void reorderDataAndFindCellStart(thrust::device_vector<uint>& cellStart,
  ...
  reorderDataAndFindCellStartD << <numBlocks, numThreads, smemSize>>> (U1CAST(cellStart),
                                                                       U1CAST(cellEnd),
...
void ProjectDensityPressureToBCandBCE(thrust::device_vector<Real4>& oldRhoPreMu,
  ...
  // execute the kernel
  ProjectDensityPressureToBCandBCE_D << <numBlocks, numThreads>>> (mR4CAST(oldRhoPreMu),
                                                                   mR3CAST(sortedPosRad),
                                                                   mR4CAST(sortedRhoPreMu),
                                                                   U1CAST(gridMarkerIndex),
                                                                   U1CAST(cellStart),
