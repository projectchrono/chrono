...
__device__ uint calcGridHash(int3 gridPos)
{
	...
	//if ((gridPos.x >= paramsD.gridSize.x)/* || (gridPos.x < 0)*/)
	//	printf("gridPos.x %d\n", gridPos.x);
...
__device__ inline float4 DifVelocityRho(	const float4 & posRadA, const float4 & posRadB,
											const float4 & velMasA, const float3 & vel_XSPH_A,
											const float4 & velMasB, const float3 & vel_XSPH_B,
											const float4 & rhoPresMuA, const float4 & rhoPresMuB) {
...
//		rhoPresMuA.x * velMasB.w / rhoPresMuB.x * dot(vel_XSPH_A - vel_XSPH_B, gradW));
	...
	//*** Artificial viscosity type 2
		...
		float3 derivV = -velMasB.w* (
			...
			+velMasB.w * 8.0f * mu0 * rAB_Dot_GradW / pow(rhoPresMuA.x + rhoPresMuB.x, 2) / ( d * d + epsilonMutualDistance * posRadA.w * posRadA.w ) * F3(velMasA - velMasB);
		return F4(derivV,
			rhoPresMuA.x * velMasB.w / rhoPresMuB.x * dot(vel_XSPH_A - vel_XSPH_B, gradW));
...
//--------------------------------------------------------------------------------------------------------------------------------
//computes dV/dt and dRho/dt, i.e. force terms. First
__device__ inline float4 DifVelocityRho2(	const float4 & posRadA, const float4 & posRadB,
											const float4 & velMasA, const float3 & vel_XSPH_A,
											const float4 & velMasB, const float3 & vel_XSPH_B,
											const float4 & rhoPresMuA, const float4 & rhoPresMuB) {
	float3 dist3 = Distance(posRadA, posRadB);
...
//		rhoPresMuA.x * velMasB.w / rhoPresMuB.x * dot(vel_XSPH_A - vel_XSPH_B, gradW));
	...
	//*** Artificial viscosity type 2
		...
		float3 derivV = -velMasB.w* (
				rhoPresMuA.y / (rhoPresMuA.x * rhoPresMuA.x) + rhoPresMuB.y / (rhoPresMuB.x * rhoPresMuB.x))* gradW
			+velMasB.w * 40.0f * mu0 * rAB_Dot_GradW / pow(rhoPresMuA.x + rhoPresMuB.x, 2) / ( d * d + epsilonMutualDistance  * posRadA.w * posRadA.w ) * F3(velMasA - velMasB);
		return F4(derivV,
			rhoPresMuA.x * velMasB.w / rhoPresMuB.x * dot(vel_XSPH_A - vel_XSPH_B, gradW));
...
//--------------------------------------------------------------------------------------------------------------------------------
//computes dV/dt and dRho/dt, i.e. force terms. First
__device__ inline float4 DifVelocityRho3(	const float4 & posRadA, const float4 & posRadB,
											const float4 & velMasA, const float3 & vel_XSPH_A,
											const float4 & velMasB, const float3 & vel_XSPH_B,
											const float4 & rhoPresMuA, const float4 & rhoPresMuB) {
	float3 dist3 = Distance(posRadA, posRadB);
...
//--------------------------------------------------------------------------------------------------------------------------------
//computes dV/dt and dRho/dt, i.e. force terms. First
__device__ inline float4 DifVelocityRho_FSI(	const float4 & posRadA, const float4 & posRadB,
...
__device__ inline float4 DifVelocityRho_FSI2(	const float4 & posRadA, const float4 & posRadB,
											const float4 & velMasA, const float4 & velMasB,
...
//--------------------------------------------------------------------------------------------------------------------------------
__device__ inline float3 DifVelocity_SSI_DEM(	const float4 & posRadA, const float4 & posRadB,
												const float4 & velMasA, const float4 & velMasB) {
//printf("** DifVelocity_SSI_DEM\n");
	...
	float3 n = dist3 / length(dist3);					//unit vector B to A
	float m_eff = (velMasA.w * velMasB.w) / (velMasA.w + velMasB.w);
	float3 force = 	(/*pow(sizeScale, 3) * */kS * l - 
					kD * m_eff * dot( F3 (velMasA - velMasB) , n )) * n;		//relative velocity at contact is simply assumed as the relative vel of the centers. If you are updating the rotation, this should be modified.
...
__device__
float3 deltaVShare(int3    gridPos,
                   uint    index,
                   float4  posRadA,
                   float4  velMasA,
				   float4  rhoPresMuA,
...
// collide a particle against all other particles in a given cell
__device__
float4 collideCell(int3    gridPos,
                   uint    index,
                   float4  posRadA,
                   float4  velMasA,
				   float3  vel_XSPH_A,
            ...
            if (j != index) {              // check not colliding with self
				...
				if (rhoPresMuA.w < -.1) {			//# A_fluid				** -1:			i.e. negative, i.e. fluid particle
					...
					if (rhoPresMuB.w < -0.1) {				//## A_Solid : B_fluid, sub rhoPresMuA with rhoPresMuB (except pressure, i.e. rhoPresMuA.y)
						...
						if ( fabs(rhoPresMuA.w - rhoPresMuB.w) > .1 ) {		//i.e collision with the other object, or boundary
...
// collide a particle against all other particles in a given cell
__device__
float collideCellDensityReInit_F1(int3    gridPos,
                   uint    index,
                   float4  posRadA,
                   float4* sortedPosRad, 
                   float4* sortedVelMas,
	...
	//if (gridPos.y == 7) {
	//	uint a = calcGridHash(I3(gridPos.x, 0, gridPos.z));
	//	printf("gridPos.y %d, hashOriginal %d, hashOtherSide %d, startInd %d, endInd %d\n", gridPos.y, gridHash, a, startIndex, endIndex);
	//}
	//if (gridPos.y == -1) {
	//	uint a = calcGridHash(I3(gridPos.x, 6, gridPos.z));
	//	printf("gridPos.y %d, hashOriginal %d, hashOtherSide %d, startInd %d, endInd %d\n", gridPos.y, gridHash, a, startIndex, endIndex);
	//}
	return densityShare;	
}
...
void calcOnCartesianShare(
    ...
    uint gridHash = calcGridHash(gridPos);
    // get start of bucket for this cell
	//float4 v_share = F4(0.0f);
	//float4 rp_share = F4(0.0f);
	...
	//vel_share = v_share;
	//rho_pres_share = rp_share;
}
//--------------------------------------------------------------------------------------------------------------------------------
// collide a particle against all other particles in a given cell
__device__
float2 collideCellDensityReInit_F2(int3    gridPos,
                   uint    index,
                   float4  posRadA,
                   float4* sortedPosRad, 
                   float4* sortedVelMas,
    ...
    for(int z=-1; z<=1; z++) {
        for(int y=-1; y<=1; y++) {
            for(int x=-1; x<=1; x++) {
                ...
                deltaV += deltaVShare(neighbourPos, index, posRadA, velMasA, rhoPreMuA, sortedPosRad, sortedVelMas, sortedRhoPreMu, cellStart, cellEnd);
            }
        }
    }
...
void collideD(float4* derivVelRhoD,               // output: new velocity
    ...
    for(int z=-1; z<=1; z++) {
        for(int y=-1; y<=1; y++) {
            for(int x=-1; x<=1; x++) {
                int3 neighbourPos = gridPos + I3(x, y, z);
                derivVelRho += collideCell(neighbourPos, index, posRadA, velMasA, vel_XSPH_A, rhoPreMuA, sortedPosRad, sortedVelMas, vel_XSPH_D, sortedRhoPreMu, cellStart, cellEnd, gridParticleIndex);
            }
        }
    }
    // write new velocity back to original unsorted location
...
__global__
void ReCalcDensityD_F1(float4* oldPosRad,
					float4* oldVelMas,
					float4* oldRhoPreMu,
					float4* sortedPosRad,
					float4* sortedVelMas,
    ...
    for(int z=-1; z<=1; z++) {
        for(int y=-1; y<=1; y++) {
            for(int x=-1; x<=1; x++) {
                int3 neighbourPos = gridPos + I3(x, y, z);
                densityShare += collideCellDensityReInit_F1(neighbourPos, index, posRadA, sortedPosRad, sortedVelMas, sortedRhoPreMu, cellStart, cellEnd);
            }
        }
	}
    // write new velocity back to original unsorted location
...
//with normalization
__global__
void ReCalcDensityD_F2(float4* oldPosRad,
					float4* oldVelMas,
					float4* oldRhoPreMu,
					float4* sortedPosRad,
					float4* sortedVelMas,
    ...
    for(int z=-1; z<=1; z++) {
        for(int y=-1; y<=1; y++) {
            for(int x=-1; x<=1; x++) {
                int3 neighbourPos = gridPos + I3(x, y, z);
                densityShare += collideCellDensityReInit_F2(neighbourPos, index, posRadA, sortedPosRad, sortedVelMas, sortedRhoPreMu, cellStart, cellEnd);
            }
        }
	}
    // write new velocity back to original unsorted location
	...
	if (rhoPreMuA.w < -.1) { rhoPreMuA.x = newDensity.x / newDensity.y; }
	rhoPreMuA.y = Eos(rhoPreMuA.x, rhoPreMuA.w);
    oldRhoPreMu[originalIndex] = rhoPreMuA;
	//printf("dens  kk %f\n", rhoPreMuA.x);
}
...
//without normalization
__global__
void CalcCartesianDataD(float4* rho_Pres_CartD,
    ...
    for(int z=-1; z<=1; z++) {
        for(int y=-1; y<=1; y++) {
            for(int x=-1; x<=1; x++) {
                int3 neighbourPos = gridPos + I3(x, y, z);
                calcOnCartesianShare(vel_share, rho_pres_share, neighbourPos, F4(gridNodePos3), sortedPosRad, sortedVelMas, sortedRhoPreMu, cellStart, cellEnd);
            }
        }
	}
    // write new velocity back to original unsorted location
...
//--------------------------------------------------------------------------------------------------------------------------------
void collide(float4* derivVelRhoD,
             float4* sortedPosRad,
			 float4* sortedVelMas,
			 float3* vel_XSPH_D,
			 float4* sortedRhoPreMu,
...
//--------------------------------------------------------------------------------------------------------------------------------
void ReCalcDensity(float4* oldPosRad,
			 float4* oldVelMas,
			 float4* oldRhoPreMu,
             float4* sortedPosRad,
			 float4* sortedVelMas,
