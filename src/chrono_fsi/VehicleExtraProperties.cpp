...
MyDriverInputs::MyDriverInputs(double delay) :
		m_delay(delay) {
}
void MyDriverInputs::onCallback(double time, double& throttle, double& steering,
		double& braking) {
	throttle = 0;
	steering = 0;
	braking = 0;
...
void MyCylindricalTire::onCallback(ChSharedPtr<ChBody> wheelBody, double radius,
		double width) {
...
void MyLuggedTire::onCallback(ChSharedPtr<ChBody> wheelBody, double radius,
		double width) {
	wheelBody->ChangeCollisionModel(new collision::ChCollisionModelParallel);
...
void MyChassisBoxModel_vis::onCallback(ChSharedPtr<ChBodyAuxRef> chassisBody) {
	...
	chassisBody->GetCollisionModel()->SetFamilyMaskNoCollisionWithFamily(
			tireFam);
...
void MyChassisBoxModel_vis::SetAttributes(const ChVector<>& otherSize,
		const ChQuaternion<>& otherRot, const ChVector<>& otherLoc) {
	size = otherSize;
	rot = otherRot;
	loc = otherLoc;
}
...
// In addition, this version overrides the visualization assets of the provided
// chassis body with the collision meshes.
void MyChassisSphereModel_vis::onCallback(
		ChSharedPtr<ChBodyAuxRef> chassisBody) {
	// Clear any existing assets (will be overriden)
...
MyChassisSimpleConvexMesh::MyChassisSimpleConvexMesh() :
	...
	utils::LoadConvexMesh(vehicle::GetDataFile(chassis_obj_file), chassis_mesh,
			chassis_convex);
}

void MyChassisSimpleConvexMesh::onCallback(
		ChSharedPtr<ChBodyAuxRef> chassisBody) {
	chassisBody->ChangeCollisionModel(new collision::ChCollisionModelParallel);
	...
	for (int c = 0; c < hull_count; c++) {
		...
		((collision::ChCollisionModelParallel*) body->GetCollisionModel())->AddConvexHull(
				convexhull, chLoc, rot);
	}
	chassisBody->GetCollisionModel()->SetFamily(chassisFam);
...
void MyChassisSimpleTriMesh_vis::onCallback(
		ChSharedPtr<ChBodyAuxRef> chassisBody) {
	chassisBody->ChangeCollisionModel(new collision::ChCollisionModelParallel);
