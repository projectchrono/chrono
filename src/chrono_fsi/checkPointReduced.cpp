...
//	printf("Job was submittet at date/time: %s\n", asctime(timeinfo));
	...
	for (int i = 0; i < referenceArray.size(); i++) {
		...
		outRefArray << ref4.x << "," << ref4.y << "," << ref4.z << ref4.w << ","
				<< endl;
	}
	...
	outProbParams << paramsH.gridSize.x << ", " << paramsH.gridSize.y << ", "
			<< paramsH.gridSize.z << endl;
	outProbParams << paramsH.worldOrigin.x << ", " << paramsH.worldOrigin.y
			<< ", " << paramsH.worldOrigin.z << endl;
	outProbParams << paramsH.cellSize.x << ", " << paramsH.cellSize.y << ", "
			<< paramsH.cellSize.z << endl;
	outProbParams << paramsH.numBodies << endl;
	outProbParams << paramsH.boxDims.x << ", " << paramsH.boxDims.y << ", "
			<< paramsH.boxDims.z << endl;
	...
	outProbParams << paramsH.deltaPress.x << ", " << paramsH.deltaPress.y
			<< ", " << paramsH.deltaPress.z << endl;
	outProbParams << paramsH.nPeriod << endl;
	outProbParams << paramsH.gravity.x << ", " << paramsH.gravity.y << ", "
			<< paramsH.gravity.z << endl;
	...
	outProbParams << paramsH.cMin.x << ", " << paramsH.cMin.y << ", "
			<< paramsH.cMin.z << endl;
	outProbParams << paramsH.cMax.x << ", " << paramsH.cMax.y << ", "
			<< paramsH.cMax.z << endl;
	outProbParams << paramsH.straightChannelBoundaryMin.x << ", "
			<< paramsH.straightChannelBoundaryMin.y << ", "
			<< paramsH.straightChannelBoundaryMin.z << endl;
	outProbParams << paramsH.straightChannelBoundaryMax.x << ", "
			<< paramsH.straightChannelBoundaryMax.y << ", "
			<< paramsH.straightChannelBoundaryMax.z << endl;
	outProbParams << paramsH.binSize0 << endl;
	outProbParams << paramsH.rigidRadius.x << ", " << paramsH.rigidRadius.y
			<< ", " << paramsH.rigidRadius.z << endl;
...
void CheckPointMarkers_Read(bool shouldIRead,
	...
	if (!inMarker) {
		cout << "Error! Unable to open file: " << "checkPointMarkersData.txt"
				<< endl;
	}
	char ch = '!';
	while (ch != '#') {
	...
	if (!inRefArray) {
		cout << "Error! Unable to open file: " << "checkPointRefrenceArrays.txt"
				<< endl;
	}
	ch = '!';
	while (ch != '#') {
	...
	while (inRefArray.good()) {
		...
		referenceArray.push_back(
				mI4(atoi(s1.c_str()), atoi(s2.c_str()), atoi(s3.c_str()),
						atoi(s4.c_str())));
	}
	inRefArray.close();
	...
	if (!inProbParams) {
		cout << "Error! Unable to open file: " << "checkPointParameters.txt"
				<< endl;
	}
	ch = '!';
	while (ch != '#') {
	...
	inProbParams >> paramsH.gridSize.x >> ddCh >> paramsH.gridSize.y >> ddCh
			>> paramsH.gridSize.z;
	inProbParams >> paramsH.worldOrigin.x >> ddCh >> paramsH.worldOrigin.y
			>> ddCh >> paramsH.worldOrigin.z;
	inProbParams >> paramsH.cellSize.x >> ddCh >> paramsH.cellSize.y >> ddCh
			>> paramsH.cellSize.z;
	inProbParams >> paramsH.numBodies;
	inProbParams >> paramsH.boxDims.x >> ddCh >> paramsH.boxDims.y >> ddCh
			>> paramsH.boxDims.z;
	...
	inProbParams >> paramsH.deltaPress.x >> ddCh >> paramsH.deltaPress.y >> ddCh
			>> paramsH.deltaPress.z;
	inProbParams >> paramsH.nPeriod;
	inProbParams >> paramsH.gravity.x >> ddCh >> paramsH.gravity.y >> ddCh
			>> paramsH.gravity.z;
	...
	inProbParams >> paramsH.cMin.x >> ddCh >> paramsH.cMin.y >> ddCh
			>> paramsH.cMin.z;
	inProbParams >> paramsH.cMax.x >> ddCh >> paramsH.cMax.y >> ddCh
			>> paramsH.cMax.z;
	inProbParams >> paramsH.straightChannelBoundaryMin.x >> ddCh
			>> paramsH.straightChannelBoundaryMin.y >> ddCh
			>> paramsH.straightChannelBoundaryMin.z;
	inProbParams >> paramsH.straightChannelBoundaryMax.x >> ddCh
			>> paramsH.straightChannelBoundaryMax.y >> ddCh
			>> paramsH.straightChannelBoundaryMax.z;
	inProbParams >> paramsH.binSize0;
	inProbParams >> paramsH.rigidRadius.x >> ddCh >> paramsH.rigidRadius.y
			>> ddCh >> paramsH.rigidRadius.z;
