...
int test_iso2631_1_wk()
{
    ...
    double expect_wk_data[] = { 31.2e-3, 48.6e-3,   79e-3,  121e-3,  182e-3, 
                                 263e-3,  352e-3,  418e-3,  459e-3,  477e-3, 
                                 482e-3,  484e-3,  494e-3,  531e-3,  631e-3,
                                 804e-3,  967e-3, 1039e-3, 1054e-3, 1036e-3, 
                                 988e-3,  902e-3,   768e-3,  636e-3, 513e-3,
                                 405e-3,  314e-3,   246e-3,  186e-3, 132e-3,
                                88.7e-3,   54e-3,  28.5e-3, 15.2e-3, 7.9e-3,
                                3.98e-3, 1.95e-3 };
    ...
    for(size_t i=0; i<nFreqWk; i++) {
        plt << tst_freq_wk[i] << "\t" << expect_wk_upper[i] << "\t" << rms_wk[i] << "\t" << expect_wk_lower[i] << std::endl;
    }
    plt << "EOD" << std::endl;
...
int test_iso2631_1_wd()
{
    ...
    double expect_wd_data[] = { 62.4e-3, 97.3e-3,  158e-3,  243e-3,  365e-3,  530e-3,
                                 713e-3,  853e-3,  944e-3,  992e-3, 1011e-3, 1008e-3,
                                 968e-3,  890e-3,  776e-3,  642e-3,  512e-3,  409e-3,
                                 323e-3,  253e-3,  202e-3,  161e-3,  125e-3,  100e-3,
                                  80e-3, 63.2e-3, 49.4e-3, 38.8e-3, 29.5e-3,  21.1e-3,
                                14.1e-3, 8.63e-3, 4.55e-3, 2.43e-3, 1.26e-3,  0.64e-3,
                                0.31e-3
                                }; 
   ...
   ChISO2631_1_Wd wd(dT);
    ...
    for(size_t i=0; i<nFreqWd; i++) {
        plt << tst_freq_wd[i] << "\t" << expect_wd_upper[i] << "\t" << rms_wd[i] << "\t" << expect_wd_lower[i] << std::endl;
    }
    plt << "EOD" << std::endl;
...
int test_iso2631_1_wf()
{
    ...
    double expect_wf_data[] = { 24.2e-3, 37.7e-3, 57.7e-3, 97.1e-3,  157e-3,   267e-3,
                                 461e-3,  695e-3,  895e-3, 1006e-3,  992e-3,   854e-3,
                                 619e-3,  384e-3,  224e-3,  116e-3,   53e-3,  23.5e-3, 
                                9.98e-3, 3.77e-3, 1.55e-3, 0.64e-3, 0.25e-3, 0.097e-3
                                }; 
   ...
   ChISO2631_1_Wf wf(dT);
    ...
    for(size_t i=0; i<nFreqWf; i++) {
        plt << tst_freq_wf[i] << "\t" << expect_wf_upper[i] << "\t" << rms_wf[i] << "\t" << expect_wf_lower[i] << std::endl;
    }
    plt << "EOD" << std::endl;
   ...
   // Correction of the first and the last values of expect_wd_lower to zero (it could not be plotted in logscale!)
    ...
    for(size_t i=0; i<nFreqWf; i++) {
        if(rms_wf[i] < expect_wf_lower[i] || rms_wf[i] > expect_wf_upper[i]) {
            if(rms_wf[i] > 0.01) {
                n_wf_errors++;
                std::cout << "Wf: Out of range at f = " << tst_freq_wf[i] << " Hz, Weighting = " << rms_wf[i] << std::endl;
            } else {
                std::cout << "Wf: Acceptable deviation at f = " << tst_freq_wf[i] << " Hz, Weighting = " << rms_wf[i] << std::endl;                
            }
        }
    }
