...
#include "ChRepresentedVolumeElement.h"
...
void VisualizeSurfaceMesh(const std::shared_ptr<ChMeshSurface>& surfaceMesh, const std::string& filename) {
    ...
    for (const auto& face : faces) {
        if (face) {
			auto Lface=std::dynamic_pointer_cast<ChLDPMFace>(face);
...
bool TieNodeToTriFace(ChSystem& sys, std::shared_ptr<ChMeshSurface> m_surface, std::shared_ptr<ChNodeFEAxyzrot> m_node,  double OffsetVal, double max_dist=0.0001) {    
    ...
    for (const auto& face : m_surface->GetFaces()) {  
		auto face_tetra = std::dynamic_pointer_cast<ChLDPMFace>(face);		
        if ( face_tetra ) {
            double val, u, v, w;
            bool is_into;
...
#pragma omp parallel for        
        for (int i = 0; i < mmesh->GetNumElements(); ++i) {
            ...
            if ( auto elem = std::dynamic_pointer_cast<ChElementLDPM>(mmesh->GetElement(i)) ) {				
            	elem->macro_strain = macro_strain;	
				//elem->mprojection_matrix=elem->ComputeProjectionMatrix();
            ...
            //}else if ( auto elem = std::dynamic_pointer_cast<ChElementCSL>(mmesh->GetElement(i)) ){
			//	elem->macro_strain = macro_strain;
			//}
            ...
            //}else if( auto elem = std::dynamic_pointer_cast<ChElementCBLCON>(mmesh->GetElement(i)) ){
			//	elem->macro_strain = macro_strain;
			//}
            ...
            //}else if ( auto elem = std::dynamic_pointer_cast<ChElementCurvilinearBeamBezier>(mmesh->GetElement(i)) ){
			//	elem->macro_strain = macro_strain;
			//}
...
void ChRepresentedVolumeElement::ApplyPeriodicBoundaryConditions() {   
	...
	for (size_t i = 0; i < CubEdges.size(); ++i) {
	    ...
	    for (size_t ind=0; ind< mmesh->GetNumNodes(); ind++) {
	    	...
	    	double val = utils::PointLineDistance(pN, p0, p1, u, is_insegment);
    ...
    //
    auto tetsurface_L = chrono_types::make_shared<ChMeshSurfaceLDPM>();
    mmesh->AddMeshSurface(tetsurface_L);
    //
    /// Front surface normal is -z directions
    //
    auto tetsurface_BACK = chrono_types::make_shared<ChMeshSurfaceLDPM>();
    mmesh->AddMeshSurface(tetsurface_BACK);
    //
    /// Bottom surface normal is -y directions
    //
    auto tetsurface_BTTM = chrono_types::make_shared<ChMeshSurfaceLDPM>();
...
#pragma omp parallel for 
        for (auto elem : mmesh->GetElements()) {
			...
			//std::shared_ptr<ChElementRVE<ChElementLDPM>> elem_ptr = std::dynamic_pointer_cast<ChElementRVE<ChElementLDPM>>(elem);
            if ( auto elem_ptr = std::dynamic_pointer_cast<ChElementLDPM>(elem) ) {                
				#pragma omp critical
				(*macro_stress) += elem_ptr->ComputeMacroStressContribution();
			//
            // Connector ElEMENTS
            //
            //}else if( auto elem_ptr = std::dynamic_pointer_cast<ChElementCBLCON>(elem) ) {
			//	#pragma omp critical
			//	(*macro_stress) += elem_ptr->ComputeMacroStressContribution();	
			//
            // Bezier Beam ElEMENTS
            //
			//}else if ( auto elem_ptr = std::dynamic_pointer_cast<ChElementCurvilinearBeamBezier>(elem) ){
			//	#pragma omp critical
			//	(*macro_stress) += elem_ptr->ComputeMacroStressContribution();	
            ...
            // to do
				...
				throw std::runtime_error("There are RVE incompatible elements in mesh");
			}
