...
int main(int argc, char* argv[])
{
  ...
  //Case 1 - Universal Joint at the origin, and aligned with the global frame
  //  Note the universal joint only allows 2 DOF(rotation about joint x & y axes)

  test_name = "Universal_Case01";
  TestUniversal(ChVector<>(0, 0, 0), QUNIT, sim_step, out_step, test_name, animate);
  if (!animate) {
  ...
  //Case 2 - Universal Joint at (1,2,3), and joint z aligned with the global axis along Y = Z
  //  Note the universal joint only allows 2 DOF(rotation about joint x & y axes)
  //    Therefore, the joint must be rotated -pi/4 about the global x-axis

  test_name = "Universal_Case02";
  TestUniversal(ChVector<>(1, 2, 3), Q_from_AngX(-CH_C_PI_4), sim_step, out_step, test_name, animate);
  //TestUniversal(ChVector<>(0, 0, 0), Q_from_AngX(-CH_C_PI_4), sim_step, out_step, test_name, animate);
  if (!animate) {
    test_passed &= ValidateReference(test_name, "Pos", 2e-3);
    test_passed &= ValidateReference(test_name, "Vel", 1e-4);
    test_passed &= ValidateReference(test_name, "Acc", 2e-2);
  ...
  TestUniversal(ChVector<>(0, 0, 0), Q_from_AngX(-CH_C_PI_2), sim_step, out_step, test_name, animate);
  if (!animate) {
    test_passed &= ValidateReference(test_name, "Pos", 2e-3);
    test_passed &= ValidateReference(test_name, "Vel", 1e-4);
    test_passed &= ValidateReference(test_name, "Acc", 2e-2);
                  ...
                  bool                  animate)          // if true, animate with Irrlich
{
  ...
  bool AxisInLineWithPendulum = true;
  ...
  if(AxisInLineWithPendulum)
  {
    cyl_g->GetCylinderGeometry().p1 = jointLoc + jointRot.Rotate(ChVector<>(0, -0.4, 0));
    cyl_g->GetCylinderGeometry().p2 = jointLoc + jointRot.Rotate(ChVector<>(0, 0.4, 0));
  }
  else
  {
  ...
  mvisual->SetColor(ChColor(1.0f,1.0f,0.0f));
  ground->AddAsset(mvisual);
  ...
  //pendulum->SetPos(jointLoc + jointRot.Rotate(ChVector<>(length / 2, 0, 0.05)));
  ...
  if(AxisInLineWithPendulum)
  {
    cyl_p2->GetCylinderGeometry().p1 = ChVector<>(-length / 2 - 0.4, 0, 0);
    cyl_p2->GetCylinderGeometry().p2 = ChVector<>(-length / 2 + 0.4, 0, 0);
  }
  else
  {
    cyl_p2->GetCylinderGeometry().p1 = ChVector<>(-length / 2, -0.4, 0);
    cyl_p2->GetCylinderGeometry().p2 = ChVector<>(-length / 2, 0.4, 0);
  }
  cyl_p2->GetCylinderGeometry().rad = 0.05;
  pendulum->AddAsset(cyl_p2);
  ChSharedPtr<ChColorAsset> mvisualp(new ChColorAsset);
  mvisualp->SetColor(ChColor(1.0f,0.0f,0.0f));
  pendulum->AddAsset(mvisualp);
  ...
  //ChSharedPtr<ChLinkLockHook>  universalJoint(new ChLinkLockHook);
  ChSharedPtr<ChLinkLockUniversal>  universalJoint(new ChLinkLockUniversal);
  universalJoint->Initialize(pendulum, ground, ChCoordsys<>(jointLoc, jointRot));
  //universalJoint->Initialize(ground,pendulum, ChCoordsys<>(jointLoc, jointRot));
...
//// =============================================================================
//
//void TestUniversal(const ChVector<>&     loc,
...
//  cyl2_g->GetCylinderGeometry().p1 = loc + revAxisRot.Rotate(ChVector<>(0, -0.2, 0));
//  cyl2_g->GetCylinderGeometry().p2 = loc + revAxisRot.Rotate(ChVector<>(0, 0.2, 0));
...
//  ChSharedPtr<ChLinkLockUniversal>  universalJoint(new ChLinkLockUniversal);
//  universalJoint->Initialize(pendulum, ground, ChCoordsys<>(loc, revAxisRot));
//  my_system.AddLink(universalJoint);
//
//
//  // Create the Irrlicht application for visualization
...
//      // the coordinate system of Body2 (in our case this is the ground).
//      ChCoordsys<> linkCoordsys = universalJoint->GetLinkRelativeCoords();
//      ChVector<double> reactForce = universalJoint->Get_react_force();
...
//
//      ChVector<double> reactTorque = universalJoint->Get_react_torque();
...
//    return 1;
//  }
//  if (ChFileutils::MakeDirectory("../VALIDATION/UNIVERSAL_JOINT") < 0) {
//    std::cout << "Error creating directory '../VALIDATION/UNIVERSAL_JOINT'" << std::endl;
//    return 1;
//  }
//
...
//
//
//  std::cout << "\nStarting Universal Test Case 01\n\n";
//  //Case 1 - Universal Joint at the origin, and aligned with the global coordinate system
//  //  Note the universal joint only allows 2 DOF(rotation about joint x & y axes)
//  TestUniversal(ChVector<> (0, 0, 0), ChQuaternion<> (Q_from_AngX(0)), .001, "UniversalJointData_Case01.txt",animate);
//
//  std::cout << "\nStarting Universal Test Case 02\n\n";
//  //Case 2 - Universal Joint at (1,2,3), and joint z aligned with the global axis along Y = Z
//  //  Note the universal joint only allows 2 DOF(rotation about joint x & y axes)
//  //    Therefore, the joint must be rotated -pi/4 about the global x-axis
//  TestUniversal(ChVector<> (1, 2, 3), ChQuaternion<> (Q_from_AngX(-CH_C_PI_4)), .001, "UniversalJointData_Case02.txt",animate);
//
//
