...
// =============================================================================
//
// Test for the translational spring damper
//
...
int main(int argc, char* argv[])
{
  ...
  TestTranSpring(ChVector<>(0, 0, 0),ChVector<>(0, 2, 0), ChCoordsys<>(ChVector<>(0, 2, 0),QUNIT), 1, sim_step, out_step, test_name, animate, save);
  if (!animate) {
  ...
  TestTranSpring(ChVector<>(1, 2, 3),ChVector<>(1, 4, 3), ChCoordsys<>(ChVector<>(-1, 4, 3),QUNIT), 2, sim_step, out_step, test_name, animate, save);
  if (!animate) {
    //test_passed &= ValidateReference(test_name, "Pos", 1e-3);
  ...
  TestTranSpring(ChVector<>(0, 0, 0),ChVector<>(0, 2, 0), ChCoordsys<>(ChVector<>(0, 4, 0),Q_from_AngZ(-CH_C_PI_2)), 2, sim_step, out_step, test_name, animate, save);
  if (!animate) {
    //test_passed &= ValidateReference(test_name, "Pos", 1e-3);
    //test_passed &= ValidateReference(test_name, "Vel", 1e-5);
    //test_passed &= ValidateReference(test_name, "Acc", 2e-2);
                  ...
                  bool                  save)             // if true, also save animation data
{
  ...
  while (simTime <= timeRecord + simTimeStep / 2)
  {
    ...
    if (simTime >= outTime - simTimeStep / 2)
    {
      ...
      ChVector<> reactForce = spring->Get_SpringReact();
      ChVector<> reactForceGlobal = reactForce*springVector;
