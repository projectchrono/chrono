...
CH_SENSOR_API void ChFilterTachometerUpdate::Apply() {
    if (m_tachSensor->m_axis == X){
        m_bufferOut->Buffer[0].rpm = m_tachSensor->m_parent->GetAngVelLocal().x() * 60 / 2 / CH_C_PI;
//        printf("tachomter x axis\n");
    } else if (m_tachSensor->m_axis == Y){
        m_bufferOut->Buffer[0].rpm = m_tachSensor->m_parent->GetAngVelLocal().y() * 60 / 2 / CH_C_PI;
//        printf("tachomter y axis\n");
    } else if (m_tachSensor->m_axis == Z){
        m_bufferOut->Buffer[0].rpm = m_tachSensor->m_parent->GetAngVelLocal().z() * 60 / 2 / CH_C_PI;
