...
class CH_SENSOR_API ChFilterDepthToRGBA8 : public ChFilter {
  ...
  public:
    ...
    /// @brief Initializes all data needed by the filter access apply function.
    /// @param pSensor A pointer to the sensor on which the filter is attached.
    /// @param bufferInOut A buffer that is passed into the filter.
    virtual void Initialize(std::shared_ptr<ChSensor> pSensor, std::shared_ptr<SensorBuffer>& bufferInOut);
...
class CH_SENSOR_API ChFilterNormalToRGBA8 : public ChFilter {
  ...
  public:
    ...
    /// @brief Apply function. Applies the normal to RGBA8 conversion to the image.
    virtual void Apply();
  ...
  private:
    std::shared_ptr<SensorDeviceNormalBuffer> m_buffer_in;  ///< holder of the input normal map
    std::shared_ptr<SensorDeviceRGBA8Buffer> m_buffer_out;  ///< holder of the output RGBA8 image
    CUstream m_cuda_stream;                                 ///< reference to the cuda stream
};
