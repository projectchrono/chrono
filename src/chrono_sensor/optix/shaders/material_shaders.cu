        ...
        for (int i = 0; i < params.num_arealights; i++) {
            ...
            if (dist_to_light < 2 * a.max_range) {
                ...
                for (int lightSampleID = 0; lightSampleID < 5; lightSampleID++) {
                    float3 tempPos =
                        a.pos + (curand_uniform(&prd_camera->rng) * a.du) + (curand_uniform(&prd_camera->rng) * a.dv);
                    ...
                    if (NdL > 0.0f) {
                        ...
                        // TODO: Re-implement this multiple times with slightly different dir_to_light values to improve
                        // data sampling
                        ...
                        float point_light_falloff =
                            (a.max_range * a.max_range / (dist_to_light * dist_to_light + a.max_range * a.max_range));
...
extern "C" __global__ void __closesthit__material_shader() {
    ...
    switch (raytype) {
        case CAMERA_RAY_TYPE:
            ...
            // printf("Use Hapke: %d | Emissive Power: %.2f | Material ID: %d\n", mat.use_hapke, mat.emissive_power,
            // material_id);
            if (!mat.use_hapke) {
                CameraShader(getCameraPRD(), mat_params, material_id, mat_params->num_blended_materials, world_normal,
