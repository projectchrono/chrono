...
static __device__ __inline__ bool CheckVisibleAndSampleSpotLight(const ContextParameters& cntxt_params,
    ...
    optixTrace(
        cntxt_params.root,           // The scene traversable handle (OptixTraversableHandle); basically the top-level
        ...
        0,  // SBT offset (used when you have multiple SBT records for the same ray type). It selects the first “ray
            // type slot”
        1,  // SBT stride (used when you have multiple SBT records for the same ray type). It usually means “one ray
            // type stride”
    ...
    // Caculate the remaining attributes of light sample
        ...
        float dist_intense_amount =
            light_data.atten_scale / (light_sample.dist * light_sample.dist);  // inverse square law
        // Angle attenuation. If angle_atten_rate < 0, no angle attenuation, else linear attenuation
        float angle_intense_amount =
            (light_data.angle_atten_rate < 0)
                ? 1.f
                : clamp(
                      // (cosf(angle) - cosf(0.5f * light_data.angle_range)) / (cosf(0.5f *
                      // light_data.angle_falloff_start) - cosf(0.5f * light_data.angle_range)), // another cosine-based
                      // attenuation formula
                      light_data.angle_atten_rate * (light_data.angle_range - 2.f * angle), 0.f, 1.f);
        angle_intense_amount *= angle_intense_amount;  // square the angle attenuation
        // float angle_intense_amount = cosf(clamp((0.5f * light_data.angle_range - angle) / (0.5f *
        // light_data.angle_range - 0.5f * light_data.angle_falloff_start), 0.f, 1.f)); angle_intense_amount =
        // (angle_intense_amount < 0) ? 1.f : angle_intense_amount;
        ...
        // light_sample.L = light_sample.NdL * light_data.color * (light_data.max_range * light_data.max_range /
        // (light_sample.dist * light_sample.dist + light_data.max_range * light_data.max_range));
