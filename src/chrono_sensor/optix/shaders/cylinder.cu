...
__device__ __inline__ void check_ends(const float3& ray_orig,
    ...
    float t = (.5 - ray_orig.y) / ray_dir.y;
    float3 p = ray_orig + ray_dir * t;

    if (p.x * p.x + p.z * p.z - 1 < 0 && t > ray_tmin && t < ray_tmax) {
        float3 shading_normal = make_float3(0, 1, 0);
        float3 tangent_vector = make_float3(-1, 0, 0);
        float2 texcoord = make_float2(-p.x / 2.f + .5f, -p.z / 2.f + .5f);
...
extern "C" __global__ void __intersection__cylinder_intersect() {
    ...
    float c = ray_orig.x * ray_orig.x + ray_orig.z * ray_orig.z - 1;
    float det = b * b - 4 * a * c;
    ...
    if (det > 0) {
        ...
        if (dist_near <= dist_far) {
            ...
            if (dist_near > ray_tmin && dist_near < ray_tmax && p_near.y < .5 && p_near.y > -.5) {
                float3 shading_normal = p_near - make_float3(0, p_near.y, 0);
                float3 tangent_vector = make_float3(p_near.z, 0, -p_near.x);
                float2 texcoord = make_float2(atan2(p_near.x, p_near.z) / (2 * CUDART_PI_F), p_near.y * 0.5 + 0.5);
                optixReportIntersection(dist_near,                                          //
                                        ...
                                        reinterpret_cast<unsigned int&>(tangent_vector.z));
            } else if (dist_far > ray_tmin && dist_far < ray_tmax && p_far.y < .5 && p_far.y > -.5) {
                float3 shading_normal = p_far - make_float3(0, p_far.y, 0);
                float3 tangent_vector = make_float3(p_far.z, 0, -p_far.x);
                float2 texcoord = make_float2(atan2(p_far.x, p_far.z) / (2 * CUDART_PI_F), p_far.y * 0.5 + 0.5);
