...
/// @param light_data point light data
/// @param light_posi position of the point light
...
static __device__ __inline__ bool CheckVisibleAndSamplePointLight(
	...
	// Caculate the remaining attributes of light sample
		...
		float geom_term = (light_data.const_color) ? 1.0f : (light_data.atten_scale / (light_sample.dist * light_sample.dist)); // inverse square law
		...
		light_sample.pdf = 1.f; // Delta light
		return true;
	}
}
