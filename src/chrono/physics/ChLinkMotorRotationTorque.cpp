...
void ChLinkMotorRotationTorque::IntLoadResidual_F(const unsigned int off, ChVectorDynamic<>& R, const double c) {
    ...
    double mT = m_func->GetVal(GetChTime());
    ...
    if (m_body2->Variables().IsActive()) {
        R.segment(m_body2->Variables().GetOffset() + 3, 3) -= c * m_body2->TransformDirectionParentToLocal(m_abs_torque).eigen();
    }

    if (m_body1->Variables().IsActive()) {
        R.segment(m_body1->Variables().GetOffset() + 3, 3) += c * m_body1->TransformDirectionParentToLocal(m_abs_torque).eigen();
    }
}
...
void ChLinkMotorRotationTorque::ConstraintsFbLoadForces(double factor) {
    ...
    m_body2->Variables().Force().segment(3, 3) -= factor * m_body2->TransformDirectionParentToLocal(m_abs_torque).eigen();
    m_body1->Variables().Force().segment(3, 3) += factor * m_body1->TransformDirectionParentToLocal(m_abs_torque).eigen();
}
