...
void ChProximityContainerMeshless::AddProximity(ChCollisionModel* modA, ChCollisionModel* modB) {
    ...
    if (lastproximity != proximitylist.end()) {
        ...
        (*lastproximity)->Reset(mnA, mnB);

        lastproximity++;
    } else {
        // add new proximity
        ChProximityMeshless* mp = new ChProximityMeshless(mnA, mnB);
...
void ChProximityContainerMeshless::ReportAllProximities(ReportProximityCallback* mcallback) {
    std::list<ChProximityMeshless*>::iterator iterproximity = proximitylist.begin();
    while (iterproximity != proximitylist.end()) {
        bool proceed = mcallback->OnReportProximity((*iterproximity)->GetNodeA()->GetCollisionModel().get(), (*iterproximity)->GetNodeB()->GetCollisionModel().get());
        if (!proceed)
            break;
        ++iterproximity;
    }
...
void ChProximityContainerMeshless::AccumulateStep1() {
    ...
    std::list<ChProximityMeshless*>::iterator iterproximity = proximitylist.begin();
    while (iterproximity != proximitylist.end()) {
        ChNodeMeshless* mnodeA = (*iterproximity)->GetNodeA();
        ChNodeMeshless* mnodeB = (*iterproximity)->GetNodeB();
