...
/// Class for translaptional spring-damper-actuator (TSDA) with the force optionally specified through a functor object
...
class ChApi ChLinkTSDA : public ChLink {
  public:
    ...
    void Set_SpringK(double k) { m_k = k; }

    /// Set spring damping coefficient (default: 0).
    /// Used only if no force functor is provided.
    void Set_SpringR(double r) { m_r = r; }

    /// Set constant actuation force (default: 0).
    /// Used only if no force functor is provided.
    void Set_SpringF(double f) { m_f = f; }
    ...
    //// RADU: obsolete all these functions
    double GetSpringDeform() const { return m_length - m_rest_length; }
    double GetSpringLength() const { return m_length; }
    double GetSpringVelocity() const { return m_length_dt; }
    double GetSpringReact() const { return m_force; }
    double GetDist() const { return m_length; }
    double GetDist_dt() const { return m_length_dt; }

    /// Get the endpoint location on 1st body (expressed in body coordinate system)
    const ChVector<>& GetEndPoint1Rel() const { return m_loc1; }

    /// Get the endpoint location on 1st body (expressed in absolute coordinate system)
    const ChVector<>& GetEndPoint1Abs() const { return m_aloc1; }

    /// Get the endpoint location on 2nd body (expressed in body coordinate system)
    const ChVector<>& GetEndPoint2Rel() const { return m_loc2; }

    /// Get the endpoint location on 1st body (expressed in absolute coordinate system)
    const ChVector<>& GetEndPoint2Abs() const { return m_aloc2; }
