...
ChLinkLock::ChLinkLock()
    : type(LNK_SPHERICAL), relC(CSYSNORM), relC_dt(CSYSNULL),
      relC_dtdt(CSYSNULL), deltaC(CSYSNORM), deltaC_dt(CSYSNULL),
      deltaC_dtdt(CSYSNULL), motion_axis(VECT_Z),
...
void ChLinkLock::SetMotion_axis(Vector m_axis) { motion_axis = m_axis; }
...
void ChLinkLock::UpdateTime(double time) {
  ...
  if (limit_X->Get_active() || limit_Y->Get_active() || limit_Z->Get_active() ||
      limit_Rx->Get_active() || limit_Ry->Get_active() ||
      limit_Rz->Get_active())
    return;
  ...
  case ANGLESET_ANGLE_AXIS:
    ...
    if ((ang != 0) || (ang_dt != 0) || (ang_dtdt != 0)) {
      ...
      deltaC_dtdt.rot =
          Qdtdt_from_AngAxis(ang_dtdt, motion_axis, deltaC.rot, deltaC_dt.rot);
    } else {
      deltaC.rot = QUNIT;
      deltaC_dt.rot = QNULL;
...
void ChLinkLock::UpdateRelMarkerCoords() {
  ...
  PQw_dtdt =
      Vsub(marker1->GetAbsCoord_dtdt().pos, marker2->GetAbsCoord_dtdt().pos);
    ...
    q_AD = //  q'qqq + qqqq'
        Qadd(Qcross(Qconjugate(marker2->GetCoord_dt().rot),
                    Qcross(Qconjugate(marker2->GetBody()->GetCoord().rot),
                           Qcross((marker1->GetBody()->GetCoord().rot),
  ...
  q_BC = // qq'qq + qqq'q
      Qadd(Qcross(Qconjugate(marker2->GetCoord().rot),
           ...
           Qcross(Qconjugate(marker2->GetCoord().rot),
                  Qcross(Qconjugate(marker2->GetBody()->GetCoord().rot),
                         Qcross((marker1->GetBody()->GetCoord_dt().rot),
                                (marker1->GetCoord().rot)))));
  ...
  if (Qnotnull(temp2))
    q_8 = Qcross(
        Qconjugate(marker2->GetCoord_dtdt().rot),
        Qcross(
  ...
  if (Qnotnull(temp1)) {
    qtemp1 = Qcross(Qconjugate(marker2->GetCoord().rot),
                    Qcross(Qconjugate(Body2->GetCoord().rot),
                           Qcross(Body1->GetCoord().rot,
                                  marker1->GetCoord_dtdt()
                                      .rot))); // q'm2 * q'o2 * q,o1 * q_dtdt,m1
    q_8 = Qadd(q_8, qtemp1);
  }
  temp2 = marker2->GetCoord_dt().rot;
  if (Qnotnull(temp2)) {
    qtemp1 =
        Qcross(Qconjugate(marker2->GetCoord_dt().rot),
               Qcross(Qconjugate(Body2->GetCoord_dt().rot),
                      Qcross(Body1->GetCoord().rot, marker1->GetCoord().rot)));
  ...
  if (Qnotnull(temp2)) {
    qtemp1 = Qcross(
        ...
        Qcross(Qconjugate(Body2->GetCoord().rot),
               Qcross(Body1->GetCoord_dt().rot, marker1->GetCoord().rot)));
  ...
  if (Qnotnull(temp2) && Qnotnull(temp1)) {
    qtemp1 = Qcross(
        Qconjugate(marker2->GetCoord_dt().rot),
        Qcross(Qconjugate(Body2->GetCoord().rot),
               Qcross(Body1->GetCoord().rot, marker1->GetCoord_dt().rot)));
  ...
  if (Qnotnull(temp1)) {
    qtemp1 = Qcross(
        Qconjugate(marker2->GetCoord().rot),
        Qcross(Qconjugate(Body2->GetCoord().rot),
               Qcross(Body1->GetCoord_dt().rot, marker1->GetCoord_dt().rot)));
  ...
  // relM_dt.pos
  relM_dt.pos = Vadd(
      Vadd(m2_Rel_A_dt.MatrT_x_Vect(Body2->GetA().MatrT_x_Vect(PQw)),
  ...
  // relM_dtdt.pos
  relM_dtdt.pos = Vadd(
      Vadd(marker2->GetA().MatrT_x_Vect(Body2->GetA_dtdt().MatrT_x_Vect(PQw)),
...
void ChLinkLock::UpdateState() {
  ...
  ChMatrixNM<double, 3, 4>
      CqxR; // the 3x4 piece of Cq_temp for trasl. link,   rotat. coords,
  ChMatrixNM<double, 4, 4>
      CqrR;       // the 4x4 piece of Cq_temp for rotat..link,   rotat. coords,
  ...
  // relC_dt.rot
  relC_dt.rot = Qadd(Qcross(Qconjugate(deltaC_dt.rot), relM.rot),
  ...
  // relC_dtdt.rot
  relC_dtdt.rot =
      Qadd(Qadd(Qcross(Qconjugate(deltaC_dtdt.rot), relM.rot),
  ...
  Ct_temp.rot = // deltaC^*(q_AD) + deltaC_dt^*q_pq
      Qadd(Qcross(Qconjugate(deltaC.rot), q_AD),
  ...
  vtemp1 = Vcross(Body1->GetWvel_loc(),
                  Vcross(Body1->GetWvel_loc(), marker1->GetCoord().pos));
  vtemp1 = Vadd(vtemp1, marker1->GetCoord_dtdt().pos);
  vtemp1 =
      Vadd(vtemp1,
           Vmul(Vcross(Body1->GetWvel_loc(), marker1->GetCoord_dt().pos), 2));
  vtemp1 = Body1->GetA().Matr_x_Vect(vtemp1);

  vtemp2 = Vcross(Body2->GetWvel_loc(),
                  Vcross(Body2->GetWvel_loc(), marker2->GetCoord().pos));
  vtemp2 = Vadd(vtemp2, marker2->GetCoord_dtdt().pos);
  vtemp2 =
      Vadd(vtemp2,
           Vmul(Vcross(Body2->GetWvel_loc(), marker2->GetCoord_dt().pos), 2));
  ...
  Qcx = Vadd(
      Qcx,
      q_4); // [Adtdt]'[A]'q + 2[Adt]'[Adt]'q + 2[Adt]'[A]'qdt + 2[A]'[Adt]'qdt
  ...
  Qcr = Qadd(Qcr, Qcross(Qconjugate(deltaC_dtdt.rot),
                         relM.rot)); // = deltaC'*q_8 + 2*deltaC_dt'*q_dt,po +
                                     // deltaC_dtdt'*q,po
...
void ChLinkLock::UpdateForces(double mytime) {
  ...
  if (limit_D->Get_active()) {
    m_force =
        Vadd(m_force, Vmul(Vnorm(relM.pos), limit_D->GetForce(dist, dist_dt)));
  }
  ...
  if (limit_Rp->Get_active()) {
    ...
    arm_xaxis =
        VaxisXfromQuat(relM.rot); // the X axis of the marker1, respect to m2.
    zenith = VangleYZplaneNorm(
        &arm_xaxis); // the angle of m1 Xaxis about normal to YZ plane
    polar = VangleRX(
        &arm_xaxis); // the polar angle of m1 Xaxis spinning about m2 Xaxis
    ...
    zenithspeed =
        Vdot(torq_axis, relWvel); // the speed of zenith rotation toward cone.

    m_torque = Vadd(m_torque, Vmul(torq_axis, limit_Rp->GetPolarForce(
                                                  zenith, zenithspeed, polar)));
  }
...
void ChLinkLock::IntStateScatterReactions(const unsigned int off_L,
...
void ChLinkLock::IntStateGatherReactions(const unsigned int off_L,
                                         ChVectorDynamic<> &L) {
  // parent (from ChConstraint objects to react vector)
  ...
  if (limit_X && limit_X->Get_active()) {
    if (limit_X->constr_lower.IsActive()) {
      Qc(off_L + local_offset) +=
          ChMax(c * (-limit_X->Get_min() + relM.pos.x), -recovery_clamp);
      ++local_offset;
    }
    if (limit_X->constr_upper.IsActive()) {
      Qc(off_L + local_offset) +=
          ChMax(c * (limit_X->Get_max() - relM.pos.x), -recovery_clamp);
      ++local_offset;
    }
  }
  if (limit_Y && limit_Y->Get_active()) {
    if (limit_Y->constr_lower.IsActive()) {
      Qc(off_L + local_offset) +=
          ChMax(c * (-limit_Y->Get_min() + relM.pos.x), -recovery_clamp);
      ++local_offset;
    }
    if (limit_Y->constr_upper.IsActive()) {
      Qc(off_L + local_offset) +=
          ChMax(c * (limit_Y->Get_max() - relM.pos.x), -recovery_clamp);
      ++local_offset;
    }
  }
  if (limit_Z && limit_Z->Get_active()) {
    if (limit_Z->constr_lower.IsActive()) {
      Qc(off_L + local_offset) +=
          ChMax(c * (-limit_Z->Get_min() + relM.pos.x), -recovery_clamp);
      ++local_offset;
    }
    if (limit_Z->constr_upper.IsActive()) {
      Qc(off_L + local_offset) +=
          ChMax(c * (limit_Z->Get_max() - relM.pos.x), -recovery_clamp);
      ++local_offset;
    }
  }
  if (limit_Rx && limit_Rx->Get_active()) {
    if (limit_Rx->constr_lower.IsActive()) {
      Qc(off_L + local_offset) += ChMax(
          c * (-sin(0.5 * limit_Rx->Get_min()) + relM.rot.e1), -recovery_clamp);
      ++local_offset;
    }
    if (limit_Rx->constr_upper.IsActive()) {
      Qc(off_L + local_offset) += ChMax(
          c * (sin(0.5 * limit_Rx->Get_max()) - relM.rot.e1), -recovery_clamp);
      ++local_offset;
    }
  }
  if (limit_Ry && limit_Ry->Get_active()) {
    if (limit_Ry->constr_lower.IsActive()) {
      Qc(off_L + local_offset) += ChMax(
          c * (-sin(0.5 * limit_Ry->Get_min()) + relM.rot.e2), -recovery_clamp);
      ++local_offset;
    }
    if (limit_Ry->constr_upper.IsActive()) {
      Qc(off_L + local_offset) += ChMax(
          c * (sin(0.5 * limit_Ry->Get_max()) - relM.rot.e2), -recovery_clamp);
      ++local_offset;
    }
  }
  if (limit_Rz && limit_Rz->Get_active()) {
    if (limit_Rz->constr_lower.IsActive()) {
      Qc(off_L + local_offset) += ChMax(
          c * (-sin(0.5 * limit_Rz->Get_min()) + relM.rot.e3), -recovery_clamp);
      ++local_offset;
    }
    if (limit_Rz->constr_upper.IsActive()) {
      Qc(off_L + local_offset) += ChMax(
          c * (sin(0.5 * limit_Rz->Get_max()) - relM.rot.e3), -recovery_clamp);
      ++local_offset;
    }
  }
}
...
void ChLinkLock::InjectConstraints(ChSystemDescriptor &mdescriptor) {
  ...
  if (limit_X && limit_X->Get_active()) {
    if (limit_X->constr_lower.IsActive()) {
      limit_X->constr_lower.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
      mdescriptor.InsertConstraint(&limit_X->constr_lower);
    }
    if (limit_X->constr_upper.IsActive()) {
      limit_X->constr_upper.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
  ...
  if (limit_Y && limit_Y->Get_active()) {
    if (limit_Y->constr_lower.IsActive()) {
      limit_Y->constr_lower.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
      mdescriptor.InsertConstraint(&limit_Y->constr_lower);
    }
    if (limit_Y->constr_upper.IsActive()) {
      limit_Y->constr_upper.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
  ...
  if (limit_Z && limit_Z->Get_active()) {
    if (limit_Z->constr_lower.IsActive()) {
      limit_Z->constr_lower.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
      mdescriptor.InsertConstraint(&limit_Z->constr_lower);
    }
    if (limit_Z->constr_upper.IsActive()) {
      limit_Z->constr_upper.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
  ...
  if (limit_Rx && limit_Rx->Get_active()) {
    if (limit_Rx->constr_lower.IsActive()) {
      limit_Rx->constr_lower.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
      mdescriptor.InsertConstraint(&limit_Rx->constr_lower);
    }
    if (limit_Rx->constr_upper.IsActive()) {
      limit_Rx->constr_upper.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
  ...
  if (limit_Ry && limit_Ry->Get_active()) {
    if (limit_Ry->constr_lower.IsActive()) {
      limit_Ry->constr_lower.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
      mdescriptor.InsertConstraint(&limit_Ry->constr_lower);
    }
    if (limit_Ry->constr_upper.IsActive()) {
      limit_Ry->constr_upper.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
  ...
  if (limit_Rz && limit_Rz->Get_active()) {
    if (limit_Rz->constr_lower.IsActive()) {
      limit_Rz->constr_lower.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
      mdescriptor.InsertConstraint(&limit_Rz->constr_lower);
    }
    if (limit_Rz->constr_upper.IsActive()) {
      limit_Rz->constr_upper.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
...
void ChLinkLock::ConstraintsBiLoad_C(double factor, double recovery_clamp,
                                     bool do_clamp) {
  // parent
    ...
    if (limit_X->constr_lower.IsActive()) {
      if (!do_clamp) {
        limit_X->constr_lower.Set_b_i(limit_X->constr_lower.Get_b_i() +
                                      factor *
                                          (-limit_X->Get_min() + relM.pos.x));
      } else {
        limit_X->constr_lower.Set_b_i(
            limit_X->constr_lower.Get_b_i() +
            ChMax(factor * (-limit_X->Get_min() + relM.pos.x),
                  -recovery_clamp));
      }
    }
    if (limit_X->constr_upper.IsActive()) {
      if (!do_clamp) {
        limit_X->constr_upper.Set_b_i(limit_X->constr_upper.Get_b_i() +
                                      factor *
                                          (limit_X->Get_max() - relM.pos.x));
      } else {
        limit_X->constr_upper.Set_b_i(
            limit_X->constr_upper.Get_b_i() +
    ...
    if (limit_Y->constr_lower.IsActive()) {
      if (!do_clamp) {
        limit_Y->constr_lower.Set_b_i(limit_Y->constr_lower.Get_b_i() +
                                      factor *
                                          (-limit_Y->Get_min() + relM.pos.y));
      } else {
        limit_Y->constr_lower.Set_b_i(
            limit_Y->constr_lower.Get_b_i() +
            ChMax(factor * (-limit_Y->Get_min() + relM.pos.y),
                  -recovery_clamp));
      }
    }
    if (limit_Y->constr_upper.IsActive()) {
      if (!do_clamp) {
        limit_Y->constr_upper.Set_b_i(limit_Y->constr_upper.Get_b_i() +
                                      factor *
                                          (limit_Y->Get_max() - relM.pos.y));
      } else {
        limit_Y->constr_upper.Set_b_i(
            limit_Y->constr_upper.Get_b_i() +
    ...
    if (limit_Z->constr_lower.IsActive()) {
      if (!do_clamp) {
        limit_Z->constr_lower.Set_b_i(limit_Z->constr_lower.Get_b_i() +
                                      factor *
                                          (-limit_Z->Get_min() + relM.pos.z));
      } else {
        limit_Z->constr_lower.Set_b_i(
            limit_Z->constr_lower.Get_b_i() +
            ChMax(factor * (-limit_Z->Get_min() + relM.pos.z),
                  -recovery_clamp));
      }
    }
    if (limit_Z->constr_upper.IsActive()) {
      if (!do_clamp) {
        limit_Z->constr_upper.Set_b_i(limit_Z->constr_upper.Get_b_i() +
                                      factor *
                                          (limit_Z->Get_max() - relM.pos.z));
      } else {
        limit_Z->constr_upper.Set_b_i(
            limit_Z->constr_upper.Get_b_i() +
  ...
  if (limit_Rx && limit_Rx->Get_active()) {
    if (limit_Rx->constr_lower.IsActive()) {
      if (!do_clamp) {
        ...
        limit_Rx->constr_lower.Set_b_i(
            limit_Rx->constr_lower.Get_b_i() +
            ChMax(factor * (-sin(0.5 * limit_Rx->Get_min()) + relM.rot.e1),
                  -recovery_clamp));
      }
    }
    if (limit_Rx->constr_upper.IsActive()) {
      if (!do_clamp) {
        ...
        limit_Rx->constr_upper.Set_b_i(
            limit_Rx->constr_upper.Get_b_i() +
            ChMax(factor * (sin(0.5 * limit_Rx->Get_max()) - relM.rot.e1),
                  -recovery_clamp));
      }
    }
  }
  if (limit_Ry && limit_Ry->Get_active()) {
    if (limit_Ry->constr_lower.IsActive()) {
      if (!do_clamp) {
        ...
        limit_Ry->constr_lower.Set_b_i(
            limit_Ry->constr_lower.Get_b_i() +
            ChMax(factor * (-sin(0.5 * limit_Ry->Get_min()) + relM.rot.e2),
                  -recovery_clamp));
      }
    }
    if (limit_Ry->constr_upper.IsActive()) {
      if (!do_clamp) {
        ...
        limit_Ry->constr_upper.Set_b_i(
            limit_Ry->constr_upper.Get_b_i() +
            ChMax(factor * (sin(0.5 * limit_Ry->Get_max()) - relM.rot.e2),
                  -recovery_clamp));
      }
    }
  }
  if (limit_Rz && limit_Rz->Get_active()) {
    if (limit_Rz->constr_lower.IsActive()) {
      if (!do_clamp) {
        ...
        limit_Rz->constr_lower.Set_b_i(
            limit_Rz->constr_lower.Get_b_i() +
            ChMax(factor * (-sin(0.5 * limit_Rz->Get_min()) + relM.rot.e3),
                  -recovery_clamp));
      }
    }
    if (limit_Rz->constr_upper.IsActive()) {
      if (!do_clamp) {
        ...
        limit_Rz->constr_upper.Set_b_i(
            limit_Rz->constr_upper.Get_b_i() +
            ChMax(factor * (sin(0.5 * limit_Rz->Get_max()) - relM.rot.e3),
                  -recovery_clamp));
      }
    }
  }
...
void Transform_Cq_to_Cqw_row(ChMatrix<> *mCq, int qrow, ChMatrix<> *mCqw,
                             int qwrow, ChBodyFrame *mbody) {
...
void ChLinkLock::ConstraintsLoadJacobians() {
  ...
  if (limit_X && limit_X->Get_active()) {
    if (limit_X->constr_lower.IsActive()) {
      limit_X->constr_lower.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 0, limit_X->constr_lower.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 0, limit_X->constr_lower.Get_Cq_b(), 0,
                              Body2);
    }
    if (limit_X->constr_upper.IsActive()) {
      limit_X->constr_upper.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 0, limit_X->constr_upper.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 0, limit_X->constr_upper.Get_Cq_b(), 0,
                              Body2);
  ...
  if (limit_Y && limit_Y->Get_active()) {
    if (limit_Y->constr_lower.IsActive()) {
      limit_Y->constr_lower.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 1, limit_Y->constr_lower.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 1, limit_Y->constr_lower.Get_Cq_b(), 0,
                              Body2);
    }
    if (limit_Y->constr_upper.IsActive()) {
      limit_Y->constr_upper.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 1, limit_Y->constr_upper.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 1, limit_Y->constr_upper.Get_Cq_b(), 0,
                              Body2);
  ...
  if (limit_Z && limit_Z->Get_active()) {
    if (limit_Z->constr_lower.IsActive()) {
      limit_Z->constr_lower.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 2, limit_Z->constr_lower.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 2, limit_Z->constr_lower.Get_Cq_b(), 0,
                              Body2);
    }
    if (limit_Z->constr_upper.IsActive()) {
      limit_Z->constr_upper.SetVariables(&Body1->Variables(),
                                         &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 2, limit_Z->constr_upper.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 2, limit_Z->constr_upper.Get_Cq_b(), 0,
                              Body2);
  ...
  if (limit_Rx && limit_Rx->Get_active()) {
    if (limit_Rx->constr_lower.IsActive()) {
      limit_Rx->constr_lower.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 4, limit_Rx->constr_lower.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 4, limit_Rx->constr_lower.Get_Cq_b(), 0,
                              Body2);
    }
    if (limit_Rx->constr_upper.IsActive()) {
      limit_Rx->constr_upper.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 4, limit_Rx->constr_upper.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 4, limit_Rx->constr_upper.Get_Cq_b(), 0,
                              Body2);
  ...
  if (limit_Ry && limit_Ry->Get_active()) {
    if (limit_Ry->constr_lower.IsActive()) {
      limit_Ry->constr_lower.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 5, limit_Ry->constr_lower.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 5, limit_Ry->constr_lower.Get_Cq_b(), 0,
                              Body2);
    }
    if (limit_Ry->constr_upper.IsActive()) {
      limit_Ry->constr_upper.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 5, limit_Ry->constr_upper.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 5, limit_Ry->constr_upper.Get_Cq_b(), 0,
                              Body2);
  ...
  if (limit_Rz && limit_Rz->Get_active()) {
    if (limit_Rz->constr_lower.IsActive()) {
      limit_Rz->constr_lower.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 6, limit_Rz->constr_lower.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 6, limit_Rz->constr_lower.Get_Cq_b(), 0,
                              Body2);
    }
    if (limit_Rz->constr_upper.IsActive()) {
      limit_Rz->constr_upper.SetVariables(&Body1->Variables(),
                                          &Body2->Variables());
      Transform_Cq_to_Cqw_row(Cq1_temp, 6, limit_Rz->constr_upper.Get_Cq_a(), 0,
                              Body1);
      Transform_Cq_to_Cqw_row(Cq2_temp, 6, limit_Rz->constr_upper.Get_Cq_b(), 0,
                              Body2);
...
ChClassRegister<ChLinkLockRevolutePrismatic>
    a_registration_ChLinkLockRevolutePrismatic;
