...
#include "chrono/physics/ChMatterMeshless.h"
#include "chrono/physics/ChProximityContainerMeshless.h"
...
ChNodeMeshless::ChNodeMeshless()
    ...
    //collision_model = new ChCollisionModelBullet; OBSOLETE
...
ChNodeMeshless::ChNodeMeshless(const ChNodeMeshless& other) : ChNodeXYZ(other) {
    ...
    //collision_model->SetContactable(this);  OBSOLETE
...
void ChNodeMeshless::SetHorizonRadius(double mr) {
    h_rad = mr;
...
void ChMatterMeshless::ResizeNnodes(int newsize, double mhorizon) {
...
void ChMatterMeshless::AddNode(ChVector<double> initial_state, double mhorizon) {
    ...
    for (int ix = 0; ix < samples_x; ix++)
        for (int iy = 0; iy < samples_y; iy++)
            for (int iz = 0; iz < samples_z; iz++) {
                ...
                AddNode(boxcoords.TransformLocalToParent(pos), horizon);
                totsamples++;
                ...
                if (do_centeredcube) {
                    ...
                    pos2 += ChVector<>(mrandomness * ChRandom() * spacing, mrandomness * ChRandom() * spacing,
                                       mrandomness * ChRandom() * spacing);
                    AddNode(boxcoords.TransformLocalToParent(pos2), horizon);
                    totsamples++;
                }
            }
...
void ChMatterMeshless::AddCollisionModelsToSystem(ChCollisionSystem* coll_sys) const {
    for (unsigned int j = 0; j < nodes.size(); j++) {
...
void ChMatterMeshless::RemoveCollisionModelsFromSystem(ChCollisionSystem* coll_sys) const {
    for (unsigned int j = 0; j < nodes.size(); j++) {
        if (nodes[j]->GetCollisionModel())
