...
class ChApi ChFeeder : public ChPhysicsItem {
  ...
  public:
    ...
    /// and the eigenvector of the vibration mode as x,y,z,rx,ry,rz about that frame; the six values also define the max
    /// speed on that axis.
    void
    SetFeederVibration(ChFrame<> mref, double mv_x, double mv_y, double mv_z, double mw_x, double mw_y, double mw_z) {
        reference = mref;
        v_x = mv_x;
        v_y = mv_y;
        v_z = mv_z;
    ...
    void GetFeederVibration(ChFrame<>& mref,
                            double& mv_x,
                            double& mv_y,
                            double& mv_z,
                            double& mw_x,
