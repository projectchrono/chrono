...
class ChApi ChSystem : public ChIntegrableIIorder {
  public:
    ...
    /// If turned on, masses and loads generated by self-loops in mech topology (namely, 
    /// ChBody and ChNode stuff) will be scaled using Wv vector as weights. The scaling of 
    /// masses can be useful for various reasons, tipycally when sharing bodies or splitting as in FETI.
    void EnableCoordWeightsWv(bool enable);

    /// Access the  Wv vector of weights. Not relevant unless EnableCoordWeightsWv(true).
    /// Inertial forces and and loads generated by self-loops in mech topology (namely, 
    /// ChBody and ChNode stuff) will be scaled using Wv vector as weights in the LoadResidual_F
    /// and LoadResidual_Mv functions.
    ChVectorDynamic<>& CoordWeightsWv() { return this->Wv; }
  ...
  public:
    ...
    void IntLoadIndicator(ChVectorDynamic<>& N     ///< result: N vector, indicating DOF of graph vertexes, 0 or 1 or greater if shared 
    );
  ...
  protected:
    ...
    bool enable_Wv_weights = false; // if true, LoadResidual_F and LoadResidual_Mv will call LoadResidual_F_scaled and LoadResidual_Mv_scaled instead.
    ChVectorDynamic<> Wv; // vector of weights to scale the mass of bodies, useful in multidomain scenarios
