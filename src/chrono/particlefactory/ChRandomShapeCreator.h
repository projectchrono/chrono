...
class ChRandomShapeCreatorSpheres : public ChRandomShapeCreator {
  public:
    ChRandomShapeCreatorSpheres() {
        // defaults
        diameter = chrono::make_shared<ChConstantDistribution>(0.02);
        density = chrono::make_shared<ChConstantDistribution>(1000);
    }
    ...
    virtual std::shared_ptr<ChBody> RandomGenerate(ChCoordsys<> mcoords) override {
        ...
        auto mbody = chrono::make_shared<ChBodyEasySphere>(mrad, density->GetRandom(), this->add_collision_shape, this->add_visualization_asset);
...
class ChRandomShapeCreatorBoxes : public ChRandomShapeCreator {
  public:
    ChRandomShapeCreatorBoxes() {
        // defaults
        x_size = chrono::make_shared<ChConstantDistribution>(0.01);
        sizeratioYZ = chrono::make_shared<ChConstantDistribution>(1.0);
        sizeratioZ = chrono::make_shared<ChConstantDistribution>(1.0);
        density = chrono::make_shared<ChConstantDistribution>(1000);
    }
    ...
    virtual std::shared_ptr<ChBody> RandomGenerate(ChCoordsys<> mcoords) override {
        ...
        auto mbody = chrono::make_shared<ChBodyEasyBox>(sx, sy, sz, fabs(density->GetRandom()), this->add_collision_shape, this->add_visualization_asset);
        mbody->SetCoord(mcoords);
        return mbody;
    };
...
class ChRandomShapeCreatorCylinders : public ChRandomShapeCreator {
  public:
    ChRandomShapeCreatorCylinders() {
        // defaults
        diameter = chrono::make_shared<ChConstantDistribution>(0.02);
        length_factor = chrono::make_shared<ChConstantDistribution>(2.0);
        density = chrono::make_shared<ChConstantDistribution>(1000);
    }
    ...
    virtual std::shared_ptr<ChBody> RandomGenerate(ChCoordsys<> mcoords) override {
        ...
        auto mbody = chrono::make_shared<ChBodyEasyCylinder>(rad, height, density->GetRandom(), this->add_collision_shape, this->add_visualization_asset);
        mbody->SetCoord(mcoords);
        return mbody;
    };
...
class ChRandomShapeCreatorConvexHulls : public ChRandomShapeCreator {
  public:
    ChRandomShapeCreatorConvexHulls() {
        // defaults
        npoints = 6;
        chord = chrono::make_shared<ChConstantDistribution>(0.01);
        sizeratioYZ = chrono::make_shared<ChConstantDistribution>(1.0);
        sizeratioZ = chrono::make_shared<ChConstantDistribution>(1.0);
        density = chrono::make_shared<ChConstantDistribution>(1000);
    }
    ...
    virtual std::shared_ptr<ChBody> RandomGenerate(ChCoordsys<> mcoords) override {
        ...
        auto mbody = chrono::make_shared<ChBodyEasyConvexHull>(points, density->GetRandom(), this->add_collision_shape, this->add_visualization_asset);
        mbody->SetCoord(mcoords);
        return mbody;
    };
...
class ChRandomShapeCreatorShavings : public ChRandomShapeCreator {
  public:
    ChRandomShapeCreatorShavings() {
        ...
        spacing_factor = 0.5;
        diameter = chrono::make_shared<ChConstantDistribution>(0.02);
        twistU = chrono::make_shared<ChConstantDistribution>(0.00);
        twistV = chrono::make_shared<ChConstantDistribution>(0.00);
        lengthratio = chrono::make_shared<ChConstantDistribution>(3.0);
        density = chrono::make_shared<ChConstantDistribution>(1000);
    }
    ...
    virtual std::shared_ptr<ChBody> RandomGenerate(ChCoordsys<> mcoords) override {
        ...
        auto mbody = chrono::make_shared<ChBodyEasyClusterOfSpheres>(points, radii, density->GetRandom(), this->add_collision_shape, this->add_visualization_asset);
