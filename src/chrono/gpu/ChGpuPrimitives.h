...
constexpr inline int3 make_int3(int x, int y, int z) {
    return {x, y, z};
}
constexpr inline int4 make_int4(int x, int y, int z, int w) {
    return {x, y, z, w};
}
constexpr inline uint2 make_uint2(unsigned int x, unsigned int y) {
    return {x, y};
}
constexpr inline uint3 make_uint3(unsigned int x, unsigned int y, unsigned int z) {
    return {x, y, z};
}
constexpr inline uint4 make_uint4(unsigned int x, unsigned int y, unsigned int z, unsigned int w) {
    return {x, y, z, w};
}
constexpr inline longlong3 make_longlong3(long long x, long long y, long long z) {
    return {x, y, z};
}
constexpr inline float2 make_float2(float x, float y) {
    return {x, y};
}
constexpr inline float3 make_float3(float x, float y, float z) {
    return {x, y, z};
}
...
// Provide cuda::std as a lightweight host fallback only when we are not compiling against the real CUDA CCCL/libcudacxx headers.
// In CUDA builds, cuda::std is provided by <cuda/std/...>; aliasing it to ::std in ordinary C++ translation units conflicts with CUDA 12/13 CCCL headers.
#if !defined(__CUDACC__) && !defined(CHRONO_USE_CUDA)
namespace cuda {
namespace std = ::std;
}
