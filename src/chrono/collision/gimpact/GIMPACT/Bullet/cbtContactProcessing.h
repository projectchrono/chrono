...
#include "cbtContactProcessingStructs.h"

class cbtContactArray : public cbtAlignedObjectArray<GIM_CONTACT>
{
public:
	cbtContactArray()
	{
	...
	SIMD_FORCE_INLINE void push_contact(
		const cbtVector3 &point, const cbtVector3 &normal,
		cbtScalar depth, int feature1, int feature2)
	{
	...
	void merge_contacts(const cbtContactArray &contacts, bool normal_contact_average = true);

	void merge_contacts_unique(const cbtContactArray &contacts);
};
