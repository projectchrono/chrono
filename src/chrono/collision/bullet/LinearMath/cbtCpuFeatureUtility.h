...
///Rudimentary cbtCpuFeatureUtility for CPU features: only report the features that Bullet actually uses (SSE4/FMA3, NEON_HPFP)
///We assume SSE2 in case BT_USE_SSE2 is defined in LinearMath/cbtScalar.h
class cbtCpuFeatureUtility
{
public:
	enum cbtCpuFeature
	{
	...
	static int getCpuFeatures()
	{
...
#ifdef BT_ALLOW_SSE4
		{
			...
			{
				capabilities |= cbtCpuFeatureUtility::CPU_FEATURE_FMA3;
			}

			const int SSE41Flag = (1 << 19);
			if (cpuInfo[2] & SSE41Flag)
			{
				capabilities |= cbtCpuFeatureUtility::CPU_FEATURE_SSE4_1;
			}
		}
#endif  //BT_ALLOW_SSE4
