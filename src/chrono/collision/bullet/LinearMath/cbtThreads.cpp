...
void cbtSpinMutex::lock()
{
	cbtAssert(!"unimplemented cbtSpinMutex::lock() called");
}

void cbtSpinMutex::unlock()
{
	cbtAssert(!"unimplemented cbtSpinMutex::unlock() called");
}

bool cbtSpinMutex::tryLock()
{
	cbtAssert(!"unimplemented cbtSpinMutex::tryLock() called");
	return true;
}
...
struct ThreadsafeCounter
{
	...
	unsigned int getNext()
	{
		...
		if (mCounter >= BT_MAX_THREAD_COUNT)
		{
			...
			////cbtAssert(!"thread counter exceeded");
...
unsigned int cbtGetCurrentThreadIndex()
{
	...
	if (sThreadIndex == kNullIndex)
	{
		...
		cbtAssert(sThreadIndex < BT_MAX_THREAD_COUNT);
	}
#if BT_DETECT_BAD_THREAD_INDEX
	if (gBtTaskScheduler && sThreadIndex > 0)
	{
		...
		else
		{
			if (gDebugThreadIds[sThreadIndex] != tid)
			{
				...
				cbtAssert(!"there are 2 or more threads with the same thread-index!");
				__debugbreak();
			}
		}
	}
...
void cbtITaskScheduler::activate()
{
...
void cbtITaskScheduler::deactivate()
{
	if (m_isActive)
	{
...
{
	int threadId = cbtGetCurrentThreadIndex();  // make sure we call this on main thread at least once before any workers run
	if (threadId != 0)
	{
		cbtAssert(!"cbtSetTaskScheduler must be called from the main thread!");
		return;
	}
	if (gBtTaskScheduler)
	{
...
cbtScalar cbtParallelSum(int iBegin, int iEnd, int grainSize, const cbtIParallelSumBody& body)
{
	...
	// non-parallel version of cbtParallelSum
	cbtAssert(!"called cbtParallelFor in non-threadsafe build. enable BT_THREADSAFE");
...
///
/// cbtTaskSchedulerSequential -- non-threaded implementation of task scheduler
///                              (really just useful for testing performance of single threaded vs multi)
///
class cbtTaskSchedulerSequential : public cbtITaskScheduler
{
public:
	cbtTaskSchedulerSequential() : cbtITaskScheduler("Sequential") {}
...
#if BT_USE_OPENMP && BT_THREADSAFE
///
/// cbtTaskSchedulerOpenMP -- wrapper around OpenMP task scheduler
///
class cbtTaskSchedulerOpenMP : public cbtITaskScheduler
{
	int m_numThreads;

public:
	cbtTaskSchedulerOpenMP() : cbtITaskScheduler("OpenMP")
	{
	...
	virtual void setNumThreads(int numThreads) BT_OVERRIDE
	{
		...
		if (m_isActive)
		{
			cbtResetThreadIndexCounter();
		}
	}
	virtual void parallelFor(int iBegin, int iEnd, int grainSize, const cbtIParallelForBody& body) BT_OVERRIDE
	{
		BT_PROFILE("parallelFor_OpenMP");
		cbtPushThreadsAreRunning();
...
#if BT_USE_TBB && BT_THREADSAFE
///
/// cbtTaskSchedulerTBB -- wrapper around Intel Threaded Building Blocks task scheduler
///
class cbtTaskSchedulerTBB : public cbtITaskScheduler
{
	int m_numThreads;
	tbb::task_scheduler_init* m_tbbSchedulerInit;

public:
	cbtTaskSchedulerTBB() : cbtITaskScheduler("IntelTBB")
	{
		m_numThreads = 0;
	...
	{
		const cbtIParallelForBody* mBody;

		ForBodyAdapter(const cbtIParallelForBody* body) : mBody(body) {}
	...
	{
		const cbtIParallelSumBody* mBody;
		cbtScalar mSum;

		SumBodyAdapter(const cbtIParallelSumBody* body) : mBody(body), mSum(cbtScalar(0)) {}
		SumBodyAdapter(const SumBodyAdapter& src, tbb::split) : mBody(src.mBody), mSum(cbtScalar(0)) {}
...
#if BT_USE_PPL && BT_THREADSAFE
///
/// cbtTaskSchedulerPPL -- wrapper around Microsoft Parallel Patterns Lib task scheduler
///
class cbtTaskSchedulerPPL : public cbtITaskScheduler
{
	int m_numThreads;
	concurrency::combinable<cbtScalar> m_sum;  // for parallelSum
public:
	cbtTaskSchedulerPPL() : cbtITaskScheduler("PPL")
	{
		m_numThreads = 0;
	}
	virtual int getMaxNumThreads() const BT_OVERRIDE
	...
	struct ForBodyAdapter
	{
		...
		ForBodyAdapter(const cbtIParallelForBody* body, int grainSize, int end) : mBody(body), mGrainSize(grainSize), mIndexEnd(end) {}
	...
	{
		const cbtIParallelSumBody* mBody;
		concurrency::combinable<cbtScalar>* mSum;
		int mGrainSize;
		int mIndexEnd;

		SumBodyAdapter(const cbtIParallelSumBody* body, concurrency::combinable<cbtScalar>* sum, int grainSize, int end) : mBody(body), mSum(sum), mGrainSize(grainSize), mIndexEnd(end) {}
		void operator()(int i) const
		{
	...
	static cbtScalar sumFunc(cbtScalar a, cbtScalar b) { return a + b; }
	virtual cbtScalar parallelSum(int iBegin, int iEnd, int grainSize, const cbtIParallelSumBody& body) BT_OVERRIDE
	{
...
{
	static cbtTaskSchedulerSequential sTaskScheduler;
...
{
#if BT_USE_TBB && BT_THREADSAFE
	static cbtTaskSchedulerTBB sTaskScheduler;
	return &sTaskScheduler;
#else
	return NULL;
#endif
...
{
#if BT_USE_PPL && BT_THREADSAFE
	static cbtTaskSchedulerPPL sTaskScheduler;
	return &sTaskScheduler;
#else
	return NULL;
#endif
