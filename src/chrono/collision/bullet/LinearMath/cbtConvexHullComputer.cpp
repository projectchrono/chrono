...
#include "cbtConvexHullComputer.h"
...
public:
	...
	public:
		...
		cbtScalar toScalar() const
		{
			return ((int64_t)high >= 0) ? cbtScalar(high) * (cbtScalar(0x100000000LL) * cbtScalar(0x100000000LL)) + cbtScalar(low)
										: -(-*this).toScalar();
		}
	...
	public:
		...
		cbtScalar toScalar() const
		{
			return sign * ((m_denominator == 0) ? SIMD_INFINITY : (cbtScalar)m_numerator / m_denominator);
		}
	};
	...
	public:
		...
		cbtScalar xvalue() const
		{
			return x.toScalar() / denominator.toScalar();
		}

		cbtScalar yvalue() const
		{
			return y.toScalar() / denominator.toScalar();
		}

		cbtScalar zvalue() const
		{
	...
	public:
		...
		cbtScalar xvalue() const
		{
			return (point.index >= 0) ? cbtScalar(point.x) : point128.xvalue();
		}

		cbtScalar yvalue() const
		{
			return (point.index >= 0) ? cbtScalar(point.y) : point128.yvalue();
		}

		cbtScalar zvalue() const
		{
			return (point.index >= 0) ? cbtScalar(point.z) : point128.zvalue();
		}
	...
	public:
		...
		void link(Edge* n)
		{
			cbtAssert(reverse->target == n->reverse->target);
			next = n;
			n->prev = this;
		}
...
private:
	...
	public:
		...
		{
			array = (T*)cbtAlignedAlloc(sizeof(T) * size, 16);
		}

		~PoolArray()
		{
			cbtAlignedFree(array);
		}
	...
	public:
		...
		T* newObject()
		{
			...
			if (!o)
			{
				...
				{
					p = new (cbtAlignedAlloc(sizeof(PoolArray<T>), 16)) PoolArray<T>(arraySize);
					p->next = arrays;
					arrays = p;
				}
				o = p->init();
	...
	Pool<Edge> edgePool;
	Pool<Face> facePool;
	cbtAlignedObjectArray<Vertex*> originalVertices;
	int mergeStamp;
	int minAxis;
	int medAxis;
	int maxAxis;
	...
	void removeEdgePair(Edge* edge)
	{
		...
		cbtAssert(edge->target && r->target);
	...
	cbtVector3 toBtVector(const Point32& v);

	cbtVector3 getBtNormal(Face* face);

	bool shiftFace(Face* face, cbtScalar amount, cbtAlignedObjectArray<Vertex*> stack);
...
public:
	...
	cbtVector3 getCoordinates(const Vertex* v);

	cbtScalar shrink(cbtScalar amount, cbtScalar clampAmount);
};

cbtConvexHullInternal::Int128 cbtConvexHullInternal::Int128::operator*(int64_t b) const
{
...
cbtConvexHullInternal::Int128 cbtConvexHullInternal::Int128::mul(int64_t a, int64_t b)
{
	Int128 result;
...
cbtConvexHullInternal::Int128 cbtConvexHullInternal::Int128::mul(uint64_t a, uint64_t b)
{
	Int128 result;
...
int cbtConvexHullInternal::Rational64::compare(const Rational64& b) const
{
	if (sign != b.sign)
	{
		return sign - b.sign;
...
int cbtConvexHullInternal::Rational128::compare(const Rational128& b) const
{
	if (sign != b.sign)
	{
		return sign - b.sign;
...
int cbtConvexHullInternal::Rational128::compare(int64_t b) const
{
	if (isInt64)
	{
...
void cbtConvexHullInternal::computeInternal(int start, int end, IntermediateHull& result)
{
	int n = end - start;
	switch (n)
	{
...
#ifdef DEBUG_CONVEX_HULL
void cbtConvexHullInternal::IntermediateHull::print()
{
	printf("    Hull\n");
...
void cbtConvexHullInternal::Vertex::printGraph()
{
	print();
	printf("\nEdges\n");
	Edge* e = edges;
...
cbtConvexHullInternal::Orientation cbtConvexHullInternal::getOrientation(const Edge* prev, const Edge* next, const Point32& s, const Point32& t)
{
	cbtAssert(prev->reverse->target == next->reverse->target);
	if (prev->next == next)
	{
		if (prev->prev == next)
		{
...
cbtConvexHullInternal::Edge* cbtConvexHullInternal::findMaxAngle(bool ccw, const Vertex* start, const Point32& s, const Point64& rxs, const Point64& sxrxs, Rational64& minCot)
{
	Edge* minEdge = NULL;
...
#ifdef DEBUG_CONVEX_HULL
				printf("      Angle is %f (%d) for ", (float)cbtAtan(cot.toScalar()), (int)cot.isNaN());
				e->print();
#endif
				if (cot.isNaN())
				{
					cbtAssert(ccw ? (t.dot(s) < 0) : (t.dot(s) > 0));
				}
				else
				{
					int cmp;
...
void cbtConvexHullInternal::findEdgeForCoplanarFaces(Vertex* c0, Vertex* c1, Edge*& e0, Edge*& e1, Vertex* stop0, Vertex* stop1)
{
	Edge* start0 = e0;
	Edge* start1 = e1;
	...
	cbtAssert(!start1 || (start1->target->point.dot(normal) == dist));
	Point64 perp = s.cross(normal);
	cbtAssert(!perp.isZero());
	...
	if (e0)
	{
		while (e0->target != stop0)
		{
			...
			cbtAssert(e->target->point.dot(normal) == dist);
			if (e->copy == mergeStamp)
			{
				break;
			}
		...
		while (true)
		{
			...
			if (e1 && (e1->target != stop1))
			{
				...
				if (f1->copy > mergeStamp)
				{
					...
					{
						cbtAssert((e1 == start1) && (d1.dot(normal) < 0));
					}
				}
			}
		...
		while (true)
		{
			...
			if (e0 && (e0->target != stop0))
			{
				...
				if (f0->copy > mergeStamp)
				{
					...
					{
						cbtAssert((e0 == start0) && (d0.dot(normal) < 0));
					}
				}
			}
...
void cbtConvexHullInternal::merge(IntermediateHull& h0, IntermediateHull& h1)
{
	if (!h1.maxXy)
	{
		return;
...
{
public:
	bool operator()(const cbtConvexHullInternal::Point32& p, const cbtConvexHullInternal::Point32& q) const
	{
		return (p.y < q.y) || ((p.y == q.y) && ((p.x < q.x) || ((p.x == q.x) && (p.z < q.z))));
	}
};

void cbtConvexHullInternal::compute(const void* coords, bool doubleCoords, int stride, int count)
{
	cbtVector3 min(cbtScalar(1e30), cbtScalar(1e30), cbtScalar(1e30)), max(cbtScalar(-1e30), cbtScalar(-1e30), cbtScalar(-1e30));
	...
	if (doubleCoords)
	{
		for (int i = 0; i < count; i++)
		{
			...
			cbtVector3 p((cbtScalar)v[0], (cbtScalar)v[1], (cbtScalar)v[2]);
			ptr += stride;
			min.setMin(p);
			max.setMax(p);
		}
	...
	center = (min + max) * cbtScalar(0.5);

	cbtAlignedObjectArray<Point32> points;
	points.resize(count);
...
cbtVector3 cbtConvexHullInternal::toBtVector(const Point32& v)
{
	cbtVector3 p;
	p[medAxis] = cbtScalar(v.x);
	p[maxAxis] = cbtScalar(v.y);
	p[minAxis] = cbtScalar(v.z);
	return p * scaling;
}
...
cbtVector3 cbtConvexHullInternal::getCoordinates(const Vertex* v)
{
	cbtVector3 p;
	p[medAxis] = v->xvalue();
	p[maxAxis] = v->yvalue();
...
cbtScalar cbtConvexHullInternal::shrink(cbtScalar amount, cbtScalar clampAmount)
{
	if (!vertexList)
	{
		return 0;
	...
	cbtAlignedObjectArray<Vertex*> stack;
	vertexList->copy = stamp;
	stack.push_back(vertexList);
	cbtAlignedObjectArray<Face*> faces;
	...
	if (clampAmount > 0)
	{
		cbtScalar minDist = SIMD_INFINITY;
		for (int i = 0; i < faceCount; i++)
		{
			cbtVector3 normal = getBtNormal(faces[i]);
			cbtScalar dist = normal.dot(toBtVector(faces[i]->origin) - hullCenter);
			if (dist < minDist)
			{
				minDist = dist;
			}
...
bool cbtConvexHullInternal::shiftFace(Face* face, cbtScalar amount, cbtAlignedObjectArray<Vertex*> stack)
{
	cbtVector3 origShift = getBtNormal(face) * -amount;
	if (scaling[0] != 0)
	{
	...
	if (cmp >= 0)
	{
		...
		do
		{
			...
			cbtAssert(dot.compare(origDot) <= 0);
#ifdef DEBUG_CONVEX_HULL
...
#endif
			e = e->reverse->prev;
			cbtAssert(e != intersection->reverse);
...
static int getVertexCopy(cbtConvexHullInternal::Vertex* vertex, cbtAlignedObjectArray<cbtConvexHullInternal::Vertex*>& vertices)
{
...
cbtScalar cbtConvexHullComputer::compute(const void* coords, bool doubleCoords, int stride, int count, cbtScalar shrink, cbtScalar shrinkClamp)
{
	if (count <= 0)
	{
		vertices.clear();
	...
	cbtAlignedObjectArray<cbtConvexHullInternal::Vertex*> oldVertices;
	getVertexCopy(hull.vertexList, oldVertices);
	int copied = 0;
	while (copied < oldVertices.size())
	{
		cbtConvexHullInternal::Vertex* v = oldVertices[copied];
		vertices.push_back(hull.getCoordinates(v));
		cbtConvexHullInternal::Edge* firstEdge = v->edges;
		if (firstEdge)
		{
			int firstCopy = -1;
			int prevCopy = -1;
			cbtConvexHullInternal::Edge* e = firstEdge;
			do
			{
				if (e->copy < 0)
				{
	...
	{
		cbtConvexHullInternal::Vertex* v = oldVertices[i];
		cbtConvexHullInternal::Edge* firstEdge = v->edges;
		if (firstEdge)
		{
			cbtConvexHullInternal::Edge* e = firstEdge;
			do
			{
				if (e->copy >= 0)
				{
					...
					cbtConvexHullInternal::Edge* f = e;
					do
					{
#ifdef DEBUG_CONVEX_HULL
