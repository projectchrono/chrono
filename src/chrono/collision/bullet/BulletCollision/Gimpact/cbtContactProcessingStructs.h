...
///The GIM_CONTACT is an internal GIMPACT structure, similar to cbtManifoldPoint.
///@todo: remove and replace GIM_CONTACT by cbtManifoldPoint.
class GIM_CONTACT
{
public:
	cbtVector3 m_point;
	cbtVector3 m_normal;
	cbtScalar m_depth;     //Positive value indicates interpenetration
	cbtScalar m_distance;  //Padding not for use
...
public:
	...
	GIM_CONTACT(const cbtVector3 &point, const cbtVector3 &normal,
				cbtScalar depth, int feature1, int feature2) : m_point(point),
															  m_normal(normal),
															  m_depth(depth),
															  m_feature1(feature1),
															  m_feature2(feature2)
	...
	SIMD_FORCE_INLINE void interpolate_normals(cbtVector3 *normals, int normal_count)
	{
		...
		cbtScalar vec_sum_len = vec_sum.length2();
		if (vec_sum_len < CONTACT_DIFF_EPSILON) return;

		//GIM_INV_SQRT(vec_sum_len,vec_sum_len); // 1/sqrt(vec_sum_len)

		m_normal = vec_sum / cbtSqrt(vec_sum_len);
	}
};
