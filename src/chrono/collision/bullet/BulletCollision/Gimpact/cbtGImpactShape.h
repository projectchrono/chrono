...
#include "cbtGImpactQuantizedBvh.h"  // box tree class

//! declare Quantized trees, (you can change to float based trees)
typedef cbtGImpactQuantizedBvh cbtGImpactBoxSet;
...
class cbtTetrahedronShapeEx : public cbtBU_Simplex1to4
{
public:
	cbtTetrahedronShapeEx()
	{
		m_numVertices = 4;
	}

	SIMD_FORCE_INLINE void setVertices(
		const cbtVector3& v0, const cbtVector3& v1,
		const cbtVector3& v2, const cbtVector3& v3)
	{
		m_vertices[0] = v0;
		m_vertices[1] = v1;
		m_vertices[2] = v2;
...
class cbtGImpactShapeInterface : public cbtConcaveShape
{
protected:
	cbtAABB m_localAABB;
	bool m_needs_update;
	cbtVector3 localScaling;
	cbtGImpactBoxSet m_box_set;  // optionally boxset
...
public:
    ...
    */
	...
	{
		cbtAABB transformedbox = m_localAABB;
	...
	SIMD_FORCE_INLINE const cbtAABB& getLocalBox()
	{
		return m_localAABB;
	}
	...
	*/
	...
	//! gets boxset
	SIMD_FORCE_INLINE const cbtGImpactBoxSet* getBoxSet() const
	{
		return &m_box_set;
	}
	...
	virtual const cbtPrimitiveManagerBase* getPrimitiveManager() const = 0;
	...
	virtual void getBulletTriangle(int prim_index, cbtTriangleShapeEx& triangle) const = 0;

	virtual void getBulletTetrahedron(int prim_index, cbtTetrahedronShapeEx& tetrahedron) const = 0;
	...
	//! if this trimesh
	SIMD_FORCE_INLINE void getPrimitiveTriangle(int index, cbtPrimitiveTriangle& triangle) const
	{
	...
	/*!
    */
	virtual void getChildAabb(int child_index, const cbtTransform& t, cbtVector3& aabbMin, cbtVector3& aabbMax) const
	{
		cbtAABB child_aabb;
	...
	//! Gets the children
	virtual cbtCollisionShape* getChildShape(int index) = 0;

	//! Gets the child
	virtual const cbtCollisionShape* getChildShape(int index) const = 0;

	//! Gets the children transform
	virtual cbtTransform getChildTransform(int index) const = 0;

	//! Sets the children transform
	/*!
	\post You must call updateBound() for update the box set.
	*/
	virtual void setChildTransform(int index, const cbtTransform& transform) = 0;

	//!@}

	//! virtual method for ray collision
	virtual void rayTest(const cbtVector3& rayFrom, const cbtVector3& rayTo, cbtCollisionWorld::RayResultCallback& resultCallback) const
	{
		(void)rayFrom;
		(void)rayTo;
		(void)resultCallback;
	...
	*/
	...
	It gives the triangles in local space
	*/
	virtual void processAllTrianglesRay(cbtTriangleCallback* /*callback*/, const cbtVector3& /*rayFrom*/, const cbtVector3& /*rayTo*/) const
	{
	}
...
//! cbtGImpactCompoundShape allows to handle multiple cbtCollisionShape objects at once
/*!
This class only can manage Convex subshapes
*/
class cbtGImpactCompoundShape : public cbtGImpactShapeInterface
{
public:
	//! compound primitive manager
	class CompoundPrimitiveManager : public cbtPrimitiveManagerBase
	{
	public:
		virtual ~CompoundPrimitiveManager() {}
		cbtGImpactCompoundShape* m_compoundShape;

		CompoundPrimitiveManager(const CompoundPrimitiveManager& compound)
			: cbtPrimitiveManagerBase()
		{
			m_compoundShape = compound.m_compoundShape;
		}

		CompoundPrimitiveManager(cbtGImpactCompoundShape* compoundShape)
		{
		...
		virtual void get_primitive_box(int prim_index, cbtAABB& primbox) const
		{
			...
			const cbtCollisionShape* shape = m_compoundShape->getChildShape(prim_index);
...
protected:
	...
	cbtAlignedObjectArray<cbtTransform> m_childTransforms;
	cbtAlignedObjectArray<cbtCollisionShape*> m_childShapes;

public:
	cbtGImpactCompoundShape(bool children_has_transform = true)
	{
	...
	virtual ~cbtGImpactCompoundShape()
	{
	}
	...
	//! Obtains the primitive manager
	virtual const cbtPrimitiveManagerBase* getPrimitiveManager() const
	{
	...
	{
		cbtAssert(shape->isConvex());
		m_childTransforms.push_back(localTransform);
		m_childShapes.push_back(shape);
	}

	//! Use this method for adding children. Only Convex shapes are allowed.
	void addChildShape(cbtCollisionShape* shape)
	{
		cbtAssert(shape->isConvex());
		m_childShapes.push_back(shape);
	}
	...
	//! Gets the children
	virtual const cbtCollisionShape* getChildShape(int index) const
	{
		return m_childShapes[index];
	}
    ...
    */
	...
	//! Gets the children transform
	virtual cbtTransform getChildTransform(int index) const
	{
		cbtAssert(m_childTransforms.size() == m_childShapes.size());
	...
	\post You must call updateBound() for update the box set.
	*/
	virtual void setChildTransform(int index, const cbtTransform& transform)
	{
		cbtAssert(m_childTransforms.size() == m_childShapes.size());
	...
	virtual void getBulletTriangle(int prim_index, cbtTriangleShapeEx& triangle) const
	{
		(void)prim_index;
		(void)triangle;
		cbtAssert(0);
	...
	virtual void getBulletTetrahedron(int prim_index, cbtTetrahedronShapeEx& tetrahedron) const
	{
		(void)prim_index;
		(void)tetrahedron;
		cbtAssert(0);
...
//! This class manages a sub part of a mesh supplied by the cbtStridingMeshInterface interface.
/*!
- Simply create this shape by passing the cbtStridingMeshInterface to the constructor cbtGImpactMeshShapePart, then you must call updateBound() after creating the mesh
...
*/
class cbtGImpactMeshShapePart : public cbtGImpactShapeInterface
{
public:
	//! Trimesh primitive manager
	/*!
	Manages the info from cbtStridingMeshInterface object and controls the Lock/Unlock mechanism
	*/
	class TrimeshPrimitiveManager : public cbtPrimitiveManagerBase
	{
	public:
		cbtScalar m_margin;
		...
		TrimeshPrimitiveManager(
			cbtStridingMeshInterface* meshInterface, int part)
		{
...
public:
	cbtGImpactMeshShapePart()
	{
		m_box_set.setPrimitiveManager(&m_primitive_manager);
	}

	cbtGImpactMeshShapePart(cbtStridingMeshInterface* meshInterface, int part);
	virtual ~cbtGImpactMeshShapePart();
	...
	*/
	...
	SIMD_FORCE_INLINE void getVertex(int vertex_index, cbtVector3& vertex) const
	{
		m_primitive_manager.get_vertex(vertex_index, vertex);
	}

	SIMD_FORCE_INLINE void setMargin(cbtScalar margin)
	{
		m_primitive_manager.m_margin = margin;
		postUpdate();
	}

	SIMD_FORCE_INLINE cbtScalar getMargin() const
	{
...
//! This class manages a mesh supplied by the cbtStridingMeshInterface interface.
/*!
Set of cbtGImpactMeshShapePart parts
- Simply create this shape by passing the cbtStridingMeshInterface to the constructor cbtGImpactMeshShape, then you must call updateBound() after creating the mesh

- You can handle deformable meshes with this shape, by calling postUpdate() every time when changing the mesh vertices.

*/
class cbtGImpactMeshShape : public cbtGImpactShapeInterface
{
	cbtStridingMeshInterface* m_meshInterface;

protected:
	cbtAlignedObjectArray<cbtGImpactMeshShapePart*> m_mesh_parts;
...
public:
	...
	cbtGImpactMeshShapePart* getMeshPart(int index)
	{
		return m_mesh_parts[index];
	}

	const cbtGImpactMeshShapePart* getMeshPart(int index) const
	{
		return m_mesh_parts[index];
	}
...
struct cbtGImpactMeshShapeData
{
...
SIMD_FORCE_INLINE int cbtGImpactMeshShape::calculateSerializeBufferSize() const
{
	return sizeof(cbtGImpactMeshShapeData);
}
