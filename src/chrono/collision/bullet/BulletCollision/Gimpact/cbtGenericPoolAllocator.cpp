/*! \file cbtGenericPoolAllocator.cpp
...
#include "cbtGenericPoolAllocator.h"

/// *************** cbtGenericMemoryPool ******************///////////

size_t cbtGenericMemoryPool::allocate_from_free_nodes(size_t num_elements)
{
...
size_t cbtGenericMemoryPool::allocate_from_pool(size_t num_elements)
{
...
void cbtGenericMemoryPool::init_pool(size_t element_size, size_t element_count)
{
	m_allocated_count = 0;
	...
	m_pool = (unsigned char *)cbtAlignedAlloc(m_element_size * m_max_element_count, 16);
	m_free_nodes = (size_t *)cbtAlignedAlloc(sizeof(size_t) * m_max_element_count, 16);
	m_allocated_sizes = (size_t *)cbtAlignedAlloc(sizeof(size_t) * m_max_element_count, 16);
...
void cbtGenericMemoryPool::end_pool()
{
	cbtAlignedFree(m_pool);
	cbtAlignedFree(m_free_nodes);
	cbtAlignedFree(m_allocated_sizes);
	m_allocated_count = 0;
	m_free_nodes_count = 0;
}
...
bool cbtGenericMemoryPool::freeMemory(void *pointer)
{
...
/// *******************! cbtGenericPoolAllocator *******************!///

cbtGenericPoolAllocator::~cbtGenericPoolAllocator()
{
	// destroy pools
	size_t i;
	...
	{
		m_pools[i]->end_pool();
		cbtAlignedFree(m_pools[i]);
	}
}

// creates a pool
cbtGenericMemoryPool *cbtGenericPoolAllocator::push_new_pool()
{
	if (m_pool_count >= BT_DEFAULT_MAX_POOLS) return NULL;

	cbtGenericMemoryPool *newptr = (cbtGenericMemoryPool *)cbtAlignedAlloc(sizeof(cbtGenericMemoryPool), 16);
...
void *cbtGenericPoolAllocator::failback_alloc(size_t size_bytes)
{
	cbtGenericMemoryPool *pool = NULL;
	...
	{
		return cbtAlignedAlloc(size_bytes, 16);
	}

	return pool->allocate(size_bytes);
}

bool cbtGenericPoolAllocator::failback_free(void *pointer)
{
	cbtAlignedFree(pointer);
	return true;
}
...
\param size_bytes size in bytes of the buffer
*/
void *cbtGenericPoolAllocator::allocate(size_t size_bytes)
{
	void *ptr = NULL;
...
bool cbtGenericPoolAllocator::freeMemory(void *pointer)
{
	bool result = false;
...
// main allocator
class GIM_STANDARD_ALLOCATOR : public cbtGenericPoolAllocator
{
public:
	GIM_STANDARD_ALLOCATOR() : cbtGenericPoolAllocator(BT_DEFAULT_POOL_ELEMENT_SIZE, BT_DEFAULT_POOL_SIZE)
	{
	}
};
...
void *cbtPoolAlloc(size_t size)
{
	return g_main_allocator.allocate(size);
}

void *cbtPoolRealloc(void *ptr, size_t oldsize, size_t newsize)
{
	void *newptr = cbtPoolAlloc(newsize);
