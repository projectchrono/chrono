...
SIMD_FORCE_INLINE cbtScalar bt_distance_point_plane(const cbtVector4 &plane, const cbtVector3 &point)
{
...
SIMD_FORCE_INLINE void bt_vec_blend(cbtVector3 &vr, const cbtVector3 &va, const cbtVector3 &vb, cbtScalar blend_factor)
{
...
SIMD_FORCE_INLINE void bt_plane_clip_polygon_collect(
	const cbtVector3 &point0,
	const cbtVector3 &point1,
	cbtScalar dist0,
	cbtScalar dist1,
	cbtVector3 *clipped,
	int &clipped_count)
{
	...
	{
		cbtScalar blendfactor = -dist0 / (dist1 - dist0);
...
SIMD_FORCE_INLINE int bt_plane_clip_triangle(
	...
	const cbtVector3 &point0,
	const cbtVector3 &point1,
	const cbtVector3 &point2,
	cbtVector3 *clipped  // an allocated array of 16 points at least
)
{
	int clipped_count = 0;

	//clip first point0
	cbtScalar firstdist = bt_distance_point_plane(plane, point0);
	;
	if (!(firstdist > SIMD_EPSILON))
	{
	...
	// point 1
	cbtScalar olddist = firstdist;
	cbtScalar dist = bt_distance_point_plane(plane, point1);
