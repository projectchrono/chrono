...
cbtCollisionObject::cbtCollisionObject()
	: m_interpolationLinearVelocity(0.f, 0.f, 0.f),
	  m_interpolationAngularVelocity(0.f, 0.f, 0.f),
	  m_anisotropicFriction(1.f, 1.f, 1.f),
	  m_hasAnisotropicFriction(false),
	  m_contactProcessingThreshold(BT_LARGE_FLOAT),
	  m_broadphaseHandle(nullptr),
	  m_collisionShape(nullptr),
	  m_extensionPointer(nullptr),
	  m_rootCollisionShape(nullptr),
	  m_collisionFlags(cbtCollisionObject::CF_STATIC_OBJECT),
	  m_islandTag1(-1),
	  m_companionId(-1),
	  m_worldArrayIndex(-1),
	  m_activationState1(1),
	  m_deactivationTime(cbtScalar(0)),
	  m_friction(cbtScalar(0.5)),
	  m_restitution(cbtScalar(0)),
	  m_rollingFriction(cbtScalar(0)),
	  m_spinningFriction(cbtScalar(0)),
	  m_contactDamping(cbtScalar(0.1)),
	  ...
	  m_ccdSweptSphereRadius(cbtScalar(0)),
	  m_ccdMotionThreshold(cbtScalar(0)),
...
void cbtCollisionObject::setActivationState(int newState) const
{
	if ((m_activationState1 != DISABLE_DEACTIVATION) && (m_activationState1 != DISABLE_SIMULATION))
		m_activationState1 = newState;
}

void cbtCollisionObject::forceActivationState(int newState) const
{
	m_activationState1 = newState;
}

void cbtCollisionObject::activate(bool forceActivation) const
{
...
void cbtCollisionObject::serializeSingleObject(class cbtSerializer* serializer) const
{
