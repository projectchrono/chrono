 ...
 cbtVector3	cbtCEtriangleShape::localGetSupportingVertexWithoutMargin(const cbtVector3& vec0)const
{
 ...
 void	cbtCEtriangleShape::batchedUnitVectorGetSupportingVertexWithoutMargin(const cbtVector3* vectors,cbtVector3* supportVerticesOut,int numVectors) const
{
...
void	cbtCEtriangleShape::calculateLocalInertia(cbtScalar mass,cbtVector3& inertia) const
{
	...
	halfExtents.setValue((cbtScalar)(radius), 
						 (cbtScalar)(radius),
						 (cbtScalar)(radius));
...
void cbtCEtriangleShape::getAabb(const cbtTransform& t,cbtVector3& aabbMin,cbtVector3& aabbMax) const
{
	...
	cbtVector3 p1_w = t.getOrigin()+ t.getBasis()*cbtVector3((cbtScalar)this->p1->x(), (cbtScalar)this->p1->y(), (cbtScalar)this->p1->z());
    cbtVector3 p2_w = t.getOrigin()+ t.getBasis()*cbtVector3((cbtScalar)this->p2->x(), (cbtScalar)this->p2->y(), (cbtScalar)this->p2->z());
    cbtVector3 p3_w = t.getOrigin()+ t.getBasis()*cbtVector3((cbtScalar)this->p3->x(), (cbtScalar)this->p3->y(), (cbtScalar)this->p3->z());
    ...
    cbtVector3 vsphereswept ((cbtScalar)this->sphereswept_rad,(cbtScalar)this->sphereswept_rad,(cbtScalar)this->sphereswept_rad);

	aabbMin = cbtVector3((cbtScalar)ChMin(ChMin(p1_w.x(),p2_w.x()),p3_w.x()),
                        (cbtScalar)ChMin(ChMin(p1_w.y(),p2_w.y()),p3_w.y()),
                        (cbtScalar)ChMin(ChMin(p1_w.z(),p2_w.z()),p3_w.z())) - venvelope - vsphereswept;

    aabbMax = cbtVector3((cbtScalar)ChMax(ChMax(p1_w.x(),p2_w.x()),p3_w.x()),
                        (cbtScalar)ChMax(ChMax(p1_w.y(),p2_w.y()),p3_w.y()),
                        (cbtScalar)ChMax(ChMax(p1_w.z(),p2_w.z()),p3_w.z())) + venvelope + vsphereswept;
}
