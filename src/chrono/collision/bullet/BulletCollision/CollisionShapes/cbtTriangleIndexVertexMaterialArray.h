...
cbtMaterialProperties
{
	///m_materialBase ==========> 2 cbtScalar values make up one material, friction then restitution
	int m_numMaterials;
	const unsigned char* m_materialBase;
	int m_materialStride;
	PHY_ScalarType m_materialType;
	///m_numTriangles <=========== This exists in the cbtIndexedMesh object for the same subpart, but since we're
...
typedef cbtAlignedObjectArray<cbtMaterialProperties> MaterialArray;

///Teh cbtTriangleIndexVertexMaterialArray is built on TriangleIndexVertexArray
...
cbtTriangleIndexVertexMaterialArray : public cbtTriangleIndexVertexArray
{
protected:
...
public:
	...
	cbtTriangleIndexVertexMaterialArray()
	{
	}

	cbtTriangleIndexVertexMaterialArray(int numTriangles, int* triangleIndexBase, int triangleIndexStride,
	...
	virtual ~cbtTriangleIndexVertexMaterialArray() {}

	void addMaterialProperties(const cbtMaterialProperties& mat, PHY_ScalarType triangleType = PHY_INTEGER)
	{
