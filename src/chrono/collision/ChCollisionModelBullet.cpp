...
bool ChCollisionModelBullet::AddCone(std::shared_ptr<ChMaterialSurface> material,
                                     double radius,
                                     double height,
                                     const ChVector<>& pos,
                                     const ChMatrix33<>& rot) {
    ...
    auto shape = new ChCollisionShapeBullet(ChCollisionShape::Type::CONE, material);

    shape->m_bt_shape = new cbtConeShapeZ(radius + GetEnvelope(), height + GetEnvelope());
    shape->m_bt_shape->setMargin((cbtScalar)GetSuggestedFullMargin());
...
bool ChCollisionModelBullet::AddCylindricalShell(std::shared_ptr<ChMaterialSurface> material,
    ...
    shape->m_bt_shape =
        new cbtCylindricalShellShape((cbtScalar)(radius + GetEnvelope()), (cbtScalar)(hlen + GetEnvelope()));
    shape->m_bt_shape->setMargin((cbtScalar)GetSuggestedFullMargin());
...
std::vector<double> ChCollisionModelBullet::GetShapeDimensions(int index) const {
    ...
    switch (m_shapes[index]->GetType()) {
        ...
        case ChCollisionShape::Type::CYLINDER: {
            auto bt_cyl = static_cast<cbtCylinderShape*>(shape->m_bt_shape);
