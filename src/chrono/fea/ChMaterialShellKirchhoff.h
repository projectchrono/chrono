...
#ifndef CH_MATERIAL_SHELL_KIRCHHOFF_H
#define CH_MATERIAL_SHELL_KIRCHHOFF_H
...
///    {n,m}=f({e,k})
/// that is per-unit-length forces/torques n_11,n22,n12,m_11,m_22,m_12 given the strains/curvatures e_11 e_22 e_12 k_11 k_22 k_12.
...
/// This class implements material properties for a layer of a Kirchhoff thin shell, for the case of isotropic linear elastic material.
/// This is probably the most common material when using thin shells.
...
/// This class implements material properties for a layer of a Kirchhoff thin shell, for the case of orthotropic linear elastic material.
/// This is useful for laminated shells. One direction can be made softer than the other.
/// Note that the angle and the thickness are defined when adding a material with this elasticity to a shell finite element as a layer.
...
/// This uses a 6x6 matrix [E] from user-input data. The [E] matrix can be computed from a preprocessing stage using a FEA analysis over a detailed 3D model
...
class ChApi ChElasticityKirchhoffGeneric : public ChElasticityKirchhoff {
  public:
    ...
    virtual void ComputeStiffnessMatrix(ChMatrixRef mC,         ///< tangent matrix
                                        ...
                                        const double z_inf,     ///< layer lower z value (along thickness coordinate)
                                        const double z_sup,     ///< layer upper z value (along thickness coordinate)
                                        const double angle      ///< layer angle respect to x (if needed)      inate
                                        ) override;
...
class ChApi ChPlasticityKirchhoff {
  public:
    ...
    /// This MUST be implemented by subclasses.
    virtual bool ComputeStressWithReturnMapping(ChVector3d& n,                             ///< forces  n_11, n_22, n_12 (per unit length)
                                                ChVector3d& m,                             ///< torques m_11, m_22, m_12 (per unit length)
...
class ChApi ChDampingKirchhoffRayleigh : public ChDampingKirchhoff {
  public:
    ...
    ChDampingKirchhoffRayleigh(std::shared_ptr<ChElasticityKirchhoff> elasticity, const double& mbeta = 0);
...
/// Material for a single layer of a thin shell (Kirchoff-Love shell theory, i.e. shells without shear effects).
