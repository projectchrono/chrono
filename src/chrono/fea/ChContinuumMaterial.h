...
#ifndef CH_CONTINUUM_MATERIAL_H
#define CH_CONTINUUM_MATERIAL_H
...
class ChApi ChContinuumElastic : public ChContinuumMaterial {
  private:
    ...
    ChMatrixDynamic<> StressStrainMatrix;  ///< Elasticity (stiffness) matrix

    double m_rayl_damping_alpha;  ///< Rayleigh damping coefficient, M proportional
    double m_rayl_damping_beta;   ///< Rayleigh damping coefficient, K proportional
  ...
  public:
    ...
    /// The Poisson ratio is set as v = -transverse_strain/axial_strain, so that it takes into account the 'squeezing' effect of materials that are pulled.
...
class ChApi ChContinuumElastoplastic : public ChContinuumElastic {
  public:
    ChContinuumElastoplastic(double young = 10000000, double poisson = 0.4, double density = 1000) : ChContinuumElastic(young, poisson, density) {}
    ...
    /// Compute plastic strain flow (flow derivative dE_plast/dt) from strain, according to Von Mises strain yield theory.
    virtual void ComputePlasticStrainFlow(ChStrainTensor<>& plasticstrainflow, const ChStrainTensor<>& totstrain) const = 0;

    /// Correct the strain-stress by enforcing that elastic stress must remain on the yield surface.
    /// This computes a plastic flow to be added to plastic strain while integrating.
    ...
    /// Set the plastic flow rate, i.e. the 'creep' speed.
    /// The lower the value, the slower the plastic flow during dynamic simulations, with delayed plasticity.
...
class ChApi ChContinuumPlasticVonMises : public ChContinuumElastoplastic {
  ...
  public:
    ...
    /// This also allows defining plastic and elastic maximum stress (yield limits for transition elastic->plastic and plastic->fracture).
    ...
    /// Set the elastic yield modulus as the maximum Von Mises equivalent strain that can be withstood by material before starting plastic flow.
    /// This defines the transition elastic->plastic.
    ...
    /// Set the plastic yield modulus as the maximum Von Mises equivalent strain that can be withstood by material before fracture.
    /// This defines the transition plastic->fracture.
...
class ChApi ChContinuumDruckerPrager : public ChContinuumElastoplastic {
  ...
  public:
    ...
    /// Set the C and A parameters of the Drucker-Prager model starting from 'practical' values of inner friction angle phi and cohesion, as used in the faceted Mohr-Coulomb model.
    ...
    /// Set the internal dilatation coefficient.
    /// Usually, 0.. < internal friction.
    ...
    /// Set the hardening inverse speed coefficient for exponential hardening.
    /// The larger this value, the slower the hardening or softening process that will asymptotically make yield = hardening_limit.
