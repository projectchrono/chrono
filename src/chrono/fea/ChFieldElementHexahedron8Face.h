...
/// Loadable of an hexahedron, with reference to some interpolated field. 
...
public:
    ...
    virtual std::shared_ptr<ChNodeFEAbase> GetNode(unsigned int i) override {
        ...
        case 4:
            return (m_element->GetNode(iface4[i]));
        case 5:
            return (m_element->GetNode(iface5[i]));
        }
        return nullptr;
    }
...
public:
    ...
    ChFieldHexahedronVolume(std::shared_ptr<ChFieldElementHexahedron8> element, std::shared_ptr<ChFieldBase> field) : m_element(element), m_field(field) {}

    ~ChFieldHexahedronVolume() {}
    ...
    /// The ChFieldHexahedron has no density - it requires a ChMaterial etc.
