...
#ifndef CHFIELDELEMENTLOADABLESURFACE_H
#define CHFIELDELEMENTLOADABLESURFACE_H
...
/// This is used to apply UV loads of ChLoaderUVdistributed type, like pressure (for problems 
/// where the field is displacement), heat flux (for problems where the field is temperature), etc.

class ChApi ChFieldElementLoadableSurface : public ChLoadableUV {
public:
    /// Construct a loadable surface the given a surface element (ex. a ChFieldElementTetrahedron4Face) and a field.
    /// For example if the field is ChFieldTemperature, this can be used to receive a 
    /// surface heat flux loader. If the field is ChFieldDisplacement, this can receive pressure loaders, etc.
    ChFieldElementLoadableSurface(std::shared_ptr<ChFieldElementSurface> element, std::shared_ptr<ChFieldBase> field) : m_element(element), m_field(field) {}
    ...
    virtual bool IsTriangleIntegrationNeeded() override { return m_element->IsTriangleIntegrationCompatible(); }
...
private:
    std::shared_ptr<ChFieldElementSurface> m_element;  ///< associated element
