...
void ChMeshSurface::AddFacesFromNodeSet(std::vector<std::shared_ptr<ChNodeFEAbase> >& node_set) {
    ...
    for (unsigned int ie = 0; ie < this->mmesh->GetNelements(); ++ie) {
        if (auto mtetra = std::dynamic_pointer_cast<ChElementTetra_4>(this->mmesh->GetElement(ie))) {
            ...
            if (n0 && n1 && n2) {
                auto mface = chrono::make_shared<ChFaceTetra_4>(mtetra, 3);
                this->AddFace(mface);
            }
            if (n1 && n2 && n3) {
                auto mface = chrono::make_shared<ChFaceTetra_4>(mtetra, 0);
                this->AddFace(mface);
            }
            if (n0 && n2 && n3) {
                auto mface = chrono::make_shared<ChFaceTetra_4>(mtetra, 1);
                this->AddFace(mface);
            }
            if (n0 && n1 && n3) {
                auto mface = chrono::make_shared<ChFaceTetra_4>(mtetra, 2);
                this->AddFace(mface);
            }
        }
        ...
        if (auto mtetra = std::dynamic_pointer_cast<ChElementTetra_4>(mmesh->GetElement(ie))) {
            for (int nface = 0; nface < 4; ++nface) {
                ...
                if (face_map.count(mface_key) == 1) {
                    ...
                    auto boundary_face = chrono::make_shared<ChFaceTetra_4>(mtetra, nface);
