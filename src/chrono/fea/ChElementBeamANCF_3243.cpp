...
void ChElementBeamANCF_3243::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA, std::shared_ptr<ChNodeFEAxyzDDD> nodeB) {
    assert(nodeA);
    assert(nodeB);
...
void ChElementBeamANCF_3243::SetDimensions(double lenX, double thicknessY, double thicknessZ) {
    m_lenX = lenX;
...
void ChElementBeamANCF_3243::SetMaterial(std::shared_ptr<ChMaterialBeamANCF> beam_mat) {
    m_material = beam_mat;
...
void ChElementBeamANCF_3243::SetAlphaDamp(double a) {
    m_Alpha = a;
...
void ChElementBeamANCF_3243::SetIntFrcCalcMethod(IntFrcMethod method) {
    m_method = method;
...
void ChElementBeamANCF_3243::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementBeamANCF_3243::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementBeamANCF_3243::ComputeNodalMass() {
...
void ChElementBeamANCF_3243::LoadableStateIncrement(const unsigned int off_x,
                                                    ChState& x_new,
                                                    const ChState& x,
...
void ChElementBeamANCF_3243::PrecomputeInternalForceMatricesWeights() {
    if (m_method == IntFrcMethod::ContInt)
...
void ChElementBeamANCF_3243::PrecomputeInternalForceMatricesWeightsContInt() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementBeamANCF_3243::PrecomputeInternalForceMatricesWeightsPreInt() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    unsigned int GQ_idx_xi = NP - 1;        // Gauss-Quadrature table index for xi
    unsigned int GQ_idx_eta_zeta = NT - 1;  // Gauss-Quadrature table index for eta and zeta
...
void ChElementBeamANCF_3243::ComputeInternalJacobianContIntNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    // Calculate the Jacobian of the generalize internal force vector using the "Continuous Integration" style of method
...
void ChElementBeamANCF_3243::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta) {
...
ChQuadratureTables* ChElementBeamANCF_3243::GetStaticGQTables() {
