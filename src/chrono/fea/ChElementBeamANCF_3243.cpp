...
void ChElementBeamANCF_3243::GetGreenLagrangeStrain(const double xi,
                                                    const double eta,
                                                    const double zeta,
                                                    ChMatrix33<>& E) {
...
void ChElementBeamANCF_3243::GetPK2Stress(const double xi, const double eta, const double zeta, ChMatrix33<>& SPK2) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
void ChElementBeamANCF_3243::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    // assumed constant throughout the entire simulation.  However, this implementation assumes that the acceleration due
    // to gravity, while a constant for the entire system, can change from step to step which could be useful for gravity
    // loaded units tests as an example.
    if (m_gravity_on) {
