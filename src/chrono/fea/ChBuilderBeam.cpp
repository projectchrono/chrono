...
void ChBuilderBeam::BuildBeam(std::shared_ptr<ChMesh> mesh,                 ///< mesh to store the resulting elements
    ...
    auto nodeA = chrono::make_shared<ChNodeFEAxyzrot>(ChFrame<>(A, mrot));
    mesh->AddNode(nodeA);
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        auto nodeB = chrono::make_shared<ChNodeFEAxyzrot>(ChFrame<>(pos, mrot));
        mesh->AddNode(nodeB);
        beam_nodes.push_back(nodeB);

        auto element = chrono::make_shared<ChElementBeamEuler>();
...
void ChBuilderBeam::BuildBeam(std::shared_ptr<ChMesh> mesh,                 ///< mesh to store the resulting elements
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        if (i < N) {
            nodeBi = chrono::make_shared<ChNodeFEAxyzrot>(ChFrame<>(pos, mrot));
            mesh->AddNode(nodeBi);
        } else
...
void ChBuilderBeam::BuildBeam(std::shared_ptr<ChMesh> mesh,                 ///< mesh to store the resulting elements
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        auto nodeBi = chrono::make_shared<ChNodeFEAxyzrot>(ChFrame<>(pos, mrot));
        mesh->AddNode(nodeBi);
        beam_nodes.push_back(nodeBi);
...
void ChBuilderBeamIGA::BuildBeam(std::shared_ptr<ChMesh> mesh,                 ///< mesh to store the resulting elements
    ...
    for (int i_node = 0; i_node < N + p; ++i_node) {
        ...
        auto hnode_i = chrono::make_shared<ChNodeFEAxyzrot>(ChFrame<>(pos, mrot));
    ...
    for (int i_el = 0; i_el < N; ++i_el) {
        ...
        auto belement_i = chrono::make_shared<ChElementBeamIGA>();
...
void ChBuilderBeamANCF::BuildBeam(std::shared_ptr<ChMesh> mesh,              ///< mesh to store the resulting elements
    ...
    auto nodeA = chrono::make_shared<ChNodeFEAxyzD>(A, bdir);
    mesh->AddNode(nodeA);
    beam_nodes.push_back(nodeA);
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        auto nodeB = chrono::make_shared<ChNodeFEAxyzD>(pos, bdir);
        mesh->AddNode(nodeB);
        beam_nodes.push_back(nodeB);
...

{
    ...
    auto nodeA = chrono::make_shared<ChNodeFEAxyzDD>(A, bdir, bcur);
    mesh->AddNode(nodeA);
    beam_nodes.push_back(nodeA);
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        auto nodeB = chrono::make_shared<ChNodeFEAxyzDD>(posB, bdir, bcur);
        auto nodeC = chrono::make_shared<ChNodeFEAxyzDD>(posC, bdir, bcur);
        mesh->AddNode(nodeB);
        mesh->AddNode(nodeC);
...
ChExtruderBeamEuler::ChExtruderBeamEuler(
    ...
    ground = chrono::make_shared<ChBody>();
    ground->SetBodyFixed(true);
    mysystem->Add(ground);

    auto nodeA = chrono::make_shared<ChNodeFEAxyzrot>(ChFrame<>(outlet));
    ...
    /*
        actuator = chrono::make_shared<ChLinkMotorLinearPosition>();
        mysystem->Add(actuator);
        actuator->Initialize(nodeA, ground, false, ChFrame<>(outlet), ChFrame<>(outlet));
        actuator->SetMotionFunction(chrono::make_shared<ChFunction_Ramp>(0,this->speed));
        actuator->SetMotionOffset( this->h);
    */
    actuator = chrono::make_shared<ChLinkMotorLinearSpeed>();
    mysystem->Add(actuator);
    actuator->Initialize(nodeA, ground, false, ChFrame<>(outlet), ChFrame<>(outlet));
    actuator->SetSpeedFunction(chrono::make_shared<ChFunction_Const>(this->speed));
    actuator->SetMotionOffset(this->h);
}
...
void ChExtruderBeamEuler::SetContact(std::shared_ptr<ChMaterialSurfaceSMC> mcontact_material, double mcontact_radius) {
    ...
    this->contactcloud = chrono::make_shared<ChContactSurfaceNodeCloud>();
...
void ChExtruderBeamEuler::Update() {
    ...
    if (d1 >= 0) {
        ...
        auto node0 = chrono::make_shared<ChNodeFEAxyzrot>(ChFrame<>(C0));
    ...
    if (d1 >= 0) {
        ...
        if (this->beam_nodes.size() > p) {
            ...
            */
            ...
            auto element = chrono::make_shared<ChElementBeamIGA>();
