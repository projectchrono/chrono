...
void ChMesh::SetupInitial() {
    ...
    for (unsigned int i = 0; i < vnodes.size(); i++) {
        if (!vnodes[i]->IsFixed()) {
            ...
            n_dofs += vnodes[i]->GetNdofX();
            n_dofs_w += vnodes[i]->GetNdofW();
        }
    }
...
void ChMesh::IntStateGather(const unsigned int off_x,
    ...
    for (unsigned int j = 0; j < vnodes.size(); j++) {
        if (!vnodes[j]->IsFixed()) {
            ...
            local_off_x += vnodes[j]->GetNdofX();
            local_off_v += vnodes[j]->GetNdofW();
        }
    }
    ...
    if (automatic_gravity_load && this->system) {
        ...
        for (int in = 0; in < vnodes.size(); in++) {
            if (!vnodes[in]->IsFixed()) {
                ...
                local_off_v += vnodes[in]->GetNdofW();
            }
        }
    }
}
