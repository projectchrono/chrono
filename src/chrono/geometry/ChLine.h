...
class ChApi ChLine : public ChGeometry {
  public:
    ...
    virtual ChVector3d Evaluate(double parU) const = 0;
    ...
    /// For a line, this is always 1.
    virtual int GetManifoldDimension() const override { return 1; }

    /// Find the parameter resU for the nearest point on curve to "point".
    bool FindNearestLinePoint(const ChVector3d& point, double& resU, double approxU, double tol) const;
    ...
    /// Return adimensional information on "how much" this curve is similar to another in its overall shape.
    /// This is independent of parametrization or start point (e.g., try with 20 samples).
    /// The return value is somewhat the "average distance between the two curves".
    /// Note that the result is affected by "weight" of curves. If it changes from default 1.0, the
    /// distance estimation is higher/lower (e.g.: if a curve defines low 'weight' in its central segment,
    /// its CurveCurveDistance from another segment is not much affected by errors near the central segment).
    double CurveCurveDist(const ChLine* compline, int samples) const;

    /// Same as before, but returns "how near" is \a complinesegm to whatever segment of this line (does not matter the percentual of line).
    /// Again, this is affected by "weight" of curves.
    /// If weight changes along curves -> 'weighted' distance.
    double CurveSegmentDist(const ChLine* complinesegm, int samples) const;

    /// Same as above, but instead of making average of the distances, these functions return the maximum of the distances.
    double CurveCurveDistMax(const ChLine* compline, int samples) const;
    double CurveSegmentDistMax(const ChLine* complinesegm, int samples) const;
