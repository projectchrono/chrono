...
/// Note: differently from ChFunctionRecorder, this function does not allow
/// not-uniform dx spacing between points, but may be faster and simpler to
/// use in many cases.
class ChApi ChFunctionOscilloscope : public ChFunction {
  private:
  ...
  public:
    ChFunctionOscilloscope() : end_x(0), dx(0.01), max_amount(100), amount(0) {}
    ChFunctionOscilloscope(const ChFunctionOscilloscope& other);
    ~ChFunctionOscilloscope() {}

    /// "Virtual" copy constructor (covariant return type).
    virtual ChFunctionOscilloscope* Clone() const override { return new ChFunctionOscilloscope(*this); }
...
CH_CLASS_VERSION(ChFunctionOscilloscope, 0)
