...
CH_FACTORY_REGISTER(ChFunctionMocap)

ChFunctionMocap::ChFunctionMocap() {
    Set_samples(2);  // this creates arrays
    Set_samp_freq(50);
}

ChFunctionMocap::ChFunctionMocap(int m_samples, double freq) {
    Set_samples(m_samples);  // this creates arrays
    Set_samp_freq(freq);
}

ChFunctionMocap::ChFunctionMocap(const ChFunctionMocap& other) {
...
void ChFunctionMocap::Estimate_x_range(double& xmin, double& xmax) const {
    xmin = 0.0;
    xmax = Get_timetot();
}

void ChFunctionMocap::Set_samp_freq(double m_fr) {
    samp_freq = m_fr;
    timetot = ((double)samples / samp_freq);
}

void ChFunctionMocap::Set_samples(int m_samples) {
    samples = m_samples;
...
void ChFunctionMocap::Compute_array_dt(const ChArray<>& array_A, ChArray<>& array_A_dt) const {
    int i, ia, ib;
    double y_dt;
...
double ChFunctionMocap::LinInterp(const ChArray<>& array, double x, double x_max) const {
    double position;
...
void ChFunctionMocap::Set_array_y(const ChArray<>& m_array_y) {
    array_y = m_array_y;
...
void ChFunctionMocap::Set_array_y_dt(const ChArray<>& m_array_y_dt) {
    // *** TO DO  ***
}

void ChFunctionMocap::Set_array_y_dtdt(const ChArray<>& m_array_y_dtdt) {
    // *** TO DO  ***
}

// Parsing of external files, to create mocap streams
// from the output of mocap devices
bool ChFunctionMocap::Parse_array_AOA() {
    // *** TO DO **** //
    return true;
}

bool ChFunctionMocap::Parse_array_Elite() {
    // *** TO DO **** //
    return true;
}
...
double ChFunctionMocap::Get_y(double x) const {
    return LinInterp(array_y, x, timetot);
}

double ChFunctionMocap::Get_y_dx(double x) const {
    return LinInterp(array_y_dt, x, timetot);
}

double ChFunctionMocap::Get_y_dxdx(double x) const {
    return LinInterp(array_y_dtdt, x, timetot);
}

void ChFunctionMocap::ArchiveOut(ChArchiveOut& marchive) {
    // version number
    marchive.VersionWrite<ChFunctionMocap>();
...
void ChFunctionMocap::ArchiveIn(ChArchiveIn& marchive) {
    // version number
    /*int version =*/ marchive.VersionRead<ChFunctionMocap>();
