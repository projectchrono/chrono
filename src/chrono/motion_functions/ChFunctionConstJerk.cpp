...
ChFunctionConstJerk::ChFunctionConstJerk(
    double q0, 
    double q1, 
    double v0, 
    double v1, 
...
ChFunctionConstJerk::ChFunctionConstJerk(double q0,    
                                       double q1,    
                                       double vmax,  
                                       double amax,  
                                       double jmax   
...
void ChFunctionConstJerk::Setup_Data(double q0, double q1, double v0, double v1, double T, double Ta, double Tj) {
...
void ChFunctionConstJerk::Setup_Data(double q0, double q1, double vmax, double amax, double jmax) {
    // Adjust motion law sign
    m_sign = (q1 > q0) ? 1 : -1;
...
void ChFunctionConstJerk::Setup_Data(bool& feasible,
                                    double q0,
                                    double q1,
                                    double v0,
                                    double v1,
    ...
    if (m_Tv <= 0) {                  // no constant speed tract
        ...
        double delta = pow(m_amax, 4) / pow(m_jmax, 2) + 2 * (pow(m_v0, 2) + pow(m_v1, 2)) +
    ...
    if (m_Ta < 0 || m_Td < 0) {
        if (m_Ta < 0) {
            m_Ta = 0;  // adjust
            m_Td = 2 * h / (m_v1 + m_v0);
            m_Tj2 = (m_jmax * h - sqrt(m_jmax * (m_jmax * pow(h, 2) + pow(m_v1 + m_v0, 2) * (m_v1 - m_v0)))) /
                    (m_jmax * (m_v1 + m_v0));
        } else if (m_Td < 0) {
            m_Td = 0;  // adjust
            m_Ta = 2 * h / (v1 + v0);
            m_Tj1 = (m_jmax * h - sqrt(m_jmax * (m_jmax * pow(h, 2) - pow(m_v1 + m_v0, 2) * (m_v1 - m_v0)))) /
                    (m_jmax * (m_v1 + m_v0));
        }
    } else {
...
double ChFunctionConstJerk::Get_y(double x) const {
    double y = 0;
    ...
    if (x >= 0 && x < m_Tj1)
        ...
        y = m_q0 + (m_vlim + m_v0) * m_Ta / 2. - m_vlim * (m_Ta - x) +
            m_jmax * pow((m_Ta - x), 3) / 6.;  // +jmax = -jmin
    ...
    // Deceleration phase
        ...
        y = m_q1 - (m_vlim + m_v1) * m_Td / 2. + m_vlim * (x - m_T + m_Td) +
            m_alim_d / 6. * (3 * pow((x - m_T + m_Td), 2) - 3 * m_Tj2 * (x - m_T + m_Td) + pow(m_Tj2, 2));
...
double ChFunctionConstJerk::Get_y_dx(double x) const {
    double y_dx = 0;
...
double ChFunctionConstJerk::Get_y_dxdx(double x) const {
    double y_dxdx = 0;
...
double ChFunctionConstJerk::Get_y_dxdxdx(double x) const {
    double y_dxdxdx = 0;
...
void ChFunctionConstJerk::Get_Bounds(double& q0, double& q1, double& v0, double& v1) {
    q0 = m_q0;
    q1 = m_q1;
    v0 = m_v0;
    v1 = m_v1;
...
void ChFunctionConstJerk::Get_Constraints(double& vmax, double& amax, double& jmax) {
    vmax = m_vmax;
    amax = m_amax;
    jmax = m_jmax;
}

void ChFunctionConstJerk::Get_Times(double& T, double& Ta, double& Tv, double& Td, double& Tj1, double& Tj2) {
    T = m_T;
    Ta = m_Ta;
    Tv = m_Tv;
    Td = m_Td;
...
void ChFunctionConstJerk::Get_Limits(double& vlim, double& alim_a, double& alim_d) {
    vlim = m_vlim;
    alim_a = m_alim_a;
    alim_d = m_alim_d;
}
