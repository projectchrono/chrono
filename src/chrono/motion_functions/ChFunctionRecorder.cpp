...
CH_FACTORY_REGISTER(ChFunctionRecorder)

ChFunctionRecorder::ChFunctionRecorder(const ChFunctionRecorder& other) {
    m_points = other.m_points;
    m_last = m_points.end();
}

void ChFunctionRecorder::Estimate_x_range(double& xmin, double& xmax) const {
    if (m_points.empty()) {
        xmin = 0.0;
        xmax = 1.2;
        return;
...
void ChFunctionRecorder::AddPoint(double mx, double my, double mw) {
...
double ChFunctionRecorder::Get_y(double x) const {
    if (m_points.empty()) {
        return 0;
    }
...
double ChFunctionRecorder::Get_y_dx(double x) const {
    //// TODO:  can we do better?
    return ChFunction::Get_y_dx(x);
}

double ChFunctionRecorder::Get_y_dxdx(double x) const {
    //// TODO: can we do better?
    return ChFunction::Get_y_dxdx(x);
}

void ChFunctionRecorder::ArchiveOut(ChArchiveOut& marchive) {
    // version number
    marchive.VersionWrite<ChFunctionRecorder>();
...
void ChFunctionRecorder::ArchiveIn(ChArchiveIn& marchive) {
    // version number
    /*int version =*/ marchive.VersionRead<ChFunctionRecorder>();
