...
int ChLinearAlgebra::BestPivotRow(ChMatrix<>& A, int current) {
    double temp = 0;
    int i;
...
// Gets the best row-col index to swap with the "i"current, using the diagonal pivot.
int ChLinearAlgebra::BestPivotDiag(ChMatrix<>& A, int current) {
    double temp = 0;
    int i;
    int pivotrow = current;
...
void ChLinearAlgebra::DiagPivot(ChMatrix<>& A, int rowA, int rowB) {
    A.SwapColumns(rowA, rowB);
    A.SwapRows(rowA, rowB);
}

// Performs a diagonal pivot on a _symmetric_ square matrix, acting only on the _UPPER_ triangle, for storage
// convenience and speed optimizations.
void ChLinearAlgebra::DiagPivotSymmetric(ChMatrix<>& A, int rowA, int rowB) {
    int elcol, elrow;
    double temp;
...
int ChLinearAlgebra::Solve_LinSys(ChMatrix<>& A, ChMatrix<>* B, ChMatrix<>* X, int* pivarray, double* det) {
...
void ChLinearAlgebra::Solve_LinSys(ChMatrix<>& A, ChMatrix<>* B, ChMatrix<>* X) {
    double det;
...
int ChLinearAlgebra::Decompose_LU(ChMatrix<>& A, int* pivarray, double* det) {
    int i, j, k, pivrow, eqpivoted;
    double r, aj, pivot;
    int err = 0;
    int rows = A.GetRows();
...
// After LU decomposition, with Decompose_LU(), call this to solve [A]X=B, as [L][U]X=B
void ChLinearAlgebra::Solve_LU(ChMatrix<>& A, ChMatrix<>* B, ChMatrix<>* X, int* pivarray) {
    int k, j;
    double sum, x;
    int rows = A.GetRows();
...
void ChLinearAlgebra::Extract_LU(ChMatrix<>& A, ChMatrix<>* mL, ChMatrix<>* mU) {
    int i, j;
    int rows = A.GetRows();
...
int ChLinearAlgebra::Decompose_LDL(ChMatrix<>& A, int* pivarray, double* det) {
    int i, j, k, pivrow, eqpivoted;
...
// After LDL decomposition with Decompose_LDL(), call this to solve [A]X=B, as [L][D][L]'X=B
void ChLinearAlgebra::Solve_LDL(ChMatrix<>& A, ChMatrix<>* B, ChMatrix<>* X, int* pivarray) {
    double sum, x;
    int j, k;
    int rows = A.GetRows();
...
void ChLinearAlgebra::Extract_LDL(ChMatrix<>& A, ChMatrix<>* L, ChMatrix<>* D, ChMatrix<>* Lu) {
    int i, j;
    int rows = A.GetRows();
    int columns = A.GetColumns();
...
double ChLinearAlgebra::Det(ChMatrix<>& A) {
...
double ChLinearAlgebra::Invert(ChMatrix<>& A, ChMatrix<>* mA) {
...
double ChLinearAlgebra::Invert(ChMatrix<>& A) {
...
double ch_sign(double a, double b) {
    if (b >= 0.0)
        return (fabs(a));
    else
        return (-fabs(a));
...
int ChLinearAlgebra::SVD(ChMatrix<>& mA, ChMatrix<>& U, ChMatrix<>& W, ChMatrix<>& V, double& cond_num) {
...
double ChLinearAlgebra::ConditionNumber(ChMatrix<>& A) {
