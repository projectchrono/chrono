...
void ChCSR3Matrix::SetElement(int insrow, int inscol, double insval, bool overwrite) {
    ...
    int trailInd_sel = leadIndex_vect[insleaddim];
    while (1) {
        // case: element not found in the row OR another element with a higher col number is already been stored
        if (trailInd_sel >= leadIndex_vect[insleaddim + 1] || trailIndex_vect[trailInd_sel] > instraildim) {
            if (insval != 0 || m_lock)  // avoid to insert zero elements
                insert(insleaddim, instraildim, insval, trailInd_sel);
            break;
        }

        // case: empty space
        if (trailIndex_vect[trailInd_sel] == -1) {
            if (insval != 0 || m_lock)  // avoid to insert zero elements
            {
                values_vect[trailInd_sel] = insval;
                trailIndex_vect[trailInd_sel] = instraildim;
            }
            break;
        }

        // case: element already allocated
        if (trailIndex_vect[trailInd_sel] == instraildim) {
...
void ChCSR3Matrix::insert(int insleaddim, int instraildim, double insval, int& trailInd_sel) {
    m_lock_broken = true;
    int traildim_shift = 1;            // an offset from the current position that points to the empty location found
    int trailInd_sel_empty = trailInd_sel;  // the location in which the new element will be put (if a space will be found it
                                  // will be different from trailInd_sel)
    ...
    int leadInd_sel_bw = insleaddim - 1;
    int trailInd_sel_fw = insleaddim + 1;
    ...
    // very rarely.
    while (traildim_shift < max_shifts && trailInd_sel - traildim_shift > -1 && trailInd_sel + traildim_shift < leadIndex_vect[m_num_rows]) {
        // backward check
        if (trailIndex_vect[trailInd_sel - traildim_shift] == -1 && !m_lock) {
            // This part is very specific: it avoids to write to another row that has only one element that it's
            // uninitialized;
            for (; leadIndex_vect[leadInd_sel_bw] > trailInd_sel - traildim_shift && leadInd_sel_bw >= 0; leadInd_sel_bw--) {
            }
            if (leadIndex_vect[leadInd_sel_bw] == trailInd_sel - traildim_shift) {
                if (leadInd_sel_bw == 0)
                    break;
                traildim_shift++;
                continue;
            ...
            trailInd_sel_empty = trailInd_sel - traildim_shift;
            break;
        }

        // forward check
        if (trailIndex_vect[trailInd_sel + traildim_shift] == -1) {
            // This part is very specific: it avoids to write to another row that has only one element that it's
            // uninitialized;
            for (; leadIndex_vect[trailInd_sel_fw] < trailInd_sel + traildim_shift && trailInd_sel_fw <= m_num_rows; trailInd_sel_fw++) {
            }
            if (leadIndex_vect[trailInd_sel_fw] == trailInd_sel + traildim_shift) {
                if (trailInd_sel_fw == m_num_rows)
                    break;
                traildim_shift++;
                continue;
            }
    ...
    while (!m_lock && trailInd_sel_empty == trailInd_sel && traildim_shift < max_shifts && trailInd_sel - traildim_shift > -1) {
        // backward check
        if (trailIndex_vect[trailInd_sel - traildim_shift] == -1) {
            // This part is very specific: it avoids to write to another row that has only one element that it's
            // uninitialized;
    ...
    while (trailInd_sel_empty == trailInd_sel && traildim_shift < max_shifts && trailInd_sel + traildim_shift < leadIndex_vect[m_num_rows]) {
        // forward check
        if (trailIndex_vect[trailInd_sel + traildim_shift] == -1) {
            // This part is very specific: it avoids to write to another row that has only one element that it's
            // uninitialized;
    ...
    // If an uninitialized location is found "trailInd_sel_empty" should point at it, so it would be different from
    // "trailInd_sel".

    /****************** STEP 2 ******************/

    // STEP 2: shift the array to make space for the new element; eventually update "trailInd_sel"
    // case 1: the uninitialized location is found forward;
    // case 2: the uninitialized location is found backward;
    // case 3: the location is not found in the neighborhood ("max_shifts") of the "trailInd_sel" cell; a reallocation is
    // needed.

    if (trailInd_sel_empty > trailInd_sel && trailInd_sel + traildim_shift < leadIndex_vect[m_num_rows] && traildim_shift < max_shifts) {
        // case 1
        for (int trailInd_sel_temp = trailInd_sel_empty; trailInd_sel_temp > trailInd_sel; trailInd_sel_temp--) {
            values_vect[trailInd_sel_temp] = values_vect[trailInd_sel_temp - 1];
            trailIndex_vect[trailInd_sel_temp] = trailIndex_vect[trailInd_sel_temp - 1];
        }

        for (int leadInd_sel = insleaddim + 1; leadIndex_vect[leadInd_sel] < trailInd_sel_empty; leadInd_sel++) {
            leadIndex_vect[leadInd_sel]++;
        }
    } else if (trailInd_sel_empty < trailInd_sel && trailInd_sel - traildim_shift > -1 && traildim_shift < max_shifts) {
        // case 2
        assert(!m_lock);
        trailInd_sel--;
        for (int trailInd_sel_temp = trailInd_sel_empty; trailInd_sel_temp < trailInd_sel; trailInd_sel_temp++) {
        ...
        for (int leadInd_sel = insleaddim; leadInd_sel > -1 && leadIndex_vect[leadInd_sel] > trailInd_sel_empty; leadInd_sel--) {
            leadIndex_vect[leadInd_sel]--;
        }
    } else {
        // case 3
        ...
        if (trailIndex_vect.capacity() > GetLeadingIndexLength()) {
            copy(values_vect.data(), trailIndex_vect.data(), false, insleaddim, trailInd_sel, 1);
        } else {
            // Actual reallocation
            std::vector<double, aligned_allocator<double, 64>> new_values_vect;
            std::vector<int, aligned_allocator<int, 64>> new_leadInd_vect_vect;

            int new_capacity = trailIndex_vect.capacity()*1.75;
            new_values_vect.resize(new_capacity);
            new_leadInd_vect_vect.resize(new_capacity);

            copy(new_values_vect.data(), new_leadInd_vect_vect.data(), false, insleaddim, trailInd_sel, new_capacity-trailIndex_vect.capacity());
            
            values_vect = std::move(new_values_vect);
            trailIndex_vect = std::move(new_leadInd_vect_vect);
...
void ChCSR3Matrix::initialize(int* nonzeros_vector) {
    ...
    leadIndex_vect[0] = 0;
    for (int leadInd_sel = 0; leadInd_sel < m_num_rows; leadInd_sel++)  // leadIndex_vect is initialized with equally spaced indexes
    {
        leadIndex_vect[leadInd_sel + 1] = leadIndex_vect[leadInd_sel] + nonzeros_vector[leadInd_sel];
    }

    initialize_ValuesColIndex();
}

void ChCSR3Matrix::initialize(int leadInd_vect_length) {
    if (!m_lock || m_lock_broken) {
        if (leadInd_vect_length == 0)
            leadInd_vect_length = trailIndex_vect.capacity();

        // leadIndex_vect is initialized with equally spaced indexes
        for (int leadInd_sel = 0; leadInd_sel <= m_num_rows; leadInd_sel++) {
            leadIndex_vect[leadInd_sel] = static_cast<int>(ceil(static_cast<double>(leadInd_sel) * (static_cast<double>(leadInd_vect_length) + 1.0) /
...
void ChCSR3Matrix::initialize_ValuesColIndex() {
    if (m_lock && !m_lock_broken)
        for (int trailInd_sel = 0; trailInd_sel < leadIndex_vect[m_num_rows]; trailInd_sel++)
            values_vect[trailInd_sel] = 0;

    else {
        // trailIndex_vect is initialized with -1; it means that the cell has been stored but contains an uninitialized value
        for (int trailInd_sel = 0; trailInd_sel < leadIndex_vect[m_num_rows]; trailInd_sel++)
            trailIndex_vect[trailInd_sel] = -1;

        isCompressed = false;
    }
}

/** Copies from/to \c values_vect and \c trailIndex_vect arrays to/from the specified arrays \c values_temp and \c leadInd_vect_temp.
...
*		1. at \c trailInd_sel a new space is created;
...
void ChCSR3Matrix::copy(double* values_temp,
    ...
    int *leadInd_vect_destination, *leadInd_vect_source;
    ...
    if (to_internal_arrays) {
        ...
        leadInd_vect_destination = trailIndex_vect.data();
        values_source = values_temp;
        leadInd_vect_source = leadIndex_temp;
    } else {
        values_destination = values_temp;
        leadInd_vect_destination = leadIndex_temp;
        values_source = values_vect.data();
        leadInd_vect_source = trailIndex_vect.data();
    }
    ...
    int trailInd_sel_destination;
    for (trailInd_sel_destination = leadIndex_vect[m_num_rows] + shifts - 1;
         trailInd_sel_destination - remaining_shifts >= leadIndex_vect[insleaddim + 1]; trailInd_sel_destination--) {
        values_destination[trailInd_sel_destination] = values_source[trailInd_sel_destination - remaining_shifts];
        leadInd_vect_destination[trailInd_sel_destination] = leadInd_vect_source[trailInd_sel_destination - remaining_shifts];
    }
    // The destination array	|| | | | | || | | | ||  ||  |  ||  |26|31||49|21|39|44|~|

    for (remaining_shifts = shifts; remaining_shifts > 1; remaining_shifts--, trailInd_sel_destination--) {
        leadInd_vect_destination[trailInd_sel_destination] = -1;
    }
    // The destination array	|| | | | | || | | | ||  ||-1|-1||-1|26|31||49|21|39|44|~|

    while (trailInd_sel_destination - remaining_shifts >= trailInd_sel) {
        values_destination[trailInd_sel_destination] = values_source[trailInd_sel_destination - remaining_shifts];
        leadInd_vect_destination[trailInd_sel_destination] = leadInd_vect_source[trailInd_sel_destination - remaining_shifts];
        trailInd_sel_destination--;
    }
    // The destination array	|| | | | | || | |14|17||47||-1|-1||-1|26|31||49|21|39|44|~|

    if (values_destination != values_source || leadInd_vect_destination != leadInd_vect_source) {
        while (trailInd_sel_destination >= 0) {
            values_destination[trailInd_sel_destination] = values_source[trailInd_sel_destination];
            leadInd_vect_destination[trailInd_sel_destination] = leadInd_vect_source[trailInd_sel_destination];
            trailInd_sel_destination--;
        }
    }
    ...
    if (shifts > 0) {
        int leadInd_sel = insleaddim + 1;
        while (leadInd_sel <= leading_dimension) {
            leadIndex_vect[leadInd_sel] += shifts;
            leadInd_sel++;
        }
    }
...
void ChCSR3Matrix::GetNonZerosDistribution(int* nonzeros_vector) const {
    for (int leadInd_sel = 0; leadInd_sel < leading_dimension; leadInd_sel++) {
        nonzeros_vector[leadInd_sel] = leadIndex_vect[leadInd_sel + 1] - leadIndex_vect[leadInd_sel];
    }
}
...
int ChCSR3Matrix::VerifyMatrix() const {
    ...
    for (int leadInd_sel = 0; leadInd_sel < leading_dimension; leadInd_sel++) {
        ...
        if (leadIndex_vect[leadInd_sel] >= leadIndex_vect[leadInd_sel + 1])
            return -1;

        bool initialized_elements_found = false;

        int trailInd_sel = leadIndex_vect[leadInd_sel + 1];
        while (trailInd_sel > leadIndex_vect[leadInd_sel]) {
            trailInd_sel--;
            if (trailIndex_vect[trailInd_sel] == -1) {
                ...
                if (trailInd_sel > leadIndex_vect[leadInd_sel] && trailIndex_vect[trailInd_sel] <= trailIndex_vect[trailInd_sel - 1])
                    return -4;
            }
        }
        if (!initialized_elements_found)
...
void ChCSR3Matrix::ImportFromDatFile(std::string path) {
    ...
    for (leadInd_sel = 0; leadInd_sel <= leading_dimension; leadInd_sel++)
        ia_file >> leadIndex_vect[leadInd_sel];
    ...
    int trailInd_sel;
    for (trailInd_sel = 0; trailInd_sel < leadIndex_vect[leadInd_sel]; trailInd_sel++) {
        a_file >> values_vect[trailInd_sel];
        ja_file >> trailIndex_vect[trailInd_sel];
    }

    if (trailInd_sel != leadIndex_vect[leadInd_sel])
        assert(0);
...
double& ChCSR3Matrix::Element(int row, int col) {
    ...
    // case 2: an empty location is found: no arrangement needed. (trailIndex_vect from the beginning of the row until trailInd_sel
    // should be non-"-1" i.e. should have stored element)
    // case 3: an element with exactly the same column index is found
    // IN ANY CASE, exiting the WHILE cycle, "trailInd_sel" should point to the place where the new element should be!

    int trailInd_sel = leadIndex_vect[leaddim];
    while (1) {
        if (trailInd_sel >= leadIndex_vect[leaddim + 1] || trailIndex_vect[trailInd_sel] > traildim) {  // case 1a and 1b
            insert(leaddim, traildim, 0.0, trailInd_sel);
        ...
        if (trailIndex_vect[trailInd_sel] == -1)  // case 2
        {
            values_vect[trailInd_sel] = 0.0;
            // TODO: if Element is called as a constant i.e. as a rvalue you could not-write the value and return 0
            // without storing any extra space
            trailIndex_vect[trailInd_sel] = traildim;
            break;
        }

        if (trailIndex_vect[trailInd_sel] == traildim) {
            break;
        }
        trailInd_sel++;
    }
...
bool ChCSR3Matrix::Compress() {
    ...
    // Compress the matrix
    int trailInd_sel_new = 0;
    int leadInd_sel = 0;

    for (int trailInd_sel = 0; trailInd_sel < leadIndex_vect[leading_dimension]; trailInd_sel++) {
        // if an element is not initialized it would simply skip its copy
        if (trailIndex_vect[trailInd_sel] > -1) {
            trailIndex_vect[trailInd_sel_new] = trailIndex_vect[trailInd_sel];
            values_vect[trailInd_sel_new] = values_vect[trailInd_sel];
            trailInd_sel_new++;
        }
        ...
        if (trailInd_sel == leadIndex_vect[leadInd_sel]) {
            if (trailIndex_vect[trailInd_sel] == -1) {
                trailIndex_vect[trailInd_sel_new] = std::min(leadInd_sel, trailing_dimension);
                values_vect[trailInd_sel_new] = 0;
                trailInd_sel_new++;
            }
            leadIndex_vect[leadInd_sel] = trailInd_sel_new - 1;
            leadInd_sel++;
        }
    }

    leadIndex_vect[leadInd_sel] = trailInd_sel_new;
    isCompressed = true;
    m_lock_broken = false;
...
void ChCSR3Matrix::Prune(double pruning_threshold) {
    if (pruning_threshold == 0) {
        for (int trailInd_sel = 0; trailInd_sel < leadIndex_vect[m_num_rows]; trailInd_sel++) {
            if (values_vect[trailInd_sel] == 0)
                trailIndex_vect[trailInd_sel] = -1;
        }
    } else {
        for (int trailInd_sel = 0; trailInd_sel < leadIndex_vect[m_num_rows]; trailInd_sel++) {
            if (std::abs(values_vect[trailInd_sel]) < pruning_threshold)
                trailIndex_vect[trailInd_sel] = -1;
        }
    }
}
