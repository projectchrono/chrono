...
class ChHashTable {
  public:
    ...
    /// exception
    /// the non-const version is used by the compiler on non-const hashes and is like map - it creates a new key/data
    /// pair if find fails
...
template <typename K, typename T, class H, class E, typename V>
ch_hash_iterator<K, T, H, E, V>::ch_hash_iterator(const ChHashTable<K, T, H, E>* owner,
                                                  unsigned bin,
                                                  hash_element<K, T>* element)
...
template <typename K, typename T, class H, class E>
std::pair<typename ChHashTable<K, T, H, E>::iterator, bool> ChHashTable<K, T, H, E>::insert(
    ...
    for (hash_element<K, T>* current = m_values[hash_value]; current; previous = current, current = current->m_next) {
        ...
        if (current->m_hash != hash_value_full)
            continue;

        // next try the equality operator
        if (!E()(current->m_value.first, value.first))
            continue;
...
bool ChHashTable<K, T, H, E>::erase(const K& key) {
    ...
    for (hash_element<K, T>* current = m_values[hash_value]; current; previous = current, current = current->m_next) {
        ...
        // next try the equality operator
        if (!E()(current->m_value.first, key))
            continue;
