...
///   ensures use of aligned memory. As such, user code should **always** use `chrono_types::make_shared` as in <pre>
...
/// alignment issues (classes with members that are fixed-sized Eigen matrices have overriden operator new).
/// Note:
///   Since std::make_unique was only introduced in C++14, the default C++11 implementation of chrono_types::make_unique
