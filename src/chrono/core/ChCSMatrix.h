...
/** \class ChCSMatrix
    ...
    When this feature is enabled, the matrix will hold the \e position of the elements - i.e. the sparsity pattern - even after calling a #Reset().
    So, if an element already existed in a given position, the next call to #SetElement() in that position will be much faster.

    \warning When the <em>sparsity lock</em> feature is turned on, also non-zero elements will be stored in the matrix in order to maximize
    the efficiency.

    While building the matrix, the most demanding operation is the \e new elements insertion (specifically, those elements whose indexes are not stored in the matrix yet).\n
    The performance is slow when: (most demanding first)
    1. the arrays capacity is not enough to fit the matrix;
    2. the arrays size is not enough to fit the matrix;
    3. a new element must be inserted in a row that already contains elements with greater column index (swap 'column' and 'row' for CSC).

    Some suggestions:
    - it is better to overestimate the number of non-zero, rather than underestimate;
    - it is better to store the elements in increasing column order, even at the cost of jumping from a row to another (swap 'column' and 'row' for CSC);
    - use <em>sparsity lock</em> feature whenever possible.
*/

class ChApi ChCSMatrix : public ChSparseMatrix {
  private:
    const bool row_major_format = true; ///< if \c true the arrays are stored according to CSR format, otherwise in CSC format
	...
	typedef std::vector<int, aligned_allocator<int, array_alignment>> index_vector_t; ///< type for #leadIndex and #trailIndex vectors (aligned)
	typedef std::vector<double, aligned_allocator<double, array_alignment>> values_vector_t; ///< type for #value vector (aligned)
#else
	...
	index_vector_t leadIndex;    ///< CS vector: #leadIndex[i] tells that #trailIndex[#leadIndex[i]] is the first element of the i-th row (if row-major)
  ...
  protected:
	/// (internal) The \a vector elements will contain equally spaced indexes, going from \a initial_number to \a final_number.
	void static distribute_integer_range_on_vector(index_vector_t& vector, int initial_number, int final_number);
    /// (internal) Actually reset the internal arrays to the given dimensions.
    ...
    /// (internal) Insert a non existing element in the position \a trai_i, given the row(CSR) or column(CSC) \a lead_sel 
	void insert(int& trail_i, const int& lead_sel);
    /// (internal) Copies arrays from source vectors to destination vectors (source and destination vectors can coincide).
    /// In the meantime it puts a given number of not-initialized spaces (\p storage_augm) in the matrix, equally distributed between the rows.
    /// \param[in] trailIndex_src trailing dimension index (source)
    /// \param[in] values_src values array (source)
    /// \param[in] initialized_element_src array with initialized flags (source)
	/// \param[out] trailIndex_dest trailing dimension index (destination)
    /// \param[out] values_dest values array (destination)
    /// \param[out] initialized_element_dest array with initialized flags (destination)
    /// \param[in,out] trail_ins the position in the #trailIndex where a new element must be inserted (the element that caused the insertion)
    /// \param[in] lead_ins the position, referred to the leading dimension, where a new element must be inserted (the element that caused the insertion)
    /// \param[in] storage_augm number of not-initialized spaces to add
  ...
  public:
    /// Create a CSx matrix with the given dimensions.
    /// \param[in] nrows number of rows
    /// \param[in] ncols number of columns
    /// \param[in] row_major_format_on create a Row Major matrix
    /// \param[in] nonzeros number of non-zeros
	...
	/// The \a nonzeros_hint value is just a _hint_.\n
	/// The matrix can be _fully_ reset, or _partially_ reset (i.e. maintaining the sparsity pattern).\n
    ...
    /// The same as #Compress(), but also removes elements whose absolute value is less or equal to \a pruning_threshold.
    ...
    /// Advanced use. While inserting new elements in the matrix ::SetMaxShifts() tells how far in the array the internal algorithm
    /// should look for a not-initialized space.
