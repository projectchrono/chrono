...
// =============================================================================
//
// Chrono-specific extensions to Eigen::SparseMatrix
//
// =============================================================================
...
//ChCSMatrix_Eigen(int nrows, int ncols, bool row_major_format_on, int nonzeros)
...
void SetElement(int row_sel, int col_sel, double insval, bool overwrite) {
...
double GetElement(int row_sel, int col_sel) const {

    return coeff(row_sel, col_sel);
}

double& Element(int row_sel, int col_sel) {
    return coeffRef(row_sel, col_sel);
}

void Reset(int nrows, int ncols, int nonzeros_hint) {
    this->makeCompressed();

    resize(nrows, ncols);
    reserve(nonzeros_hint);

}

int* GetCS_LeadingIndexArray() const {
    return stati_cast<int>(outerIndexPtr());
}

int* GetCS_TrailingIndexArray() const {
    return innerIndexPtr();
}

double* GetCS_ValueArray() const {
    return valuePtr();
}
...
int Inflate(int storage_augm, int lead_sel, int trail_sel) {
...
void Prune(double pruning_threshold) {
...
//}
//
//void ImportFromDatFile(std::string path, bool row_major_format_on) {
...
//}
//
//void ExportToDatFile(std::string filepath, int precision) const {
...
//}
//
//void LoadSparsityPattern(ChSparsityPatternLearner& sparsity_learner) {
...
//}
//
//void insert(int& trail_i_sel, const int& lead_sel) {
//    isCompressed = false;
//    m_lock_broken = true;
//
...
//}
//
//ChCSMatrix_Eigen& operator=(const ChCSMatrix_Eigen& mat_source) {
...
//}
//
//ChCSMatrix_Eigen& apply_operator(const ChCSMatrix_Eigen& mat_source, std::function<void(double&, const double&)> f) {
...
//}
//
//ChCSMatrix_Eigen& operator*=(const double coeff) {
//    assert(coeff != 0 && "operator*= cannot accept coeff = zero");
//
...
//}
//
//bool operator==(const ChCSMatrix_Eigen& mat_source) const {
...
//}
//
//void MatrMultiply(ChMatrixConstRef matB, ChMatrixRef mat_out, bool transposeA) const {
//    transposeA
