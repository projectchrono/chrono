...
#ifndef CH_TRANSFORM_H
#define CH_TRANSFORM_H
...
///
/// A coordinate system (a 'frame') has a translation and a rotation respect to a 'parent' coordinate system, usually
/// the absolute (world) coordinates.  This class implements useful static functions to perform the typical
/// local->parent or parent->local transformations of points. Such functions are static, so there is no need to
/// instantiate a ChTransform object.
...
class ChTransform {
  public:
    ...
    static ChVector3<Real> TransformParentToLocal(
        const ChVector3<Real>& parent,  ///< point to transform, given in parent coordinates;
        const ChVector3<Real>&
        ...
        return ChVector3<Real>(alignment(0, 0) * mx + alignment(1, 0) * my + alignment(2, 0) * mz,
    ...
    static ChVector3<Real> TransformLocalToParent(
        const ChVector3<Real>& local,      ///< point to transform, given in local coordinates
        const ChVector3<Real>& origin,     ///< origin of frame respect to parent, in parent coords,
        const ChMatrix33<Real>& alignment  ///< rotation of frame respect to parent, in parent coords.
    ) {
        return ChVector3<Real>(
    ...
    static ChVector3<Real> TransformParentToLocal(
        ...
        return ChVector3<Real>(((e0e0 + e1e1) * 2. - 1.) * dx + ((e1e2 - e0e3) * 2.) * dy + ((e1e3 + e0e2) * 2.) * dz,
    ...
    static ChVector3<Real> TransformLocalToParent(
        ...
        Real e1e3 = alignment.e1() * alignment.e3();
        Real e2e3 = alignment.e2() * alignment.e3();
        return ChVector3<Real>(origin.x() + ((e0e0 + e1e1) * 2. - 1.) * local.x() + ((e1e2 - e0e3) * 2.) * local.y() +
                                   ((e1e3 + e0e2) * 2.) * local.z(),
