...
/// This class implements the functionality for a constraint between a couple of two objects of type ChVariablesBody.
...
class ChApi ChConstraintTwoBodies : public ChConstraintTwo {
  ...
  public:
    ...
    /// Set references to the constrained objects, each of ChVariablesBody type, automatically creating/resizing
    /// jacobians if needed. If variables are not from ChVariablesBody class, an assert failure happens.
    ...
    /// This function must computes the product between the row-jacobian of this constraint '[Cq_i]' and the vector of
    /// variables 'v', that is CV=[Cq_i]*v.
    /// This is used for some iterative solvers.
    virtual double Compute_Cq_q() override;

    /// This function must increment the vector of variables 'v' with the quantity [invM]*[Cq_i]'*deltal, that is
    ...
    /// NOTE: the 'vect' vector must already have the size of the total variables&constraints in the system; the
    /// procedure will use the ChVariable offsets (that must be already updated) to know the indexes in result and vect;
    virtual void MultiplyAndAdd(double& result, const ChVectorDynamic<double>& vect) const override;

    /// Computes the product of the corresponding transposed blocks in the system matrix (ie. the TRANSPOSED jacobian
    ...
    /// that the sparse matrix is kept symmetric.
