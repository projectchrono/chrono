...
protected:
  eChConstraintMode
      mode;   ///< mode of the constraint: free / lock / complementar
...
public:
  /// Default constructor
  ChConstraint()
      : c_i(0), g_i(0), b_i(0), l_i(0), cfm_i(0), valid(false), disabled(false),
        redundant(false), broken(false), _active(true), mode(CONSTRAINT_LOCK) {}
  ...
  virtual void MultiplyAndAdd(double &result,
                              const ChMatrix<double> &vect) const = 0;
...
private:
  void UpdateActiveFlag() {
    this->_active = (valid && !disabled && !redundant && !broken &&
                     mode != (CONSTRAINT_FREE));
  }
};
