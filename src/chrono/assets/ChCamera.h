...
class ChApi ChCamera {
  public:
    ...
    /// Set the position of the observer (eye point).
    /// Expressed in the local coordinate system.
    void SetPosition(const ChVector3d& pos) { position = pos; }

    /// Get the position of the observer (eye point).
    /// Expressed in the local coordinate system.
    const ChVector3d& GetPosition() const { return position; }

    /// Set the position of the target point (aim point).
    /// Expressed in the local coordinate system.
    void SetAimPoint(const ChVector3d& point) { aimpoint = point; }

    /// Get the position of the target point (aim point).
    /// Expressed in the local coordinate system.
    const ChVector3d& GetAimPoint() const { return aimpoint; }

    /// Set the position of the 'up' direction of the camera (default is vertical, VECT_Y).
    /// Expressed in the local coordinate system.
    void SetUpVector(const ChVector3d& up) { upvector = up.GetNormalized(); }

    /// Get the position of the 'up' direction of the camera (default is vertical, VECT_Y).
    /// Expressed in the local coordinate system.
    ...
    /// Get the Field Of View, if the visualization supports focusing.
    double GetFOV() const { return fov; }

    /// Set the Horizontal/Vertical size ratio. Default = 4/3.
    /// For instance, 4/3 can be used if rendering to 800x600 images with square pixels.
    void SetHVratio(double mf) { hvratio = mf; }

    /// Get the Horizontal/Vertical size ratio. Default = 4/3.
    /// For instance, 4/3 can be used if rendering to 800x600 images with square pixels.
