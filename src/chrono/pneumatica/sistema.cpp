...
// Metodo che alloca memoria per stato e derivatastato
{
    ...
    stato = new double[n];
    memset(stato, 0x0, sizeof(double) * n);
    derivatastato = new double[n];
    memset(derivatastato, 0x0, sizeof(double) * n);
    // Aggiungo a nc 6 per poter contenere i contributi del PID cfr->controllore.cpp
    comandi = new double[nc + 20];
    memset(comandi, 0x0, sizeof(double) * (nc + 20));
...
// Metodo che salva i dati sul disco
{
    ...
    /*
        dataout<<t<<" "<<*(stato)<<" "<<*(stato+1)<<" "<<*(derivatastato+1)<<" "<<*(stato+2)<<" "<<*(stato+3)<<"
       "<<*(comandi)<<" "<<*(comandi+1)<<"\n";
    */
}
