...
{
    if (comando > comandomax)
        comando = comandomax;
    else if (comando < comandomin)
        comando = comandomin;

    if (comando < (comandochius + 1e-3) &&
        comando > (comandochius - 1e-3))  // comando < (comandochius+1e-3) && comando < (comandochius-1e-3)
        stato = chiusa;
    else if ((comando < comandochius)) {
        ((ugello_controllato_PA*)ug1)
            ->SetComando((comandochius - comando) /
                         fabs(comandomin - comandochius));  // SetComando(comandochius-comando);
        stato = alimentazione;
    } else if ((comando > comandochius)) {
        ((ugello_controllato_RA*)ug2)
            ->SetComando((comando - comandochius) /
                         fabs(comandomax - comandochius));  // SetComando(comando-comandochius);
        stato = scarico;
    }
