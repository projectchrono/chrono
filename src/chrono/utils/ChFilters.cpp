...
double ChButterworth_Lowpass::Filter(double u)
{
    ...
    for(size_t i=0; i<m_n_biquad; i++) {
        y = m_biq_b0[i]*u + m_biq_b1[i]*m_biq_u_hist1[i] + m_biq_b2[i]*m_biq_u_hist2[i] - m_biq_a1[i]*m_biq_y_hist1[i] - m_biq_a2[i]*m_biq_y_hist2[i];
...
const double ChISO2631_5_Wz::m_w[13][7] = {
...
double ChISO2631_Vibration_SeatCushionLogger::GetVDV()
{
    ...
    return sqrt(pow(kx*m_data_vdv_x_avg.back(),2.0) + pow(ky*m_data_vdv_y_avg.back(),2.0) + pow(kz*m_data_vdv_z_avg.back(),2.0));
}
