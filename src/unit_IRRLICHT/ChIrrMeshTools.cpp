...
// -----------------------------------------------------------------------------
IMesh* createCylinderMesh(f32 radius,
                          f32 length, 
                          u32 tesselation) 
{
  f32 oblique = 0;
...
// -----------------------------------------------------------------------------
void fillChTrimeshFromIrlichtMesh(chrono::geometry::ChTriangleMesh* chTrimesh,
                                  IMesh*                            pMesh)
{
...
// ***OBSOLETE***
// -----------------------------------------------------------------------------
void fillIrlichtMeshFromChTrimesh(IMesh*                            pMesh,
                                  ...
                                  video::SColor                     clr)
{
  ...
  for (int i = 0; i < chTrimesh->getNumTriangles(); i++)
  {
    ...
    buffer->Vertices[i*3+0] = video::S3DVertex((f32)pos.x, (f32)pos.y, (f32)pos.z,
      (f32)normal.x, (f32)normal.y, (f32)normal.z,
      clr, 0, 0);
    pos = chTrimesh->getTriangle(i).p2;
    buffer->Vertices[i*3+1] = video::S3DVertex((f32)pos.x, (f32)pos.y, (f32)pos.z,
      (f32)normal.x, (f32)normal.y, (f32)normal.z,
      clr, 0, 0);
    pos = chTrimesh->getTriangle(i).p3;
    buffer->Vertices[i*3+2] = video::S3DVertex((f32)pos.x, (f32)pos.y, (f32)pos.z,
      (f32)normal.x, (f32)normal.y, (f32)normal.z,
      clr, 0, 0);
