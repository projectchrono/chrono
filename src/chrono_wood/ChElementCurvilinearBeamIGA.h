...
#ifndef CHELEMENT_CURVILINEAR_BEAM_IGA_H
#define CHELEMENT_CURVILINEAR_BEAM_IGA_H
...
class ChWoodApi ChElementCurvilinearBeamIGA : public ChElementBeam, public ChLoadableU, public ChLoadableUVW {
  public:
    ChElementCurvilinearBeamIGA();
    ...
    ChElementCurvilinearBeamIGA(const ChElementCurvilinearBeamIGA&) = delete;
    ChElementCurvilinearBeamIGA& operator=(const ChElementCurvilinearBeamIGA&) = delete;
	...
	ChMatrix33<> FrenetSerret(std::vector<std::shared_ptr<ChNodeFEAxyzrot>>& nodes, ChVector3d& jacob, 
				double invJacob, ChVectorDynamic<>& ddRddxi, ChVectorDynamic<>& dddRdddxi, double& ba_curvature, double& ba_torsion);
	...
	/// Access the local weights of this element 
    ChVectorDynamic<>& GetWeights() { return this->weights; }
    ...
    virtual void EvaluateSectionPoint(const double eta, ChVector3d& point) {
        ...
        ChBasisToolsNurbs::BasisEvaluate(this->order, u, weights, knots,
                                                     N);  ///< here return  in N
