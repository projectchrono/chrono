...
ChColor ChVisualShapeWOOD::ComputeFalseColor(double mv) {
...
double ChVisualShapeWOOD::ComputeScalarOutput(std::shared_ptr<ChNodeFEAxyz> mnode,
                                             int nodeID,
...
double ChVisualShapeWOOD::ComputeScalarOutput(std::shared_ptr<ChNodeFEAxyzP> mnode,
                                             int nodeID,
                                             std::shared_ptr<ChElementBase> melement) {
    switch (fem_data_type) {
        case DataType::SURFACE:
...
void ChVisualShapeWOOD::UpdateBuffers_Tetrahedron(std::shared_ptr<fea::ChElementBase> element,
...
void ChVisualShapeWOOD::UpdateBuffers_Tetra_4_P(std::shared_ptr<fea::ChElementBase> element,
                                               ChTriangleMeshConnected& trianglemesh,
                                               unsigned int& i_verts,
                                               unsigned int& i_vnorms,
                                               unsigned int& i_vcols,
...
void ChVisualShapeWOOD::UpdateBuffers_Hex(std::shared_ptr<ChElementBase> element,
                                         ChTriangleMeshConnected& trianglemesh,
                                         unsigned int& i_verts,
                                         unsigned int& i_vnorms,
                                         unsigned int& i_vcols,
...
void ChVisualShapeWOOD::UpdateBuffers_Beam(std::shared_ptr<fea::ChElementBase> element,
                                          ChTriangleMeshConnected& trianglemesh,
                                          unsigned int& i_verts,
                                          unsigned int& i_vnorms,
                                          unsigned int& i_vcols,
    ...
    } else if (auto beamIGA = std::dynamic_pointer_cast<ChElementCurvilinearBeamIGA>(beam)) {
        sectionshape = beamIGA->GetSection()->GetDrawShape();
    } else if (auto beamBezier = std::dynamic_pointer_cast<ChElementCurvilinearBeamBezier>(beam)) {
        sectionshape = beamBezier->GetSection()->GetDrawShape();
    } else if (auto connector = std::dynamic_pointer_cast<ChElementCBLCON>(beam)) {    	
        sectionshape = connector->GetSection()->GetDrawShape();        
    ...
    if (sectionshape) {
	    ...
	    if(std::dynamic_pointer_cast<ChElementCurvilinearBeamBezier>(beam)){
	    	mcol=ChColor(1.0f, 0.0f, 0.0f);
            ...
            for (int il = 0; il < sectionshape->GetNumLines(); ++il) {
                ...
                if (in > 0) {
                    ChVector3<int> ivert_offset(ivert_el, ivert_el, ivert_el);
                    ChVector3<int> islice_offset((in - 1) * n_section_pts, (in - 1) * n_section_pts,
                                                (in - 1) * n_section_pts);
                    for (size_t is = 0; is < msubline_pts.size() - 1; ++is) {
                        ...
                        trianglemesh.GetIndicesVertexes()[i_triindex] =
                            ChVector3<int>(ipa, ipbb, ipaa) + islice_offset + ivert_offset;
                        if (smooth_faces) {
                        ...
                        trianglemesh.GetIndicesVertexes()[i_triindex] =
                            ChVector3<int>(ipa, ipb, ipbb) + islice_offset + ivert_offset;
                        if (smooth_faces) {
                            trianglemesh.GetIndicesNormals()[i_triindex] =
                                ChVector3<int>(ipa, ipb, ipbb) + islice_offset + ivert_offset;
                        }
...
void ChVisualShapeWOOD::UpdateBuffers_Shell(std::shared_ptr<fea::ChElementBase> element,
                                           ChTriangleMeshConnected& trianglemesh,
                                           unsigned int& i_verts,
                                           unsigned int& i_vnorms,
                                           unsigned int& i_vcols,
...
void ChVisualShapeWOOD::UpdateBuffers_LoadSurface(std::shared_ptr<ChMeshSurface> surface,
                                                 ChTriangleMeshConnected& trianglemesh,
                                                 unsigned int& i_verts,
                                                 unsigned int& i_vnorms,
                                                 unsigned int& i_vcols,
...
void ChVisualShapeWOOD::UpdateBuffers_ContactSurfaceMesh(std::shared_ptr<ChContactSurface> surface,
                                                        ChTriangleMeshConnected& trianglemesh,
                                                        unsigned int& i_verts,
                                                        unsigned int& i_vnorms,
                                                        unsigned int& i_vcols,
...
void ChVisualShapeWOOD::Update(ChPhysicsItem* updater, const ChFrame<>& frame) {
    if (!FEMmesh)
        return;
            ...
            for (const auto& element : FEMmesh->GetElements()) {
                if (std::dynamic_pointer_cast<ChElementTetrahedron>(element)) {
                    ...
                    if (auto beamEuler = std::dynamic_pointer_cast<ChElementBeamEuler>(beam)) {
                        ...
                        sectionshape = beamIGA->GetSection()->GetDrawShape();
		    } else if (auto curvybeamIGA = std::dynamic_pointer_cast<ChElementCurvilinearBeamIGA>(beam)) {
                        sectionshape = curvybeamIGA->GetSection()->GetDrawShape();
                    } else if (auto curvybeamBezier = std::dynamic_pointer_cast<ChElementCurvilinearBeamBezier>(beam)) {
                        sectionshape = curvybeamBezier->GetSection()->GetDrawShape();
                    } else if (auto connector = std::dynamic_pointer_cast<ChElementCBLCON>(beam)) {
                        sectionshape = connector->GetSection()->GetDrawShape();
                    } else if (auto beamTimoshenko = std::dynamic_pointer_cast<ChElementBeamTaperedTimoshenko>(beam)) {
                        sectionshape = beamTimoshenko->GetTaperedSection()->GetSectionA()->GetDrawShape();
