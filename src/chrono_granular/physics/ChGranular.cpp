...
template <int> __global__ void primingOperationsRectangularBox(int*, int* , int* , unsigned int* , unsigned int* , unsigned int);
...
chrono::ChGRN_DE_Container::~ChGRN_DE_Container() {
    ...
    if (p_d_CM_XDOT != nullptr) {
        cudaFree(p_d_CM_XDOT);
        p_d_CM_XDOT = nullptr;
    }
    if (p_d_CM_YDOT != nullptr) {
        cudaFree(p_d_CM_YDOT);
        p_d_CM_YDOT = nullptr;
    }
    if (p_d_CM_ZDOT != nullptr) {
        cudaFree(p_d_CM_ZDOT);
        p_d_CM_ZDOT = nullptr;
    }
}
