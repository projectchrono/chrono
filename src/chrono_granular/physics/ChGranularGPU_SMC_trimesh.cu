...
/**
 * This kernel carries out broad-phase for mesh-grMat.
 *
 * Nomenclature:
 *   - SD: subdomain.
 *   - BD: the big-domain, which is the union of all SDs
 *   - BKT: a bucket, which contains triangles associated with a specific set of SDs.
...
__global__ void triangleSoupBroadPhase(
    const Triangle_Soup<int>& d_triangleSoup,
    ParamsPtr gran_params,
    unsigned int*
        BUCKET_countsOfTrianglesTouching,  //!< Array that for each SD indicates how many triangles touch this SD
    unsigned int*
        triangles_in_BUCKET_composite,  //!< Big array that works in conjunction w/ d_BUCKET_countsOfTrianglesTouching.
                                        //!< "triangles_in_BUCKET_composite" says which BUCKET contains what triangles.
    unsigned int*
        SD_isTouchedByTriangleSoup  //!< If SD 629 has any triangle touching it, then SD_isTouchedByTriangleSoup[629]>0.
) {
    ...
    // Ear mark SDs that are touched by at least one triangle. This step is needed since when computing the
    ...
    for (unsigned int i = 0; i < MAX_SDs_TOUCHED_BY_TRIANGLE; i++) {
        if (head_flags[i] && (SDsTouched[i] != NULL_GRANULAR_ID))
            atomicAdd(SD_isTouchedByTriangleSoup, 1);
    }
...
*/
#define BulkGranMat chrono::granular::ChManyBodyStateWrapper

template <unsigned int N_CUDATHREADS,
          unsigned int TRIANGLE_FAMILIES>
__global__ void interactionGranMat_TriangleSoup(
    Triangle_Soup<int>& d_triangleSoup,           //!< Contains information pertaining to triangle soup (in device mem.)
    BulkGranMat& d_blkGrnMat,                     //!< Wrapper that stores granMat information available on the device
    unsigned int* BKT_countsOfTrianglesTouching,  //!< Array that for each BKT says how many triangles touch this BKT
    unsigned int*
        triangles_in_BKT_composite,  //!< Big array that works in conjunction with BKT_countsOfTrianglesTouching.
                                     //!< "triangles_in_BKT_composite" says which BKT contains what triangles.
    unsigned int* SD_countsOfGrElemsTouching,  //!< Array that for each SD indicates how many DEs touch this SD
    ...
    unsigned int* SD_isTouchedByTriangleSoup)  //!< The length of this array is equal to number of SDs. If SD 423 is
                                               //!< touched by any triangle, then SD_isTouchedByTriangleSoup[423]>0.
{
...
#define thisSD blockIdx.x
    // return right away if this SD is not touched by any triangle
    if (SD_isTouchedByTriangleSoup[thisSD] == 0)
        return;

    // return right away if this SD is not touched by any DE
    ...
    for (unsigned int sphereTrip = 0; sphereTrip < tripsToCoverSpheres; sphereTrip++) {
        ...
        if (local_ID < nSD_spheres) {
            ...
            sphX[local_ID] = d_blkGrnMat.grElem_X[globalID];
            sphY[local_ID] = d_blkGrnMat.grElem_Y[globalID];
            sphZ[local_ID] = d_blkGrnMat.grElem_Z[globalID];
        }
    }
    ...
    for (unsigned int triangTrip = 0; triangTrip < tripsToCoverTriangles; triangTrip++) {
        ...
        if (local_ID < nBKT_triangles) {
            ...
            //familyID[localID] = d_triangleSoup.triangleFamily_ID[globalID];
        ...
        if (sphere_Local_ID < nSD_spheres) {
            ...
            for (unsigned int triangTrip = 0; triangTrip < tripsToCoverTriangles; triangTrip++) {
                ...
                if (targetTriangle < nBKT_triangles) {
                    ...
                    face_sphere_cd(A, B, C, sphCntr, d_blkGrnMat.sphereRadius, norm, depth, pt1, pt2, eff_radius);
