...
public:
	...
	virtual bool TriggerEvent(ChSharedPtr<ChBody> mbody, ChSystem& msystem)
	{
		...
		if(((fabs(localpos.x) < mbox.Size.x) &&
		    (fabs(localpos.y) < mbox.Size.y) &&
		    (fabs(localpos.z) < mbox.Size.z)) 
		     ^ invert_volume ) // XOR
			return true;
		else
			return false;
	...
	virtual bool TriggerEvent(ChSharedPtr<ChBody> mbody, ChSystem& msystem)
	{
				...
				&& (fabs(localpos.y)< 0.5*Ysize+margin))
		{
			...
			if (mcached != last_positions.end())
			{
				...
				if ((fabs(pColl.x)< 0.5*Xsize) &&
					(fabs(pColl.y)< 0.5*Ysize))
				{
