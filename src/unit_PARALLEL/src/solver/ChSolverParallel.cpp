...
#pragma omp parallel for
    ...
    //DEM code does not use rigid rigid, only bilaterals for ShurA
    if(rigid_rigid) {
        rigid_rigid->ShurA(x);
    }
    bilateral->ShurA(x);
...
void ChSolverParallel::ShurBilaterals(
    custom_vector<real> &x_t, custom_vector<real> & AX) {
...
void ChSolverParallel::UpdateContacts() {
    if (rigid_rigid->solve_sliding == true
...
void ChSolverParallel::Solve(
    ...
    if (number_of_constraints > 0) {
        ...
        if (solver_type == STEEPEST_DESCENT) {
            total_iteration += SolveSD(data_container->host_data.gamma_data,
                                       data_container->host_data.rhs_data, max_iteration);
        } else if (solver_type == GRADIENT_DESCENT) {
            total_iteration += SolveGD(data_container->host_data.gamma_data,
                                       data_container->host_data.rhs_data, max_iteration);
        } else if (solver_type == CONJUGATE_GRADIENT) {
            total_iteration += SolveCG(data_container->host_data.gamma_data,
                                       data_container->host_data.rhs_data, max_iteration);
        } else if (solver_type == CONJUGATE_GRADIENT_SQUARED) {
            total_iteration += SolveCGS(data_container->host_data.gamma_data,
                                        data_container->host_data.rhs_data, max_iteration);
        } else if (solver_type == BICONJUGATE_GRADIENT) {
            total_iteration += SolveBiCG(data_container->host_data.gamma_data,
                                         data_container->host_data.rhs_data, max_iteration);
        } else if (solver_type == BICONJUGATE_GRADIENT_STAB) {
            total_iteration += SolveBiCGStab(
                                   data_container->host_data.gamma_data,
                                   data_container->host_data.rhs_data, max_iteration);
        } else if (solver_type == MINIMUM_RESIDUAL) {
            total_iteration += SolveMinRes(data_container->host_data.gamma_data,
                                           data_container->host_data.rhs_data, max_iteration);
        }
...
void ChSolverParallel::VelocityStabilization(
    ChParallelDataManager *data_container_) {}

uint ChSolverParallel::SolveStab(
    custom_vector<real> &x, const custom_vector<real> &mb, const uint max_iter) {
    uint N = mb.size();
    //	bool verbose = false;
