...
ChSystemParallel::ChSystemParallel(unsigned int max_objects) :
	...
	((ChCollisionSystemParallel *) (collision_system))->data_container =
			gpu_data_manager;
	((ChLcpSystemDescriptorParallel *) (LCP_descriptor))->data_container =
			gpu_data_manager;
	((ChLcpSolverParallel *) (LCP_solver_speed))->data_container =
			gpu_data_manager;
	((ChContactContainerParallel*) contact_container)->data_container =
			gpu_data_manager;
...
int ChSystemParallel::Integrate_Y_impulse_Anitescu() {
	...
	if (use_aabb_active) {
		...
		((ChCollisionSystemParallel*) collision_system)->GetOverlappingAABB(
				body_active, aabb_min, aabb_max);
		for (int i = 0; i < bodylist.size(); i++) {
			if (bodylist[i]->IsActive() == true
					&& bodylist[i]->GetCollide() == true) {
	...
	std::vector<ChLcpConstraint *> &mconstraints =
			(*this->LCP_descriptor).GetConstraintsList();
	for (uint ic = 0; ic < mconstraints.size(); ic++) {
		if (mconstraints[ic]->IsActive() == false) {
			continue;
		}
		ChLcpConstraintTwoBodies *mbilateral =
				(ChLcpConstraintTwoBodies *) (mconstraints[ic]);
		mconstraints[ic]->Set_l_i(
				gpu_data_manager->host_data.gamma_bilateral[counter]);
		counter++;
	}
...
#pragma omp parallel for
	...
	if (ChCollisionSystemParallel* coll_sys =
			dynamic_cast<ChCollisionSystemParallel*>(collision_system)) {
...
void ChSystemParallel::RecomputeThreads() {
	double sum_of_elems = std::accumulate(timer_accumulator.begin(),
			timer_accumulator.end(), 0.0);
...
void ChSystemParallel::PerturbBins(bool increase, int number) {
	...
	if (increase) {
		int3 grid_size =
				((ChCollisionSystemParallel *) (GetCollisionSystem()))->broadphase.getBinsPerAxis();

		grid_size.x = grid_size.x + number;
		grid_size.y = grid_size.y + number;
		grid_size.z = grid_size.z + number;

		((ChCollisionSystemParallel *) (GetCollisionSystem()))->broadphase.setBinsPerAxis(
				grid_size);
	} else {
...
void ChSystemParallel::RecomputeBins() {
	double sum_of_elems_cd = std::accumulate(cd_accumulator.begin(),
			cd_accumulator.end(), 0.0);
...
void ChSystemParallel::AddBody(ChSharedPtr<ChBody> newbody) {
	...
	gpu_data_manager->host_data.vel_data.push_back(
			R3(mbodyvar->Get_qb().GetElementN(0),
	...
	gpu_data_manager->host_data.omg_data.push_back(
			R3(mbodyvar->Get_qb().GetElementN(3),
	...
	gpu_data_manager->host_data.rot_data.push_back(
			R4(newbody->GetRot().e0, newbody->GetRot().e1, newbody->GetRot().e2,
					newbody->GetRot().e3));
	gpu_data_manager->host_data.inr_data.push_back(
			R3(inertia.GetElement(0, 0), inertia.GetElement(1, 1),
					inertia.GetElement(2, 2)));
	gpu_data_manager->host_data.frc_data.push_back(
			R3(mbodyvar->Get_fb().ElementN(0), mbodyvar->Get_fb().ElementN(1),
					mbodyvar->Get_fb().ElementN(2)));     //forces
	gpu_data_manager->host_data.trq_data.push_back(
			R3(mbodyvar->Get_fb().ElementN(3), mbodyvar->Get_fb().ElementN(4),
					mbodyvar->Get_fb().ElementN(5)));     //torques
	...
	gpu_data_manager->host_data.fric_roll_data.push_back(
			newbody->GetMaterialSurface()->GetRollingFriction());
	gpu_data_manager->host_data.fric_spin_data.push_back(
			newbody->GetMaterialSurface()->GetSpinningFriction());
	gpu_data_manager->host_data.cohesion_data.push_back(
			newbody->GetMaterialSurface()->GetCohesion());
	gpu_data_manager->host_data.compliance_data.push_back(
			newbody->GetMaterialSurface()->GetCompliance());
...
void ChSystemParallel::RemoveBody(ChSharedPtr<ChBody> mbody) {
	assert(
			std::find<std::vector<ChBody *>::iterator>(bodylist.begin(),
					bodylist.end(), mbody.get_ptr()) != bodylist.end());
...
void ChSystemParallel::UpdateBodies() {
	...
	real *compliance_pointer =
			gpu_data_manager->host_data.compliance_data.data();
...
#pragma omp parallel for
	for (int i = 0; i < bodylist.size(); i++) {
		...
		vel_pointer[i] = (R3(bodylist[i]->Variables().Get_qb().ElementN(0),
				bodylist[i]->Variables().Get_qb().ElementN(1),
				bodylist[i]->Variables().Get_qb().ElementN(2)));
		omg_pointer[i] = (R3(bodylist[i]->Variables().Get_qb().ElementN(3),
				bodylist[i]->Variables().Get_qb().ElementN(4),
				bodylist[i]->Variables().Get_qb().ElementN(5)));
		pos_pointer[i] = (R3(bodylist[i]->GetPos().x, bodylist[i]->GetPos().y,
				bodylist[i]->GetPos().z));
		rot_pointer[i] = (R4(bodylist[i]->GetRot().e0, bodylist[i]->GetRot().e1,
				bodylist[i]->GetRot().e2, bodylist[i]->GetRot().e3));
		inr_pointer[i] = (R3(inertia.GetElement(0, 0), inertia.GetElement(1, 1),
				inertia.GetElement(2, 2)));
		frc_pointer[i] = (R3(bodylist[i]->Variables().Get_fb().ElementN(0),
				bodylist[i]->Variables().Get_fb().ElementN(1),
				bodylist[i]->Variables().Get_fb().ElementN(2)));     //forces
		trq_pointer[i] = (R3(bodylist[i]->Variables().Get_fb().ElementN(3),
				bodylist[i]->Variables().Get_fb().ElementN(4),
				bodylist[i]->Variables().Get_fb().ElementN(5)));     //torques
		...
		fric_roll_pointer[i] =
				((bodylist[i]))->GetMaterialSurface()->GetRollingFriction();
		fric_spin_pointer[i] =
				((bodylist[i]))->GetMaterialSurface()->GetSpinningFriction();
		cohesion_pointer[i] =
				((bodylist[i]))->GetMaterialSurface()->GetCohesion();
		compliance_pointer[i] =
				((bodylist[i]))->GetMaterialSurface()->GetCompliance();
		lim_pointer[i] = (R3(bodylist[i]->GetLimitSpeed(), .05 / GetStep(),
				.05 / GetStep()));
...
void ChSystemParallel::UpdateBilaterals() {
	for (it = linklist.begin(); it != linklist.end(); it++) {
		...
		(*it)->ConstraintsBiLoad_C(1.0 / GetStep(),
				max_penetration_recovery_speed, true);
	...
	gpu_data_manager->host_data.correction_bilateral.resize(
			number_of_bilaterals);
...
#pragma omp parallel for
	for (int i = 0; i < number_of_bilaterals; i++) {
		int cntr = mapping[i];
		ChLcpConstraintTwoBodies *mbilateral =
				(ChLcpConstraintTwoBodies *) (mconstraints[cntr]);
		int idA =
				((ChBody *) ((ChLcpVariablesBody *) (mbilateral->GetVariables_a()))->GetUserData())->GetId();
		int idB =
				((ChBody *) ((ChLcpVariablesBody *) (mbilateral->GetVariables_b()))->GetUserData())->GetId();
		...
		real3 A, B, C, D;
		A = R3(mbilateral->Get_Cq_a()->GetElementN(0),
				mbilateral->Get_Cq_a()->GetElementN(1),
				mbilateral->Get_Cq_a()->GetElementN(2));     //J1x
		B = R3(mbilateral->Get_Cq_b()->GetElementN(0),
				mbilateral->Get_Cq_b()->GetElementN(1),
				mbilateral->Get_Cq_b()->GetElementN(2));     //J2x
		C = R3(mbilateral->Get_Cq_a()->GetElementN(3),
				mbilateral->Get_Cq_a()->GetElementN(4),
				mbilateral->Get_Cq_a()->GetElementN(5));     //J1w
		D = R3(mbilateral->Get_Cq_b()->GetElementN(3),
				mbilateral->Get_Cq_b()->GetElementN(4),
				mbilateral->Get_Cq_b()->GetElementN(5));     //J2w
		...
		gpu_data_manager->host_data.residual_bilateral[i] =
				mbilateral->Get_b_i();     // b_i is residual b
		gpu_data_manager->host_data.correction_bilateral[i] = 1.0
				/ mbilateral->Get_g_i();     // eta = 1/g
...
void ChSystemParallel::ChangeCollisionSystem(ChCollisionSystem *newcollsystem) {
	...
	if (ChCollisionSystemParallel* coll_sys =
			dynamic_cast<ChCollisionSystemParallel*>(newcollsystem)) {
		((ChCollisionSystemParallel *) (collision_system))->data_container =
				gpu_data_manager;
	} else if (ChCollisionSystemBulletParallel* coll_sys =
			dynamic_cast<ChCollisionSystemBulletParallel*>(newcollsystem)) {
		((ChCollisionSystemBulletParallel *) (collision_system))->data_container =
				gpu_data_manager;
	}
}

void ChSystemParallel::ChangeLcpSystemDescriptor(
		ChLcpSystemDescriptor* newdescriptor) {
	assert(newdescriptor);
	if (this->LCP_descriptor)
		delete (this->LCP_descriptor);
	this->LCP_descriptor = newdescriptor;

	((ChLcpSystemDescriptorParallel *) (this->LCP_descriptor))->data_container =
			gpu_data_manager;
}
