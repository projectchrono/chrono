...
namespace chrono {
	...
	class ChApiGPU ChContactContainerGPU: public ChContactContainerBase {
		CH_RTTI(ChContactContainerGPU, ChContactContainerBase)
			;
		...
		public:
			...
			void SetNcontacts(int contacts) {
				n_added = contacts;
			}
			...
			double Get_load_C_factor() {
				return load_C;
			}
			double Get_load_max_recovery_speed() {
				return load_max_recovery_speed;
			}
			bool Get_load_do_clamp() {
				return load_do_clamp;
			}
...
//			bool Compare(std::list<ChContact*> contactlist) {
//				if (contactlist.size() != data_container->number_of_contacts) {
//					cout << "FAIL: Numcontacts" << endl;
//					return false;
//				}
//				int counter = 0;
//
//				for (std::list<ChContact *>::iterator it = contactlist.begin(); it != contactlist.end(); it++) {
//					Vector cptA = (*it)->GetContactP1();
//
//
//
//
