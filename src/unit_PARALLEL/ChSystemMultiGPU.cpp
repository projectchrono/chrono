...
//		}
	...
	double ChSystemMultiGPU::ComputeCollisions() {
		...
		//step 1 determine number of subdivisions and size of domain
		...
		//step 2 determine location of each subdomain
		int gpu_counter = 0;
		for (int i = 0; i < slices.x; i++) {
			for (int j = 0; j < slices.y; j++) {
				for (int k = 0; k < slices.z; k++) {
					gpu_subdomains[i * slices.x * slices.y + j * slices.y + k].min_bounding_point = F3(subdomain_dim.x * (i + 0), subdomain_dim.y * (j + 0), subdomain_dim.z * (k + 0));
					gpu_subdomains[i * slices.x * slices.y + j * slices.y + k].max_bounding_point = F3(subdomain_dim.x * (i + 1), subdomain_dim.y * (j + 1), subdomain_dim.z * (k + 1));
					gpu_subdomains[i].gpu_id = gpu_counter;
					gpu_counter++;
					if (gpu_counter > num_gpu - 1) {
						gpu_counter = 0;
					}
				}
			}
		}
//		//step 3 determine what objects are in each subdomain
//
		...
		for (int i = 0; i < gpu_data_manager->number_of_objects; i++) {
			for (int j = 0; j < gpu_subdomains.size(); j++) {
				if (gpu_subdomains[j].bod_list[i]==true&& gpu_subdomains[j].gpu_data.number_of_contacts>0){
					gpu_data_manager->host_vel_data[i] += gpu_subdomains[j].vel_update[gpu_subdomains[j].update_counter];
					gpu_data_manager->host_omg_data[i] += gpu_subdomains[j].omg_update[gpu_subdomains[j].update_counter];
					gpu_subdomains[j].update_counter++;
				};
			}
		}
