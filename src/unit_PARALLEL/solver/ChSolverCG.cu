...
__global__ void device_Project(real *gam, real *fric, int2 *ids) {
    ...
    function_Project(index, gamma, fric,  ids, number_of_contacts);
}
...
custom_vector<real> ChSolverCG::ShurProduct(const custom_vector<real> &x_t) {
...
void ChSolverCG::Solve(real step, gpu_container &gpu_data_) {
    ...
    host_RHS(
        ...
        rhs.data());
#endif
    SolveCG(gpu_data->device_gam_data, rhs);
    Thrust_Fill(gpu_data->device_QXYZ_data, R3(0));
    Thrust_Fill(gpu_data->device_QUVW_data, R3(0));
    host_shurA(
        gpu_data->device_JXYZA_data.data(),
...
uint ChSolverCG::SolveCG(custom_vector<real> &x, const custom_vector<real> &b, const uint max_iter) {
    ...
    for (current_iteration = 0; current_iteration < max_iteration; current_iteration++) {
        ...
        host_Project(gpu_data->device_gam_data.data(),
