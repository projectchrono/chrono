...
__host__ __device__ void function_Project(uint &index, real *gam, real *fric, int2 *ids, uint number_of_contacts) {
...
__global__ void device_Project(real *gam, real *fric, int2 *ids) {
	INIT_CHECK_THREAD_BOUNDED(INDEX1D, number_of_contacts_const);
	function_Project(index, gam, fric, ids, number_of_contacts_const);
}
void ChSolverGPU::host_Project(real *gam, real *fric, int2 *ids) {
	for (uint index = 0; index < number_of_contacts; index++) {
		function_Project(index, gam, fric, ids, number_of_contacts);
	}
}
...
void ChSolverGPU::host_shurA(real3 *JXYZA, real3 *JXYZB, real3 *JUVWA, real3 *JUVWB, real *gamma, int2 *ids, real *inv_mass, real3 *inv_inertia, bool *active, real3 *QXYZ, real3 *QUVW) {
...
void ChSolverGPU::host_shurB(real3 *JXYZA, real3 *JXYZB, real3 *JUVWA, real3 *JUVWB, real *gamma, int2 *ids, real *inv_mass, real3 *inv_inertia, bool *active, real3 *QXYZ, real3 *QUVW, real *AX) {
#pragma omp parallel for schedule(guided)
	for (uint index = 0; index < number_of_constraints; index++) {
		function_shurB(index, JXYZA, JXYZB, JUVWA, JUVWB, gamma, inv_mass, inv_inertia, ids, active, QXYZ, QUVW, AX);
	}
...
__host__ __device__ void function_RHS(uint &index, int2 *ids, real3 *JXYZA, real3 *JXYZB, real3 *JUVWA, real3 *JUVWB, real3 *vel, real3 *omega, real *correction, real &step_size, real *rhs) {
	uint b1 = ids[index].x;
	uint b2 = ids[index].y;
	real temp = 0;
	...
	real inv_hpa = 1.0 / (h + alpha);
	//cout << "CORR " << fmaxf(inv_hpa * -fabs(correction[index]), -.01) << endl;
	//rhs[index] = -((-temp) + fmaxf(inv_hpa * -fabs(correction[index]), 0));
	rhs[index] = -((-temp) + inv_hpa * -fabs(correction[index]));
}

__global__ void device_RHS(int2 *ids, real3 *JXYZA, real3 *JXYZB, real3 *JUVWA, real3 *JUVWB, real3 *vel, real3 *omega, real *correction, real step_size, real *rhs) {
	INIT_CHECK_THREAD_BOUNDED(INDEX1D, number_of_constraints_const);
	function_RHS(index, ids, JXYZA, JXYZB, JUVWA, JUVWB, vel, omega, correction, step_size, rhs);
}

void ChSolverCG::host_RHS(int2 *ids, real3 *JXYZA, real3 *JXYZB, real3 *JUVWA, real3 *JUVWB, real3 *vel, real3 *omega, real *correction, real step_size, real *rhs) {
#pragma omp parallel for schedule(guided)
	for (uint index = 0; index < number_of_constraints; index++) {
		function_RHS(index, ids, JXYZA, JXYZB, JUVWA, JUVWB, vel, omega, correction, step_size, rhs);
	}
