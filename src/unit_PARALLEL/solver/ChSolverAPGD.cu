...
__host__ __device__  void partA_function(const uint &i,
                                             const real* b,
                                             const real* mg_tmp1,
                                             const real* my,
                                             const real t_k,
...
__global__ void partA_device(const uint size,
                             const real* b,
                             const real* mg_tmp1,
                             const real* my,
                             const real t_k,
...
void partA_host(const uint size,
...
__host__ __device__  void partOne_function(const uint &i,
                                      const real* b,
                                      const real* mx,
                                      const real* my,
                                      const real* mg_tmp,
    ...
    mg_tmp2[i]=_mg_tmp-_b;
    
    obj1_tmp[i]=0.5*_mg_tmp-_b;
    obj1_tmp[i]=obj1_tmp[i]*_mx;
    
    obj2_tmp[i]=0.5*mg_tmp1[i]-_b;
    obj2_tmp[i]=obj2_tmp[i]*_my;
    ms[i]=_mx-_my;
}

__global__ void partOne_device(const uint size,
                               const real* b,
                               const real* mx,
                               const real* my,
                               const real* mg_tmp,
...
void partOne_host(const uint size,
                  const custom_vector<real> b,
...
__host__ __device__  void partTwo_function(const uint &i,
                                           const real* ms,
                                           const real* mg,
                                           real* obj1_tmp,
                                           real* obj2_tmp){
...
__global__ void partTwo_device(const uint size,
                               const real* ms,
                               const real* mg,
                               real* obj1_tmp,
                               real* obj2_tmp){
...
real partTwo_host(const uint size,
                  const real &obj2,
                  const real &L_k,
                  const custom_vector<real> ms,
...
#pragma omp parallel for schedule(guided)
    ...
    return obj2 + temp1 + 0.5 * L_k * pow(temp2, 2.0);
    
}



__host__ __device__  void partThree_function(const uint &i,
                                           const real* mx,
                                           const real* ml,
                                           const real* mg,
                                           const real beta_k1,
...
__global__ void partThree_device(const uint size,
                                 const real* mx,
                                 const real* ml,
                                 const real* mg,
                                 const real beta_k1,
...
void partThree_host(const uint size,
                  const custom_vector<real> mx,
...
uint ChSolverGPU::SolveAPGD(custom_vector<real> &x, const custom_vector<real> &b, const uint max_iter) {
    ...
    custom_vector<real> obj1_tmp(x.size()), obj2_tmp(x.size());
	...
	for (current_iteration = 0; current_iteration < max_iter; current_iteration++) {
        ...
        //obj2 + Dot(mg, ms) + 0.5 * L_k * pow(Norm(ms), 2.0)
