...
void ChOpenGLHUD::GenerateSystem(ChSystem* physics_system) {
  ...
  text.Render(buffer, RIGHT, 0.925 - SPACING * 0, sx, sy);
  sprintf(buffer, "BODIES     %04d", num_bodies);
  text.Render(buffer, RIGHT, 0.925 - SPACING * 1, sx, sy);
  int num_shapes = 0;
  ...
  text.Render(buffer, RIGHT, 0.925 - SPACING * 2, sx, sy);

  sprintf(buffer, "CONTACTS   %04d", num_contacts);
  text.Render(buffer, RIGHT, 0.925 - SPACING * 3, sx, sy);
  sprintf(buffer, "AVGCONPB   %04d", average_contacts_per_body);
  text.Render(buffer, RIGHT, 0.925 - SPACING * 4, sx, sy);

  sprintf(buffer, "TIMING INFO");
  text.Render(buffer, RIGHT, -0.925 + SPACING * 11, sx, sy);
  sprintf(buffer, "STEP     %04f", physics_system->GetTimerStep());
  text.Render(buffer, RIGHT, -0.925 + SPACING * 10, sx, sy);
  sprintf(buffer, "BROAD    %04f", physics_system->GetTimerCollisionBroad());
  text.Render(buffer, RIGHT, -0.925 + SPACING * 9, sx, sy);
  sprintf(buffer, "NARROW   %04f", physics_system->GetTimerCollisionNarrow());
  text.Render(buffer, RIGHT, -0.925 + SPACING * 8, sx, sy);
  sprintf(buffer, "SOLVE    %04f", physics_system->GetTimerLcp());
  text.Render(buffer, RIGHT, -0.925 + SPACING * 7, sx, sy);
  sprintf(buffer, "UPDATE   %04f", physics_system->GetTimerUpdate());
  text.Render(buffer, RIGHT, -0.925 + SPACING * 6, sx, sy);
}
