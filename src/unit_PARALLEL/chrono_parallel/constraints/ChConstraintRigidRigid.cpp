...
#pragma omp parallel for
   ...
   host_RHS(data_container->host_data.bids_rigid_rigid.data(), data_container->host_data.dpth_rigid_rigid.data(), data_container->settings.solver.alpha,
            contact_active_pairs.data(), data_container->host_data.norm_rigid_rigid.data(), data_container->host_data.vel_data.data(), data_container->host_data.omg_data.data(),
...
void ChConstraintRigidRigid::UpdateRHS() {
   host_RHS(data_container->host_data.bids_rigid_rigid.data(), data_container->host_data.dpth_rigid_rigid.data(), data_container->settings.solver.alpha,
            contact_active_pairs.data(), data_container->host_data.norm_rigid_rigid.data(), data_container->host_data.vel_new_data.data(),
            data_container->host_data.omg_new_data.data(), data_container->host_data.cpta_rigid_rigid.data(), data_container->host_data.cptb_rigid_rigid.data(),
            contact_rotation.data(), data_container->host_data.rhs_data.data());
   if (solve_spinning) {
...
void ChConstraintRigidRigid::ShurB(real*x,
   ...
   if (solve_spinning) {
      ...
      host_shurB_spinning(data_container->settings.solver.alpha, data_container->host_data.bids_rigid_rigid.data(), contact_active_pairs.data(),
                          data_container->host_data.norm_rigid_rigid.data(), comp_rigid_rigid.data(), x, data_container->host_data.cpta_rigid_rigid.data(),
                          data_container->host_data.cptb_rigid_rigid.data(), contact_rotation.data(), data_container->host_data.QXYZ_data.data(),
      ...
      host_shurB_sliding(data_container->settings.solver.alpha, data_container->host_data.bids_rigid_rigid.data(), contact_active_pairs.data(),
                         data_container->host_data.norm_rigid_rigid.data(), comp_rigid_rigid.data(), x, data_container->host_data.cpta_rigid_rigid.data(),
                         data_container->host_data.cptb_rigid_rigid.data(), contact_rotation.data(), data_container->host_data.QXYZ_data.data(),
                         data_container->host_data.QUVW_data.data(), output);
      ...
      host_shurB_normal(data_container->settings.solver.alpha, data_container->host_data.bids_rigid_rigid.data(), contact_active_pairs.data(),
                        data_container->host_data.norm_rigid_rigid.data(), comp_rigid_rigid.data(), x, JUA_rigid_rigid.data(), JUB_rigid_rigid.data(),
...
void ChConstraintRigidRigid::Build_D(SOLVERMODE solver_mode) {
   ...
   for (int index = 0; index < num_contacts; index++) {
      ...
      data_container->host_data.D_T.append(index_mult + 0, body_id.x * 6 + 0, -U.x);
      data_container->host_data.D_T.append(index_mult + 0, body_id.x * 6 + 1, -U.y);
      data_container->host_data.D_T.append(index_mult + 0, body_id.x * 6 + 2, -U.z);

      if (solver_mode == SLIDING || solver_mode == SPINNING) {
         data_container->host_data.D_T.append(index_mult + 0, body_id.x * 6 + 3, T3.x);
         data_container->host_data.D_T.append(index_mult + 0, body_id.x * 6 + 4, T3.y);
         data_container->host_data.D_T.append(index_mult + 0, body_id.x * 6 + 5, T3.z);
      }

      data_container->host_data.D_T.append(index_mult + 0, body_id.y * 6 + 0, U.x);
      data_container->host_data.D_T.append(index_mult + 0, body_id.y * 6 + 1, U.y);
      data_container->host_data.D_T.append(index_mult + 0, body_id.y * 6 + 2, U.z);
      if (solver_mode == SLIDING || solver_mode == SPINNING) {
         data_container->host_data.D_T.append(index_mult + 0, body_id.y * 6 + 3, -T6.x);
         data_container->host_data.D_T.append(index_mult + 0, body_id.y * 6 + 4, -T6.y);
         data_container->host_data.D_T.append(index_mult + 0, body_id.y * 6 + 5, -T6.z);
      }
      data_container->host_data.D_T.finalize(index_mult + 0);
      data_container->host_data.D_T.append(index_mult + 1, body_id.x * 6 + 0, -V.x);
      data_container->host_data.D_T.append(index_mult + 1, body_id.x * 6 + 1, -V.y);
      data_container->host_data.D_T.append(index_mult + 1, body_id.x * 6 + 2, -V.z);

      if (solver_mode == SLIDING || solver_mode == SPINNING) {
         data_container->host_data.D_T.append(index_mult + 1, body_id.x * 6 + 3, T4.x);
         data_container->host_data.D_T.append(index_mult + 1, body_id.x * 6 + 4, T4.y);
         data_container->host_data.D_T.append(index_mult + 1, body_id.x * 6 + 5, T4.z);

      }

      data_container->host_data.D_T.append(index_mult + 1, body_id.y * 6 + 0, V.x);
      data_container->host_data.D_T.append(index_mult + 1, body_id.y * 6 + 1, V.y);
      data_container->host_data.D_T.append(index_mult + 1, body_id.y * 6 + 2, V.z);

      if (solver_mode == SLIDING || solver_mode == SPINNING) {

         data_container->host_data.D_T.append(index_mult + 1, body_id.y * 6 + 3, -T7.x);
         data_container->host_data.D_T.append(index_mult + 1, body_id.y * 6 + 4, -T7.y);
         data_container->host_data.D_T.append(index_mult + 1, body_id.y * 6 + 5, -T7.z);
      }
      data_container->host_data.D_T.finalize(index_mult + 1);
      data_container->host_data.D_T.append(index_mult + 2, body_id.x * 6 + 0, -W.x);
      data_container->host_data.D_T.append(index_mult + 2, body_id.x * 6 + 1, -W.y);
      data_container->host_data.D_T.append(index_mult + 2, body_id.x * 6 + 2, -W.z);

      if (solver_mode == SLIDING || solver_mode == SPINNING) {
         data_container->host_data.D_T.append(index_mult + 2, body_id.x * 6 + 3, T5.x);
         data_container->host_data.D_T.append(index_mult + 2, body_id.x * 6 + 4, T5.y);
         data_container->host_data.D_T.append(index_mult + 2, body_id.x * 6 + 5, T5.z);

      }

      data_container->host_data.D_T.append(index_mult + 2, body_id.y * 6 + 0, W.x);
      data_container->host_data.D_T.append(index_mult + 2, body_id.y * 6 + 1, W.y);
      data_container->host_data.D_T.append(index_mult + 2, body_id.y * 6 + 2, W.z);

      if (solver_mode == SLIDING || solver_mode == SPINNING) {

         data_container->host_data.D_T.append(index_mult + 2, body_id.y * 6 + 3, -T8.x);
         data_container->host_data.D_T.append(index_mult + 2, body_id.y * 6 + 4, -T8.y);
         data_container->host_data.D_T.append(index_mult + 2, body_id.y * 6 + 5, -T8.z);

      }
      data_container->host_data.D_T.finalize(index_mult + 2);
      if (solver_mode == SPINNING) {
         ...
         data_container->host_data.D_T.append(index_mult + 3, body_id.x * 6 + 3, -TA.x);
         data_container->host_data.D_T.append(index_mult + 3, body_id.x * 6 + 4, -TA.y);
         data_container->host_data.D_T.append(index_mult + 3, body_id.x * 6 + 5, -TA.z);
         data_container->host_data.D_T.append(index_mult + 3, body_id.y * 6 + 3, TD.x);
         data_container->host_data.D_T.append(index_mult + 3, body_id.y * 6 + 4, TD.y);
         data_container->host_data.D_T.append(index_mult + 3, body_id.y * 6 + 5, TD.z);
         data_container->host_data.D_T.finalize(index_mult + 3);
         data_container->host_data.D_T.append(index_mult + 4, body_id.x * 6 + 3, -TB.x);
         data_container->host_data.D_T.append(index_mult + 4, body_id.x * 6 + 4, -TB.y);
         data_container->host_data.D_T.append(index_mult + 4, body_id.x * 6 + 5, -TB.z);
         data_container->host_data.D_T.append(index_mult + 4, body_id.y * 6 + 3, TE.x);
         data_container->host_data.D_T.append(index_mult + 4, body_id.y * 6 + 4, TE.y);
         data_container->host_data.D_T.append(index_mult + 4, body_id.y * 6 + 5, TE.z);
         data_container->host_data.D_T.finalize(index_mult + 4);
         data_container->host_data.D_T.append(index_mult + 5, body_id.x * 6 + 3, -TC.x);
         data_container->host_data.D_T.append(index_mult + 5, body_id.x * 6 + 4, -TC.y);
         data_container->host_data.D_T.append(index_mult + 5, body_id.x * 6 + 5, -TC.z);
         data_container->host_data.D_T.append(index_mult + 5, body_id.y * 6 + 3, TF.x);
         data_container->host_data.D_T.append(index_mult + 5, body_id.y * 6 + 4, TF.y);
         data_container->host_data.D_T.append(index_mult + 5, body_id.y * 6 + 5, TF.z);
         data_container->host_data.D_T.finalize(index_mult + 5);
      }
   }
}
