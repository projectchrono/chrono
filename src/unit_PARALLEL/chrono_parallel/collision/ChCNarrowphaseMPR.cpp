...
int RefinePortal(const ConvexShape& shapeA, const ConvexShape& shapeB, const real& envelope, simplex& portal) {
  real3 n;
  for (int i = 0; i < MAX_ITERATIONS; i++) {
...
bool chrono::collision::MPRCollision(const ConvexShape& shapeA, const ConvexShape& shapeB, real envelope, real3& normal, real3& pointA, real3& pointB, real& depth) {
 ...
 // PreSolve(shapeA,shapeB , envelope);
...
void PreSolve(const ConvexShape& shapeA, const ConvexShape& shapeB, real envelope) {
  ...
  real3 normalInB(real(0.), real(0.), real(0.));
  ...
  std::cout<<"OA ["<<A_origin.x<<" "<<A_origin.y<<" "<<A_origin.z<<"] ";
  std::cout<<"OB ["<<B_origin.x<<" "<<B_origin.y<<" "<<B_origin.z<<"] \n";
  real marginA = envelope;
  real marginB = envelope;
...

  {
    ...
    for (;;) {
      real3 TA = quatRotateT(-m_cachedSeparatingAxis, shapeA.R);
      real3 TB = quatRotateT(m_cachedSeparatingAxis, shapeB.R);

      std::cout<<"SA ["<<TA.x<<" "<<TA.y<<" "<<TA.z<<"] ";
      std::cout<<"SB ["<<TB.x<<" "<<TB.y<<" "<<TB.z<<"] \n";
      real3 PA = SupportVert(shapeA,TA, 0 );
      real3 PB = SupportVert(shapeB,TB, 0 );
      std::cout<<"PA ["<<PA.x<<" "<<PA.y<<" "<<PA.z<<"] ";
      std::cout<<"PB ["<<PB.x<<" "<<PB.y<<" "<<PB.z<<"] \n";
      real3 pWorld = TransformLocalToParent(A_origin,shapeA.R,PA ) ;
      real3 qWorld = TransformLocalToParent(B_origin,shapeB.R,PB ) ;


      std::cout<<"QA ["<<pWorld.x<<" "<<pWorld.y<<" "<<pWorld.z<<"] ";
      std::cout<<"QB ["<<qWorld.x<<" "<<qWorld.y<<" "<<qWorld.z<<"] \n";


      //real3 pWorld =TransformSupportVert(shapeA, -m_cachedSeparatingAxis, 0);
      //real3 qWorld =TransformSupportVert(shapeB, m_cachedSeparatingAxis, 0);
  ...
  std::cout<<"PRESOLVE: "<<distance<<" ";
  std::cout<<"N ["<<normalInB.x<<" "<<normalInB.y<<" "<<normalInB.z<<"] ";
  std::cout<<"A ["<<pointOnA.x<<" "<<pointOnA.y<<" "<<pointOnA.z<<"] ";
  std::cout<<"B ["<<pointOnB.x<<" "<<pointOnB.y<<" "<<pointOnB.z<<"] \n";
