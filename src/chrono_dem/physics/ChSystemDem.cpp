...
ChSystemDem::ChSystemDem(const std::string& checkpoint) {
    m_sys = new ChSystemDem_impl(1.f, 1.f, make_float3(100, 100, 100), make_float3(0, 0, 0));
...
ChSystemDemMesh::ChSystemDemMesh(const std::string& checkpoint) : mesh_verbosity(CHDEM_MESH_VERBOSITY::QUIET) {
    m_sys = new ChSystemDemMesh_impl(1.f, 1.f, make_float3(100, 100, 100), make_float3(0, 0, 0));
    ReadCheckpointFile(checkpoint, true);
}

ChSystemDem::~ChSystemDem() {
    delete m_sys;
}

ChSystemDemMesh::~ChSystemDemMesh() {}

// -----------------------------------------------------------------------------

void ChSystemDem::SetGravitationalAcceleration(const ChVector3f& g) {
    m_sys->X_accGrav = (float)g.x();
    m_sys->Y_accGrav = (float)g.y();
    m_sys->Z_accGrav = (float)g.z();
}

void ChSystemDem::SetGravitationalAcceleration(const float3 g) {
    m_sys->X_accGrav = g.x;
    m_sys->Y_accGrav = g.y;
    m_sys->Z_accGrav = g.z;
}

void ChSystemDem::SetBDFixed(bool fixed) {
    m_sys->BD_is_fixed = fixed;
}

void ChSystemDem::SetBDCenter(const ChVector3f& O) {
    m_sys->user_coord_O_X = O.x();
    m_sys->user_coord_O_Y = O.y();
    m_sys->user_coord_O_Z = O.z();
}

void ChSystemDem::SetParticleDensity(float density) {
    m_sys->sphere_density_UU = density;
}

void ChSystemDem::SetParticleRadius(float rad) {
    m_sys->sphere_radius_UU = rad;
}

void ChSystemDem::SetParticleFixed(const std::vector<bool>& fixed) {
    m_sys->user_sphere_fixed = fixed;
}

// Set particle output file format
void ChSystemDem::SetParticleOutputMode(CHDEM_OUTPUT_MODE mode) {
    m_sys->file_write_mode = mode;
}

// Set particle output file content
void ChSystemDem::SetParticleOutputFlags(unsigned int flags) {
    m_sys->output_flags = flags;
}

void ChSystemDem::SetFixedStepSize(float size_UU) {
    m_sys->stepSize_UU = size_UU;
}

float ChSystemDem::GetFixedStepSize() const {
    return m_sys->stepSize_UU;
}

void ChSystemDem::EnableMinLength(bool useMinLen) {
    m_sys->use_min_length_unit = useMinLen;
}

void ChSystemDem::SetTimeIntegrator(CHDEM_TIME_INTEGRATOR new_integrator) {
    m_sys->gran_params->time_integrator = new_integrator;
    m_sys->time_integrator = new_integrator;
}

void ChSystemDem::SetFrictionMode(CHDEM_FRICTION_MODE new_mode) {
    m_sys->gran_params->friction_mode = new_mode;
}

void ChSystemDem::SetRollingMode(CHDEM_ROLLING_MODE new_mode) {
    m_sys->gran_params->rolling_mode = new_mode;
}

void ChSystemDem::SetDefragmentOnInitialize(bool defragment) {
    m_sys->defragment_on_start = defragment;
}

void ChSystemDem::SetRecordingContactInfo(bool record) {
    m_sys->gran_params->recording_contactInfo = record;
}

void ChSystemDem::SetMaxSafeVelocity_SU(float max_vel) {
    m_sys->gran_params->max_safe_vel = max_vel;
}

void ChSystemDem::SetPsiFactors(unsigned int psi_T, unsigned int psi_L, float psi_R) {
    m_sys->psi_T = psi_T;
    m_sys->psi_L = psi_L;
    m_sys->psi_R = psi_R;
}

void ChSystemDem::SetPsiT(unsigned int psi_T) {
    m_sys->psi_T = psi_T;
}

void ChSystemDem::SetPsiL(unsigned int psi_L) {
    m_sys->psi_L = psi_L;
}

void ChSystemDem::SetPsiR(float psi_R) {
    m_sys->psi_R = psi_R;
}

void ChSystemDem::SetSimTime(float time) {
    m_sys->elapsedSimTime = time;
}

// -----------------------------------------------------------------------------

void ChSystemDem::SetStaticFrictionCoeff_SPH2SPH(float mu) {
    m_sys->gran_params->static_friction_coeff_s2s = mu;
}

void ChSystemDem::SetStaticFrictionCoeff_SPH2WALL(float mu) {
    m_sys->gran_params->static_friction_coeff_s2w = mu;
}

void ChSystemDem::SetRollingCoeff_SPH2SPH(float mu) {
    m_sys->rolling_coeff_s2s_UU = mu;
}

void ChSystemDem::SetRollingCoeff_SPH2WALL(float mu) {
    m_sys->rolling_coeff_s2w_UU = mu;
}

void ChSystemDem::SetSpinningCoeff_SPH2SPH(float mu) {
    m_sys->spinning_coeff_s2s_UU = mu;
}

void ChSystemDem::SetSpinningCoeff_SPH2WALL(float mu) {
    m_sys->spinning_coeff_s2w_UU = mu;
}

void ChSystemDem::SetKn_SPH2SPH(double someValue) {
    m_sys->K_n_s2s_UU = someValue;
}

void ChSystemDem::SetKn_SPH2WALL(double someValue) {
    m_sys->K_n_s2w_UU = someValue;
}

void ChSystemDem::SetGn_SPH2SPH(double someValue) {
    m_sys->Gamma_n_s2s_UU = someValue;
}

void ChSystemDem::SetGn_SPH2WALL(double someValue) {
    m_sys->Gamma_n_s2w_UU = someValue;
}

void ChSystemDem::SetKt_SPH2SPH(double someValue) {
    m_sys->K_t_s2s_UU = someValue;
}

void ChSystemDem::SetGt_SPH2SPH(double someValue) {
    m_sys->Gamma_t_s2s_UU = someValue;
}

void ChSystemDem::SetKt_SPH2WALL(double someValue) {
    m_sys->K_t_s2w_UU = someValue;
}

void ChSystemDem::SetGt_SPH2WALL(double someValue) {
    m_sys->Gamma_t_s2w_UU = someValue;
}

void ChSystemDem::SetCohesionRatio(float someValue) {
    m_sys->cohesion_over_gravity = someValue;
}

void ChSystemDem::SetAdhesionRatio_SPH2WALL(float someValue) {
...
void ChSystemDem::SetYoungModulus_WALL(double someValue) {
    m_sys->YoungsModulus_wall_UU = someValue;
}

void ChSystemDem::SetPoissonRatio_SPH(double someValue) {
    m_sys->PoissonRatio_sphere_UU = someValue;
}

void ChSystemDem::SetPoissonRatio_WALL(double someValue) {
    m_sys->PoissonRatio_wall_UU = someValue;
}

void ChSystemDem::SetRestitution_SPH(double someValue) {
    m_sys->COR_sphere_UU = someValue;
}

void ChSystemDem::SetRestitution_WALL(double someValue) {
    m_sys->COR_wall_UU = someValue;
}

// -----------------------------------------------------------------------------

void ChSystemDemMesh::UseMaterialBasedModel(bool val) {
...
void ChSystemDemMesh::SetStaticFrictionCoeff_SPH2MESH(float mu) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->tri_params->static_friction_coeff_s2m = mu;
}

void ChSystemDemMesh::SetRollingCoeff_SPH2MESH(float mu) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->rolling_coeff_s2m_UU = mu;
}

void ChSystemDemMesh::SetSpinningCoeff_SPH2MESH(float mu) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->spinning_coeff_s2m_UU = mu;
}

void ChSystemDemMesh::SetKn_SPH2MESH(double someValue) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->K_n_s2m_UU = someValue;
}

void ChSystemDemMesh::SetGn_SPH2MESH(double someValue) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->Gamma_n_s2m_UU = someValue;
}

void ChSystemDemMesh::SetKt_SPH2MESH(double someValue) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->K_t_s2m_UU = someValue;
}

void ChSystemDemMesh::SetGt_SPH2MESH(double someValue) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->Gamma_t_s2m_UU = someValue;
}

void ChSystemDemMesh::SetAdhesionRatio_SPH2MESH(float someValue) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->adhesion_s2m_over_gravity = someValue;
}

void ChSystemDemMesh::SetYoungModulus_MESH(double someValue) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->YoungsModulus_mesh_UU = someValue;
}

void ChSystemDemMesh::SetPoissonRatio_MESH(double someValue) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->PoissonRatio_mesh_UU = someValue;
}

void ChSystemDemMesh::SetRestitution_MESH(double someValue) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
...
void ChSystemDem::SetVerbosity(CHDEM_VERBOSITY level) {
    m_sys->verbosity = level;
}

void ChSystemDemMesh::SetMeshVerbosity(CHDEM_MESH_VERBOSITY level) {
...
void ChSystemDem::SetBCSpherePosition(size_t sphere_bc_id, const ChVector3f& pos) {
    m_sys->SetBCSpherePosition(sphere_bc_id, make_float3(pos.x(), pos.y(), pos.z()));
}

ChVector3f ChSystemDem::GetBCSpherePosition(size_t sphere_bc_id) const {
    float3 pos = m_sys->GetBCSpherePosition(sphere_bc_id);
    return ChVector3f(pos.x, pos.y, pos.z);
}

void ChSystemDem::SetBCSphereVelocity(size_t sphere_bc_id, const ChVector3f& velo) {
    m_sys->SetBCSphereVelocity(sphere_bc_id, make_float3(velo.x(), velo.y(), velo.z()));
}

ChVector3f ChSystemDem::GetBCSphereVelocity(size_t sphere_bc_id) const {
...
size_t ChSystemDem::CreateBCPlane(const ChVector3f& pos, const ChVector3f& normal, bool track_forces) {
...
size_t ChSystemDem::CreateCustomizedPlate(const ChVector3f& pos_center, const ChVector3f& normal, float hdim_y) {
...
size_t ChSystemDem::CreateBCCylinderZ(const ChVector3f& center, float radius, bool outward_normal, bool track_forces) {
...
bool ChSystemDem::DisableBCbyID(size_t BC_id) {
    return m_sys->DisableBCbyID(BC_id);
}

bool ChSystemDem::EnableBCbyID(size_t BC_id) {
    return m_sys->EnableBCbyID(BC_id);
}

bool ChSystemDem::SetBCOffsetFunction(size_t BC_id, const GranPositionFunction& offset_function) {
    return m_sys->SetBCOffsetFunction(BC_id, offset_function);
}

void ChSystemDem::setBDWallsMotionFunction(const GranPositionFunction& pos_fn) {
    m_sys->setBDWallsMotionFunction(pos_fn);
}

// -----------------------------------------------------------------------------

unsigned int ChSystemDem::GetNumSDs() const {
    return m_sys->nSDs;
}

ChVector3f ChSystemDem::GetBCPlanePosition(size_t plane_id) const {
    // todo: throw an error if BC not a plane type
    float3 pos = m_sys->GetBCPlanePosition(plane_id);
    return ChVector3f(pos.x, pos.y, pos.z);
}

void ChSystemDem::SetBCPlaneRotation(size_t plane_id, ChVector3d center, ChVector3d omega) {
...
bool ChSystemDem::GetBCReactionForces(size_t BC_id, ChVector3f& force) const {
    float3 frc;
...
unsigned int ChSystemDem::GetNumContacts() const {
    return m_sys->GetNumContacts();
}

float ChSystemDem::GetSimTime() const {
    return m_sys->elapsedSimTime;
}

size_t ChSystemDem::GetNumParticles() const {
    return m_sys->nSpheres;
}

float ChSystemDem::GetParticleRadius() const {
    return m_sys->sphere_radius_UU;
}

ChVector3f ChSystemDem::GetParticlePosition(int nSphere) const {
    float3 pos = m_sys->GetParticlePosition(nSphere);
    return ChVector3f(pos.x, pos.y, pos.z);
}

void ChSystemDem::SetParticlePosition(int nSphere, const ChVector3d pos) {
    double3 position = make_double3(pos.x(), pos.y(), pos.z());
    m_sys->SetParticlePosition(nSphere, position);
}

ChVector3f ChSystemDem::GetParticleVelocity(int nSphere) const {
    float3 vel = m_sys->GetParticleLinVelocity(nSphere);
    return ChVector3f(vel.x, vel.y, vel.z);
}

void ChSystemDem::SetParticleVelocity(int nSphere, const ChVector3d velo) {
    double3 velocity = make_double3(velo.x(), velo.y(), velo.z());
    m_sys->SetParticleVelocity(nSphere, velocity);
}

ChVector3f ChSystemDem::GetParticleAngVelocity(int nSphere) const {
    if (m_sys->gran_params->friction_mode == CHDEM_FRICTION_MODE::FRICTIONLESS)
        return ChVector3f(0);

    float3 omega = m_sys->GetParticleAngVelocity(nSphere);
    return ChVector3f(omega.x, omega.y, omega.z);
}

ChVector3f ChSystemDem::GetParticleLinAcc(int nSphere) const {
    float3 acc = m_sys->GetParticleLinAcc(nSphere);
    return ChVector3f(acc.x, acc.y, acc.z);
}

bool ChSystemDem::IsFixed(int nSphere) const {
    return m_sys->IsFixed(nSphere);
}

float ChSystemDem::GetParticlesKineticEnergy() const {
    float KE = (float)(m_sys->ComputeTotalKE());
    return KE;
}

double ChSystemDem::GetMaxParticleZ() const {
    return m_sys->GetMaxParticleZ(true);
}

double ChSystemDem::GetMinParticleZ() const {
    // Under the hood, GetMaxParticleZ(false) returns the lowest Z.
    return m_sys->GetMaxParticleZ(false);
}

unsigned int ChSystemDem::GetNumParticleAboveZ(float ZValue) const {
    return m_sys->GetNumParticleAboveZ(ZValue);
}

unsigned int ChSystemDem::GetNumParticleAboveX(float XValue) const {
    return m_sys->GetNumParticleAboveX(XValue);
}

size_t ChSystemDem::EstimateMemUsage() const {
    return m_sys->EstimateMemUsage();
}

// -----------------------------------------------------------------------------

unsigned int ChSystemDemMesh::AddMesh(std::shared_ptr<ChTriangleMeshConnected> mesh, float mass) {
...
unsigned int ChSystemDemMesh::AddMesh(const std::string& filename,
    ...
    if (!flag)
        CHDEM_ERROR("ERROR! Mesh %s failed to load in!\n", filename.c_str());
...
std::vector<unsigned int> ChSystemDemMesh::AddMeshes(const std::vector<std::string>& objfilenames,
    ...
    if (size != rotscales.size() || size != translations.size() || size != masses.size())
        CHDEM_ERROR("ERROR! Mesh loading vectors must all have same size!\n");
    if (size == 0)
...
void ChSystemDemMesh::SetMeshes() {
    int nTriangles = 0;
    for (const auto& mesh : m_meshes)
        nTriangles += mesh->GetNumTriangles();

    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    ChSystemDemMesh_impl::TriangleSoup* pMeshSoup = sys_trimesh->getMeshSoup();
    ...
    for (const auto& mesh : m_meshes) {
        for (unsigned int i = 0; i < mesh->GetNumTriangles(); i++) {
            ...
            if (use_mesh_normals) {
                ...
                ChVector3d normal = mesh->m_normals.at(normal_i);

                // Generate normal using RHR from nodes 1, 2, and 3
                ChVector3d AB = tri.p2 - tri.p1;
                ChVector3d AC = tri.p3 - tri.p1;
                ChVector3d cross;
                cross.Cross(AB, AC);
...
unsigned int ChSystemDemMesh::GetNumMeshes() const {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    return sys_trimesh->meshSoup->numTriangleFamilies;
}

void ChSystemDemMesh::EnableMeshCollision(bool val) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
...
static void convertChVector2Float3Vec(const std::vector<ChVector3f>& points, std::vector<float3>& pointsFloat3) {
...
inline unsigned int quarryCsvFormat(const std::string& line) {
    ...
    if (line.find("x,y,z") == std::string::npos)
        CHDEM_ERROR("ERROR! Checkpoint file (kinematics) does not contain xyz information!\n");
...
void ChSystemDem::ReadCsvParticles(std::ifstream& ifile, unsigned int totRow) {
...
void ChSystemDem::ReadHstHistory(std::ifstream& ifile, unsigned int totItem) {
...
void ChSystemDem::ReadContactHistoryFile(const std::string& infilename) {
    std::ifstream ifile(infilename.c_str());
    if (!ifile.is_open()) {
        CHDEM_ERROR("ERROR! Checkpoint file (contact history) did not open successfully!\n");
    }
...
void ChSystemDem::ReadParticleFile(const std::string& infilename) {
    std::ifstream ifile(infilename.c_str());
    if (!ifile.is_open()) {
        CHDEM_ERROR("ERROR! Checkpoint file (kinematics) did not open successfully!\n");
    }
...
bool ChSystemDem::SetParamsFromIdentifier(const std::string& identifier, std::istringstream& iss1, bool overwrite) {
    ...
    switch (hash_charr(identifier.c_str())) {
        ...
        case ("verbosity"_):
            iss1 >> i;
            SetVerbosity(static_cast<CHDEM_VERBOSITY>(i));
            break;
        ...
        case ("particleFileMode"_):
            iss1 >> i;
            SetParticleOutputMode(static_cast<CHDEM_OUTPUT_MODE>(i));
            break;
        ...
        case ("frictionMode"_):
            iss1 >> i;
            SetFrictionMode(static_cast<CHDEM_FRICTION_MODE>(i));
            break;
        case ("rollingMode"_):
            iss1 >> i;
            SetRollingMode(static_cast<CHDEM_ROLLING_MODE>(i));
            break;
        case ("timeIntegrator"_):
            iss1 >> i;
            SetTimeIntegrator(static_cast<CHDEM_TIME_INTEGRATOR>(i));
            break;
        case ("maxSafeVelSU"_):
            iss1 >> f;
...
// Read in a (Chrono::Dem generated) checkpoint file to restart a simulation. It calls ReadHstHistory,
// ReadCsvParticles and ReadDatParams to parse in the entire checkpoint file.
void ChSystemDem::ReadCheckpointFile(const std::string& infilename, bool overwrite) {
    // Open the file
    std::ifstream ifile(infilename.c_str());
    if (!ifile.is_open()) {
        CHDEM_ERROR("ERROR! Checkpoint file did not open successfully!\n");
    }
    ...
    if (line != "ChSystemDem")
        printf(
            "WARNING! Header of checkpoint file indicates this is not for ChSystemDem (and it seems to be for %s)! "
...
bool ChSystemDemMesh::SetParamsFromIdentifier(const std::string& identifier, std::istringstream& iss1, bool overwrite) {
    ...
    // Is the parameter one of the ChSystemDem's? If not, we then search it in ChSystemDemMesh.
    // The parent method returns true if it did not find a match.
    if (ChSystemDem::SetParamsFromIdentifier(identifier, iss1, overwrite)) {
        switch (hash_charr(identifier.c_str())) {
            case ("K_n_s2m"_):
                iss1 >> d;
                SetKn_SPH2MESH(d);
...
// DemMesh version of checkpointing loading subroutine.
void ChSystemDemMesh::ReadCheckpointFile(const std::string& infilename, bool overwrite) {
    // Open the file
    std::ifstream ifile(infilename.c_str());
    if (!ifile.is_open()) {
        CHDEM_ERROR("ERROR! Checkpoint file did not open successfully!\n");
    ...
    while (line.find_first_not_of(' ') == std::string::npos)
        std::getline(ifile, line);
    if (line != "ChSystemDemMesh")
        printf(
            "WARNING! Header of checkpoint file indicates this is not for ChSystemDemMesh (and it seems to be for "
            "%s)! "
            "There may still be parameters defaulted after loading this checkpoint file, and you may want to set "
            "them "
            "manually.\n",
...
void ChSystemDemMesh::Initialize() {
    if (m_meshes.size() > 0)
        SetMeshes();
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
...
void ChSystemDemMesh::InitializeMeshes() {
    if (m_meshes.size() > 0)
        SetMeshes();
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    sys_trimesh->initializeTriangles();
...
void ChSystemDem::Initialize() {
    m_sys->initializeSpheres();
    if (m_sys->verbosity == CHDEM_VERBOSITY::INFO || m_sys->verbosity == CHDEM_VERBOSITY::METRICS) {
...
double ChSystemDemMesh::AdvanceSimulation(float duration) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    return sys_trimesh->AdvanceSimulation(duration);
}

double ChSystemDem::AdvanceSimulation(float duration) {
    m_timer.reset();
    m_timer.start();
...
void ChSystemDem::WriteCheckpointParams(std::ofstream& cpFile) const {
    ...
    if (m_sys->time_integrator == CHDEM_TIME_INTEGRATOR::CHUNG)
        printf(
...
void ChSystemDem::WriteCheckpointFile(const std::string& outfilename) {
...
void ChSystemDem::WriteRawParticles(std::ofstream& ptFile) const {
    m_sys->WriteRawParticles(ptFile);
}

void ChSystemDem::WriteCsvParticles(std::ofstream& ptFile) const {
    m_sys->WriteCsvParticles(ptFile);
}

#ifdef USE_HDF5
void ChSystemDem::WriteH5Particles(H5::H5File ptFile) const {
    m_sys->WriteH5Particles(ptFile);
}
#endif

void ChSystemDem::WriteParticleFile(const std::string& outfilename) const {
    // The file writes are a pretty big slowdown in CSV mode
    if (m_sys->file_write_mode == CHDEM_OUTPUT_MODE::BINARY) {
        std::ofstream ptFile(outfilename, std::ios::out | std::ios::binary);
        WriteRawParticles(ptFile);
    } else if (m_sys->file_write_mode == CHDEM_OUTPUT_MODE::CSV) {
        std::ofstream ptFile(outfilename, std::ios::out);
        WriteCsvParticles(ptFile);
    } else if (m_sys->file_write_mode == CHDEM_OUTPUT_MODE::HDF5) {
#ifdef USE_HDF5
        H5::H5File ptFile(outfilename.c_str(), H5F_ACC_TRUNC);
        WriteH5Particles(ptFile);
#else
        CHDEM_ERROR("ERROR! HDF5 Installation not found. Recompile with HDF5.\n");
#endif
    }
}

void ChSystemDem::WriteContactInfoFile(const std::string& outfilename) const {
    m_sys->WriteContactInfoFile(outfilename);
}

void ChSystemDem::WriteHstHistory(std::ofstream& histFile) const {
    ...
    switch (m_sys->gran_params->friction_mode) {
        case (CHDEM_FRICTION_MODE::FRICTIONLESS):
            printf("WARNING! Currently using FRICTIONLESS model. There is no contact history to write!\n");
            return;
        case (CHDEM_FRICTION_MODE::SINGLE_STEP):
            formatMode += 1;
            break;
        case (CHDEM_FRICTION_MODE::MULTI_STEP):
            formatMode += (1 + 2);
            break;
        default:
            CHDEM_ERROR("ERROR! Unknown friction model, failed to write contact history file!\n");
    }
...
void ChSystemDem::WriteContactHistoryFile(const std::string& outfilename) const {
...
// DemMesh version of checkpoint params writing. Including extra params that only a meshed system has.
void ChSystemDemMesh::WriteCheckpointMeshParams(std::ofstream& cpFile) const {
    std::ostringstream paramStream;
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
...
// DemMesh version of checkpoint writing
void ChSystemDemMesh::WriteCheckpointFile(const std::string& outfilename) {
    printf("Writing checkpoint data to file \"%s\"\n", outfilename.c_str());
    std::ofstream cpFile(outfilename, std::ios::out);

    // Header, indicating the system
    cpFile << std::string("ChSystemDemMesh\n");
...
void ChSystemDemMesh::WriteMesh(const std::string& outfilename, unsigned int i) const {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    if (sys_trimesh->file_write_mode == CHDEM_OUTPUT_MODE::NONE) {
        return;
    }
    if (i >= m_meshes.size()) {
...
void ChSystemDem::getNeighbors(unsigned int ID, std::vector<unsigned int>& neighborList) {
    m_sys->getNeighbors(ID, neighborList);
}

/// Get rolling friction torque between body i and j, return 0 if not in contact
ChVector3f ChSystemDem::getRollingFrictionTorque(unsigned int i, unsigned int j) {
    float3 m_roll = m_sys->getRollingFrictionTorque(i, j);
    return ChVector3f(m_roll.x, m_roll.y, m_roll.z);
}

/// Get v_rot for rolling friction
ChVector3f ChSystemDem::getRollingVrot(unsigned int i, unsigned int j) {
    float3 vrot = m_sys->getRollingVrot(i, j);
    return ChVector3f(vrot.x, vrot.y, vrot.z);
}

/// get contact char time
float ChSystemDem::getRollingCharContactTime(unsigned int i, unsigned int j) {
    return m_sys->getRollingCharContactTime(i, j);
}

/// Get tangential friction force between body i and j, return 0 if not in contact
ChVector3f ChSystemDem::getSlidingFrictionForce(unsigned int i, unsigned int j) {
    float3 fr = m_sys->getSlidingFrictionForce(i, j);
    return ChVector3f(fr.x, fr.y, fr.z);
}

/// Get normal friction force between body i and j, return 0 if not in contact
ChVector3f ChSystemDem::getNormalForce(unsigned int i, unsigned int j) {
    float3 N = m_sys->getNormalForce(i, j);
    return ChVector3f(N.x, N.y, N.z);
}

void ChSystemDemMesh::WriteMeshes(const std::string& outfilename) const {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    if (sys_trimesh->file_write_mode == CHDEM_OUTPUT_MODE::NONE) {
        return;
    }
...
void ChSystemDemMesh::CollectMeshContactForces(std::vector<ChVector3d>& forces, std::vector<ChVector3d>& torques) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    ...
    for (unsigned int i = 0; i < m_num_meshes; i++) {
        ...
        forces[i] = ChVector3d(fx, fy, fz) * force_factor;  // Divide by C_F to go from SU to UU
        ...
        torques[i] = ChVector3d(tx, ty, tz) * torque_factor;  // Divide by C_TAU to go from SU to UU
    }
}

void ChSystemDemMesh::CollectMeshContactForces(int mesh, ChVector3d& force, ChVector3d& torque) {
    ChSystemDemMesh_impl* sys_trimesh = static_cast<ChSystemDemMesh_impl*>(m_sys);
    double force_factor = sys_trimesh->FORCE_SU2UU;
    double torque_factor = sys_trimesh->TORQUE_SU2UU;
    ...
    force = ChVector3d(fx, fy, fz) * force_factor;  // Divide by C_F to go from SU to UU
    ...
    torque = ChVector3d(tx, ty, tz) * torque_factor;  // Divide by C_TAU to go from SU to UU
}
