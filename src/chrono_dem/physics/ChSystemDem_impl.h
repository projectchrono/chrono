...
// Underlying implementation of the Chrono::Dem system.
// used to control and dispatch the DEM sphere-only solver.
class ChSystemDem_impl {
  public:
    virtual ~ChSystemDem_impl();
  ...
  protected:
    ...
    struct GranParams {
        ...
        CHDEM_FRICTION_MODE friction_mode;      ///< Which friction mode is active for the simulation
        CHDEM_ROLLING_MODE rolling_mode;        ///< Which rolling resistance model is active
        CHDEM_TIME_INTEGRATOR time_integrator;  ///< Which time integrator is active
    ...
    /// Structure of pointers to kinematic quantities of the ChSystemDem_impl.
    ...
    ChSystemDem_impl() = delete;

    /// Construct Chrono::Dem system with given sphere radius, density, big domain dimensions and the frame origin.
    ChSystemDem_impl(float sphere_rad, float density, float3 boxDims, float3 O);
    ...
    /// Set with the CHDEM_OUTPUT_FLAGS enum.
    unsigned int output_flags;

    /// How to write the output files?
    /// Default is CSV.
    CHDEM_OUTPUT_MODE file_write_mode;
  ...
  public:
    ...
    typedef const ChSystemDem_impl::GranParams* GranParamsPtr;

    /// Get handle for the sphere data that skips namespacing and enforces const-ness
    typedef const ChSystemDem_impl::SphereData* GranSphereDataPtr;

    friend class ChSystemDem;
