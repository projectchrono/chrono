...
/// @addtogroup dem_physics
/// @{

// Forward declarations
class ChSystemDem_impl;
class ChSystemDemMesh_impl;

// -----------------------------------------------------------------------------

/// Interface to a Chrono::Dem system.
class CH_DEM_API ChSystemDem {
  public:
    /// Construct system with given sphere radius, density, big domain dimensions and center.
    ChSystemDem(float sphere_rad, float density, const ChVector3f& boxDims, ChVector3f O = ChVector3f(0));

    /// Construct system with a checkpoint file.
    ChSystemDem(const std::string& checkpoint);
    ...
    // Read in a (Chrono::Dem generated) checkpoint file to restart a simulation.
    ...
    void SetParticleOutputMode(CHDEM_OUTPUT_MODE mode);

    /// Set output settings bit flags by bitwise ORing settings in CHDEM_OUTPUT_FLAGS.
    ...
    void SetTimeIntegrator(CHDEM_TIME_INTEGRATOR new_integrator);
    ...
    /// Write a one-stop checkpoint file for Chrono::Dem.
  ...
  protected:
    ...
    ChSystemDem() : m_sys(nullptr) {}

    ChSystemDem_impl* m_sys;  ///< underlying system implementation
...
/// Interface to a Chrono::Dem mesh system.
class CH_DEM_API ChSystemDemMesh : public ChSystemDem {
  public:
    /// Construct system with given sphere radius, density, big domain dimensions and center.
    ChSystemDemMesh(float sphere_rad, float density, const ChVector3f& boxDims, ChVector3f O = ChVector3f(0));

    /// Construct system with a checkpoint file.
    ChSystemDemMesh(const std::string& checkpoint);
    ...
    void SetMeshVerbosity(CHDEM_MESH_VERBOSITY level);
    ...
    /// DemMesh version of checkpoint loading from a file.
    void ReadCheckpointFile(const std::string& infilename, bool overwrite = false);

    /// DemMesh version of checkpoint generating subroutine. Has a bit more content than parent.
    void WriteCheckpointFile(const std::string& outfilename);
  ...
  private:
    /// Set triangle meshes in underlying DEM system.
    void SetMeshes();
