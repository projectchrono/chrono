...
ChSystemDem_impl::ChSystemDem_impl(float sphere_rad, float density,
                                   float3 boxDims, float3 O)
    : sphere_radius_UU(sphere_rad),
      ...
      nSpheres(0),
      elapsedSimTime(0.f),
      verbosity(CHDEM_VERBOSITY::INFO),
      use_min_length_unit(true),
      defragment_on_start(true),
      file_write_mode(CHDEM_OUTPUT_MODE::CSV),
      X_accGrav(0.f),
      Y_accGrav(0.f),
      Z_accGrav(0.f),
  ...
  demErrchk(
      cudaMallocManaged(&gran_params, sizeof(GranParams), cudaMemAttachGlobal));
  demErrchk(
      cudaMallocManaged(&sphere_data, sizeof(SphereData), cudaMemAttachGlobal));
  psi_T = PSI_T_DEFAULT;
  psi_L = PSI_L_DEFAULT;
  psi_R = PSI_R_DEFAULT;
  gran_params->friction_mode = CHDEM_FRICTION_MODE::FRICTIONLESS;
  gran_params->rolling_mode = CHDEM_ROLLING_MODE::NO_RESISTANCE;
  gran_params->time_integrator = CHDEM_TIME_INTEGRATOR::EXTENDED_TAYLOR;
  this->time_integrator = CHDEM_TIME_INTEGRATOR::EXTENDED_TAYLOR;
  this->output_flags = ABSV;  // | VEL_COMPONENTS and | ANG_VEL_COMPONENTS are
...
void ChSystemDem_impl::CreateWallBCs() {
  ...
  size_t plane_BC_X_bot =
      CreateBCPlane(plane_center_bot_X, plane_normal_bot_X, false, 0);
  size_t plane_BC_X_top =
      CreateBCPlane(plane_center_top_X, plane_normal_top_X, false, 1);
  size_t plane_BC_Y_bot =
      CreateBCPlane(plane_center_bot_Y, plane_normal_bot_Y, false, 2);
  size_t plane_BC_Y_top =
      CreateBCPlane(plane_center_top_Y, plane_normal_top_Y, false, 3);
  size_t plane_BC_Z_bot =
      CreateBCPlane(plane_center_bot_Z, plane_normal_bot_Z, false, 4);
  size_t plane_BC_Z_top =
      CreateBCPlane(plane_center_top_Z, plane_normal_top_Z, false, 5);
...
ChSystemDem_impl::~ChSystemDem_impl() { demErrchk(cudaFree(gran_params)); }
...
void ChSystemDem_impl::packSphereDataPointers() {
  ...
  sphere_data->SD_SphereCompositeOffsets_SP =
      SD_SphereCompositeOffsets_ScratchPad.data();
...
void ChSystemDem_impl::WriteRawParticles(std::ofstream& ptFile) const {
  for (unsigned int n = 0; n < nSpheres; n++) {
    ...
    x_UU += (float)(((int64_t)ownerSD_trip.x * gran_params->SD_size_X_SU) *
                    LENGTH_SU2UU);
    y_UU += (float)(((int64_t)ownerSD_trip.y * gran_params->SD_size_Y_SU) *
                    LENGTH_SU2UU);
    z_UU += (float)(((int64_t)ownerSD_trip.z * gran_params->SD_size_Z_SU) *
                    LENGTH_SU2UU);
    ...
    if (GET_OUTPUT_SETTING(ABSV)) {
      float absv = (float)(std::sqrt(pos_X_dt.at(n) * pos_X_dt.at(n) +
                                     pos_Y_dt.at(n) * pos_Y_dt.at(n) +
...
void ChSystemDem_impl::WriteCsvParticles(std::ofstream& ptFile) const {
  ...
  for (unsigned int n = 0; n < nSpheres; n++) {
    ...
    if (gran_params->friction_mode != CHDEM_FRICTION_MODE::FRICTIONLESS &&
        GET_OUTPUT_SETTING(ANG_VEL_COMPONENTS)) {
      outstrstream << "," << sphere_Omega_X.at(n) / TIME_SU2UU << ","
                   << sphere_Omega_Y.at(n) / TIME_SU2UU << ","
                   << sphere_Omega_Z.at(n) / TIME_SU2UU;
    }

    if (GET_OUTPUT_SETTING(FORCE_COMPONENTS)) {
      double fx = (sphere_acc_X.at(n) - gran_params->gravAcc_X_SU) *
                  gran_params->sphere_mass_SU * FORCE_SU2UU;
      double fy = (sphere_acc_Y.at(n) - gran_params->gravAcc_Y_SU) *
                  gran_params->sphere_mass_SU * FORCE_SU2UU;
      double fz = (sphere_acc_Z.at(n) - gran_params->gravAcc_Z_SU) *
                  gran_params->sphere_mass_SU * FORCE_SU2UU;
      outstrstream << "," << fx << "," << fy << "," << fz;
    }
...
#ifdef USE_HDF5
void ChSystemDem_impl::WriteH5Particles(H5::H5File& ptFile) const {
  ...
  for (size_t n = 0; n < nSpheres; n++) {
    ...
    x_UU +=
        ((int64_t)ownerSD_trip.x * gran_params->SD_size_X_SU) * LENGTH_SU2UU;
    y_UU +=
        ((int64_t)ownerSD_trip.y * gran_params->SD_size_Y_SU) * LENGTH_SU2UU;
    z_UU +=
        ((int64_t)ownerSD_trip.z * gran_params->SD_size_Z_SU) * LENGTH_SU2UU;
  ...
  H5::DataSet ds_x =
      ptFile.createDataSet("x", H5::PredType::NATIVE_FLOAT, dataspace);
  ds_x.write(x, H5::PredType::NATIVE_FLOAT);

  H5::DataSet ds_y =
      ptFile.createDataSet("y", H5::PredType::NATIVE_FLOAT, dataspace);
  ds_y.write(y, H5::PredType::NATIVE_FLOAT);

  H5::DataSet ds_z =
      ptFile.createDataSet("z", H5::PredType::NATIVE_FLOAT, dataspace);
  ...
  if (GET_OUTPUT_SETTING(VEL_COMPONENTS)) {
    ...
    H5::DataSet ds_vx =
        ptFile.createDataSet("vx", H5::PredType::NATIVE_FLOAT, dataspace);
    ds_vx.write(vx, H5::PredType::NATIVE_FLOAT);

    H5::DataSet ds_vy =
        ptFile.createDataSet("vy", H5::PredType::NATIVE_FLOAT, dataspace);
    ds_vy.write(vy, H5::PredType::NATIVE_FLOAT);

    H5::DataSet ds_vz =
        ptFile.createDataSet("vz", H5::PredType::NATIVE_FLOAT, dataspace);
  ...
  if (GET_OUTPUT_SETTING(ABSV)) {
    ...
    for (size_t n = 0; n < nSpheres; n++) {
      absv[n] = sqrt(pos_X_dt.at(n) * pos_X_dt.at(n) +
                     pos_Y_dt.at(n) * pos_Y_dt.at(n) +
                     pos_Z_dt.at(n) * pos_Z_dt.at(n)) *
                VEL_SU2UU;
    }
    H5::DataSet ds_absv =
        ptFile.createDataSet("absv", H5::PredType::NATIVE_FLOAT, dataspace);
  ...
  if (GET_OUTPUT_SETTING(FIXITY)) {
    ...
    H5::DataSet ds_fixed =
        ptFile.createDataSet("fixed", H5::PredType::NATIVE_UCHAR, dataspace);
  ...
  if (gran_params->friction_mode != CHDEM_FRICTION_MODE::FRICTIONLESS &&
    ...
    H5::DataSet ds_wx =
        ptFile.createDataSet("wx", H5::PredType::NATIVE_FLOAT, dataspace);
    ds_wx.write(wx, H5::PredType::NATIVE_FLOAT);

    H5::DataSet ds_wy =
        ptFile.createDataSet("wy", H5::PredType::NATIVE_FLOAT, dataspace);
    ds_wy.write(wy, H5::PredType::NATIVE_FLOAT);

    H5::DataSet ds_wz =
        ptFile.createDataSet("wz", H5::PredType::NATIVE_FLOAT, dataspace);
...
float3 ChSystemDem_impl::getRollingFrictionTorque(unsigned int i,
                                                  unsigned int j) {
  ...
  for (unsigned int neighborID = 0; neighborID < MAX_SPHERES_TOUCHED_BY_SPHERE;
       neighborID++) {
    ...
    if (theirSphereID == j) {
      return make_float3(rolling_friction_torque[theirSphereMappingID].x *
                             FORCE_SU2UU * LENGTH_SU2UU,
                         rolling_friction_torque[theirSphereMappingID].y *
                             FORCE_SU2UU * LENGTH_SU2UU,
                         rolling_friction_torque[theirSphereMappingID].z *
                             FORCE_SU2UU * LENGTH_SU2UU);
    }
  }
  return make_float3(0.0f, 0.0f, 0.0f);
...
void ChSystemDem_impl::getNeighbors(unsigned int ID,
...
float3 ChSystemDem_impl::getRollingVrot(unsigned int i, unsigned int j) {
  if (gran_params->recording_contactInfo == false) {
    printf("ERROR: recording_contactInfo set to false!\n");
    exit(1);
  }
  ...
  for (unsigned int neighborID = 0; neighborID < MAX_SPHERES_TOUCHED_BY_SPHERE;
    ...
    if (theirSphereID == j) {
      return make_float3(
          v_rot_array[theirSphereMappingID].x * LENGTH_SU2UU / TIME_SU2UU,
...
float ChSystemDem_impl::getRollingCharContactTime(unsigned int i,
                                                  unsigned int j) {
  if (gran_params->recording_contactInfo == false) {
    printf("ERROR: recording_contactInfo set to false!\n");
    exit(1);
...
/// Get tangential friction force between body i and j, return 0 if not in
/// contact
float3 ChSystemDem_impl::getSlidingFrictionForce(unsigned int i,
                                                 unsigned int j) {
  if (gran_params->recording_contactInfo == false) {
    printf("ERROR: recording_contactInfo set to false!\n");
    exit(1);
  ...
  for (unsigned int neighborID = 0; neighborID < MAX_SPHERES_TOUCHED_BY_SPHERE;
    ...
    if (theirSphereID == j) {
      return make_float3(
          tangential_friction_force[theirSphereMappingID].x * FORCE_SU2UU,
...
/// for sphere-wall contact, force only stored in one spot, associated with
/// sphere, so i has to be smaller than j, where j is the BC index, i is sphere
/// ID
float3 ChSystemDem_impl::getNormalForce(unsigned int i, unsigned int j) {
  if (gran_params->recording_contactInfo == false) {
    printf("ERROR: recording_contactInfo set to false!\n");
    exit(1);
  }
  ...
  for (unsigned int neighborID = 0; neighborID < MAX_SPHERES_TOUCHED_BY_SPHERE;
    ...
    if (theirSphereID == j) {
      return make_float3(
          normal_contact_force[theirSphereMappingID].x * FORCE_SU2UU,
...
void ChSystemDem_impl::WriteContactInfoFile(
    const std::string& outfilename) const {
  if ((gran_params->recording_contactInfo == false) ||
      (gran_params->friction_mode == CHDEM_FRICTION_MODE::FRICTIONLESS)) {
    CHDEM_ERROR(
        "ERROR! You did not enable contact info recording or are using "
        "frictionless model!\n");
  } else {
    ...
    for (unsigned int n = 0; n < nSpheres; n++) {
      ...
      // go through all possible neighbors
      for (unsigned int neighborID = 0;
           neighborID < MAX_SPHERES_TOUCHED_BY_SPHERE; neighborID++) {
        unsigned int theirSphereMappingID = bodyAoffset + neighborID;
        unsigned int theirSphereID = contact_partners_map[theirSphereMappingID];
        ...
        if (theirSphereID >= n && theirSphereID < nSpheres) {
          ...
          outstrstream << ", "
                       << Length(normal_contact_force[theirSphereMappingID]) *
                              FORCE_SU2UU;

          if (gran_params->friction_mode != CHDEM_FRICTION_MODE::FRICTIONLESS) {
            outstrstream << ", "
                         << tangential_friction_force[theirSphereMappingID].x *
                                FORCE_SU2UU
                         << ", "
                         << tangential_friction_force[theirSphereMappingID].y *
                                FORCE_SU2UU
                         << ", "
                         << tangential_friction_force[theirSphereMappingID].z *
                                FORCE_SU2UU;
          }

          if (gran_params->rolling_mode != CHDEM_ROLLING_MODE::NO_RESISTANCE) {
            outstrstream << ", "
                         << rolling_friction_torque[theirSphereMappingID].x *
                                FORCE_SU2UU * LENGTH_SU2UU
                         << ", "
                         << rolling_friction_torque[theirSphereMappingID].y *
                                FORCE_SU2UU * LENGTH_SU2UU
...
void ChSystemDem_impl::copyConstSphereDataToDevice() {
  gran_params->max_x_pos =
      ((int64_t)gran_params->SD_size_X_SU * gran_params->nSDs_X);
  gran_params->max_y_pos =
      ((int64_t)gran_params->SD_size_Y_SU * gran_params->nSDs_Y);
  gran_params->max_z_pos =
      ((int64_t)gran_params->SD_size_Z_SU * gran_params->nSDs_Z);
  ...
  int64_t true_max_pos =
      std::max(std::max(gran_params->max_x_pos, gran_params->max_y_pos),
               gran_params->max_z_pos);
  ...
  //     printf("BIG WARNING! Max possible position is greater than
  //     UINT_MAX!!!\n"); printf("You are now relying on Conlain's local
  ...
  gran_params->sphereInertia_by_r =
      (float)((2.0 / 5.0) * gran_params->sphere_mass_SU *
              gran_params->sphereRadius_SU);
}

size_t ChSystemDem_impl::CreateBCSphere(float center[3], float radius,
                                        bool outward_normal, bool track_forces,
                                        float mass) {
...
size_t ChSystemDem_impl::CreateBCConeZ(float cone_tip[3], float slope,
...
size_t ChSystemDem_impl::CreateBCPlane(float plane_pos[3],
                                       float plane_normal[3], bool track_forces,
                                       size_t position) {
  BC_params_t<float, float3> p;
  ...
  double len = std::sqrt(plane_normal[0] * plane_normal[0] +
...
size_t ChSystemDem_impl::CreateCustomizedPlate(float plate_pos_center[3],
                                               float plate_normal[3],
                                               float hdim_y) {
  BC_params_t<float, float3> p;
  ...
  double len = std::sqrt(plate_normal[0] * plate_normal[0] +
...
size_t ChSystemDem_impl::CreateBCCylinderZ(float center[3], float radius,
                                           bool outward_normal,
                                           bool track_forces) {
  BC_params_t<float, float3> p;
...
double ChSystemDem_impl::get_max_K() const {
  ...
  if (gran_params->use_mat_based == true) {
    ...
    // see reference eq 2.13, 2.14 in Book Contact Force Model, Flores and
    // Lankarani
    double sigma_sphere =
        (1 - std::pow(PoissonRatio_sphere_UU, 2)) / YoungsModulus_sphere_UU;
    double sigma_wall =
        (1 - std::pow(PoissonRatio_sphere_UU, 2)) / YoungsModulus_wall_UU;

    maxK = 4.0 / (3.0 * (sigma_sphere + std::min(sigma_sphere, sigma_wall))) *
           std::sqrt(sphere_radius_UU);

    INFO_PRINTF(
        "Use material based contact force model, maximum effective stiffnes is "
        "%e\n",
        maxK);
    return maxK;
    ...
    INFO_PRINTF(
        "Use user defined contact force model, maximum effective stiffnes is "
        "%e\n",
        maxK);
    return maxK;
  }
  ...
  switch (bc_type) {
    case BC_type::SPHERE: {
      if (params_SU.active) {
        ...
        // TODO: flag for this part? original implementation is commented out...
        // need to figure out the structure
        // TODO sphere update can be different dependent on the integrator
        float3 sphere_acc =
            params_SU.reaction_forces * 1.f / params_SU.sphere_params.mass;
        ...
        float sphere_inertia = 0.4f * params_SU.sphere_params.mass *
                               params_SU.sphere_params.radius *
                               params_SU.sphere_params.radius;

        float3 ang_acc =
            params_SU.sphere_params.reaction_torques * 1.f / sphere_inertia;
    ...
    case BC_type::CONE: {
      ...
      params_SU.cone_params.cone_tip.x = convertToPosSU<int64_t, float>(
          (float)(params_UU.cone_params.cone_tip.x + offset_UU.x));
      params_SU.cone_params.cone_tip.y = convertToPosSU<int64_t, float>(
          (float)(params_UU.cone_params.cone_tip.y + offset_UU.y));
      params_SU.cone_params.cone_tip.z = convertToPosSU<int64_t, float>(
          (float)(params_UU.cone_params.cone_tip.z + offset_UU.z));
      new_pos = params_SU.cone_params.cone_tip;

      params_SU.cone_params.hmax = convertToPosSU<int64_t, float>(
          (float)(params_UU.cone_params.hmax + offset_UU.z));
      params_SU.cone_params.hmin = convertToPosSU<int64_t, float>(
          (float)(params_UU.cone_params.hmin + offset_UU.z));
    ...
    case BC_type::PLANE: {
      ...
      params_SU.plane_params.position.x = convertToPosSU<int64_t, float>(
          (float)(params_UU.plane_params.position.x + offset_UU.x));
      params_SU.plane_params.position.y = convertToPosSU<int64_t, float>(
          (float)(params_UU.plane_params.position.y + offset_UU.y));
      params_SU.plane_params.position.z = convertToPosSU<int64_t, float>(
          (float)(params_UU.plane_params.position.z + offset_UU.z));
    ...
    case BC_type::CYLINDER: {
      ...
      params_SU.cyl_params.center.x = convertToPosSU<int64_t, float>(
          (float)(params_UU.cyl_params.center.x + offset_UU.x));
      params_SU.cyl_params.center.y = convertToPosSU<int64_t, float>(
          (float)(params_UU.cyl_params.center.y + offset_UU.y));
      params_SU.cyl_params.center.z = convertToPosSU<int64_t, float>(
          (float)(params_UU.cyl_params.center.z + offset_UU.z));
...
float3 ChSystemDem_impl::GetBCPlanePosition(size_t plane_id) const {
...
float3 ChSystemDem_impl::GetBCSpherePosition(size_t bc_id) const {
...
void ChSystemDem_impl::SetBCSpherePosition(size_t bc_id, const float3 pos) {
  BC_params_list_SU[bc_id].sphere_params.sphere_center.x =
      (int)(pos.x / LENGTH_SU2UU);
  BC_params_list_SU[bc_id].sphere_params.sphere_center.y =
      (int)(pos.y / LENGTH_SU2UU);
  BC_params_list_SU[bc_id].sphere_params.sphere_center.z =
      (int)(pos.z / LENGTH_SU2UU);
}
...
void ChSystemDem_impl::SetBCSphereVelocity(size_t bc_id, const float3 velo) {
  BC_params_list_SU[bc_id].sphere_params.sphere_velo.x =
      velo.x / LENGTH_SU2UU * TIME_SU2UU;
  BC_params_list_SU[bc_id].sphere_params.sphere_velo.y =
      velo.y / LENGTH_SU2UU * TIME_SU2UU;
  BC_params_list_SU[bc_id].sphere_params.sphere_velo.z =
      velo.z / LENGTH_SU2UU * TIME_SU2UU;
}

void ChSystemDem_impl::SetBCPlaneRotation(size_t plane_id,
                                          double3 rotation_center,
                                          double3 rotation_omega) {
  BC_params_list_UU[plane_id].plane_params.rotation_center.x =
      (int64_t)rotation_center.x;
  BC_params_list_UU[plane_id].plane_params.rotation_center.y =
      (int64_t)rotation_center.y;
  BC_params_list_UU[plane_id].plane_params.rotation_center.z =
      (int64_t)rotation_center.z;
...
bool ChSystemDem_impl::GetBCReactionForces(size_t BC_id, float3& force) const {
...
void ChSystemDem_impl::convertBCUnits() {
  for (int i = 0; i < BC_type_list.size(); i++) {
    ...
    switch (bc_type) {
      case BC_type::SPHERE: {
        ...
        params_SU.sphere_params.radius =
            (unsigned int)convertToPosSU<int64_t, float>(
                (params_UU.sphere_params.radius));
        params_SU.sphere_params.normal_sign =
            (int64_t)params_UU.sphere_params.normal_sign;

        // add BC mass
        params_SU.sphere_params.mass =
            params_UU.sphere_params.mass / MASS_SU2UU;

        // BC position
        params_SU.sphere_params.sphere_center.x =
            (int)convertToPosSU<int64_t, float>(
                (float)(params_UU.sphere_params.sphere_center.x));
        params_SU.sphere_params.sphere_center.y =
            (int)convertToPosSU<int64_t, float>(
                (float)(params_UU.sphere_params.sphere_center.y));
        params_SU.sphere_params.sphere_center.z =
            (int)convertToPosSU<int64_t, float>(
                (float)(params_UU.sphere_params.sphere_center.z));

        // BC velocity
        params_SU.sphere_params.sphere_velo.x =
            params_UU.sphere_params.sphere_velo.x / VEL_SU2UU;
        params_SU.sphere_params.sphere_velo.y =
            params_UU.sphere_params.sphere_velo.y / VEL_SU2UU;
        params_SU.sphere_params.sphere_velo.z =
            params_UU.sphere_params.sphere_velo.z / VEL_SU2UU;
      ...
      case BC_type::CONE: {
        ...
        params_SU.cone_params.normal_sign =
            (int64_t)params_UU.cone_params.normal_sign;
      ...
      case BC_type::PLANE: {
        ...
        params_SU.plane_params.normal.x =
            (float)params_UU.plane_params.normal.x;
        params_SU.plane_params.normal.y =
            (float)params_UU.plane_params.normal.y;
        params_SU.plane_params.normal.z =
            (float)params_UU.plane_params.normal.z;

        params_SU.plane_params.rotation_center.x =
            (int64_t)((float)params_UU.plane_params.rotation_center.x /
                      LENGTH_SU2UU);
        ...
        params_SU.plane_params.angular_acc.x =
            params_UU.plane_params.angular_acc.x * TIME_SU2UU;
        params_SU.plane_params.angular_acc.y =
            params_UU.plane_params.angular_acc.y * TIME_SU2UU;
        params_SU.plane_params.angular_acc.z =
            params_UU.plane_params.angular_acc.z * TIME_SU2UU;
        BC_params_list_SU.push_back(params_SU);
        break;
      }
      case BC_type::CYLINDER: {
        ...
        // TODO normalize this just in case
        // float abs = Length(params_UU);
        params_SU.cyl_params.radius =
            convertToPosSU<int64_t, float>(params_UU.cyl_params.radius);
        params_SU.cyl_params.normal_sign =
            (int64_t)params_UU.cyl_params.normal_sign;
...
void ChSystemDem_impl::initializeSpheres() {
  ...
  // Set aside memory for holding data structures worked with. Get some
  // initializations going
  partitionBD();
  ...
  // CUDA 13.0+ requires cudaMemLocation instead of int device
  cudaMemLocation location;
  location.type = cudaMemLocationTypeDevice;
  location.id = dev_ID;
  demErrchk(cudaMemAdvise(gran_params, sizeof(*gran_params),
                          cudaMemAdviseSetReadMostly, location));
  demErrchk(cudaMemAdvise(sphere_data, sizeof(*sphere_data),
                          cudaMemAdviseSetReadMostly, location));
...
void ChSystemDem_impl::SetParticlePosition(int nSphere, double3 position) {
  ...
  int sphere_pos_local_X =
      (int)(sphCenter_X_modified -
            (int64_t)ownerSD.x * gran_params->SD_size_X_SU);
  int sphere_pos_local_Y =
      (int)(sphCenter_Y_modified -
            (int64_t)ownerSD.y * gran_params->SD_size_Y_SU);
  int sphere_pos_local_Z =
      (int)(sphCenter_Z_modified -
            (int64_t)ownerSD.z * gran_params->SD_size_Z_SU);

  unsigned int SDID = ownerSD_x * gran_params->nSDs_Y * gran_params->nSDs_Z +
                      ownerSD_y * gran_params->nSDs_Z + ownerSD_z;
  ...
  //     printf("ERROR! Sphere %u has invalid SD %u, max is %u, triplet %d, %d,
  //     %d\n", nSphere, SDID,
...
float ChSystemDem_impl::ComputeTotalKE() {
  ...
  if (num_spheres == 0) return 0.f;

  // Compute sum(v^2) and sum(w^2)
  float v2_UU =
      computeArray3SquaredSum(pos_X_dt, pos_Y_dt, pos_Z_dt, num_spheres);
  v2_UU *= VEL_SU2UU * VEL_SU2UU;
  float w2_UU = computeArray3SquaredSum(sphere_Omega_X, sphere_Omega_Y,
                                        sphere_Omega_Z, num_spheres);
  w2_UU /= TIME_SU2UU * TIME_SU2UU;
  float m = CH_4_3 * CH_PI * sphere_radius_UU * sphere_radius_UU *
            sphere_radius_UU * sphere_density_UU;

  // Then, KE = 0.5 * m * sum(v^2) + 0.2 * m * r^2 * sum(w^2)
  return 0.5 * m * v2_UU +
         0.2 * m * sphere_radius_UU * sphere_radius_UU * w2_UU;
}
...
float3 ChSystemDem_impl::GetParticleLinAcc(int nSphere) const {
  double acc_x =
      sphere_acc_X.at(nSphere) * LENGTH_SU2UU / (TIME_SU2UU * TIME_SU2UU);
  double acc_y =
      sphere_acc_Y.at(nSphere) * LENGTH_SU2UU / (TIME_SU2UU * TIME_SU2UU);
  double acc_z =
      sphere_acc_Z.at(nSphere) * LENGTH_SU2UU / (TIME_SU2UU * TIME_SU2UU);
...
// Partitions the big domain (BD) and sets the number of SDs that BD is split
// in.
void ChSystemDem_impl::partitionBD() {
  ...
  double sd_length_scale =
      2.0 * sphere_radius_UU * AVERAGE_SPHERES_PER_SD_X_DIR;
  ...
  gran_params->BD_frame_X = (int64_t)(-0.5 * ((int64_t)nSDs_X * SD_size_X) +
                                      (double)user_coord_O_X / LENGTH_SU2UU);
  gran_params->BD_frame_Y = (int64_t)(-0.5 * ((int64_t)nSDs_Y * SD_size_Y) +
                                      (double)user_coord_O_Y / LENGTH_SU2UU);
  gran_params->BD_frame_Z = (int64_t)(-0.5 * ((int64_t)nSDs_Z * SD_size_Z) +
                                      (double)user_coord_O_Z / LENGTH_SU2UU);

  // permanently cache the initial frame
  BD_rest_frame_SU =
      make_longlong3(gran_params->BD_frame_X, gran_params->BD_frame_Y,
                     gran_params->BD_frame_Z);

  INFO_PRINTF("%u Sds as %u, %u, %u\n", gran_params->nSDs, gran_params->nSDs_X,
              gran_params->nSDs_Y, gran_params->nSDs_Z);
  ...
  TRACK_VECTOR_RESIZE(SD_SphereCompositeOffsets, nSDs,
                      "SD_SphereCompositeOffsets", 0);
  TRACK_VECTOR_RESIZE(SD_SphereCompositeOffsets_ScratchPad, nSDs,
                      "SD_SphereCompositeOffsets_ScratchPad", 0);
}

void ChSystemDem_impl::combineMaterialSurface() {
  // effective youngs modulus and shear modulus in user units
  double E_eff_s2s_uu, E_eff_s2w_uu, G_eff_s2s_uu, G_eff_s2w_uu;

  materialPropertyCombine(YoungsModulus_sphere_UU, YoungsModulus_sphere_UU,
                          PoissonRatio_sphere_UU, PoissonRatio_sphere_UU,
                          E_eff_s2s_uu, G_eff_s2s_uu);

  materialPropertyCombine(YoungsModulus_sphere_UU, YoungsModulus_wall_UU,
                          PoissonRatio_sphere_UU, PoissonRatio_wall_UU,
                          E_eff_s2w_uu, G_eff_s2w_uu);

  // unit conversion of youngs modulus
  double E_SU2UU = this->MASS_SU2UU /
                   (this->LENGTH_SU2UU * this->TIME_SU2UU * this->TIME_SU2UU);
...
void ChSystemDem_impl::switchToSimUnits() {
  // Compute sphere mass, highest system stiffness, and gravity magnitude
  double massSphere = CH_4_3 * CH_PI * sphere_radius_UU * sphere_radius_UU *
                      sphere_radius_UU * sphere_density_UU;
  double K_star = get_max_K();

  double magGravAcc = sqrt(X_accGrav * X_accGrav + Y_accGrav * Y_accGrav +
                           Z_accGrav * Z_accGrav);
  ...
  // new hertz way
  this->LENGTH_SU2UU =
      std::cbrt(massSphere * massSphere * magGravAcc * magGravAcc *
                sphere_radius_UU / (K_star * K_star)) /
      psi_L;
  this->LENGTH_SU2UU =
      std::min((double)(sphere_radius_UU * psi_R), this->LENGTH_SU2UU);
  ...
  if (this->use_min_length_unit) {
    this->LENGTH_SU2UU = std::min(
        (double)std::max(box_size_X, std::max(box_size_Y, box_size_Z)) /
  ...
  gran_params->sphereRadius_SU =
      (unsigned int)(sphere_radius_UU / LENGTH_SU2UU);
  ...
  gran_params->cohesionAcc_s2s =
      (float)(magGravAcc * cohesion_over_gravity / ACC_SU2UU);
  gran_params->adhesionAcc_s2w =
      (float)(magGravAcc * adhesion_s2w_over_gravity / ACC_SU2UU);

  // LULUTODO: INITIALIZATION DEPENDS ON WHETHER OR NOT MATERIAL PROPERTY IS
  // USED
  ...
  INFO_PRINTF("SU gravity is %f, %f, %f\n", gran_params->gravAcc_X_SU,
              gran_params->gravAcc_Y_SU, gran_params->gravAcc_Z_SU);
