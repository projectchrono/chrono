...
ChSystemDem_impl::ChSystemDem_impl(float sphere_rad, float density, float3 boxDims, float3 O)
...
float3 ChSystemDem_impl::getRollingFrictionTorque(unsigned int i, unsigned int j) {
...
float3 ChSystemDem_impl::getSlidingFrictionForce(unsigned int i, unsigned int j) {
    if (gran_params->recording_contactInfo == false) {
        printf("ERROR: recording_contactInfo set to false!\n");
        exit(1);
    }
...
size_t ChSystemDem_impl::CreateBCPlane(float plane_pos[3], float plane_normal[3], bool track_forces, size_t position) {
    BC_params_t<float, float3> p;
...
size_t ChSystemDem_impl::CreateCustomizedPlate(float plate_pos_center[3], float plate_normal[3], float hdim_y) {
    BC_params_t<float, float3> p;
...
size_t ChSystemDem_impl::CreateBCCylinderZ(float center[3], float radius, bool outward_normal, bool track_forces) {
    BC_params_t<float, float3> p;
