...
__host__ double ChSystemDem_impl::GetMaxParticleZ(bool getMax) {
    unsigned int num_spheres = (unsigned int)sphere_local_pos_Z.size();
    if (num_spheres == 0)
        CHDEM_ERROR("ERROR! 0 particle in system! Please call this method after Initialize().\n");
...
__host__ unsigned int ChSystemDem_impl::GetNumParticleAboveX(float XValue) {
    unsigned int num_spheres = (unsigned int)sphere_local_pos_X.size();
    if (num_spheres == 0)
        CHDEM_ERROR("ERROR! 0 particle in system! Please call this method after Initialize().\n");
...
__host__ float ChSystemDem_impl::get_max_vel() const {
    float* d_absv;
    float* d_max_vel;
    float h_max_vel;
...
__host__ int3 ChSystemDem_impl::getSDTripletFromID(unsigned int SD_ID) const {
...
__host__ void ChSystemDem_impl::setupSphereDataStructures() {
    ...
    if (user_sphere_positions.size() == 0) {
        CHDEM_ERROR("ERROR! no sphere positions given!\n");
    }
    ...
    //    sphere_owner_SDs.resize(nSpheres, NULL_CHDEM_ID);
    ...
    TRACK_VECTOR_RESIZE(sphere_owner_SDs, nSpheres, "sphere_owner_SDs", NULL_CHDEM_ID);
    ...
    // If user gives Chrono::Dem internal data from a file then it's a restart,
...
/// </summary>
/// <returns></returns>
__host__ void ChSystemDem_impl::runSphereBroadphase() {
