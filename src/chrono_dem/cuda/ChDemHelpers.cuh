...
//
// Holds of helper functions for Chrono::Dem code that need to be scoped higher
//
// =============================================================================
...
inline __host__ __device__ int3 SDIDTriplet(unsigned int SD_ID, ChSystemDem_impl::GranParamsPtr gran_params) {
    int3 SD_trip = {0, 0, 0};
...
// Convert triplet to single int SD ID
inline __device__ unsigned int SDTripletID(const int3& trip, ChSystemDem_impl::GranParamsPtr gran_params) {
    return SDTripletID(trip.x, trip.y, trip.z, gran_params);
}

// Convert triplet to single int SD ID
inline __device__ unsigned int SDTripletID(const int trip[3], ChSystemDem_impl::GranParamsPtr gran_params) {
    return SDTripletID(trip[0], trip[1], trip[2], gran_params);
}

// get an index for the current contact pair
inline __device__ size_t findContactPairInfo(ChSystemDem_impl::GranSphereDataPtr sphere_data,
                                             ChSystemDem_impl::GranParamsPtr gran_params,
                                             unsigned int body_A,
                                             unsigned int body_B) {
...
inline __device__ float3 computeRollingAngAcc(ChSystemDem_impl::GranSphereDataPtr sphere_data,
                                              ChSystemDem_impl::GranParamsPtr gran_params,
                                              float rolling_coeff,
    ...
    if (gran_params->friction_mode != CHDEM_FRICTION_MODE::FRICTIONLESS &&
        ...
        switch (gran_params->rolling_mode) {
            case CHDEM_ROLLING_MODE::SCHWARTZ: {
...
// set delta_t for the displacement
inline __device__ void computeMultiStepDisplacement(ChSystemDem_impl::GranParamsPtr gran_params,
...
inline __device__ float3 computeFrictionForces(ChSystemDem_impl::GranParamsPtr gran_params,
    ...
    } else if (gran_params->friction_mode == CHDEM_FRICTION_MODE::MULTI_STEP) {
    ...
    constexpr float CHDEM_MACHINE_EPSILON = 1e-6f;
    if (ft < CHDEM_MACHINE_EPSILON) {
        return make_float3(0.f, 0.f, 0.f);
    }
