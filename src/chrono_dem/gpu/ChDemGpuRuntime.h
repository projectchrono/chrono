...
inline cudaError_t cudaMallocManaged(void** ptr, std::size_t bytes, unsigned int flags = cudaMemAttachGlobal) {
    return hipMallocManaged(ptr, bytes, flags);
}
template <class T>
inline cudaError_t cudaMallocManaged(T** ptr, std::size_t bytes, unsigned int flags = cudaMemAttachGlobal) {
    return hipMallocManaged(reinterpret_cast<void**>(ptr), bytes, flags);
}
inline cudaError_t cudaFree(void* ptr) {
    return hipFree(ptr);
}
inline cudaError_t cudaMemcpy(void* dst, const void* src, std::size_t bytes, cudaMemcpyKind kind) {
...
inline cudaError_t cudaMemAdvise(const void* ptr, std::size_t bytes, cudaMemoryAdvise advice, int device) {
