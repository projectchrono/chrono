...
__host__ void ChSystemDemMesh_impl::runTriangleBroadphase() {
    ...
    SDsTouchedByEachTriangle_composite_out.resize(numOfTriangleTouchingSD_instances, NULL_CHDEM_ID);
    SDsTouchedByEachTriangle_composite.resize(numOfTriangleTouchingSD_instances, NULL_CHDEM_ID);
    TriangleIDS_ByMultiplicity_out.resize(numOfTriangleTouchingSD_instances, NULL_CHDEM_ID);
    ...
    demErrchk(gpuDeviceSynchronize());
    demErrchk(gpuMemcpy(SD_trianglesInEachSD_composite.data(), TriangleIDS_ByMultiplicity_out.data(),
                         numOfTriangleTouchingSD_instances * sizeof(unsigned int), gpuMemcpyDeviceToDevice));
...
__global__ void interactionGranMat_TriangleSoup_matBased(ChSystemDemMesh_impl::TriangleSoupPtr d_triangleSoup,
    ...
    unsigned int sphereIDGlobal = NULL_CHDEM_ID;
...
__global__ void interactionGranMat_TriangleSoup(ChSystemDemMesh_impl::TriangleSoupPtr d_triangleSoup,
                                                ChSystemDem_impl::GranSphereDataPtr sphere_data,
                                                const unsigned int* SD_trianglesInEachSD_composite,
                                                const unsigned int* SD_numTrianglesTouching,
                                                const unsigned int* SD_TrianglesCompositeOffsets,
...
__host__ double ChSystemDemMesh_impl::AdvanceSimulation(float duration) {
    ...
    for (; time_elapsed_SU < stepSize_SU * nsteps; time_elapsed_SU += stepSize_SU) {
        ...
        if (meshSoup->nTrianglesInSoup != 0 && mesh_collision_enabled) {
            demErrchk(
                gpuMemset(meshSoup->generalizedForcesPerFamily, 0, 6 * meshSoup->numTriangleFamilies * sizeof(float)));
        }
        demErrchk(gpuPeekAtLastError());
        demErrchk(gpuDeviceSynchronize());
