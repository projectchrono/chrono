...
/// Structure with Chrono::Dem simulation parameters.
struct ChDemSimulationParameters {
    float sphere_radius;
    float sphere_density;
    float box_X;
    float box_Y;
    float box_Z;
    CHDEM_TIME_INTEGRATOR time_integrator;
    float step_size;
    float time_end;
    float grav_X;
    float grav_Y;
    ...
    CHDEM_VERBOSITY verbose;
    CHDEM_RUN_MODE run_mode;
    unsigned int psi_T;
    unsigned int psi_L;
    float psi_R;
    std::string output_dir;
    std::string checkpoint_file;
    CHDEM_OUTPUT_MODE write_mode;
};
...
bool ParseJSON(const std::string& json_file, ChDemSimulationParameters& params, bool verbose = true) {
    ...
    if (doc.HasMember("time_integrator") && doc["time_integrator"].IsString()) {
        ...
        if (time_integrator == std::string("forward_euler"))
            params.time_integrator = CHDEM_TIME_INTEGRATOR::FORWARD_EULER;
        else if (time_integrator == std::string("chung"))
            params.time_integrator = CHDEM_TIME_INTEGRATOR::CHUNG;
        else if (time_integrator == std::string("centered_difference"))
            params.time_integrator = CHDEM_TIME_INTEGRATOR::CENTERED_DIFFERENCE;
        else if (time_integrator == std::string("extended_taylor"))
            params.time_integrator = CHDEM_TIME_INTEGRATOR::EXTENDED_TAYLOR;
        else {
    ...
    if (doc.HasMember("friction_mode") && doc["friction_mode"].IsString()) {
        ...
        if (friction_mode == std::string("frictionless"))
            params.friction_mode = CHDEM_FRICTION_MODE::FRICTIONLESS;
        else if (friction_mode == std::string("single_step"))
            params.friction_mode = CHDEM_FRICTION_MODE::SINGLE_STEP;
        else if (friction_mode == std::string("multi_step"))
            params.friction_mode = CHDEM_FRICTION_MODE::MULTI_STEP;
        else {
    ...
    if (doc.HasMember("rolling_mode") && doc["rolling_mode"].IsString()) {
        ...
        if (rolling_mode == std::string("no_resistance"))
            params.rolling_mode = CHDEM_ROLLING_MODE::NO_RESISTANCE;
        else if (rolling_mode == std::string("schwartz"))
            params.rolling_mode = CHDEM_ROLLING_MODE::SCHWARTZ;
        else {
    ...
    if (doc.HasMember("verbose") && doc["verbose"].IsInt()) {
        params.verbose = (CHDEM_VERBOSITY)doc["verbose"].GetInt();
    ...
    if (doc.HasMember("run_mode") && doc["run_mode"].IsString()) {
        ...
        if (run_mode == std::string("frictionless"))
            params.run_mode = CHDEM_RUN_MODE::FRICTIONLESS;
        else if (run_mode == std::string("one_step"))
            params.run_mode = CHDEM_RUN_MODE::ONE_STEP;
        else if (run_mode == std::string("multi_step"))
            params.run_mode = CHDEM_RUN_MODE::MULTI_STEP;
        else {
    ...
    if (doc.HasMember("write_mode") && doc["write_mode"].IsString()) {
        ...
        if (write_mode == std::string("binary"))
            params.write_mode = CHDEM_OUTPUT_MODE::BINARY;
        else if (write_mode == std::string("csv"))
            params.write_mode = CHDEM_OUTPUT_MODE::CSV;
        else if (write_mode == std::string("hdf5"))
            params.write_mode = CHDEM_OUTPUT_MODE::HDF5;
        else if (write_mode == std::string("none"))
            params.write_mode = CHDEM_OUTPUT_MODE::NONE;
        else {
            InvalidArg("write_mode");
            return false;
        }
