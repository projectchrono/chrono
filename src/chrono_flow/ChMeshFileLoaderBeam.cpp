...
void ChMeshFileLoaderBeam::FromAbaqusFile(std::shared_ptr<ChMesh> mesh,
    ...
    while (std::getline(fin, line)) {
        ...
        if (line[0] == '*') {
            ...
            if (line.find("*ELEMENT") == 0) {
                ...
                if (nty > 0) {
                    ...
                    } else if (s_ele_type == "U1") {
                        e_parse_section = E_PARSE_BEAMS_2;
                    }
        ...
        // element parsing
        if (e_parse_section == E_PARSE_TETS_10 || e_parse_section == E_PARSE_TETS_4 || e_parse_section == E_PARSE_BEAMS_2) {
            ////int idelem = 0;
            ...
            if (e_parse_section == E_PARSE_TETS_10) {
                ...
                for (int in = 0; in < 4; ++in)
                    if (tokenvals[in + 1] == -10e30)
                        throw std::invalid_argument("ERROR in in .inp file: in parsing IDs of tetrahedron: \n" + line +
                                                    "\n");
            } else if (e_parse_section == E_PARSE_BEAMS_2) {
                if (ntoken != 3)
                    throw std::invalid_argument(
                        "ERROR in .inp file: tetrahedrons require ID and 4 node IDs, see line:\n" + line + "\n");
...
void ChMeshFileLoaderBeam::ANCFShellFromGMFFile(std::shared_ptr<ChMesh> mesh,
                                            const char* filename,
...
void ChMeshFileLoaderBeam::BSTShellFromObjFile(
