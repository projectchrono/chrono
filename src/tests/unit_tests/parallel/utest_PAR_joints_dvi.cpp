...
class JointsDVI : public ::testing::TestWithParam<Options> {
  protected:
    JointsDVI() : animate(false) {
        ...
        utils::AddCylinderGeometry(wheel.get(), 0.3, 0.1, wheel_mat, ChVector<>(0, 0, 0), Q_from_AngZ(CH_C_PI_2));
...
INSTANTIATE_TEST_CASE_P(ChronoParallel, JointsDVI, ::testing::ValuesIn(options));
