...
TEST_F(Mat33Test, constructors) {
    ...
    Assert_near(Mat33(R1), FromChMatrix33(ToChQuaternion(R1)), CH_REAL_EPSILON * 3);
}
...
TEST_F(Mat33Test, functions) {
    ...
    }
    {
        ...
        Assert_near(Res1, FromChMatrix33(Res2), CH_REAL_EPSILON * 2);
    }
    {
        // Multiply Matrix T
        ChMatrix33<real> Res2 = B1 * B2.transpose();
        Assert_near(MultTranspose(A1, A2), FromChMatrix33(Res2), CH_REAL_EPSILON * 2);
    }
    {
        // Outer Product
        Mat33 Res1 = OuterProduct(a1, a2);
        Mat33 Res2(6, 12, 18, 7, 14, 21, 8, 16, 24);
        Assert_near(Res1, Res2, CH_REAL_EPSILON);
    }
    // Transpose
    Assert_near(Transpose(A4), A4_T, CH_REAL_EPSILON);

    // Determinant
    ASSERT_NEAR(Determinant(A5), 45.056, CH_REAL_EPSILON * 400);

    // Trace
    ASSERT_NEAR(Trace(A5), 4.8, CH_REAL_EPSILON);

    // Adjoint
    Assert_near(Adjoint(A3), A4, CH_REAL_EPSILON);

    // Adjoint Transpose
    Assert_near(AdjointTranspose(A4), Transpose(A3), CH_REAL_EPSILON);

    // Inverse
    Assert_near(Inverse(A3), A4, CH_REAL_EPSILON);

    // Inverse Transpose
    Assert_near(InverseTranspose(A3), Transpose(Inverse(A3)), CH_REAL_EPSILON);

    // Frobenius Norm
    ASSERT_NEAR(Norm(A5), 12.674383614203887588, CH_REAL_EPSILON);

    // Largest Column Normalized
    Assert_near(LargestColumnNormalized(A4),
                real3(-.75856744948921676267, 0.63213954124101396889, -.15803488531025349222), CH_REAL_EPSILON);

    // Normal Equations Matrix
    Assert_near(NormalEquationsMatrix(A3), Transpose(A3) * A3, CH_REAL_EPSILON);
    Assert_near(NormalEquationsMatrix(A3), Mat33(26, 32, 3, 32, 41, 10, 3, 10, 25), CH_REAL_EPSILON);
