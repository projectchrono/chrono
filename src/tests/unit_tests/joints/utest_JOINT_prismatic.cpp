                   ...
                   const std::string& testName)     // if true, also save animation data
{
    ...
    sys.GetSolver()->AsIterative()->SetTolerance(simTimeStep * 1e-6);
    ...
    while (simTime <= timeRecord + simTimeStep / 2) {
        ...
        if (simTime >= outTime - simTimeStep / 2) {
            ...
            ChFrame<> linkCoordsys = prismaticJoint->GetFrame2Rel();
            ChVector3d reactForce = prismaticJoint->GetReactForce2();
            ...
            ChVector3d reactTorque = prismaticJoint->GetReactTorque2();
