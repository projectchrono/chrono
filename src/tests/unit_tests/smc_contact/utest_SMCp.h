...
std::shared_ptr<ChBody> AddSphere(int id,
    ...
    auto body = chrono_types::make_shared<ChBody>(chrono_types::make_shared<ChCollisionModelParallel>(), ChContactMethod::SMC);
    ...
    utils::AddSphereGeometry(body.get(), radius, mat);
...
std::shared_ptr<ChBody> AddWall(int id,
    ...
    body->GetCollisionModel()->ClearModel();
    utils::AddBoxGeometry(body.get(), size / 2, mat);
    body->GetCollisionModel()->BuildModel();
...
void SetSimParameters(ChSystemParallelSMC* sys, const ChVector<>& gravity, ChSystemSMC::ContactForceModel fmodel) {
    ...
    sys->ChangeCollisionSystem(CollisionSystemType::COLLSYS_PARALLEL);
}
