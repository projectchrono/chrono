...
int main(int argc, char* argv[]) {
	...
	printf("\ntimePause %f, numPause %d\n", paramsH.timePause,
			int(paramsH.timePause / paramsH.dT));
	printf("paramsH.timePauseRigidFlex %f, numPauseRigidFlex %d\n\n",
			...
			int(
					(paramsH.timePauseRigidFlex - paramsH.timePause)
							/ paramsH.dT + paramsH.timePause / paramsH.dT));
...
#if haveFluid
	ForceSPH_LF(posRadD, velMasD, rhoPresMuD, bodyIndexD, derivVelRhoD,
			referenceArray, numObjects, paramsH, bceType, 0.5 * paramsH.dT);

	UpdateFluid_init_LF(posRadD, velMasD_half, rhoPresMuD_half, derivVelRhoD,
			referenceArray, paramsH.dT);
...
// ****************** RK2: 1/2
		...
		UpdateFluid_rho_vel_LF(velMasD, rhoPresMuD, velMasD_half,
				rhoPresMuD_half, derivVelRhoD, referenceArray,
				0.5 * paramsH.dT);
		...
		int out_steps = std::ceil((1.0 / time_step) / out_fps);
		PrintToFile(posRadD, velMasD, rhoPresMuD, referenceArray, paramsH,
				realTime, tStep, out_steps, pov_dir_fluid);
		...
		ForceSPH_LF(posRadD, velMasD, rhoPresMuD, bodyIndexD, derivVelRhoD,
				referenceArray, numObjects, paramsH, bceType, paramsH.dT); // Arman later take care of dT. It is useless and you can remove it.

		fsi_timer.stop("force_sph");
		UpdateFluid_EveryThing_LF(posRadD, velMasD_half, rhoPresMuD_half,
				derivVelRhoD, referenceArray, paramsH.dT);
