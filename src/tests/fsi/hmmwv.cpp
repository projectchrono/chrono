...
public:
	...
	virtual void onCallback(double time, double& throttle, double& steering,
			double& braking) {
		throttle = 0;
		steering = 0;
		braking = 0;
...
public:
	virtual void onCallback(ChSharedPtr<ChBody> wheelBody, double radius,
			double width) {
		wheelBody->ChangeCollisionModel(
...
public:
	MyLuggedTire() {
		...
		utils::LoadConvexMesh(vehicle::GetDataFile(lugged_file), lugged_mesh,
				lugged_convex);
	...
	virtual void onCallback(ChSharedPtr<ChBody> wheelBody, double radius,
		...
		ChCollisionModelParallel* coll_model =
				(ChCollisionModelParallel*) wheelBody->GetCollisionModel();
		...
		for (int iseg = 0; iseg < 15; iseg++) {
			ChQuaternion<> rot = Q_from_AngAxis(iseg * 24 * CH_C_DEG_TO_RAD,
					VECT_Y);
...
public:
	...
	virtual void onCallback(ChSharedPtr<ChBody> wheelBody, double radius,
		...
		wheelBody->GetCollisionModel()->ClearModel();
		for (int j = 0; j < 15; j++) {
			utils::AddConvexCollisionModel(wheelBody, lugged_mesh,
					lugged_convex, VNULL, Q_from_AngAxis(j * 24 * CH_C_DEG_TO_RAD, VECT_Y), false);
		}
...
int main(int argc, char* argv[]) {
	...
	system->GetSettings()->solver.max_iteration_bilateral =
			max_iteration_bilateral;
	system->GetSettings()->solver.max_iteration_normal = max_iteration_normal;
	system->GetSettings()->solver.max_iteration_sliding = max_iteration_sliding;
	system->GetSettings()->solver.max_iteration_spinning =
			max_iteration_spinning;
	system->GetSettings()->solver.alpha = 0;
	system->GetSettings()->solver.contact_recovery_speed =
			contact_recovery_speed;
	system->ChangeSolverType(APGD);

	system->GetSettings()->collision.narrowphase_algorithm =
	...
	// Bottom box
	utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(hdimX, hdimY, hthick),
			ChVector<>(0, 0, -hthick), ChQuaternion<>(1, 0, 0, 0), true);
	if (terrain_type == GRANULAR) {
		...
		utils::AddBoxGeometry(ground.get_ptr(),
				...
				ChQuaternion<>(1, 0, 0, 0), visible_walls);
		// Rear box
		utils::AddBoxGeometry(ground.get_ptr(),
				ChVector<>(hthick, hdimY, hdimZ + hthick),
				ChVector<>(-hdimX - hthick, 0, hdimZ - hthick),
	...
	switch (wheel_type) {
	case CYLINDRICAL: {
		vehicle = new ChWheeledVehicleAssembly(system, vehicle_file_cyl,
				simplepowertrain_file);
		tire_cb = new MyCylindricalTire();
	}
		break;
	case LUGGED: {
		vehicle = new ChWheeledVehicleAssembly(system, vehicle_file_lug,
				simplepowertrain_file);
...
// -----------------------
	...
	gl_window.SetCamera(ChVector<>(0, -10, 0), ChVector<>(0, 0, 0),
			ChVector<>(0, 0, 1));
	...
	while (time < time_end) {
		...
		if (sim_frame == next_out_frame) {
			...
			cout << "     Time:           " << time << endl;
			cout << "     Speed:          "
					<< vehicle->GetVehicle()->GetVehicleSpeed() << endl;
		...
		if (vehicle->GetVehicle()->GetChassis()->GetBodyFixed()
				&& time > time_hold) {
			cout << endl << "Release vehicle t = " << time << endl;
			vehicle->GetVehicle()->GetChassis()->SetBodyFixed(false);
			for (int i = 0; i < 2 * vehicle->GetVehicle()->GetNumberAxles();
					i++) {
...
// Advance dynamics.
		...
		if (monitor_bilaterals && sim_frame % bilateral_frame_interval == 0) {
			...
			cout << "  Max. violation = "
					<< system->CalculateConstraintViolation(cvec) << endl;
		}
