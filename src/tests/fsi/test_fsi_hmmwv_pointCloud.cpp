...
void CreateChassisBCE(
	...
	AddBCE_FromFile(posRadH, velMasH, rhoPresMuH, referenceArray, numObjects,
			paramsH, mVehicle->GetVehicle()->GetChassis(),
			dataPath);
...
void CreateMbdPhysicalSystemObjects(ChSystemParallelDVI& mphysicalSystem,
	...
	if (haveVehicle) {
		...
		switch (wheel_type) {
		case CYLINDRICAL: {
			mVehicle = new vehicle::ChWheeledVehicleAssembly(&mphysicalSystem,
		...
		mVehicle->Initialize(initLoc + ChVector<>(0, 0, vertical_offset),
				initRot);
...
// -----------------
#if haveFluid
		CreateChassisBCE(posRadH, velMasH, rhoPresMuH, referenceArray,
				FSI_Bodies, numObjects, paramsH);
...
//                                                           // X
//
//          mball->GetCollisionModel()->SetFamily(5);
//          mball->GetCollisionModel()->SetFamilyMaskNoCollisionWithFamily(5);
//
...
//	chrono::SetFsiDataPath(CHRONO_FSI_DATA_DIR); // Defined in ChronoConfic.cmake.in
	...
	for (int tStep = 0; tStep < stepEnd + 1; tStep++) {
		...
		DoStepDynamics_ChronoRK2(mphysicalSystem, mVehicle,

				pos_ChSystemBackupH, quat_ChSystemBackupH, vel_ChSystemBackupH,
				omegaLRF_ChSystemBackupH,

				paramsH, mTime, time_hold_vehicle,
				haveVehicle);
