...
void InitializeMbdPhysicalSystem(ChSystemParallelDVI& mphysicalSystem, int argc,
	...
	mphysicalSystem.GetSettings()->solver.max_iteration_normal = max_iteration
			/ 3;
	mphysicalSystem.GetSettings()->solver.max_iteration_sliding = max_iteration
			/ 3;
	...
	mphysicalSystem.GetSettings()->collision.collision_envelope =
			collisionEnvelop;
	mphysicalSystem.GetSettings()->collision.bins_per_axis = _make_int3(10, 10,
			10);  // Arman check
}

void create_system_particles(ChSystemParallelDVI& mphysicalSystem) {
	Real3 domainCenter = 0.5 * (paramsH.cMin + paramsH.cMax);
	Real3 ellipsoidSize = .3
			* mR3(10 * paramsH.HSML, 5 * paramsH.HSML, 7 * paramsH.HSML);
	ChVector<> size = ChVector<>(ellipsoidSize.x, ellipsoidSize.y,
			ellipsoidSize.z);
	...
	const ChVector<> pos = ChVector<>(domainCenter.x,
			domainCenter.y + paramsH.cMax.y, domainCenter.z);
...
void InitializeChronoGraphics(ChSystemParallelDVI& mphysicalSystem) {
	Real3 domainCenter = 0.5 * (paramsH.cMin + paramsH.cMax);
	ChVector<> CameraLocation = ChVector<>(2 * paramsH.cMax.x,
			2 * paramsH.cMax.y, 2 * paramsH.cMax.z);
	ChVector<> CameraLookAt = ChVector<>(domainCenter.x, domainCenter.y,
			domainCenter.z);
...
// return 0;
	...
	application = std::shared_ptr < ChIrrApp
			> (new ChIrrApp(&mphysicalSystem, L"Bricks test",
	...
	ChIrrWizard::add_typical_Camera(application->GetDevice(),
			core::vector3df(CameraLocation.x, CameraLocation.y,
					CameraLocation.z),
...
int DoStepChronoSystem(ChSystemParallelDVI& mphysicalSystem, Real dT) {
	...
	ChIrrTools::drawGrid(application->GetVideoDriver(), 2 * paramsH.HSML,
			2 * paramsH.HSML, 50, 50,
			ChCoordsys<>(
					ChVector<>(domainCenter.x, paramsH.worldOrigin.y,
							domainCenter.z), Q_from_AngAxis(CH_C_PI / 2, VECT_X)),
...
int main(int argc, char* argv[]) {
	...
	assert(
			posRadH.size() == numObjects.numAllMarkers
					&& "numObjects is not set correctly");
	...
	AddSphDataToChSystem(mphysicalSystem, startIndexSph, posRadH, velMasH,
			paramsH, numObjects, fluidCollisionFamily);
	...
	outSimulationInfo << " dT: " << dTc << " max_iteration: " << numIter
			...
			<< " number of bodies: " << mphysicalSystem.Get_bodylist()->size()
			<< std::endl;
	...
	for (int tStep = 0; tStep < stepEnd + 1; tStep++) {
		...
		// ****************** RK2: 1/2
		ForceSPH(posRadD, velMasD, vel_XSPH_D, rhoPresMuD, bodyIndexD,
				derivVelRhoD, referenceArray, numObjects, currentParamsH,
				bceType, 0.5 * currentParamsH.dT); //?$ right now, it does not consider paramsH.gravity or other stuff on rigid
		...
		AddChSystemForcesToSphForces(derivVelRhoChronoH, velMasH2,
				mphysicalSystem, numObjects, startIndexSph,
				0.5 * currentParamsH.dT); // assumes velMasH2 constains a copy of velMas in ChSystem right before DoStepDynamics
		CopyH2D(derivVelRhoD, derivVelRhoChronoH);
		UpdateFluid(posRadD2, velMasD2, vel_XSPH_D, rhoPresMuD2, derivVelRhoD,
				referenceArray, 0.5 * currentParamsH.dT); // assumes ...D2 is a copy of ...D
		ApplyBoundarySPH_Markers(posRadD2, rhoPresMuD2,
				numObjects.numAllMarkers);

		CopyD2H(posRadH2, velMasH2, rhoPresMuH2, posRadD2, velMasD2,
				rhoPresMuD2);
		UpdateSphDataInChSystem(mphysicalSystem, posRadH2, velMasH2, numObjects,
				startIndexSph);

		// ****************** RK2: 2/2
		ForceSPH(posRadD2, velMasD2, vel_XSPH_D, rhoPresMuD2, bodyIndexD,
				derivVelRhoD, referenceArray, numObjects, currentParamsH,
				bceType, currentParamsH.dT); //?$ right now, it does not consider paramsH.gravity or other stuff on rigid bodies.
		...
		AddChSystemForcesToSphForces(derivVelRhoChronoH, velMasH2,
				...
				0.5 * currentParamsH.dT); // assumes velMasH2 constains a copy of velMas in ChSystem right before DoStepDynamics
		CopyH2D(derivVelRhoD, derivVelRhoChronoH);
		UpdateFluid(posRadD, velMasD, vel_XSPH_D, rhoPresMuD, derivVelRhoD,
				referenceArray, currentParamsH.dT);
		...
		UpdateSphDataInChSystem(mphysicalSystem, posRadH, velMasH, numObjects,
				startIndexSph);
