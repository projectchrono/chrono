...
// another type of fluid_solid coupling where ChSystem adds constraints and solves the fluid given the other forces on
// the fluid markers (i.e. from the interior of the domain.
...
double CreateGranularBed(ChSystem* mphysicalSystem) {
	...
	utils::MixtureIngredientPtr& m1 = gen.AddMixtureIngredient(utils::SPHERE,
			1.0);
...
void CreateMbdPhysicalSystemObjects(ChSystemParallelDVI& mphysicalSystem,
	...
	switch (wheel_type) {
	case CYLINDRICAL: {
		mVehicle = new ChWheeledVehicleAssembly(&mphysicalSystem,
				vehicle_file_cyl, simplepowertrain_file);
		tire_cb = new MyCylindricalTire();
	}
		break;
	case LUGGED: {
		mVehicle = new ChWheeledVehicleAssembly(&mphysicalSystem,
				vehicle_file_lug, simplepowertrain_file);
...
void SavePovFilesMBD(ChSystemParallelDVI& mphysicalSystem, int tStep,
		double mTime, int& num_contacts, double exec_time) {
...
// =============================================================================
void SetMarkersVelToZero(thrust::device_vector<Real4>& velMasD,
		thrust::host_vector<Real4>& velMasH) {
...
void printSimulationParameters() {
	simParams << " time_hold_vehicle: " << time_hold_vehicle << endl
			...
			<< " maxFlowVelocity " << maxFlowVelocity << endl << " time_step: "
			<< time_step << endl << " time_end: " << time_end << endl;
}
// =============================================================================
...
int main(int argc, char* argv[]) {
	...
	simParams << " Job was submittet at date/time: " << asctime(timeinfo)
			<< endl;
	...
	AddSphDataToChSystem(mphysicalSystem, startIndexSph, posRadH, velMasH,
			paramsH, numObjects, fluidCollisionFamily, sphMarkerMass);
	...
	for (int tStep = 0; tStep < stepEnd + 1; tStep++) {
		...
		mphysicalSystem.Set_G_acc(
				ChVector<>(currentParamsH.gravity.x, currentParamsH.gravity.y,
						currentParamsH.gravity.z));
		...
		thrust::host_vector<short int> numContactsOnAllSph(
				numObjects.numFluidMarkers);  // numObjects.numFluidMarkers
...
// ****************** RK2: 1/2
		...
		ForceSPH(posRadD, velMasD, vel_XSPH_D, rhoPresMuD, bodyIndexD,
				...
				bceType, 0.5 * currentParamsH.dT);

		fsi_timer.stop("force_sph");

		CopyForceSphToChSystem(mphysicalSystem, numObjects, startIndexSph,
				derivVelRhoD, numContactsOnAllSph, sphMarkerMass);
#endif

		fsi_timer.start("stepDynamic_mbd");

		mTime += 0.5 * currentParamsH.dT;
		DoStepChronoSystem(mphysicalSystem, 0.5 * currentParamsH.dT, mTime); // Keep only this if you are just interested in the rigid sys

		fsi_timer.stop("stepDynamic_mbd");

#if haveFluid
		CountNumContactsPerSph(numContactsOnAllSph, mphysicalSystem, numObjects,
				startIndexSph);

		UpdateFluid(posRadD2, velMasD2, vel_XSPH_D, rhoPresMuD2, derivVelRhoD,
				referenceArray, 0.5 * currentParamsH.dT);
		ApplyBoundarySPH_Markers(posRadD2, rhoPresMuD2,
				numObjects.numAllMarkers);

		CopyD2HPosVel(posRadH2, velMasH2, posRadD2, velMasD2);
		CopyCustomChSystemPosVel2HostThrust(posRadH2, velMasH2, mphysicalSystem,
				numObjects, startIndexSph, numContactsOnAllSph);
		...
		DoStepChronoSystem(mphysicalSystem, 0.5 * currentParamsH.dT, mTime); // Keep only this if you are just interested in the rigid sys
#if haveFluid
		// note: you don't set numContactsOnAllSph to zero. That's the beauty of it. you dont update fluid markers who have
		// had contacts in either of the middle steps
		CountNumContactsPerSph(numContactsOnAllSph, mphysicalSystem, numObjects,
				startIndexSph);
		...
		CopyCustomChSystemPosVel2HostThrust(posRadH, velMasH, mphysicalSystem,
				numObjects, startIndexSph, numContactsOnAllSph);
