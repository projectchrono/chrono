...
ANCFshell<N>::ANCFshell() {
    ...
    auto vis_surf = chrono::make_shared<ChVisualizationFEAmesh>(*mesh);
    ...
    auto vis_node = chrono::make_shared<ChVisualizationFEAmesh>(*mesh);
    ...
    auto nodeA = chrono::make_shared<ChNodeFEAxyzD>(ChVector<>(0, 0, -width / 2), dir);
    auto nodeB = chrono::make_shared<ChNodeFEAxyzD>(ChVector<>(0, 0, +width / 2), dir);
    nodeA->SetFixed(true);
    nodeB->SetFixed(true);
    mesh->AddNode(nodeA);
    mesh->AddNode(nodeB);
    ...
    for (int i = 1; i < N; i++) {
        auto nodeC = chrono::make_shared<ChNodeFEAxyzD>(ChVector<>(i * dx, 0, -width / 2), dir);
        auto nodeD = chrono::make_shared<ChNodeFEAxyzD>(ChVector<>(i * dx, 0, +width / 2), dir);
        mesh->AddNode(nodeC);
        mesh->AddNode(nodeD);
