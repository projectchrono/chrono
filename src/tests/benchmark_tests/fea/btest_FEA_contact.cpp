...
FEAcontactTest::FEAcontactTest() {
    ...
    auto cmat = chrono::make_shared<ChMaterialSurfaceSMC>();
...
void FEAcontactTest::CreateFloor(std::shared_ptr<ChMaterialSurfaceSMC> cmat) {
    auto mfloor = chrono::make_shared<ChBodyEasyBox>(2, 0.1, 2, 2700, true);
    mfloor->SetBodyFixed(true);
    mfloor->SetMaterialSurface(cmat);
    m_system->Add(mfloor);

    auto masset_texture = chrono::make_shared<ChTexture>();
...
void FEAcontactTest::CreateBeams(std::shared_ptr<ChMaterialSurfaceSMC> cmat) {
    auto mesh = chrono::make_shared<ChMesh>();
    m_system->Add(mesh);

    auto emat = chrono::make_shared<ChContinuumElastic>();
    emat->Set_E(0.01e9);
    emat->Set_v(0.3);
    ...
    auto surf = chrono::make_shared<ChContactSurfaceMesh>();
    mesh->AddContactSurface(surf);
    surf->SetMaterialSurface(cmat);
    surf->AddFacesFromBoundary(0.002);

    auto vis_speed = chrono::make_shared<ChVisualizationFEAmesh>(*(mesh.get()));
...
void FEAcontactTest::CreateCables(std::shared_ptr<ChMaterialSurfaceSMC> cmat) {
    ...
    auto section = chrono::make_shared<ChBeamSectionCable>();
    ...
    auto cloud = chrono::make_shared<ChContactSurfaceNodeCloud>();
    ...
    auto vis_nodes = chrono::make_shared<ChVisualizationFEAmesh>(*(mesh.get()));
