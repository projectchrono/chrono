...
ChainTest<N>::ChainTest() : m_length(0.25), m_step(1e-3) {
    ...
    // Create ground
    auto ground = chrono::make_shared<ChBody>(contact_method);
    ...
    for (int ib = 0; ib < N; ib++) {
        ...
        auto pend = chrono::make_shared<ChBodyEasyBox>(m_length, width, width, density, false, true, contact_method);
        pend->SetPos(ChVector<>((ib + 0.5) * m_length, 0, 0));
        pend->AddAsset(chrono::make_shared<ChColorAsset>(0.5f * (ib % 2), 0.0f, 0.5f * (ib % 2 - 1)));
        m_system->AddBody(pend);

        auto rev = chrono::make_shared<ChLinkLockRevolute>();
