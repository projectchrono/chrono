...
int main(int argc, char* argv[]) {
    ...
    sys.Add(mirror_left);
    mirror_left->GetVisualModel()->GetShapeInstances()[0].shape->AddMaterial(reflective_color);
    ...
    mirror_right->GetVisualModel()->GetShapeInstances()[0].shape->AddMaterial(reflective_color);

    auto ball = chrono_types::make_shared<ChBodyEasySphere>(.25, 1000, true, false);
    ball->SetPos({4, 0, 0});
    ball->SetFixed(true);
    sys.Add(ball);
    ball->GetVisualModel()->GetShapeInstances()[0].shape->AddMaterial(red_color);
