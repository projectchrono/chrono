...
ChLcpVariablesGenericDiagonalMass& ChLcpVariablesGenericDiagonalMass::operator=(const ChLcpVariablesGenericDiagonalMass& other)
{
...
void ChLcpVariablesGenericDiagonalMass::Compute_inc_invMb_v(ChMatrix<float>& result, const ChMatrix<float>& vect) const
{
    assert(result.GetRows() == vect.GetRows());
    assert(vect.GetRows() == Get_ndof());
    for (int i = 0; i< vect.GetRows(); ++i)
...
void ChLcpVariablesGenericDiagonalMass::Compute_inc_invMb_v(ChMatrix<double>& result, const ChMatrix<double>& vect) const
{
    assert(result.GetRows() == vect.GetRows());
    assert(vect.GetRows() == Get_ndof());
    for (int i = 0; i< vect.GetRows(); ++i)
