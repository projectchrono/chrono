...
class ChApiPeridynamics ChMatterPeriBB : public ChMatterPeri<ChMatterDataPerNode, ChMatterDataPerBondBB> {
  public:
    ...
    virtual void ComputeForces() {
        ...
        for (auto& bond : this->bonds) {
            ...
            if (!mbond.broken) {
                ...
                double c_gmuller = 0.5 * ((18.0 * k_bulk / mbond.nodeA->vol_accumulator) +
                                          (18.0 * k_bulk / mbond.nodeB->vol_accumulator));
    ...
    void SetupInitial() override {
        ...
        Peridynamics"]
        // loop on nodes for resetting volume accumulator   // note! could be necessary to update in runtime if bonds
        are deleted/fractured/.. for (auto& node : this->nodes) { node.first->vol_accumulator = node.first->volume;
        }
        // loop on bonds for sum of connected volumes to nodes   // note! could be necessary to update in runtime if
        bonds are deleted/fractured/.. for (auto& bond : this->bonds) { ChMatterDataPerBondBB& mbond = bond.second; if
        (!mbond.broken) { mbond.nodeA->vol_accumulator += mbond.nodeB->volume; //*VolumeCorrection(old_sdist, horizon,
        vol_size); // vol corr. not relevant in Ganzenmueller? mbond.nodeB->vol_accumulator += mbond.nodeA->volume;
