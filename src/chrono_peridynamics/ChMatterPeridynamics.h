...
class ChMatterPeri : public ChMatterPeriBase {
  ...
  public:
    ...
    ChMatterPeri() { matsurface = chrono_types::make_shared<ChContactMaterialNSC>(); }
    ...
    const std::unordered_map<ChNodePeri*, T_per_node>& GetMapOfNodes() { return nodes; }
    ...
    virtual bool AddProximity(ChNodePeri* nodeA, ChNodePeri* nodeB) override {
        ...
        if (!(anodeA->is_requiring_bonds && anodeB->is_requiring_bonds) && anodeA->is_boundary && anodeB->is_boundary &&
            (!anodeA->is_fluid) && (!anodeB->is_fluid))
            return false;
