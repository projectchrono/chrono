...
ChBodyEasyCascade::ChBodyEasyCascade(TopoDS_Shape& shape,
                                     double density,
...
ChBodyEasyCascade::ChBodyEasyCascade(const std::string& shape_name,
...
void ChBodyEasyCascade::Init(TopoDS_Shape& shape,
                             double density,
                             std::shared_ptr<ChCascadeTriangulate> vis_params,
                             bool create_collision,
                             std::shared_ptr<ChContactMaterial> mat) {
    ChFramed* user_ref_to_abs = nullptr;  // as parameter?
    ...
    topods_shape = shape;
    topods_shape.Location(TopLoc_Location());
...
ChBodyEasyCascadeProfile::ChBodyEasyCascadeProfile(std::vector<std::shared_ptr<::chrono::ChLinePath>> wires,
...
void ChBodyEasyCascadeProfile::AddProfile(std::vector<std::shared_ptr<::chrono::ChLinePath>> wires,
                                          std::vector<std::shared_ptr<::chrono::ChLinePath>> holes,
                                          double thickness,
                                          double density,
                                          std::shared_ptr<ChCascadeTriangulate> vis_params,
...
void ChBodyEasyCascadeProfile::ClearProfiles() {
    faces.clear();
    UpdateCollisionAndVisualizationShapes();
}

void ChBodyEasyCascadeProfile::UpdateCollisionAndVisualizationShapes() {
    ...
    for (const auto& face : faces) {
        ...
        double vol;
        double mass;
        ChCascadeDoc::GetVolumeProperties(prism, face.density, cog, inertiaXX, inertiaXY, vol, mass);
    ...
    if (!user_ref_to_abs) {
        ...
        ChCascadeDoc::ConvertFrameCascadeToChrono(loc_shape_to_abs, frame_ref_to_abs);
    } else {
        frame_ref_to_abs = *user_ref_to_abs;
    }

    // Reset shape location to local ref csys (identity).
    topods_shape = mcompound;
    topods_shape.Location(TopLoc_Location());

    // Set the total mass and total inertia

    SetMass(inertia_composer.GetMass());
    ...
    SetInertiaXX(m_inertiaXX);
    SetInertiaXY(m_inertiaXY);

    // SetFrameRefToAbs(frame_ref_to_abs); //not needed
...
TopoDS_Wire ChBodyEasyCascadeProfile::FromChronoPathToCascadeWire(std::shared_ptr<::chrono::ChLinePath> profile) const {
        ...
        if (auto msegment = std::dynamic_pointer_cast<::chrono::ChLineSegment>(profile->GetSubLineN(i))) {
            if (msegment->pA.z() != msegment->pB.z())
                throw std::runtime_error(
                    "Error! ChBodyEasyCascadeProfile: sub segment of ChLinePath not parallel to XY plane!");
            ...
            if ((marc->origin.rot.e1() != 0) || (marc->origin.rot.e2() != 0))
                throw std::runtime_error(
                    "Error! ChBodyEasyCascadeProfile: a sub arc of ChLinePath not parallel to XY plane!");
            ...
            throw std::runtime_error(
                "Error! ChBodyEasyCascadeProfile: ChLinePath must contain only ChLineArc and/or ChLineSegment.");
        }
    }

    if (!mwirebuilder.IsDone()) {
        throw std::runtime_error("Error! ChBodyEasyCascadeProfile: profile is not closed.");
    }
