        ...
        while (getline(fin, line)) {
            ...
            if (parse_tet) {
                ...
                if (my_material.IsType<ChContinuumElastic>()) {
                    ...
                    mel->SetNodes(this->GetNode(nodes_offset + n1 - 1).DynamicCastTo<ChNodeFEAxyz>(),
                    ...
                    mel->SetNodes(this->GetNode(nodes_offset + n1 - 1).DynamicCastTo<ChNodeFEAxyzP>(),
...
void ChMesh::LoadFromAbaqusFile(const char* filename,
    ...
    while (getline(fin, line)) {
        ...
        if (e_parse_section == E_PARSE_TETS_10) {
            ...
            if (my_material.IsType<ChContinuumElastic>()) {
                ChSharedPtr<ChElementTetra_4> mel(new ChElementTetra_4);
                mel->SetNodes(this->GetNode(nodes_offset + tokenvals[1] - 1).DynamicCastTo<ChNodeFEAxyz>(),
                ...
                ChSharedPtr<ChElementTetra_4_P> mel(new ChElementTetra_4_P);
                mel->SetNodes(this->GetNode(nodes_offset + tokenvals[1] - 1).DynamicCastTo<ChNodeFEAxyzP>(),
...
void ChMesh::IntToLCP(const unsigned int off_v,  ///< offset in v, R
...
void ChMesh::IntFromLCP(const unsigned int off_v,  ///< offset in v
                        ChStateDelta& v,
