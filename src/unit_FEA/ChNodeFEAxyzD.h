...
class ChNodeFEAxyzD : public ChNodeFEAbase
  ...
  public:
    ChNodeFEAxyzD(ChVector<> initial_pos = VNULL, ChVector<> initial_dir = VECT_X) {
        ...
        variables->GetMassDiagonal().FillElem(
            0.0);  // default: no atomic mass associated to fea node, the fea element will add mass matrix
    }

    ~ChNodeFEAxyzD(){};

    ChNodeFEAxyzD(const ChNodeFEAxyzD& other) : ChNodeFEAbase(other) {
        this->X0 = other.X0;
        this->pos = other.pos;
