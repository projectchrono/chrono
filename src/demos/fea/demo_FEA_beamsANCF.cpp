...
void IGA_test(ChSystem& mysys, float beam_tip_load, double ANCF_res, int nsections = 32, int order = 2) {
	...
	double analytic_timoshenko_displ = (beam_tip_load * pow(beamL, 3)) / (3 * melasticity->GetYoungModulus()* (1. / 12.)*beam_wz*pow(beam_wy, 3))
		+ (beam_tip_load * beamL) / (Ks_y*melasticity->GetGshearModulus()*beam_wz*beam_wy); // = (P*L^3)/(3*E*I) + (P*L)/(k*A*G)
	...
	GetLog() << "\n LINEAR STATIC ANCF cantilever, nsections= " << nsections
		<< "  rel.error=  "
		<< fabs((ANCF_res - analytic_timoshenko_displ) / analytic_timoshenko_displ)
		<< "\n";
