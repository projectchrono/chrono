...
int main(int argc, char* argv[]) {
    ...
    int method = 3;
    std::cout << "Options:\n";
    std::cout << "  1. Linear static analysis" << std::endl;
    std::cout << "  2. Non-linear static analysis" << std::endl;
    std::cout << "  3. Non-linear static incremental analysis [DEFAULT]" << std::endl;
    std::cout << "\nSelect method: ";
    std::getline(std::cin, input);
    if (!input.empty()) {
        std::istringstream stream(input);
        stream >> method;
        ChClampValue(method, 1, 3);
    }
    ...
    // Create a beam from a point to another
    ...
    // Use BuildBeam for creating another beam
    // This time, use one node (the last node created by the last beam) and one point
    ...
    // Visualization of the FEM meshes
    // -------------------------------
    ...
    */
    ...
    switch (method) {
        ...
        case 3: {
            ...
            // Instead of using sys.DoStaticNonLinear(), which is quite basic, we will use
            // ChStaticNonLinearAnalysisIncremental. This requires a custom callback for incrementing the external
            // loads:
            ...
            class MyCallback : public ChStaticNonLinearAnalysisIncremental::LoadIncrementCallback {
              public:
                ...
                virtual void OnLoadScaling(
                    const double load_scaling,                      // ranging from 0 to 1
