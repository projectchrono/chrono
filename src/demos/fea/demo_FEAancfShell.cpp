...
int main(int argc, char* argv[]) {
  ...
  application.AddTypicalCamera(
      core::vector3df(-0.4, -0.3, 0),  // camera location
      core::vector3df(0, 0.5, -0.1));  // "look at" location
  GetLog() << "-----------------------------------------------------------\n";
  GetLog() << "-----------------------------------------------------------\n";
  ...
  for (int i = 0; i < MaxLayNum; i++) {
    ...
    for (int j = 0; j < NumLayer(i, 0); j++) {
      ...
      if (MaxMNUM < MNUM(i, j)) MaxMNUM = MNUM(i, j);
    }
  }
  ...
  while (i < TotalNumNodes) {
    ChSharedPtr<ChNodeFEAxyzD> node(new ChNodeFEAxyzD(
        ChVector<>(COORDFlex(i, 0), COORDFlex(i, 1), COORDFlex(i, 2)),
  ...
  while (elemcount < TotalNumElements) {
    ...
    for (int j = 0; j < NumLayer(LayNum(i, 0) - 1, 0); j++) {
      ...
      InertFlexVec(ij + 3) =
          LayPROP[LayNum(i, 0) - 1][j][0];  // Thickness per layer
    ...
    element->SetNodes(
        my_mesh->GetNode(NumNodes[elemcount][0]).DynamicCastTo<ChNodeFEAxyzD>(),
        ...
        my_mesh->GetNode(NumNodes[elemcount][3])
    ...
    ChMatrixNM<double, 35, 1>
        StockAlpha_EAS;  // StockAlpha(5*7,1): Max #Layer is 7
  ...
  my_system.SetLcpSolverType(
      ChSystem::LCP_ITERATIVE_MINRES);  // <- NEEDED because other solvers can't
