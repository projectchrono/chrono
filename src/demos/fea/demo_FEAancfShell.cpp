...
// Create the Irrlicht visualization (open the Irrlicht device,
  ...
  //  ChIrrApp application(&my_system, L"ANCF Shells", core::dimension2d<u32>(800, 600), false, true);
    ...
    //application.AddTypicalCamera(core::vector3df(0.5f, 1.f, 0.f));
    ...
    GetLog() << "\tPlate_lenght_x = " << plate_lenght_x << " Plate_lenght_y =" << plate_lenght_y << "\n";
    GetLog() << "\tMesh properties: Structured uniform mesh in each direction \n";
    GetLog() << "\tNumber of devision in x = " << numDiv_x << " Number of devision in y =" << numDiv_y << "\n";
    GetLog() << "----------------------------------------------------------------------------------------------\n";
    ...
    GetLog() << "\tCreating the mesh based on number of nodes in each direction ...\n";
    ...
    for (int i = 0; i < TotalNumElements; i++) {
        ...
        //  ElemLengthXY(i,2)=plate_lenght_z/numDiv_z;
            ...
            GetLog() << "\tNodes of element " << i << " are  \t " << NumNodes(i,0) << "\t" << NumNodes(i,1) << "\t"
                     << NumNodes(i,2) << "\t" << NumNodes(i,3) << "\n\t\t\n";
    ...
    //!-----------------------------------------------------------------------------------------------------------!
    GetLog() << "\tDefining initial coordinates and boundary conditions ... \n";
    ...
    for (int i = 0; i < TotalNumNodes; i++) {
        ...
        NDR(i,5) = (i % (numDiv_x + 1) == 0) ? 1 : 0;
            ...
            GetLog() << "\t Position constrains\t" << NDR(i,0) << "\t" << NDR(i,1) << "\t" << NDR(i,2)
                     << "\n \t Derivative constrains \t" << NDR(i,3) << "\t" << NDR(i,4) << "\t" << NDR(i,5) << "\n";
        ...
        COORDFlex(i,4) = 0;
        COORDFlex(i,5) = 1;
            GetLog() << " \t Positions \t\t" << COORDFlex(i,0) << "\t" << COORDFlex(i,1) << "\t" << COORDFlex(i,2)
                     << "\n \t Slope \t\t\t" << COORDFlex(i,3) << "\t" << COORDFlex(i,4) << "\t" << COORDFlex(i,5)
                     << "\n";
        ...
        VELCYFlex(i,4) = 0;
        VELCYFlex(i,5) = 0;
            GetLog() << "\t Velocities\t\t" << VELCYFlex(i,0) << "\t" << VELCYFlex(i,1) << "\t" << VELCYFlex(i,2)
                     << "\n\t Slopes V\t\t" << VELCYFlex(i,3) << "\t" << VELCYFlex(i,4) << "\t" << VELCYFlex(i,5)
                     << "\n\n";
    }
