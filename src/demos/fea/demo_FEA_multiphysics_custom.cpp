...
int main(int argc, char* argv[]) {
    ...
    // In Example 1 we already introduced all classes that would allow us the simulation of Fick
    ...
    class ChFieldElementChemicalEdge : public ChFieldElementLine {
      public:
        ...
        ChFieldElementChemicalEdge(std::shared_ptr<ChNodeFEAfieldXYZ> nodeA, std::shared_ptr<ChNodeFEAfieldXYZ> nodeB) {
    ...
    class ChMaterialChemicalDiffusion : public ChMaterial {
      public:
        ...
        /// your custom class with additional states/properties per-point. For the moment return nullptr.
    ...
    class ExtractConcentration : public ChVisualDataExtractorScalar<ExtractConcentration,              // (just repeat the class name here - used to generate clone() automatically)
    ...
    {
        virtual double ExtractImpl(const ChFieldDataChemicalConcentration* fdata) const override { return const_cast<ChFieldDataChemicalConcentration*>(fdata)->Concentration(); }
    };

    class ExtractMolarFlux : public ChVisualDataExtractorScalar<ExtractMolarFlux, ChFieldDataAuxiliaryChemical1D, DataAtMaterialpoint> {
        virtual double ExtractImpl(const ChFieldDataAuxiliaryChemical1D* fdata) const override { return fdata->molar_flux; }
    };
    ...
    if (true) {
        ...
        // The web of chemical channels has a bunch of ChNodeFEAfieldXYZ as nodes.
    ...
    // - override ComputeMolarFlux1D, the constitutive law: return a "capped chemical flow", and flow in excess causes step damage
    ...
    class ExtractChemicalDamage : public ChVisualDataExtractorScalar<ExtractChemicalDamage, ChFieldDataChemicalDamage, DataAtMaterialpoint> {
        virtual double ExtractImpl(const ChFieldDataChemicalDamage* fdata) const override { return fdata->damage; }
    };
    ...
    if (true) {
        ...
        auto sine_concentr = chrono_types::make_shared<ChFunctionSine>(1.5, 0.02, 0.0, 1.5);  // amplitude, freq, phase, shift
        auto constraint_c2 = chrono_types::make_shared<ChLinkField>();
        constraint_c2->Initialize(node2, chemical_field);
        constraint_c2->SetOffset(sine_concentr);
        sys.Add(constraint_c2);
