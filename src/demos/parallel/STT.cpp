...
bool cylinder = true;
 ...
 std::string out_dir = out_dir2 + "/3_SET_dt1e-4_iter700_min/DEM-C/MAIN_crs_1/SET_06"; // tu budziet: out_dir2 + argv[1500] + "/MAIN"
...
//	ChBody::ContactMethod contact_method = ChBody::DVI;
	...
	L7_0 = sqrt(wall_7->GetPos().x*wall_7->GetPos().x + wall_7->GetPos().y*wall_7->GetPos().y) - thickness / 2;
	L8_0 = sqrt(wall_8->GetPos().x*wall_8->GetPos().x + wall_8->GetPos().y*wall_8->GetPos().y) - thickness / 2;
	L9_0 = sqrt(wall_9->GetPos().x*wall_9->GetPos().x + wall_9->GetPos().y*wall_9->GetPos().y) - thickness / 2;
	L10_0 = sqrt(wall_10->GetPos().x*wall_10->GetPos().x + wall_10->GetPos().y*wall_10->GetPos().y) - thickness / 2;
	L11_0 = sqrt(wall_11->GetPos().x*wall_11->GetPos().x + wall_11->GetPos().y*wall_11->GetPos().y) - thickness / 2;
	L12_0 = sqrt(wall_12->GetPos().x*wall_12->GetPos().x + wall_12->GetPos().y*wall_12->GetPos().y) - thickness / 2;
	L13_0 = sqrt(wall_13->GetPos().x*wall_13->GetPos().x + wall_13->GetPos().y*wall_13->GetPos().y) - thickness / 2;
	L14_0 = sqrt(wall_14->GetPos().x*wall_14->GetPos().x + wall_14->GetPos().y*wall_14->GetPos().y) - thickness / 2;
	...
	while (my_system->GetChTime() < simulation_time) {
		...
		if (my_system->GetChTime() >= data_out_frame * data_out_step) {
			...
			double force_7 = sqrt(force8.x*force8.x + force8.y*force8.y) + sqrt(force7.x*force7.x + force7.y*force7.y);
			double force_9 = sqrt(force10.x*force10.x + force10.y*force10.y) + sqrt(force9.x*force9.x + force9.y*force9.y);
			double force_11 = sqrt(force12.x*force12.x + force12.y*force12.y) + sqrt(force11.x*force11.x + force11.y*force11.y);
			double force_13 = sqrt(force14.x*force14.x + force14.y*force14.y) + sqrt(force13.x*force13.x + force13.y*force13.y);

			double force_r = (force_x + force_y + force_7 + force_9 + force_11 + force_13); // total force!!! do not divide by 12 to have the mean one
			double sig_r = force_r / (Lz * CH_C_PI * diam); // divide total force by total side area !!! as simple as that 

//			double force_r = (force_x + force_y);
//			double sig_r = force_r / (Lz * CH_C_PI * diam / 3);

			catStream << eps_ax << "\t" << eps_r << "\t" << eps_vol << "\t" 
