...
enum SpecimenDistrib {
    UNIFORM_BEADS_5mm,
    MONO_SPHERES_1mm,
    TRI_SPHERES_4mm_5mm_6mm,
    OTTAWA_SAND,
    MONTEREY_SAND,
    CAT_LIMESTONE,
    CAT_LIMESTONE_2,
...
int main(int argc, char* argv[]) {
    ...
    switch (material) {
        case GLASS:
            for (i = 0; i < numParticleTypes; i++) {
                ...
                particleMass[i] = particleDensity[i] * CH_C_PI * particleDiameter[i] * particleDiameter[i] *
                                  particleDiameter[i] / 6.0;
    ...
    for (i = 0; i < numParticleTypes; i++) {
        if (particleDiameter[i] > max_diameter)
            max_diameter = particleDiameter[i];
        if (particleDiameter[i] < min_diameter)
            min_diameter = particleDiameter[i];
        if (particleMass[i] > max_mass)
            max_mass = particleMass[i];
        if (particleMass[i] < min_mass)
            min_mass = particleMass[i];
        if (Y[i] > Y_max)
            Y_max = Y[i];
        if (Y[i] < Y_min)
            Y_min = Y[i];
    ...
    switch (geom) {
        ...
        case TRIAXIAL_CLAY:
            ...
            nu_ext = 0.3f;
            COR_ext = 0.2f;
            mu_ext = 0.4f;  // particle-on-wall friction // 0.1 more than particle-on-particle friction (it's just my
                            // guess...)
            break;
    ...
    wall_7->SetPos(ChVector<>(-(Lx0 / 2 + thickness / 2) * cos(CH_C_PI / 6),
                              -(Ly0 / 2 + thickness / 2) * sin(CH_C_PI / 6), Lz0 / 2));
    wall_7->SetRot(z30);
    ...
    wall_9->SetPos(ChVector<>(-(Lx0 / 2 + thickness / 2) * cos(CH_C_PI / 3),
                              -(Ly0 / 2 + thickness / 2) * sin(CH_C_PI / 3), Lz0 / 2));
    wall_9->SetRot(z60);
    ...
    wall_11->SetPos(ChVector<>((Lx0 / 2 + thickness / 2) * sin(CH_C_PI / 6),
                               -(Ly0 / 2 + thickness / 2) * cos(CH_C_PI / 6), Lz0 / 2));
    wall_11->SetRot(z30);
    ...
    wall_13->SetPos(ChVector<>((Lx0 / 2 + thickness / 2) * sin(CH_C_PI / 3),
                               -(Ly0 / 2 + thickness / 2) * cos(CH_C_PI / 3), Lz0 / 2));
    wall_13->SetRot(z60);
    ...
    if (cylinder == true) {
        gen.createObjectsCylinderZ(utils::POISSON_DISK, max_diameter, ChVector<>(0, 0, Lz0 / 2),
                                   (Lx0 - max_diameter) / 2, (Lz0 - max_diameter) / 2);
    } else {
        gen.createObjectsBox(utils::POISSON_DISK, max_diameter, ChVector<>(0, 0, Lz0 / 2),
    ...
    if (cylinder == true) {
        stressStream << "time"
                     << "\t";
        stressStream << "strain_a"
                     << "\t"
                     << "strain_b"
                     << "\t";
        stressStream << "stress_a"
                     << "\t"
                     << "stress_b"
                     << "\n";
    } else {
    ...
    while (my_system->GetChTime() < simulation_time) {  // 2 * time_step){//
        ...
        //	//if (fabs(wall_2->GetPos_dt().z()) < prestress_tolerance_vel && fabs(wall_2->GetPos_dtdt().z()) <
        // prestress_tolerance_acc) { 	if (fabs(wall_2->GetPos_dt().z()) < prestress_tolerance_vel &&
        // fabs(wall_2->GetPos_dtdt().z()) < prestress_tolerance_acc && 		max_vel < prestress_tolerance_vel &&
        // max_acc
        //< prestress_tolerance_acc) { 		cout << "** simulation finished at " << my_system->GetChTime() << "\n";
        //		simulation_time = 0;
        //	}
        //}
    }
