...
enum PenetratorGeom {
	P_SPHERE, P_CONE1, P_CONE2, P_CONE1_SPHERETIP, P_CONE2_SPHERETIP
};
...
void CreateContainer(ChSystemParallel* msystem) {
	...
	utils::CreateCylindricalContainerFromBoxes(msystem, binId, mass_pilka, mat_c,
		ChVector<>(hDimX, hDimY, hDimZ), hThickness, 40,
		ChVector<>(0, 0, 0), ChQuaternion<>(1, 0, 0, 0), true, true, false,
		true, false);
}
...
int CreateObjects(ChSystemParallel* msystem, int mikstura) {
	...
	if (mikstura == 3){
		...
		m3->setDefaultSize(ChVector<>(r_g, (fiMult - 1.0) * 2 * r_g, -1.0)); // the third parameter does not matter in here. I set it to -1.0.
		m3->setDefaultMaterial(mat_g);
	}

	gen.setBodyIdentifier(Id_g);

	double hDist = multDist * r_g; // O.K. That parameter has correct value for generating: SPHERES, BISPHERES, ELLIPSOIDS_1D

	for (int i = 0; i < numLayers; i++) {
		double center = hDist + layerHeight / 2 + i * (2 * hDist + layerHeight);
		gen.createObjectsCylinderZ(utils::POISSON_DISK, 2 * hDist,
			ChVector<>(0, 0, center), hDimX - hDist, layerHeight / 2);
		cout << "Layer " << i << "  total bodies: " << gen.getTotalNumBodies()
			<< endl;
...
int CreateObjects2(ChSystemParallel* msystem) {
	...
	for (int i = 0; i < numLayers; i++) {
		...
		utils::Generator::PointVector points = sampler.SampleCylinderZ(ChVector<>(0, 0, center), hDimX - hDist, layerHeight / 2);
		CreateEllipsoids(msystem, points);
		cout << "Layer " << i << "  total bodies: " << points.size()
			<< endl;
...
void CreatePenetratorGeometry(std::shared_ptr<ChBody> obj) {
	...
	case P_CONE1_SPHERETIP:
		// apex angle = 30 de
		utils::AddConeGeometry(obj.get(), R_bc1, H_bc1);
		utils::AddSphereGeometry(obj.get(), r_tip,
			ChVector<>(0, .75 * H_bc1, 0));
		break;
	case P_CONE2_SPHERETIP:
		// apex angle = 60 de
		utils::AddConeGeometry(obj.get(), R_bc2, H_bc2);
		utils::AddSphereGeometry(obj.get(), r_tip,
			ChVector<>(0, .75 * H_bc2, 0));
		break;
	}
...
void FindSurfaceProfile(ChSystem* sys, int howManyLayers, double h_max, int pov_out_frame, int layerSize){
	...
	for (int lay_nr = 1; lay_nr <= howManyLayers; lay_nr++){
		...
		for (; itIdVector != idVector.end();
			++ii, ++itIdVector, ++itxVector, ++ityVector, ++itzVector) {

			surfaceProfileStream << ii << "\t" << *itIdVector << "\t" <<
				*itxVector << "\t" << *ityVector << "\t" << *itzVector << "\n";
		}
	}
...
double FindHighestPrintStats(ChSystem* sys) {
	...
	settingsFile << "  pile height 1: " << high1 << ", pile height 2: " << high2
		<< "\n";
	settingsFile << "  bulk density 1: " << eqDens1 << ", bulk density 2: "
		<< eqDens2 << "\n";
	settingsFile << "  packing ratio 1: " << eqDens1 / rho_g
		<< ", packing ratio 2: " << eqDens2 / rho_g << "\n";
	settingsFile.Flush();
	return high1;
}
...
void AdjustAcceleration(std::shared_ptr<ChBody> penetrator) {
	...
	penetrator->Accumulate_force(ChVector<>(0, 0, force_f),
		penetrator->GetPos(), false);
}
...
// set friction
// -----------------------------------------------------------------------------
void SetSystemMuCr(ChSystemParallel* sys, double newFriction,
	double newRestitution) {
	for (size_t i = 0; i < sys->Get_bodylist()->size(); i++) {
		auto body = (*sys->Get_bodylist())[i];
...
std::shared_ptr<ChBody> CreatePenetrator(ChSystemParallel* msystem) {
	...
	if (penetGeom == P_CONE1 || penetGeom == P_CONE2
...
void printProblemInfo(int pType){
	...
	settingsFile << "  density of penetrator: " << rho_b << "\n"
		...
		<< "\n" << "  problem type (0: settling, 1: dropping): "
		<< problemTypeInt << "\n" << "  dropping height: " << h << "\n"
		<< "\n\n";
	settingsFile.Flush();
...
// ========================================================
	...
	msystem->GetSettings()->solver.max_iteration_sliding =
		max_iteration_sliding;
	msystem->GetSettings()->solver.max_iteration_spinning =
		max_iteration_spinning;
	msystem->GetSettings()->solver.alpha = 0;
	msystem->GetSettings()->solver.contact_recovery_speed =
	...
	if (problem == DROPPING) {
		...
		cout << "  done.  Read " << msystem->Get_bodylist()->size()
			<< " bodies." << endl;
	...
	gl_window.SetCamera(ChVector<>(0, -10 * hDimY, hDimZ),
		ChVector<>(0, 0, hDimZ), ChVector<>(0, 0, 1), .02);
	...
	while (time <  time_end) {  
		...
		if (sim_frame == next_out_frame) {
			...
			for (size_t i = 0; i < msystem->Get_bodylist()->size(); ++i) {
				...
				if (body->GetIdentifier() > 0) {
					...
					double kinen_i = 0.5 * body->GetMass() * vel2 + rotEnergyComps.x + rotEnergyComps.y + rotEnergyComps.z;
			...
			maxvelStream << time << "\t" << max_vel << "\t" << max_acc << "\t" << 
				avg_kinen << "\t" << var_kinen << "\n";
				...
				//#endif
					...
					//std::vector<double> history = ((ChSolverParallel*)(*msystem->GetLcpSolverSpeed()))->GetViolationHistory();
		...
		if (problem == SETTLING && write_checkpts_data && msystem->GetChTime() >= checkpt_out_frame * checkpt_out_step) {
			char filename[100];
		...
		if (chrono::ChSystemParallel* parallel_sys = dynamic_cast<chrono::ChSystemParallel*>(msystem)) {
			...
			//printf("   %8.5f | %7.4f | %7.4f | %7.4f | %7.4f | %7.4f | %7d | %7d | %7d | %7.4f |\n", TIME, STEP, BROD, NARR,
			//	 SOLVER, UPDT, BODS, CNTC, REQ_ITS);
			//timing_outputStream << TIME << "\t" << STEP << "\t" << BROD << "\t" << NARR << "\t" <<
			//	SOLVER << "\t" << UPDT << "\t" << BODS << "\t" << CNTC << "\t" << REQ_ITS << "\t" <<
	...
	if (problem == SETTLING) {
		...
		cout << "  done.  Wrote " << msystem->Get_bodylist()->size()
			<< " bodies." << endl;
	}
