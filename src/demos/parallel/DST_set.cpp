...
ChVector<> plate_dim(plate_width / 2., plate_length / 2., plate_thickness / 2.); //Divided by two because the dimensions given in here are symmetric values 
...
#ifdef USE_DEMP
	auto ref_sphere = std::make_shared<ChBody>(new ChCollisionModelParallel,
...
void AddFixedFrame(ChSystemParallel* sys) {
...
#ifdef USE_DEMP
	auto fixed_frame = std::make_shared<ChBody>(new ChCollisionModelParallel,
		ChMaterialSurfaceBase::DEM);
#else
	auto fixed_frame = std::make_shared<ChBody>(new ChCollisionModelParallel);
#endif
	...
	ChVector<> sideFrameDim(side_frame_width / 2., side_frame_length / 2., frame_height / 2.); //Divided by two because the dimensions given in here are symmetric values 
...
#ifdef USE_DEMP
	auto moving_frame = std::make_shared<ChBody>(new ChCollisionModelParallel,
		ChMaterialSurfaceBase::DEM);
#else
	auto moving_frame = std::make_shared<ChBody>(new ChCollisionModelParallel);
#endif
...
#ifdef USE_DEMP
	auto klin = std::make_shared<ChBody>(new ChCollisionModelParallel,
		ChMaterialSurfaceBase::DEM);
#else
	auto klin = std::make_shared<ChBody>(new ChCollisionModelParallel);
#endif
		...
		for (int jj = 0; jj <= 37; jj++){ // 39
			...
			if (jj % 2 == 1){
				for (int ii = 0; ii <= 34; ii++){ //34
					float coord_x = -0.5 * (container_inner_width - 2 * radius_bead) + radius_bead + ii * 2 * radius_bead;
					ballId++;
				...
				for (int ii = 0; ii <= 33; ii++){ // 33
					float coord_x = -0.5 * (container_inner_width - 2 * radius_bead) + 2 * radius_bead + ii * 2 * radius_bead;
					ballId++;
					AddSphere(sys, ballId, coord_x, coord_y, coord_z);
				}
			}
