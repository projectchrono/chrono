...
void test_pendulum() {
    ...
    auto cyl = chrono_types::make_shared<ChCylinderShape>(0.05, length);
    cyl->SetColor(ChColor(0.7f, 0.8f, 0.8f));
    my_mass->AddVisualShape(cyl, ChFrame<>(VNULL, Q_from_AngY(CH_C_PI_2)));
...
void test_mooringline() {
    ...
    auto BuildMooringLine_body = [&](std::shared_ptr<ChBody> pA, std::shared_ptr<ChBody> pB, int N) {
        ...
        for (int i_body = 0; i_body < N; i_body++) {
            ...
            auto cyl_rev = chrono_types::make_shared<ChCylinderShape>(0.1, 0.6 * len);
