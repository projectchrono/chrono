...
int main(int argc, char* argv[])
{
  ...
  while (application.GetDevice()->run())
  {
    ...
    ChIrrTools::drawSpring(application.GetVideoDriver(), 0.05,
      spring_1->GetEndPoint1Abs(),
      spring_1->GetEndPoint2Abs(),
      video::SColor(255, 150, 20, 20), 80, 15, true);

    ChIrrTools::drawSpring(application.GetVideoDriver(), 0.05,
      spring_2->GetEndPoint1Abs(),
      spring_2->GetEndPoint2Abs(),
      video::SColor(255, 20, 20, 150), 80, 15, true);

    application.DoStep();

    if (frame % 50 == 0) {
      GetLog() << system.GetChTime() << "  "
        << spring_1->Get_SpringLength() << "  "
        << spring_1->Get_SpringVelocity() << "  "
        << spring_1->Get_SpringReact() << "\n";

      GetLog() << "            " 
        << spring_2->Get_SpringLength() << "  "
        << spring_2->Get_SpringVelocity() << "  "
