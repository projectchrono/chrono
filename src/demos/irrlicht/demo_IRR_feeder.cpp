...
void create_debris(ChVisualSystemIrrlicht& vis, ChSystem& sys, double dt, double particles_second) {
    ...
    for (int i = 0; i < particles_dt; i++) {
        ...
        rigidBody->SetPos(ChVector<>(
            ((particlelist.size() % 3)/3.0  -0.5 )* xnozzlesize , 
...
int main(int argc, char* argv[]) {
    ...
    // To this end, you just need to set a x,y,z,Rx,Ry,Rz "virtual" vibration mode for the bowl, respect to some reference.
    // Example. typical vibration bowls for helicoidal feeding are vibrating both torsionally and up-down, like turning a screw
    // left & right, so our example has a mode with both Y vertical displacement and rotation on Y axis:
    feeder->SetFeederVibration(
        ChFrame<>(ChVector<>(0,-0.1,0)), // this is the coordinate frame respect to whom we assume the virtual vibration
