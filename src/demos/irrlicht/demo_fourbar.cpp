...
int main(int argc, char* argv[])
{
    ...
    while(device->run())
    {
        ...
        // .. draw a small circle representing joint BC
        ChIrrTools::drawCircle(device->getVideoDriver(), 0.06, ChCoordsys<>(my_link_BC->GetMarker1()->GetAbsCoord().pos,
QUNIT));
        // .. draw a small circle representing joint CD
        ChIrrTools::drawCircle(device->getVideoDriver(), 0.06, ChCoordsys<>(my_link_CD->GetMarker1()->GetAbsCoord().pos,
QUNIT));
        // .. draw a small circle representing joint DA
        ChIrrTools::drawCircle(device->getVideoDriver(), 0.06, ChCoordsys<>(my_link_DA->GetMarker1()->GetAbsCoord().pos,
QUNIT));
        // .. draw the rod (from joint BC to joint CD)
