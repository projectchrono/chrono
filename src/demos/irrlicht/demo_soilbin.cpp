  ...
  public:
    ...
    const bool create_some_falling_items(double pSize, double pDev, int nParticles, int nBoxes = 0) {
        ...
        if ((msys->GetChTime() - this->simTime_lastPcreated) >= minTime_betweenCreate) {
            ...
            ChSharedPtr<ChTexture> cubeMap(new ChTexture());
            cubeMap->SetTextureFilename(GetChronoDataFile("concrete.jpg"));
            ChSharedPtr<ChTexture> rockMap(new ChTexture());
            ...
            for (int bi = 0; bi < nParticles; bi++) {
                ...
                auto currRigidBody = ChSharedPtr<ChBodyEasySphere>(new ChBodyEasySphere(sphrad, this->sphDens, true, true));
            ...
            for (int bi = 0; bi < nBoxes; bi++) {
                ...
                auto currRigidBody = ChSharedPtr<ChBodyEasyBox>(
                    new ChBodyEasyBox(pSize * xscale, pSize * yscale, pSize * zscale, this->boxDens, true, true));
                currRigidBody->SetPos(currPos);
                currRigidBody->SetRot(randrot);
...
class SoilbinWheel {
  public:
    ...
    SoilbinWheel(ChSystem* system, ChVector<> mposition, double mass, ChVector<>& inertia) {
        ...
        ChSharedPtr<ChTriangleMeshShape> tireMesh_asset(new ChTriangleMeshShape());
...
class TestMech {
  public:
    // data
    ChSharedPtr<ChBodyEasyBox> truss;       // spindle truss
    ChSharedPtr<ChBodyEasyBox> suspweight;  // suspended weight
    ChSharedPtr<ChBodyEasyBox> floor;
    ChSharedPtr<ChBodyEasyBox> wall1;
    ChSharedPtr<ChBodyEasyBox> wall2;
    ChSharedPtr<ChBodyEasyBox> wall3;
    ChSharedPtr<ChBodyEasyBox> wall4;
    ...
    TestMech(ChSystem* system,
        ...
        floor = ChSharedPtr<ChBodyEasyBox>(
            new ChBodyEasyBox(binWidth + wallWidth / 2.0, wallWidth, binLength + wallWidth / 2.0, 1.0, true, true));
        ...
        wall1 = ChSharedPtr<ChBodyEasyBox>(new ChBodyEasyBox(wallWidth, binHeight, binLength, 1.0, true, true));
        wall1->SetPos(ChVector<>(-binWidth / 2.0 - wallWidth / 2.0, 0, 0));
        wall1->SetBodyFixed(true);
        system->AddBody(wall1);

        wall2 = ChSharedPtr<ChBodyEasyBox>(new ChBodyEasyBox(wallWidth, binHeight, binLength, 1.0, true, false));
        wall2->SetPos(ChVector<>(binWidth / 2.0 + wallWidth / 2.0, 0, 0));
        wall2->SetBodyFixed(true);
        system->AddBody(wall2);

        wall3 = ChSharedPtr<ChBodyEasyBox>(
            new ChBodyEasyBox(binWidth + wallWidth / 2.0, binHeight, wallWidth, 1.0, true, false));
        wall3->SetPos(ChVector<>(0, 0, -binLength / 2.0 - wallWidth / 2.0));
        wall3->SetBodyFixed(true);
        system->AddBody(wall3);

        // wall 4
        wall4 = ChSharedPtr<ChBodyEasyBox>(
            new ChBodyEasyBox(binWidth + wallWidth / 2.0, binHeight, wallWidth, 1.0, true, true));
        ...
        // *****
        ChSharedPtr<ChTexture> bluMap(new ChTexture());
        bluMap->SetTextureFilename(GetChronoDataFile("blu.png"));
        ChVector<> trussCM = wheelBody->GetPos();

        truss = ChSharedPtr<ChBodyEasyBox>(new ChBodyEasyBox(0.2, 0.2, 0.4, 300.0, false, true));
        ...
        suspweight = ChSharedPtr<ChBodyEasyBox>(new ChBodyEasyBox(0.2, 0.4, 0.2, 5000.0, false, true));
