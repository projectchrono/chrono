...
int main(int argc, char* argv[]) {
    ...
    application.AddTypicalCamera(core::vector3df(0, 0, cable_length), core::vector3df(0, 0, 0));
    ...
    link_cable->Initialize(ball, ceiling, false, ChVector<>(0, 0, 0), ChVector<>(0, 0, cable_length));
    my_system.AddLink(link_cable);    
    linkvec.push_back(*link_cable);

    auto link_cable2 = chrono_types::make_shared<ChLinkDistance>();  // right, front, upper, 1
    link_cable2->Initialize(ball, ceiling, false, ChVector<>(0, 0, 0), ChVector<>(0, 0, -cable_length));
    my_system.AddLink(link_cable2);
    linkvec.push_back(*link_cable2);
