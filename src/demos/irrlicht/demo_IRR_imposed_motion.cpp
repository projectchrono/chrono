...
int main(int argc, char* argv[]) {
    ...
    auto mfloor = chrono_types::make_shared<ChBodyEasyBox>(3, 0.2, 3, 1000, false, false); // no collide, not shown
    ...
    auto mmoved_1 = chrono_types::make_shared<ChBodyEasyMesh>(GetChronoDataFile("support.obj"),1000); // mesh file, density
	...
	f_rot_axis->SetFunctionAngle(chrono_types::make_shared<ChFunction_Sine>(0,0.15,chrono::CH_C_PI)); // phase freq ampl
	...
	// Create the object to move 
    auto mmoved_2 = chrono_types::make_shared<ChBodyEasyMesh>(GetChronoDataFile("support.obj"),1000); // mesh file, density
    ...
    auto mspline = chrono_types::make_shared<ChLineBspline>(
		3,												// spline order
		std::vector<ChVector<>> {						// std::vector with ChVector<> controlpoints
	...
	// Create the object to move 
    auto mmoved_3 = chrono_types::make_shared<ChBodyEasyMesh>(GetChronoDataFile("support.obj"),1000); // mesh file, density
	...
	// Create the object to move 
    auto mmoved_4 = chrono_types::make_shared<ChBodyEasyMesh>(GetChronoDataFile("support.obj"),1000); // mesh file, density
    ...
    ChIrrApp application(&mphysicalSystem, L"Imposing rotation and position to bodies", core::dimension2d<u32>(800, 600), false, true);
