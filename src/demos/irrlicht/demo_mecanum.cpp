...
public:
	...
	bool OnEvent(const SEvent& event)
	{
		...
		if (event.EventType == irr::EET_KEY_INPUT_EVENT && !event.KeyInput.PressedDown)
		{
			...
			{
			case irr::KEY_KEY_Q: 
				STATIC_x_speed+=1.5; if (STATIC_x_speed>MAX_XZ_SPEED) STATIC_x_speed=MAX_XZ_SPEED;
				return true;
			case irr::KEY_KEY_W: 
				STATIC_x_speed-=1.5; if (STATIC_x_speed<-MAX_XZ_SPEED) STATIC_x_speed=-MAX_XZ_SPEED;
				return true;
			case irr::KEY_KEY_A: 
				STATIC_z_speed+=1.5; if (STATIC_z_speed>MAX_XZ_SPEED) STATIC_z_speed=MAX_XZ_SPEED;
				return true;
			case irr::KEY_KEY_Z: 
				STATIC_z_speed-=1.5; if (STATIC_z_speed<-MAX_XZ_SPEED) STATIC_z_speed=-MAX_XZ_SPEED; 
				return true;
			case irr::KEY_KEY_E: 
				STATIC_rot_speed+=0.05; if (STATIC_rot_speed>MAX_ROT_SPEED) STATIC_rot_speed=MAX_ROT_SPEED;
				return true;
			case irr::KEY_KEY_R: 
				STATIC_rot_speed-=0.05; if (STATIC_rot_speed<-MAX_ROT_SPEED) STATIC_rot_speed=-MAX_ROT_SPEED; 
				return true;
										   ...
										   double spindle_mass)
{
	...
	for (int iroller = 0; iroller < n_rollers; iroller++)
	{
		...
		ChFrameMoving<> f1( ChVector<>(0, 0, -(wheel_radius - roller_midradius)) , Q_from_AngAxis(roller_angle, ChVector<>(0,0,1) )  ); 
		...
		// (preconcatenate rotation 90°on X, to set axis of revolute joint)
		ChFrameMoving<> fr( ChVector<>(0, 0, 0) , Q_from_AngAxis(CH_C_PI/2.0, ChVector<>(1,0,0) )  ); 
		ChFrameMoving<> frabs = fr >> f3;
		ChSharedPtr<ChLinkLockRevolute>  my_link_roller(new ChLinkLockRevolute);
...
int main(int argc, char* argv[])
{
	...
	ChIrrAppInterface application(&mphysicalSystem, L"Mecanum robot simulator",core::dimension2d<u32>(800,600),false); 

	// create text with info
	IGUIStaticText* textFPS = application.GetIGUIEnvironment()->addStaticText(L"Use keys Q,W, A,Z, E,R to move the robot", rect<s32>(150,10,430,40), true);
	...
	ChBodySceneNode* spindle_A = create_mecanum_wheel(mphysicalSystem, application.GetSceneManager(), application.GetVideoDriver(),
	...
	ChBodySceneNode* spindle_B = create_mecanum_wheel(mphysicalSystem, application.GetSceneManager(), application.GetVideoDriver(),
							...
							0.1,		// mass of single roller
							0.2);		// mass of the spindle
	ChSharedPtr<ChLinkEngine>  my_link_shaftB(new ChLinkEngine);
	 my_link_shaftB->Initialize( spindle_B->GetBody(), 
								 mTrussPlatform->GetBody(), 
							     (f1 >> f2_wB).GetCoord());
	 my_link_shaftB->Set_eng_mode(ChLinkEngine::ENG_MODE_SPEED);
	 my_link_shaftB->Get_spe_funct()->Set_yconst(0.0);
	 mphysicalSystem.AddLink(my_link_shaftB);



	ChBodySceneNode* spindle_C = create_mecanum_wheel(mphysicalSystem, application.GetSceneManager(), application.GetVideoDriver(),
	...
	ChSharedPtr<ChLinkEngine>  my_link_shaftC(new ChLinkEngine);
	 ...
	 my_link_shaftC->Get_spe_funct()->Set_yconst(0.0); 
	...
	while(application.GetDevice()->run()) 
	{
		...
		my_link_shaftA->Get_spe_funct()->Set_yconst(wheel_A_rotspeed);
		my_link_shaftB->Get_spe_funct()->Set_yconst(wheel_B_rotspeed);
		my_link_shaftC->Get_spe_funct()->Set_yconst(wheel_C_rotspeed);
