...
///////////////////////////////////////////////////
//
//  Summary: A LoopChain derived system, with lots of extra 
//            idlers and rolling elements
//        
...
#include "subsys/trackVehicle/LoopChain.h"
#include "ModelDefs.h"
...
bool do_shadows = false; // shadow map is experimental
  ...
  const std::string out_dir = "../test_LoopChain";
...
int main(int argc, char* argv[])
{
  ...
  // --------------------------
  // Create Chain system (drive gear, idler, chain)
  // Inherits ChSystem.
  // Specify the visualization and collision type .
  LoopChain chainLoopSystem("Justins LoopChain system", 
  ...
  chainLoopSystem.Initialize(ChCoordsys<>(initLoc, initRot));

  // if writing an output file, setup what debugInformation we want added each step data is saved.
#ifdef WRITE_OUTPUT
  chainLoopSystem.Setup_log_to_file(what_to_save,
    "test_LoopChain_all.csv");
#endif
...
*/
  ...
  ChIrrApp application(chainLoopSystem.GetSystem(),
                      L"test LoopChain demo",
  ...
  // the GUI driver
  ChIrrGuiTrack driver(application, chainLoopSystem, trackPoint, chaseDist, chaseHeight,window_x_len-150);
...
*/
  ...
  chainLoopSystem.GetSystem()->ShowHierarchy(GetLog() );
#endif
...
// write data to file?
#ifdef WRITE_OUTPUT
      chainLoopSystem.Log_to_file();  // needs to already be setup before sim loop calls it
#endif
  ...
  while (application.GetDevice()->run())
    ...
    // Update
    time = chainLoopSystem.GetSystem()->GetChTime();

    driver.Update(time);

    chainLoopSystem.Update(time, throttle_input[0], braking_input[0]);
    ...
    if( !application.GetPaused() )
      chainLoopSystem.Advance(step_size);
    ...
    if (step_number % output_steps == 0) 
    {
     // log desired output to console?
      ...
      chainLoopSystem.Log_to_console(what_to_console);
#endif
      ...
      GetLog() << "\n total compute time: " << total_step_time << ", Avg. time per step " << time_since_last_output * chainLoopSystem.GetSystem()->GetStep() / output_step_size;
...
/*
  ...
  chainLoopSystem.ExportMeshPovray(out_dir);
  ...
  while (time < tend)
  {
    if (step_number % render_steps == 0) {
      ...
      utils::WriteShapesPovray(chainLoopSystem.GetSystem(), filename);
    ...
    chainLoopSystem.Update(time, throttle_input, braking_input);
