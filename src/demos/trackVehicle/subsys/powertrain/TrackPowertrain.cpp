...
// -----------------------------------------------------------------------------
TrackPowertrain::TrackPowertrain(const std::string& name, const ChVector<>& dir_motor_block)
    : m_drive_mode(NEUTRAL), m_dir_motor_block(dir_motor_block), m_last_time_gearshift(0), m_gear_shift_latency(0.5) {
...
void TrackPowertrain::Initialize(ChSharedPtr<ChBody> chassis, ChSharedPtr<ChShaft> driveshaft) {
    assert(chassis);
    assert(driveshaft);
...
void TrackPowertrain::Update(double time, double throttle, double shaft_speed) {
