...
const ChVector<> LoopChain::m_idlerPos(-2.1904, -0.1443, 0.2447);  // relative to local csys
const ChQuaternion<> LoopChain::m_idlerRot(QUNIT);
...
LoopChain::LoopChain(const std::string& name,
    ...
    for (int j = 0; j < m_num_rollers; j++) {
        ...
        m_rollers[j] = ChSharedPtr<SupportRoller>(
            new SupportRoller(r_s.str(), VisualizationType::Primitives, CollisionType::Primitives));
    }
...
void LoopChain::Initialize(const ChCoordsys<>& gear_Csys) {
    ...
    // add the first idler to the lists passed into the track chain Init()
    ...
    m_rollers[3]->Initialize(m_chassis, m_chassis->GetFrame_REF_to_abs(), ChCoordsys<>(roller_loc, QUNIT));
...
void LoopChain::Update(double time, double throttle, double braking) {
...
void LoopChain::SetShoePinDamping(double damping) {
    m_chain->Set_pin_friction(damping);
}

// Log constraint violations
// -----------------------------------------------------------------------------
void LoopChain::LogConstraintViolations(bool include_chain) {
...
// -----------------------------------------------------------------------------
void LoopChain::SaveConstraintViolations(std::stringstream& ss, bool include_chain) {
...
void LoopChain::Setup_log_to_file(int what, const std::string& filename) {
...
void LoopChain::Log_to_console(int console_what) {
...
void LoopChain::create_fileHeader(int what) {
    // open the data file for writing the header
