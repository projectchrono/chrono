        ...
        virtual bool ReportContactCallback(const ChVector<>& pA,
            ...
            if (modA->GetFamily() == (int)CollisionFam::Gear || modB->GetFamily() == (int)CollisionFam::Gear) {
                ...
                if (react_forces.x > 0) {
                    ...
                    double Fn_sig2 = (m_Fn_var * (m_num_gear_contacts - 1) +
                                      (react_forces.x - Fn_bar_n1) * (react_forces.x - Fn_bar)) /
                                     m_num_gear_contacts;
      ...
      public:
        ...
        const ChVector<> getContactLocRel(const ChVector<>& pA,
            ...
            if (modA->GetFamily() == (int)fam) {
                ...
                pos_abs =
                    pA -
                    ((ChBody*)modA->GetPhysicsItem())->GetPos();  // abs dist. of contact point from the gear center
                body_frame.SetRot(((ChBody*)modA->GetPhysicsItem())->GetRot());
            } else {
                // assume gear is body B
                pos_abs =
                    pB -
                    ((ChBody*)modB->GetPhysicsItem())->GetPos();  // abs dist. of contact point from the gear center
        ...
        virtual bool ReportContactCallback(const ChVector<>& pA,
            ...
            if ((modA->GetFamily() == (int)CollisionFam::Gear &&
                 modB->GetPhysicsItem()->GetNameString() == m_shoe_name) ||
                (modB->GetFamily() == (int)CollisionFam::Gear &&
                 modA->GetPhysicsItem()->GetNameString() == m_shoe_name)) {
                    ...
                    if (!m_is_persistentContact_set[index]) {
                        ...
                        if (0) {
                            GetLog() << " \n\n ***********     setting the gear contact point at time : "
                                     << modA->GetPhysicsItem()->GetSystem()->GetChTime()
...
void DriveChain::Setup_log_to_file(int what, const std::string& filename, const std::string& data_dirname) {
