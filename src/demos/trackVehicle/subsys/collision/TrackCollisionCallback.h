...
class GearPinCollisionCallback : public ChSystem::ChCustomComputeCollisionCallback
{
  ...
  // all length units in meters
	GearPinCollisionCallback(const double geom_envelope = 0.005,
  ...
  bool BroadphasePassed(const ChVector<>& pin_gear_Pz_bar,
    const ChVector<>& pin_gear_Nz_bar,
    size_t p_idx) const
  {
    ...
    const size_t p_idx)
  {
    ...
    if( r_pitch_pin_XY.Length() + m_bound_rad_Pin[p_idx] >= m_bound_gear_seat_rad[p_idx] && r1r2_dot < m_r1r2_dot_limit[p_idx] )
    {
      ...
      ChVector<> contact_pos_gear_bar = pitch_circle_cenXY_bar + r_pitch_pin_XY * m_geom[p_idx]->gear_concave_radius;
  ...
  size_t Get_GearToothIdx(const ChVector<>& pin_cen,
    const size_t p_idx) const
  {
  ...
  // function implementation
    ...
    for(size_t p_idx = 0; p_idx < m_shoes.size(); p_idx++)
    {
        ...
        iter_shoe != shoe_bodies.end(); iter_shoe++)
      {
        ...
        ChVector<> pin_pos_Pz = shoe->GetPos()
          + shoe->GetRot().Rotate( m_pin_pos_bar[p_idx] );
        ChVector<> pin_pos_Nz = shoe->GetPos()
          + shoe->GetRot().Rotate( ChVector<>(m_pin_pos_bar[p_idx].x,
          m_pin_pos_bar[p_idx].y,
        ...
        if( BroadphasePassed(pin_gear_bar_Pz, pin_gear_bar_Nz, p_idx) )
        {
          ...
          ChVector<> gear_seat_pos_Pz = gear->GetPos() 
            + gear->GetRot().Rotate( m_seat_pos_bar[p_idx] );
          // same as Pz_bar, negate z
          ChVector<> gear_seat_pos_Nz = gear->GetPos() 
            + gear->GetRot().Rotate( ChVector<>(m_seat_pos_bar[p_idx].x,
            m_seat_pos_bar[p_idx].y,
            -m_seat_pos_bar[p_idx].z) );
  ...
  int Get_sum_Pz_contacts(size_t p_idx = 0) const { assert(p_idx < m_sum_Pz_contacts.size()); return m_sum_Pz_contacts[p_idx]; }

  int Get_sum_Nz_contacts(size_t p_idx = 0) const { assert(p_idx < m_sum_Nz_contacts.size()); return m_sum_Nz_contacts[p_idx]; }
...
private:
  ...
  std::vector<double> m_bound_rad_Pin;      ///< geometry bounding sphere circumscribes the outside circumference of the pins
