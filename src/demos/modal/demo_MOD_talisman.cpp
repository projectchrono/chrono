...
void MakeAndRunDemo_SlewingBeam(bool do_modal_reduction, ChMatrixDynamic<>& mdeflection) {
    ...
    auto BuildDumbBody = [&](const ChVector<>& mpos) {
    ...
    BuildDumbBody(ChVector<>(0, 0, 0));  // ground, A0
    BuildDumbBody(ChVector<>(0, 0, h));  // ground, B0
    BuildDumbBody(ChVector<>(0, w, h));  // ground, C0
    BuildDumbBody(ChVector<>(0, w, 0));  // ground, D0

    // Settings
    std::shared_ptr<ChModalAssembly> modal_assembly = chrono_types::make_shared<ChModalAssembly>();
    modal_assembly->use_linear_inertial_term = USE_LINEAR_INERTIAL_TERM;
    if (USE_HERTING)
        modal_assembly->modal_reduction_type = chrono::modal::ChModalAssembly::Reduction_Type::Herting;
    else
        modal_assembly->modal_reduction_type = chrono::modal::ChModalAssembly::Reduction_Type::Craig_Bampton;
    ...
    auto BuildNode_Root = [&](const ChVector<>& mpos) {
    ...
    BuildNode_Root(ChVector<>(0, 0, 0));  // A0
    BuildNode_Root(ChVector<>(0, 0, h));  // B0
    BuildNode_Root(ChVector<>(0, w, h));  // C0
    BuildNode_Root(ChVector<>(0, w, 0));  // D0
    ...
    auto MakeSingleUnit = [&](const std::vector<std::shared_ptr<ChNodeFEAxyzrot>>& mnode_list_left,
        ...
        auto mnode_A1 =
            chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(mnode_A0->GetPos() + ChVector<>(2 * d, 0, 0), QUNIT));
        mnode_A1->SetMass(0);
        mnode_A1->GetInertia().setZero();
        auto mnode_B1 =
            chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(mnode_A0->GetPos() + ChVector<>(2 * d, 0, h), QUNIT));
        mnode_B1->SetMass(0);
        mnode_B1->GetInertia().setZero();
        auto mnode_C1 =
            chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(mnode_A0->GetPos() + ChVector<>(2 * d, w, h), QUNIT));
        mnode_C1->SetMass(0);
        mnode_C1->GetInertia().setZero();
        auto mnode_D1 =
            chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(mnode_A0->GetPos() + ChVector<>(2 * d, w, 0), QUNIT));
        mnode_D1->SetMass(0);
        mnode_D1->GetInertia().setZero();

        auto mnode_E0 =
            chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(mnode_A0->GetPos() + ChVector<>(d, 0, 0), QUNIT));
        mnode_E0->SetMass(0);
        mnode_E0->GetInertia().setZero();
        auto mnode_F0 =
            chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(mnode_A0->GetPos() + ChVector<>(d, w, h), QUNIT));
        mnode_F0->SetMass(0);
    ...
    for (auto node : modal_assembly->Get_meshlist().front()->GetNodes())
        all_beam_nodes.push_back(std::dynamic_pointer_cast<ChNodeFEAxyzrot>(node));
    for (auto node : modal_assembly->Get_internal_meshlist().front()->GetNodes())
        all_beam_nodes.push_back(std::dynamic_pointer_cast<ChNodeFEAxyzrot>(node));
    GetLog() << "There are " << all_beam_nodes.size() << " nodes\n";

    std::vector<std::shared_ptr<ChElementBeamEuler>> all_beam_elements;
    for (auto ele : modal_assembly->Get_meshlist().front()->GetElements())
        all_beam_elements.push_back(std::dynamic_pointer_cast<ChElementBeamEuler>(ele));
    for (auto ele : modal_assembly->Get_internal_meshlist().front()->GetElements())
        all_beam_elements.push_back(std::dynamic_pointer_cast<ChElementBeamEuler>(ele));
    GetLog() << "There are " << all_beam_elements.size() << " elements\n";
    ...
    if (do_modal_reduction) {
        ...
        modal_assembly->SwitchModalReductionON(modes_settings, damping_beam);
        modal_assembly->DumpSubassemblyMatrices(true, true, true, true, (out_dir + "/modal_assembly"));
    }
    ...
    if (DO_DYNAMICS) {
        ...
        while (frame < Nframes) {
            ...
            if (t < 1.0 || (t > 2.0 && t < 3.0))
                P1 = P0 * (1 - cos(CH_C_2PI * t)) / 2.0;
            else
                P1 = 0;

            node_B6->SetForce(ChVector<>(0, 0, P1));
            node_C6->SetForce(ChVector<>(0, 0, P1));
            ...
            node_A0->TransformParentToLocal(node_A6->Frame(), relative_frame_tip);
            ...
            if (do_modal_reduction) {
                ...
                mdeflection(frame, 11) = floating_frame_F.GetRot().Q_to_Rotv().x();
                mdeflection(frame, 12) = floating_frame_F.GetRot().Q_to_Rotv().y();
                mdeflection(frame, 13) = floating_frame_F.GetRot().Q_to_Rotv().z();

                // residual of constraint equations on configuration of F
                mdeflection.block(frame, 14, 1, 6) = modal_assembly->res_CF.transpose().segment(0, 6);
            }

            if (frame % itv_frame == 0) {
                GetLog() << "t: " << sys.GetChTime() << "\t";
                GetLog() << "Tip force: " << mdeflection(frame, 1) << "\t";
                GetLog() << "Tip. Def.:\t" << relative_frame_tip.GetPos().x() << "\t" << relative_frame_tip.GetPos().y()
