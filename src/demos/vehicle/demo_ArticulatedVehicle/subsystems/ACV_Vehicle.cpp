...
void ACV_Vehicle::Initialize(const ChCoordsys<>& chassisPos, double chassisFwdVel) {
    ...
    m_chassis_rear[0]->Initialize(m_chassis, ChVector<>(-2.0, 0, 0), WheeledCollisionFamily::CHASSIS);
    ...
    m_axles[0]->Initialize(m_chassis->GetBody(), ChVector<>(0.5, 0, 0), ChVector<>(0, 0, 0.0), m_chassis->GetBody(), -1,
                           0.0);
    m_axles[1]->Initialize(m_chassis_rear[0]->GetBody(), ChVector<>(-0.5, 0, 0), ChVector<>(0, 0, 0),
                           m_chassis_rear[0]->GetBody(), -1, 0.0);
