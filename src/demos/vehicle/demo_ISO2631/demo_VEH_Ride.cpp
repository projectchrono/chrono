...
#else
	// better than rigid tire, best would be a validated flexible tire
...
#ifdef USE_RIGID_TIRE
    std::wstring windowTitle = L"Vehicle Ride Quality Demo (Rigid Tire) - " + std::to_wstring(rmsVals[iTerrain]) + L" mm RMS";
    ChVehicleIrrApp app(&vehicle, &powertrain, windowTitle.c_str());
#else
    std::wstring windowTitle =  L"Vehicle Ride Quality Demo (TMeasy Tire) - " + std::to_wstring(rmsVals[iTerrain]) + L" mm RMS";
    ChVehicleIrrApp app(&vehicle, &powertrain, windowTitle.c_str());
#endif
