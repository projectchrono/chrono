...
// =============================================================================
//
// Main driver function for the Sedan full model with lockable differential gear box (experimental)
//
...
const double maxLockingTorque = 100.0;
...
int main(int argc, char* argv[])
{
    ...
    if(lock_the_diffbox) {
        GetLog() << "\tThe Diffbox is locked\n";
    } else {
        GetLog() << "\tThe Diffbox is unlocked\n";
    }
    ...
    my_sedan.SetLockingTorqueLimit(maxLockingTorque);
    my_sedan.LockDiffBox(lock_the_diffbox);

    // Create the terrain
    RigidTerrain terrain(my_sedan.GetSystem());

    std::shared_ptr<RigidTerrain::Patch> patch, patch1;

    patch = terrain.AddPatch(
        ChCoordsys<>(ChVector<>(0, -25, terrainHeight - 5), QUNIT), ChVector<>(terrainLength, terrainWidth, 10));
    ...
    patch1->SetColor(ChColor(0.5f, 0.5f, 0.8f));
    terrain.Initialize();
    ...
    utils::CSV_writer frontwheels_csv("\t");
    ...
    while(app.GetDevice()->run()) {
        ...
        double xpos = my_sedan.GetVehicle().GetVehiclePos().x();
        // End simulation
        if(time >= t_end)
            break;

        // End before falling off the road
        if(xpos > xpos_max) {
            break;
        }
        ...
        frontwheels_csv << time << omega_front_left << omega_front_right << std::endl;
    ...
    if(lock_the_diffbox) {
        frontwheels_csv.write_to_file("../SedanFrontWheelOmegaLocked.csv");
    } else {
        frontwheels_csv.write_to_file("../SedanFrontWheelOmegaUnlocked.csv");
    }
    return 0;
}
