...
// =============================================================================
//
// Demo program for UAZBUS with SAE leafsprings simulation.
//
// The detailed leafspring models imposes a new side effect:
// due to the longitudinal axle movement we get a steering offset It happens
// when the front axle preload is changed or when doing a braking manoeuver.
//
...
int main(int argc, char* argv[]) {
    ...
    {
        auto suspR = std::static_pointer_cast<ChSAELeafspringAxle>(uaz.GetVehicle().GetSuspension(1));
    ...
    auto patch = terrain.AddPatch(patch_mat, ChVector<>(0, 0, 0), ChVector<>(0, 0, 1), 600, 600);
    ...
    ChWheeledVehicleIrrApp app(&uaz.GetVehicle(), L"UAZBUS demo with kinematic leafspring model");
    app.SetSkyBox();
