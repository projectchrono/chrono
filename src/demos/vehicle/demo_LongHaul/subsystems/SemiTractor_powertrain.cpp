...
SemiTractor_powertrain::SemiTractor_powertrain(const std::string& name) : ChSimpleMapPowertrain(name) {}

double SemiTractor_powertrain::GetMaxEngineSpeed() {
    return 2700 * rpm2rads;
}

void SemiTractor_powertrain::SetEngineTorqueMaps(ChFunction_Recorder& map0, ChFunction_Recorder& mapF) {
...
void SemiTractor_powertrain::SetGearRatios(std::vector<double>& fwd_gear_ratios, double& reverse_gear_ratio) {
...
void SemiTractor_powertrain::SetShiftPoints(std::vector<std::pair<double, double>>& shift_bands) {
