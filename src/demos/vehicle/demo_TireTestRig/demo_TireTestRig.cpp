...
std::string out_dir1 = "../RESULTS/";
std::string out_dir2 = out_dir1 + "TireTestRig/";
std::string out_dir3 = out_dir2 + "Fiala/";
...
int main()
{
  ...
  actuator->Initialize(ground, chassis, false, ChCoordsys<>(pt1,QUNIT), ChCoordsys<>(pt1 + ChVector<>(1, 0, 0),QUNIT));
  ...
  revolute_set_camber_rim->Initialize(rim, set_camber, ChCoordsys<>(ChVector<>(0, 0, radius), Q_from_AngX(CH_C_PI_2)));
  ...
  if (ChFileutils::MakeDirectory(out_dir2.c_str()) < 0) {
    std::cout << "Error creating directory " << out_dir2 << std::endl;
    return 1;
  }
  if (ChFileutils::MakeDirectory(out_dir3.c_str()) < 0) {
    std::cout << "Error creating directory " << out_dir3 << std::endl;
    return 1;
  }
  ...
  // Write headers
  out_force_moment << "Time" << "X_Frc" << "Y_Frc" << "Z_Frc" << "X_Trq" << "Y_Trq" << "Z_Trq" << std::endl;
  out_wheelstate   << "Time" << "X_Pos" << "Y_Pos" << "Z_Pos" << "e0" << "e1" << "e2" << "e3" << 
                      "X_Vel" << "Y_Vel" << "Z_Vel" << "w_x" << "w_y" << "w_z" << "omega" << std::endl;
  ...
  while (simTime <= sim_endtime + sim_step / 2)
  {
    ...
    if (simTime >= outTime - sim_step / 2)
    {
      ...
      out_wheelstate << simTime << wheelstate.pos << wheelstate.rot << wheelstate.lin_vel << 
        wheelstate.ang_vel << wheelstate.omega << std::endl;
  ...
  // Write output files
  out_force_moment.write_to_file(out_dir3 + "Fiala_FM_out.csv", "Fiala Tire Forces and Moments\n\n");
  out_wheelstate.write_to_file(out_dir3 + "Fiala_WheelStates_out.csv", "Fiala Wheel States\n\n");
