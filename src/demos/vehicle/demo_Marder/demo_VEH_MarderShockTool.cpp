...
ChVector<> trackPoint(0.0, 3.0, 0.0);
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 5) {
        GetLog() << "usage: prog height_inch startSpeed_mph endSpeed_mph stepSpeed_mph\n";
        return 1;
    }

    int obsHeightInch = atoi(argv[1]);
    int startSpeedInch = atoi(argv[2]);
    int endSpeedInch = atoi(argv[3]);
    int stepSpeedInch = atoi(argv[4]);

    ChFunction_Recorder resLogger;
    resLogger.AddPoint(0, 0);  // needed, if the first simulation run is beyond limit

    for (int iVel = startSpeedInch; iVel <= endSpeedInch; iVel += stepSpeedInch) {
        int velMph = iVel;
        double target_speed = double(velMph) * mphToMetersPerSec;
        double obs_xpos = accTravel.Get_y(target_speed) + 1.0;
        double obs_radius = double(obsHeightInch) * inchToMeters;
        ...
        for (int i = 0; i < 6; i++) {
            ...
            auto patch = terrain.AddPatch(patch_mat, ChVector<>(terrainLength * double(i), 0, 0), ChVector<>(0, 0, 1),
        ...
        while (app.GetDevice()->run()) {
            ...
            if (dbg_output) {
                ...
                cout << "      chassis:    " << c_pos.x() << "  " << c_pos.y() << "  " << c_pos.z() << endl;
                {
                    ...
                    cout << "      L idler:    " << i_pos_rel.x() << "  " << i_pos_rel.y() << "  " << i_pos_rel.z()
                         << endl;
                    cout << "      L sprocket: " << s_pos_rel.x() << "  " << s_pos_rel.y() << "  " << s_pos_rel.z()
                         << endl;
                }
                {
                    ...
                    ChVector<> i_pos_rel = c_ref.TransformPointParentToLocal(i_pos_abs);
                    ChVector<> s_pos_rel = c_ref.TransformPointParentToLocal(s_pos_abs);
                    cout << "      R idler:    " << i_pos_rel.x() << "  " << i_pos_rel.y() << "  " << i_pos_rel.z()
                         << endl;
                    cout << "      R sprocket: " << s_pos_rel.x() << "  " << s_pos_rel.y() << "  " << s_pos_rel.z()
                         << endl;
                }
            ...
            if (speed_found) {
                double az = marder.GetVehicle()
                                .GetVehiclePointAcceleration(marder.GetChassis()->GetLocalDriverCoordsys().pos)
                                .z();
        ...
        kurs.close();
        resLogger.AddPoint(azmax, contact_speed);
        if (azmax >= 2.5)
            break;
    }  // iVel
    double amin, amax;
    ...
    GetLog() << "ObsHeight       = " << obsHeightInch << " in\n";
