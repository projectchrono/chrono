...
std::string tmeasytire_file("MAN_Kat1/tire/MAN_5t_TMeasyTire.json");
...
int main(int argc, char* argv[]) {
    ...
    auto powertrain = chrono_types::make_shared<SimpleCVTPowertrain>(vehicle::GetDataFile(simplecvtpowertrain_file));
    ...
    for (auto& axle : vehicle.GetAxles()) {
        switch (tire_model) {
            ...
            case TireModelType::TMEASY: {
                ...
                vehicle.InitializeTire(tireL, axle->m_wheels[0], VisualizationType::NONE);
                vehicle.InitializeTire(tireR, axle->m_wheels[1], VisualizationType::NONE);
                break;
            }
