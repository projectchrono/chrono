...
int main(int argc, char* argv[]) {
    // for (int z = 0; z < num_run; z++) {
    for (int q = start_q_per_dim; q <= end_q_per_dim; q++) {
        // int q = start_q_per_dim;
        ...
        // mmesh->LoadWavefrontMesh("../data/vehicle/hmmwv/hmmwv_chassis.obj", false, true);
        ...
        for (int i = 0; i < x_instances; i++) {
            for (int j = 0; j < y_instances; j++) {
                for (int k = 0; k < z_instances; k++) {
                    ...
                    auto box_body =
                        std::make_shared<ChBodyEasyBox>(randf() + .1, randf() + .1, randf() + .1, 1000, true, false);
                    box_body->SetPos({x_spread * (i + .5 - x_instances / 2.), y_spread * (j + .5 - y_instances / 2.),
                                      z_spread * (k + .5 - z_instances / 2.)});
                    // q = {randf(), randf(), randf(), randf()};
                    // q.Normalize();
                    box_body->SetRot(q);
                    box_body->SetBodyFixed(true);
                    mphysicalSystem.Add(box_body);

                    auto sphere_body = std::make_shared<ChBodyEasySphere>(randf() + .1, 1000, true, false);
                    sphere_body->SetPos({x_spread * (i + .5 - x_instances / 2.), y_spread * (j + .5 - y_instances / 2.),
                                         z_spread * (k + .5 - z_instances / 2.)});
                    // q = {randf(), randf(), randf(), randf()};
                    // q.Normalize();
                    sphere_body->SetRot(q);
                    sphere_body->SetBodyFixed(true);
                    mphysicalSystem.Add(sphere_body);

                    auto cyl_body = std::make_shared<ChBodyEasyCylinder>(randf() + .1, randf() + .1, 1000, true, false);
                    cyl_body->SetPos({x_spread * (i + .5 - x_instances / 2.), y_spread * (j + .5 - y_instances / 2.),
                                      z_spread * (k + .5 - z_instances / 2.)});
                    // q = {randf(), randf(), randf(), randf()};
                    // q.Normalize();
                    cyl_body->SetRot(q);
                    ...
                    auto texcyl_asset = cyl_body->GetAssets()[0];
                    if (std::shared_ptr<ChVisualization> visual_asset =
                            std::dynamic_pointer_cast<ChVisualization>(texcyl_asset)) {
        ...
        cam_body->SetPos({-8, 0, 0});
        cam_body->SetRot(Q_from_AngAxis(0, {0, 0, 1}));
        ...
        for (int c = 0; c < num_cameras; c++) {
            auto cam = std::make_shared<ChCameraSensor>(
                ...
                25.0f * (randf() + 1),                                             // update rate in Hz
        ...
        while (ch_time < end_time) {
            cam_body->SetPos({-orbit_radius * cos(ch_time * orbit_rate), -orbit_radius * sin(ch_time * orbit_rate), 0});
            cam_body->SetRot(Q_from_AngAxis(ch_time * orbit_rate, {0, 0, 1}));
            // cam_body->SetPos({-8, 0, 0});
            // cam_body->SetRot(Q_from_AngAxis(0, {0, 0, 1}));
