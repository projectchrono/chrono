...
lidar = sens.ChLidarSensor(
    ...
    chrono.ChFrameD(chrono.ChVectorD(0, 0, .3), chrono.Q_from_AngAxis(0, chrono.ChVectorD(0, 1, 0))),  # offset pose
...
while (app.GetDevice().run()) :
    ...
    if (step_number % control_steps == 0) :
        ...
        steering,throttle,braking = ControlCar(steering,velocity,camera_data_RGBA8,lidar_data_DI,np.copy(lidar_data_XYZI))
