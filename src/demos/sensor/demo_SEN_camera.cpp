...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        ...
        std::cout << "demo_SEN_camera <number of samples per pixel> <light type: point, spot, directional, environment> ";
    ...
    if (light_type_str.find("point") != std::string::npos) {
        ...
        light_idx = manager->scene->AddDirectionalLight(
            {intensity, intensity, intensity}, light_elevation * CH_PI/180, light_azimuth * CH_PI/180
        ); 
    }
    ...
    auto cam1 = chrono_types::make_shared<ChCameraSensor>(ground_body,   // body camera is attached to
    ...
    auto normal = chrono_types::make_shared<ChNormalCamera>(
    ...
    while (ch_time < end_time) {
        ...
        cam1->SetOffsetPose(chrono::ChFrame<double>(
        ...
        normal->SetOffsetPose(chrono::ChFrame<double>(
            {orbit_radius * cos(orbit_angle), orbit_radius * sin(orbit_angle), 2},
            QuatFromAngleAxis(orbit_angle + CH_PI, {0, 0, 1})));
