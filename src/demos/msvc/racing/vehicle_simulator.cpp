...
MySimulator::MySimulator() {
    ...
    device = createDevice(video::EDT_DIRECT3D9, core::dimension2d<u32>(640, 480),  // resolution
                          32,                                                      // 32 bit depth
                          false,                                                   // full screen
    ...
    my_ground = (ChBodySceneNode*)addChBodySceneNode_easyBox(my_system, device->getSceneManager(), 1.0,
                                                             ChVector<>(0, -2, 0), QUNIT, ChVector<>(180, 2, 180));
    ...
    ChBodySceneNode* my_ground2 = (ChBodySceneNode*)addChBodySceneNode_easyGenericMesh(
        my_system, device->getSceneManager(), 1.0, ChVector<>(0, -1, 0), QUNIT,
    ...
    IAnimatedMeshSceneNode* details_node = device->getSceneManager()->addAnimatedMeshSceneNode(details_mesh,
...
void MySimulator::Run() {
    ...
    while (device->run()) {
        ...
        ChIrrTools::drawGrid(driver, 2, 2, 30, 30, ChCoordsys<>(ChVector<>(0, -0.99, 0), Q_from_AngX(CH_C_PI_2)),
        ...
        ChIrrTools::drawChFunction(this->device, &this->mycar->speed_recorder, this->my_system->GetChTime() - 2,
                                   this->my_system->GetChTime(), -3, 100);
...
void MySimulator::UpdateVideoCamera() {
    ...
    if ((camera_view_type == 2) || (camera_view_type == 3) || (camera_view_type == 4)) {
