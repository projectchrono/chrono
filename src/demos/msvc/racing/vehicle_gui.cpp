...
MyEventReceiver::MyEventReceiver(MySimulator* asimulator) {
    ...
    scrollbar_throttle =
        msimulator->device->getGUIEnvironment()->addScrollBar(true, rect<s32>(10, 85, 150, 100), 0, 100);
    scrollbar_throttle->setMax(100);
    scrollbar_throttle->setPos(0);
    text_throttle =
        msimulator->device->getGUIEnvironment()->addStaticText(L"Throttle", rect<s32>(150, 85, 250, 100), false);

    // ..add a GUI slider to control steering via mouse
    scrollbar_steer = msimulator->device->getGUIEnvironment()->addScrollBar(true, rect<s32>(10, 105, 150, 120), 0, 101);
    scrollbar_steer->setMax(100);
    scrollbar_steer->setPos(50);
    text_steer = msimulator->device->getGUIEnvironment()->addStaticText(L"Steer", rect<s32>(150, 105, 250, 120), false);

    // ..add a GUI text and GUI slider to control the stiffness
    scrollbar_FspringK =
        msimulator->device->getGUIEnvironment()->addScrollBar(true, rect<s32>(10, 125, 150, 140), 0, 102);
    ...
    text_FspringK = msimulator->device->getGUIEnvironment()->addStaticText(L"Spring K [N/m]:    ",
                                                                           rect<s32>(150, 125, 250, 140), false);

    // ..add a GUI text and GUI slider to control the damping
    scrollbar_FdamperR =
        msimulator->device->getGUIEnvironment()->addScrollBar(true, rect<s32>(10, 145, 150, 160), 0, 103);
    scrollbar_FdamperR->setMax(100);
    scrollbar_FdamperR->setPos(FromValToSlider(msimulator->mycar->link_springRF->Get_SpringR(), 0, 1900));
    text_FdamperR = msimulator->device->getGUIEnvironment()->addStaticText(L"Damper R [Ns/m]:   ",
                                                                           rect<s32>(150, 145, 250, 160), false);

    // ..add a GUI text and GUI slider to control the original undeformed spring length
    scrollbar_FspringL =
        msimulator->device->getGUIEnvironment()->addScrollBar(true, rect<s32>(10, 165, 150, 180), 0, 104);
    ...
    text_FspringL = msimulator->device->getGUIEnvironment()->addStaticText(L"Spring L [m]:     ",
                                                                           rect<s32>(150, 165, 250, 180), false);

    // ..add a GUI text and GUI slider to control the mass
    scrollbar_Mass = msimulator->device->getGUIEnvironment()->addScrollBar(true, rect<s32>(10, 185, 150, 200), 0, 105);
    scrollbar_Mass->setMax(100);
    scrollbar_Mass->setPos(
        FromValToSlider(msimulator->mycar->truss->GetBody()->GetMass(), 10, 800));  // map from max min range
    text_Mass = msimulator->device->getGUIEnvironment()->addStaticText(L"Mass [kg]:     ",
                                                                       rect<s32>(150, 185, 250, 200), false);

    // ..add a GUI text and GUI slider to control the length of both the rods of the steer
    scrollbar_rods_steer =
        msimulator->device->getGUIEnvironment()->addScrollBar(true, rect<s32>(10, 205, 150, 220), 0, 106);
    scrollbar_rods_steer->setMax(100);
    scrollbar_rods_steer->setPos(FromValToSlider(msimulator->mycar->link_distLSTEER->GetImposedDistance(), 0.23,
                                                 0.26));  // map from max min range
    text_rods_steer = msimulator->device->getGUIEnvironment()->addStaticText(L"Steer rods length [m]:",
                                                                             rect<s32>(150, 205, 280, 220), false);

    // ..add a GUI text and GUI slider to control the length of both the rods of the steer
    scrollbar_brake = msimulator->device->getGUIEnvironment()->addScrollBar(true, rect<s32>(10, 225, 150, 240), 0, 107);
    scrollbar_brake->setMax(100);
    scrollbar_brake->setPos(0);
    text_brake =
        msimulator->device->getGUIEnvironment()->addStaticText(L"Braking", rect<s32>(150, 225, 280, 240), false);
        ...
        if (event.MouseInput.Event == EMIE_MOUSE_WHEEL) {
            ...
            if (incbrakethrottle) {
                ...
                if (msimulator->mycar->throttle == 0) {
                    ...
                    if (newbraking < 0) {
                        newbraking = 0;
                        msimulator->mycar->throttle = 0.001;
                    }
                    if (newbraking > 1)
                        newbraking = 1;
        ...
        switch (event.GUIEvent.EventType) {
            case EGET_SCROLL_BAR_CHANGED:
                ...
                if (id == 105)  // id of 'spring rest length' slider..
                {
                    ...
                    char message[50];
                    sprintf(message, "Mass [kg]: %g", newmass);
                ...
                if (id == 106)  // id of 'length of rods of steer' slider..
                {
                    ...
                    // show mass as formatted text in interface screen
                    char message[50];
                    sprintf(message, "Steer rods length [m]:%g", newlen);
    ...
    if (event.EventType == irr::EET_KEY_INPUT_EVENT && !event.KeyInput.PressedDown) {
        switch (event.KeyInput.Key) {
            ...
            case irr::KEY_F5:  // camera will stay up in sky, fixed but looking at car
                ...
                msimulator->camera->setPosition(
                    vector3dfCH((this->msimulator->mycar->truss->GetBody()->GetPos() + ChVector<>(-4, 8, -2))));
