...
MyOscillatorSystem::MyOscillatorSystem(IrrlichtDevice* mIrrDevice) {
    ...
    this->my_truss = (ChBodySceneNode*)addChBodySceneNode_easyBox(
        &this->physical_system, mIrrDevice->getSceneManager(), 1.0, ChVector<>(0, 0, 0), QUNIT, ChVector<>(0.1, 1, 1));
    this->my_truss->GetBody()->SetBodyFixed(true);
    this->my_truss->GetBody()->SetCollide(false);

    this->my_body = (ChBodySceneNode*)addChBodySceneNode_easyBox(&this->physical_system, mIrrDevice->getSceneManager(),
                                                                 3.0, ChVector<>(X0, 0, 0), QUNIT, ChVector<>(1, 1, 1));
    ...
    this->my_spring->Initialize(my_truss->GetBody(), my_body->GetBody(), true, ChVector<>(0.0, 0.0, 0.0),
