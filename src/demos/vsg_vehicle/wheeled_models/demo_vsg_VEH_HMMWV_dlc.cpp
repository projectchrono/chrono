...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < helper.GetConePositions(); i++) {
        ...
        ChVector<> objSize(1, 1, 1);
        ChColor redCol(0.8, 0.2, 0.2);
        vis->SetDecoObject(GetChronoDataFile("models/traffic_cone/trafficCone750mm.obj"), ChCoordsys<>(pL, QUNIT),
                           objSize, redCol);
        /*
                ...
                irr::scene::IAnimatedMeshSceneNode* node_coneL =
           vis->GetSceneManager()->addAnimatedMeshSceneNode(mesh_coneL); node_coneL->getMaterial(0).EmissiveColor =
           irr::video::SColor(0, 100, 0, 0); node_coneL->setPosition(irr::core::vector3dfCH(pL));
        */

        ChVector<> pR = helper.GetConePosition(i, false) + ChVector<>(0, -coneBaseWidth / 2, 0.05);
        ChColor greenCol(0.2, 0.8, 0.2);
        vis->SetDecoObject(GetChronoDataFile("models/traffic_cone/trafficCone750mm.obj"), ChCoordsys<>(pR, QUNIT),
                           objSize, greenCol);
        /*
                irr::scene::IAnimatedMesh* mesh_coneR =
                        vis->GetSceneManager()->getMesh(GetChronoDataFile("models/traffic_cone/trafficCone750mm.obj").c_str());
                irr::scene::IAnimatedMeshSceneNode* node_coneR =
           vis->GetSceneManager()->addAnimatedMeshSceneNode(mesh_coneR); node_coneR->getMaterial(0).EmissiveColor =
           irr::video::SColor(0, 0, 100, 0); node_coneR->setPosition(irr::core::vector3dfCH(pR));
        */
    }
    vis->Initialize();
