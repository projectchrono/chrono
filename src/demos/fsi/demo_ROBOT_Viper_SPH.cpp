...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < numPart; i++) {
        ...
        myFsiSystem.AddSphMarker(
            ...
            ChVector<>(paramsH->rho0, pre_ini, paramsH->mu0),  // initial presssure modified as 0.0
            paramsH->HSML, -1,
            ChVector<>(0.0e0),
    ...
    size_t numPhases = myFsiSystem.GetFsiData()->fsiGeneralData->referenceArray.size();
...
void SaveParaViewFiles(fsi::ChSystemFsi& myFsiSystem,
    ...
    if (pv_output && std::abs(mTime - (next_frame)*frame_time) < 1e-7) {
        ...
        fsi::utils::PrintToFile(myFsiSystem.GetFsiData()->sphMarkersD2->posRadD,
                                myFsiSystem.GetFsiData()->sphMarkersD2->velMasD,
                                myFsiSystem.GetFsiData()->sphMarkersD2->rhoPresMuD,
                                myFsiSystem.GetFsiData()->fsiGeneralData->sr_tau_I_mu_i,
                                myFsiSystem.GetFsiData()->fsiGeneralData->referenceArray,
                                myFsiSystem.GetFsiData()->fsiGeneralData->referenceArray_FEA, demo_dir, true);
