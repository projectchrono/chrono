...
void CreateMbdPhysicalSystemObjects(ChSystemParallelDVI &mphysicalSystem,
  ...
  // Ground body
  auto ground =
      std::make_shared<ChBody>(new collision::ChCollisionModelParallel);
  ...
  double bottomWidth =
      midSecDim - basinDepth / tan(phi); // for a 45 degree slope
  ...
  double x1I =
      -midSecDim + inclinedWidth * cos(phi) - hthick * sin(phi) - smallBuffer;
  ...
  //      auto mball = std::make_shared<ChBody>(new
  //      collision::ChCollisionModelParallel);
  ...
  //                                                       //
  //                                                       utils::AddEllipsoidGeometry(body.get(),
  //                                                       size);
  //                                                       // X
  //
...
void SavePovFilesMBD(fsi::ChSystemFsi &myFsiSystem,
                     ...
                     chrono::fsi::SimParams *paramsH, int tStep, double mTime) {
  ...
  if (povray_output && tStep % out_steps == 0) {
    ...
    chrono::fsi::utils::PrintToFile(
        myFsiSystem.GetDataManager()->sphMarkersD1.posRadD,
    ...
    if (tStep / out_steps == 0) {
      const std::string rmCmd =
          std::string("rm ") + pov_dir_mbd + std::string("/*.dat");
      system(rmCmd.c_str());
    }
...
int main(int argc, char *argv[]) {
  ...
  //****************************************************************************************
  const std::string simulationParams =
      out_dir + "/simulation_specific_parameters.txt";
  ...
  chrono::fsi::Real3 boxCenter =
      chrono::fsi::mR3(0, 0, paramsH->cMin.z + 0.5 * basinDepth);
  boxCenter.z += 2 * paramsH->HSML;
  chrono::fsi::Real3 boxHalfDim =
      chrono::fsi::mR3(1.8, .9 * hdimY, 0.5 * basinDepth);
  ...
  int numPhases =
      myFsiSystem.GetDataManager()
          ->fsiGeneralData.referenceArray.size(); // Arman TODO: either rely on
                                                  // pointers, or stack
  ...
  ChVector<> gravity =
      ChVector<>(paramsH->gravity.x, paramsH->gravity.y, paramsH->gravity.z);
