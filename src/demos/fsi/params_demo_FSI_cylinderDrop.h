...
void SetupParamsH(SimParams *paramsH, Real hdimX, Real hdimY, Real hthick,
  ...
  paramsH->rho0 = 1000;
  paramsH->markerMass =
      pow(paramsH->MULT_INITSPACE * paramsH->HSML, 3) * paramsH->rho0;
  paramsH->mu0 = .001;
  ...
  // paramsH->HSML);
  int3 side0 =
      mI3(floor((paramsH->cMax.x - paramsH->cMin.x) / (2 * paramsH->HSML)),
  ...
  paramsH->cMaxInit =
      mR3(fluidInitDimX, paramsH->cMax.y, paramsH->cMinInit.z + fluidHeight);
  //****************************************************************************************
  //*** initialize straight channel
  paramsH->straightChannelBoundaryMin =
      paramsH->cMinInit; // mR3(0, 0, 0);  // 3D channel
  paramsH->straightChannelBoundaryMax =
      paramsH->cMaxInit; // SmR3(3, 2, 3) * paramsH->sizeScale;
  //************************** modify pressure ***************************
  ...
  //stuff *****************************
  int3 SIDE =
      mI3(int((paramsH->cMax.x - paramsH->cMin.x) / paramsH->binSize0 + .1),
