...
bool GetProblemSpecs(int argc, char** argv, SimParams& params) {
    ...
    cli.AddOption<int>("Simulation", "testing_mode", "Testing mode", std::to_string(params.testing_mode));
    ...
    params.testing_mode = cli.GetAsType<int>("testing_mode");
    return true;
}
...
void SimulateMaterial(int i, const SimParams& params) {
    ...
    for (const auto& p : points) {
        ...
        switch (params.testing_mode) {
            case 0:
                pre_ini = sysSPH.GetDensity() * gz * (-p.z() + fzDim);
            ...
            case 2:
                pre_ini = sysSPH.GetDensity() * gz * (-p.z() + fzDim);
                // Density with depth function from
