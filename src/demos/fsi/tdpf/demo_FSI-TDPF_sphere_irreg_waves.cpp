...
bool GetProblemSpecs(int argc, char** argv, double& t_end, bool& verbose, bool& realtime, bool& render, bool& snapshots) {
    ChCLI cli(argv[0], "FSI_TDPF sphere irregular waves");
...
int main(int argc, char* argv[]) {
    ...
    // Add FSI body
    sysFSI.AddRigidBody(sphere, nullptr, false);
    ...
    if (render) {
        ...
        visFSI->SetWaveMeshParams({0, 0}, {100, 100}, 64);
        visFSI->SetWaveMeshColormap(ChColormap::Type::BLUE, 0.95f);
        visFSI->SetWaveMeshColorMode(ChTdpfVisualizationVSG::ColorMode::HEIGHT, {-wave_height, +wave_height});
        visFSI->SetWaveMeshUpdateFrequency(render_fps);

        vis->AttachPlugin(visFSI);
        vis->AttachSystem(&sysMBS);
        vis->SetWindowTitle("FSI-TDPF sphere irregular waves");
        vis->SetWindowSize(1280, 720);
        vis->SetBackgroundColor(ChColor(0.04f, 0.11f, 0.18f));
        vis->AddCamera(ChVector3d(30, -35, 15), ChVector3d(0, 0, 0));
