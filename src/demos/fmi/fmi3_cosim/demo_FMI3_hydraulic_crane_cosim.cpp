...
void CreateCraneFMU(FmuChronoUnit& crane_fmu,
    ...
    try {
        crane_fmu.Load(FmuType::COSIMULATION, fmu_filename, fmu_unpack_dir);
        ////crane_fmu.Load(FmuType::COSIMULATION, fmu_filename); // will go in TEMP/_fmu_temp
    } catch (std::exception&) {
        throw;
    }
...
void CreateActuatorFMU(FmuChronoUnit& actuator_fmu,
                       ...
                       const std::vector<std::string>& logCategories) {
    try {
        actuator_fmu.Load(FmuType::COSIMULATION, fmu_filename, fmu_unpack_dir);
        ////actuator_fmu.Load(FmuType::COSIMULATION, fmu_filename); // will go in TEMP/_fmu_temp
    } catch (std::exception&) {
        throw;
    }
