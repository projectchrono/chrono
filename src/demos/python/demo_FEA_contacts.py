...
if (do_mesh_collision_floor) :
    ...
    mfloor.GetCollisionModel().AddTriangleMesh(mmeshbox, False, False, chrono.VNULL, chrono.ChMatrix33D(1),
    ...
    mfloor = chrono.ChBodyEasyBox(2, 0.1, 2, 2700, True)
    mfloor.SetBodyFixed(True)
    mfloor.SetMaterialSurface(mysurfmaterial)
    my_system.Add(mfloor)
...
mcube = chrono.ChBodyEasyBox(0.1, 0.1, 0.1, 2700, True)
mcube.SetPos(chrono.ChVectorD(0.6, 0.5, 0.6))
mcube.SetMaterialSurface(mysurfmaterial)
my_system.Add(mcube)

msphere = chrono.ChBodyEasySphere(0.1, 2700, True)
...
mcontactsurf = fea.ChContactSurfaceMesh()
