...
void CreateSliderGuide(std::shared_ptr<ChBody>& mguide,
                       ...
                       ChSystem& msystem,
                       const ChVector<> mpos) {
    mguide = chrono_types::make_shared<ChBodyEasyBox>(4, 0.3, 0.6, 1000, material, ChCollisionSystemType::CHRONO);
    mguide->SetPos(mpos);
    mguide->SetBodyFixed(true);
    msystem.Add(mguide);

    mslider = chrono_types::make_shared<ChBodyEasySphere>(0.14, 1000, material, ChCollisionSystemType::CHRONO);
    mslider->SetPos(mpos + ChVector<>(0, 0.3, 0));
    msystem.Add(mslider);

    auto obstacle =
        chrono_types::make_shared<ChBodyEasyBox>(0.4, 0.4, 0.4, 8000, material, ChCollisionSystemType::CHRONO);
...
void CreateStatorRotor(std::shared_ptr<ChBody>& mstator,
    ...
    mrotor = chrono_types::make_shared<ChBodyEasyBox>(1, 0.1, 0.1, 1000, material, ChCollisionSystemType::CHRONO);
...
int main(int argc, char* argv[]) {
    ...
    // Create ground body
    auto floorBody = chrono_types::make_shared<ChBodyEasyBox>(20, 2, 20, 3000, material, ChCollisionSystemType::CHRONO);
