...
int main(int argc, char* argv[]) {
    ...
    body->AddVisualShape(capsule, ChFrame<>(ChVector3d(-2, 0, 1), QUNIT));
    body->AddVisualShape(chrono_types::make_shared<ChVisualShapeSphere>(0.03),
                         ChFrame<>(ChVector3d(-2, 0, 0.1), QUNIT));
    body->AddVisualShape(chrono_types::make_shared<ChVisualShapeSphere>(0.03),
                         ChFrame<>(ChVector3d(-2, 0, 1.9), QUNIT));

    // Attach a cone shape
    auto coneShape = chrono_types::make_shared<ChVisualShapeCone>(0.3, 1.0);
    coneShape->SetMaterial(0, orange_mat);
    body->AddVisualShape(coneShape, ChFrame<>(ChVector3d(1, 0, 0), QUNIT));
    body->AddVisualShape(chrono_types::make_shared<ChVisualShapeSphere>(0.03), ChFrame<>(ChVector3d(1, 0, 0), QUNIT));
    body->AddVisualShape(chrono_types::make_shared<ChVisualShapeSphere>(0.03), ChFrame<>(ChVector3d(1, 0, 1), QUNIT));
    ...
    for (int np = 0; np < 100; ++np)
        particles->AddParticle(ChCoordsys<>(ChVector3d(ChRandom::Get() - 2, 0.5, ChRandom::Get() + 2)));
    ...
    vis.Initialize();
    vis.AddCamera(ChVector3d(-2.0, 3.0, -4.0), ChVector3d(0, 0, 0));
