...
// dot product of each column of a matrix with itself
CUDA_HOST_DEVICE CH_MULTICORE_API inline real3 DotMM(const real* M) {
    real3 result;
    result.x = M[0] * M[0] + M[1] * M[1] + M[2] * M[2];
    result.y = M[4] * M[4] + M[5] * M[5] + M[6] * M[6];
    result.z = M[8] * M[8] + M[9] * M[9] + M[10] * M[10];
...
CUDA_HOST_DEVICE CH_MULTICORE_API inline real3 DotMM(const real* M, const real* N) {
    real3 result;
    result.x = M[0] * N[0] + M[1] * N[1] + M[2] * N[2];
    result.y = M[4] * N[4] + M[5] * N[5] + M[6] * N[6];
    result.z = M[8] * N[8] + M[9] * N[9] + M[10] * N[10];
...
CUDA_HOST_DEVICE CH_MULTICORE_API inline Mat33 MulMM(const real* M, const real* N) {
    Mat33 r;
...
CUDA_HOST_DEVICE CH_MULTICORE_API inline Mat33 MulM_TM(const real* M, const real* N) {
    // c1 c2 c3    // c1 c2 c3
    // 0  1  2     // 0  4  8
    // 4  5  6     // 1  5  9
    // 8  9  10    // 2  6  10
...
CUDA_HOST_DEVICE CH_MULTICORE_API inline real3 MulMV(const real* M, const real* N) {
    real3 r;
    r[0] = M[0] * N[0] + M[4] * N[1] + M[8] * N[2];
    r[1] = M[1] * N[0] + M[5] * N[1] + M[9] * N[2];
    r[2] = M[2] * N[0] + M[6] * N[1] + M[10] * N[2];
...
CUDA_HOST_DEVICE CH_MULTICORE_API inline Mat33 OuterProductVV(const real* A, const real* B) {
    return Mat33(A[0] * B[0], A[1] * B[0], A[2] * B[0], A[0] * B[1], A[1] * B[1], A[2] * B[1], A[0] * B[2], A[1] * B[2],
                 A[2] * B[2]);
}

CUDA_HOST_DEVICE CH_MULTICORE_API inline Mat33 ScaleMat(const real* M, const real b) {
    Mat33 r;
    r[0] = M[0] * b;
    r[1] = M[1] * b;
    r[2] = M[2] * b;
...
CUDA_HOST_DEVICE CH_MULTICORE_API inline SymMat33 NormalEquations(const real* A) {
    SymMat33 T;
...
CUDA_HOST_DEVICE CH_MULTICORE_API inline Mat33 MAbs(const real* M) {
...
CUDA_HOST_DEVICE CH_MULTICORE_API OPERATOR_EQUALSALT(*, real, Mat33) CUDA_HOST_DEVICE CH_MULTICORE_API
    OPERATOR_EQUALSALT(*, Mat33, Mat33) CUDA_HOST_DEVICE CH_MULTICORE_API
    OPERATOR_EQUALSALT(+, Mat33, Mat33) CUDA_HOST_DEVICE CH_MULTICORE_API OPERATOR_EQUALSALT(-, Mat33, Mat33)

        CUDA_HOST_DEVICE CH_MULTICORE_API Mat33
