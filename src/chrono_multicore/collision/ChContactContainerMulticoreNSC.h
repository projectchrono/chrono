...
/// Specialization of the multicore contact container for NSC contacts.
class CH_MULTICORE_API ChContactContainerMulticoreNSC_mc : public ChContactContainerMulticore_mc {
  public:
    ChContactContainerMulticoreNSC_mc(ChMulticoreDataManager* dc);
    ChContactContainerMulticoreNSC_mc(const ChContactContainerMulticoreNSC_mc& other);

    /// "Virtual" copy constructor (covariant return type).
    virtual ChContactContainerMulticoreNSC_mc* Clone() const override {
        return new ChContactContainerMulticoreNSC_mc(*this);
    }
