...
void ChCNarrowphaseDispatch::ClearContacts() {
    ...
    if (num_potential_rigid_contacts == 0) {
        data_manager->cd_data->norm_rigid_rigid.resize(0);
        data_manager->cd_data->cpta_rigid_rigid.resize(0);
        data_manager->cd_data->cptb_rigid_rigid.resize(0);
        data_manager->cd_data->dpth_rigid_rigid.resize(0);
        data_manager->cd_data->erad_rigid_rigid.resize(0);
        data_manager->cd_data->bids_rigid_rigid.resize(0);
    }
}
...
int ChCNarrowphaseDispatch::PreprocessCount() {
    ...
    if (narrowphase_algorithm == ChNarrowphase::Algorithm::MPR) {
        ...
        const long long* pair_shapeIDs = data_manager->cd_data->pair_shapeIDs.data();
...
#pragma omp parallel for
    ...
    data_manager->cd_data->contact_shapeIDs.resize(num_potentialContacts);
    Thrust_Expand(contact_index.begin(), contact_index.end() - 1, data_manager->cd_data->pair_shapeIDs.begin(),
                  data_manager->cd_data->contact_shapeIDs.begin());
...
void ChCNarrowphaseDispatch::Dispatch_Init(uint index,
    ...
    const custom_vector<long long>& pair_shapeIDs = data_manager->cd_data->pair_shapeIDs;
...
void ChCNarrowphaseDispatch::Dispatch_Finalize(uint icoll, uint ID_A, uint ID_B, int nC) {
    custom_vector<vec2>& body_ids = data_manager->cd_data->bids_rigid_rigid;
...
void ChCNarrowphaseDispatch::DispatchMPR() {
    custom_vector<real3>& norm = data_manager->cd_data->norm_rigid_rigid;
    custom_vector<real3>& ptA = data_manager->cd_data->cpta_rigid_rigid;
    custom_vector<real3>& ptB = data_manager->cd_data->cptb_rigid_rigid;
    custom_vector<real>& contactDepth = data_manager->cd_data->dpth_rigid_rigid;
    custom_vector<real>& effective_radius = data_manager->cd_data->erad_rigid_rigid;
...
void ChCNarrowphaseDispatch::DispatchR() {
    ...
    real* contactDepth = data_manager->cd_data->dpth_rigid_rigid.data();
    real* effective_radius = data_manager->cd_data->erad_rigid_rigid.data();
...
void ChCNarrowphaseDispatch::DispatchRigid() {
    ...
    custom_vector<real3>& norm_data = data_manager->cd_data->norm_rigid_rigid;
    custom_vector<real3>& cpta_data = data_manager->cd_data->cpta_rigid_rigid;
    custom_vector<real3>& cptb_data = data_manager->cd_data->cptb_rigid_rigid;
    custom_vector<real>& dpth_data = data_manager->cd_data->dpth_rigid_rigid;
    custom_vector<real>& erad_data = data_manager->cd_data->erad_rigid_rigid;
    custom_vector<vec2>& bids_data = data_manager->cd_data->bids_rigid_rigid;
    custom_vector<long long>& contact_shapeIDs = data_manager->cd_data->contact_shapeIDs;
...
void ChCNarrowphaseDispatch::DispatchRigidFluid() {
    ...
    RigidSphereContact(data_manager->node_container->kernel_radius,  //
                       ...
                       data_manager->cd_data->norm_rigid_fluid,      //
                       data_manager->cd_data->cpta_rigid_fluid,      //
                       data_manager->cd_data->dpth_rigid_fluid,      //
                       data_manager->cd_data->neighbor_rigid_fluid,  //
                       data_manager->cd_data->c_counts_rigid_fluid,  //
...
#pragma omp parallel for
    ...
    for (int index = 0; index < (signed)f_number_of_bins_active; index++) {
        ...
        if (rigid_index != 1000000000) {
            ...
            uint rigid_start = data_manager->cd_data->bin_start_index[rigid_index];
            uint rigid_end = data_manager->cd_data->bin_start_index[rigid_index + 1];
#pragma omp parallel for
            for (int i = start; i < (signed)end; i++) {
                ...
                for (uint j = rigid_start; j < rigid_end; j++) {
                    if (contact_counts[p] < max_rigid_neighbors) {
                        uint shape_id_a = data_manager->cd_data->bin_aabb_number[j];
                        real3 Amin = data_manager->cd_data->aabb_min[shape_id_a];
                        real3 Amax = data_manager->cd_data->aabb_max[shape_id_a];
