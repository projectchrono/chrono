...
ChContactContainerMulticore::ChContactContainerMulticore(ChMulticoreDataManager* dc)
    : data_manager(dc) {
...
void ChContactContainerMulticore::ReportAllContacts(std::shared_ptr<ReportContactCallback> callback) {
    ...
    const auto& ptA = cd_data->host_data.cpta_rigid_rigid;
    const auto& ptB = cd_data->host_data.cptb_rigid_rigid;
    const auto& nrm = cd_data->host_data.norm_rigid_rigid;
    const auto& depth = cd_data->host_data.dpth_rigid_rigid;
    const auto& erad = cd_data->host_data.erad_rigid_rigid;
    const auto& bids = cd_data->host_data.bids_rigid_rigid;
...
ChContactContainerMulticore_mc::ChContactContainerMulticore_mc(ChMulticoreDataManager* dc) : data_manager(dc) {
    contactlist_6_6.clear();
    n_added_6_6 = 0;
}

ChContactContainerMulticore_mc::ChContactContainerMulticore_mc(const ChContactContainerMulticore_mc& other)
    : ChContactContainer(other) {
    //// TODO
}

ChContactContainerMulticore_mc::~ChContactContainerMulticore_mc() {
    RemoveAllContacts();
}

void ChContactContainerMulticore_mc::RemoveAllContacts() {
    std::list<ChContact_6_6*>::iterator itercontact = contactlist_6_6.begin();
    while (itercontact != contactlist_6_6.end()) {
        delete (*itercontact);
        (*itercontact) = 0;
...
void ChContactContainerMulticore_mc::BeginAddContact() {
    lastcontact_6_6 = contactlist_6_6.begin();
    n_added_6_6 = 0;
}

void ChContactContainerMulticore_mc::EndAddContact() {
    // remove contacts that are beyond last contact
    while (lastcontact_6_6 != contactlist_6_6.end()) {
        delete (*lastcontact_6_6);
        lastcontact_6_6 = contactlist_6_6.erase(lastcontact_6_6);
...
void ChContactContainerMulticore_mc::ComputeContactForces() {
    // Defer to associated system
    static_cast<ChSystemMulticore*>(GetSystem())->CalculateContactForces();
}

ChVector<> ChContactContainerMulticore_mc::GetContactableForce(ChContactable* contactable) {
    // If contactable is a body, defer to associated system
    if (auto body = dynamic_cast<ChBody*>(contactable)) {
        real3 frc = static_cast<ChSystemMulticore*>(GetSystem())->GetBodyContactForce(body->GetId());
        return ToChVector(frc);
...
ChVector<> ChContactContainerMulticore_mc::GetContactableTorque(ChContactable* contactable) {
    // If contactable is a body, defer to associated system
    if (auto body = dynamic_cast<ChBody*>(contactable)) {
        real3 trq = static_cast<ChSystemMulticore*>(GetSystem())->GetBodyContactTorque(body->GetId());
        return ToChVector(trq);
