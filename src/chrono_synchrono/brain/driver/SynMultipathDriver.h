#ifndef SYN_MULTIPATHDRIVER_H
#define SYN_MULTIPATHDRIVER_H
...
class SYN_API ChMultiplePathSteeringController : public vehicle::ChSteeringController {
  public:
    ...
    ChMultiplePathSteeringController(std::vector<std::pair<std::shared_ptr<ChBezierCurve>, bool>> path_pairs,
                                     int lane = 0);
...
/// A driver model that is very similar with ChPathFollowerACCDriver but
/// it uses multiple path steering controller. So the driver can switch
/// to other paths when needed
///
