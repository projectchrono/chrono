...
struct btSubSimplexClosestResult {
  ...
  void setBarycentricCoordinates(real a = real(0.), real b = real(0.), real c = real(0.), real d = real(0.)) {
...
class CH_PARALLEL_API ChCNarrowphaseVoronoiSimplex {
 ...
 public:
  ...
  void addVertex(const real3& w, const real3& p, const real3& q);

  void setEqualVertexThreshold(real threshold) { m_equalVertexThreshold = threshold; }

  real getEqualVertexThreshold() const { return m_equalVertexThreshold; }

  bool closest(real3& v);

  real maxVertex();

  bool fullSimplex() const { return (m_numVertices == 4); }

  int getSimplex(real3* pBuf, real3* qBuf, real3* yBuf) const;
  ...
  void compute_points(real3& p1, real3& p2);
