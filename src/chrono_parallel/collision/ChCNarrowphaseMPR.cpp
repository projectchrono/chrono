...
bool IsCurvedShape(real& minRadius, const ConvexShape& shape) {
    switch (shape.type) {
        case SPHERE:
        case CAPSULE:
            minRadius = shape.B.x;
            return true;
        case ELLIPSOID:
            minRadius = Min(shape.B.x, shape.B.y);
            minRadius = Min(minRadius, shape.B.z);
            return true;
        case CONE:
        case ROUNDEDCONE:
        case CYLINDER:
        case ROUNDEDCYL:
            minRadius = Min(shape.B.x, shape.B.z);
            return true;
        default:
            return false;
    }
...
real ContactEffectiveRadius(const ConvexShape& shapeA, const ConvexShape& shapeB, real defaultRadius) {
    if ((shapeA.type == SPHERE && shapeB.type == SPHERE) || (shapeA.type == CAPSULE && shapeB.type == SPHERE) ||
        (shapeA.type == SPHERE && shapeB.type == CAPSULE)) {
        return shapeA.B.x * shapeB.B.x / (shapeA.B.x + shapeB.B.x);
    }
    //	if (shapeA.type == CYLINDER && shapeB.type == SPHERE) {
    //		// TODO	: grab reimplement the combinations that appear in ChCNarrowphaseR
    //		return defaultRadius;
    //	} if (shapeA.type == CYLINDER && shapeB.type == SPHERE) {
    //		// TODO
    //		return defaultRadius;
    //	}
    real rad1 = defaultRadius;
    real rad2 = defaultRadius;
    bool isCurved1 = IsCurvedShape(rad1, shapeA);
    bool isCurved2 = IsCurvedShape(rad2, shapeB);
    if (isCurved1 && isCurved2) {
        return Min(rad1, rad2);
    }
    if (isCurved1) {
        return rad1;
    }
...
bool chrono::collision::MPRCollision(const ConvexShape& shapeA,
    ...
    effectiveRadius = ContactEffectiveRadius(shapeA, shapeB, defaultRadius);
    return true;
}
