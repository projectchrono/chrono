...
#pragma omp parallel for
    ...
    bin_num_contact.resize(num_bins_active + 1);
    bin_num_contact[num_bins_active] = 0;
...
#pragma omp parallel for
    for (int index = 0; index < num_bins_active; index++) {
        f_Store_AABB_AABB_Intersection(index, inv_bin_size, bins_per_axis, aabb_min, aabb_max, bin_number_out,
                                       bin_aabb_number, bin_start_index, bin_num_contact, fam_data, obj_active, obj_data_id,
                                       contact_pairs);
    }
