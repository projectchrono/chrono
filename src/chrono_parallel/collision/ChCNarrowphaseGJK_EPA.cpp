...
bool GJKCollide(const ConvexShape& shape0,
...

    {
        ...
        for (;;) {
            ...
            //      printf("pWorld: [%f %f %f], qWorld:  [%f %f %f]\n", pWorld.x, pWorld.y, pWorld.z, qWorld.x,
            //      qWorld.y,
            //      qWorld.z);
    ...
    if (isValid && ((distance < 0) || (distance * distance < C_LARGE_REAL)) &&
        distance < (marginA + marginB + envelope)) {
    ...
    if (m_numPerturbationIterations && manifold.num_contact_points < 4) {
        ...
        for (i = 0; i < m_numPerturbationIterations; i++) {
            if (Dot(v0) > C_EPSILON) {
                ...
                //                    << perturbed_manifold.points[i].pointB << perturbed_manifold.points[i].depth <<
                //                    std::endl;
                //        }
            }
        }
