...
static inline __m128d permute2d(__m128d  const & a) {
	...
	if (do_zero && !do_shuffle) {  // zeroing, not shuffling
		...
		// zero some elements
		__m128i mask1 = constant4i<-int(i0 >= 0), -int(i0 >= 0), -int(i1 >= 0),
				-int(i1 >= 0)>();
...
static inline __m256d permute4d(__m256d  const & a) {
	...
	if (!do_shuffle) {  // no shuffling needed
		if (do_zero) {  // zeroing
			...
			__m256d  const mask = _mm256_castps_pd(
					constant8f<-int(i0 >= 0), -int(i0 >= 0), -int(i1 >= 0),
							-int(i1 >= 0), -int(i2 >= 0), -int(i2 >= 0),
							-int(i3 >= 0), -int(i3 >= 0)>());
	...
	const int s1 = (i0 < 0 ? 3 : (i0 & 2) >> 1)
			| (i2 < 0 ? 0x30 : (i2 & 2) << 3);
	const int s2 = (i1 < 0 ? 3 : (i1 & 2) >> 1)
			| (i3 < 0 ? 0x30 : (i3 & 2) << 3);
	// permute mask
	const int sm = (i0 < 0 ? 0 : (i0 & 1)) | (i1 < 0 ? 1 : (i1 & 1)) << 1
			| (i2 < 0 ? 0 : (i2 & 1)) << 2 | (i3 < 0 ? 1 : (i3 & 1)) << 3;
	...
	if (s1 == 0x01 || s1 == 0x11 || s2 == 0x01 || s2 == 0x11) {
		...
		return _mm256_castps_pd(
				set_m128r(_mm_castpd_ps(rlo), _mm_castpd_ps(rhi)));
	}
	...
	case 0x00:  // LL
		...
		break;
	case 0x03:  // LZ
		r1 = _mm256_insertf128_pd(do_zero ? _mm256_setzero_pd() : __m256d(a),
				_mm256_castpd256_pd128(a), 1);
		break;
	case 0x10:  // LH
		r1 = a;
		break;
	...
	if (s2 == s1) {
		...
		case 0x00:  // LL
			...
			break;
		case 0x03:  // ZL
			r2 = _mm256_insertf128_pd(
					do_zero ? _mm256_setzero_pd() : __m256d(a),
					_mm256_castpd256_pd128(a), 1);
			break;
		case 0x10:  // LH
			r2 = a;
...
static const __m256d NEGATEMASK = _mm256_castsi256_pd(
		_mm256_set1_epi64x(0x8000000000000000));
static const __m256d ABSMASK = _mm256_castsi256_pd(
		_mm256_setr_epi32(-1, 0x7FFFFFFF, -1, 0x7FFFFFFF, -1, 0x7FFFFFFF, -1,
				0x7FFFFFFF));

// Mask gets the first 3 elements out of 4, sets last element to zero
static const __m256d REAL3MASK = _mm256_castsi256_pd(
		_mm256_setr_epi64x(0xffffffffffffffff, 0xffffffffffffffff,
...
static __m256d change_sign(__m256d a) {
	...
	__m256d mask = _mm256_castsi256_pd(
			_mm256_setr_epi32(0, i0 ? (int) 0x80000000 : 0, 0,
					i1 ? (int) 0x80000000 : 0, 0, i2 ? (int) 0x80000000 : 0, 0,
					i3 ? (int) 0x80000000 : 0));
