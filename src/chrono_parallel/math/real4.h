...
class real4 {
  public:
    ...
    inline explicit real4(real a) : mmvalue(_mm256_set1_pd(a)) {}
    inline real4(real a, real b, real c, real d) : mmvalue(_mm256_setr_pd(a, b, c, d)) {}
    inline real4(const real3& v, real w) : mmvalue(_mm256_setr_pd(v.x, v.y, v.z, w)) {}
    inline real4(const __m256& m) : mmvalue(m) {}
    ...
    inline real4(const real3& v, real w) : mmvalue(_mm256_setr_ps(v.x, v.y, v.z, w)) {}
...
static inline real3 AbsRotate(const quaternion& q, const real3& v) {
    ...
    result.x =
        Abs((e0e0 + e1e1) * 2.0f - 1.0f) * v.x + Abs((e1e2 - e0e3) * 2.0f) * v.y + Abs((e1e3 + e0e2) * 2.0f) * v.z;
    result.y =
        Abs((e1e2 + e0e3) * 2.0f) * v.x + Abs((e0e0 + e2e2) * 2.0f - 1.0f) * v.y + Abs((e2e3 - e0e1) * 2.0f) * v.z;
    result.z =
        Abs((e1e3 - e0e2) * 2.0f) * v.x + Abs((e2e3 + e0e1) * 2.0f) * v.y + Abs((e0e0 + e3e3) * 2.0f - 1.0f) * v.z;
    return result;
}
