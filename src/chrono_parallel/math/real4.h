...
public:
	...
	static inline __m256d Set(real x, real y, real z, real w) {
	...
	static inline __m128 Set(real x, real y, real z, real w) {
		return _mm_setr_ps(x, y, z, w);
	}
#else
	static inline real4 Set(real x) {
		return real4(x);
	}
	static inline real4 Set(real x, real y, real z, real w) {
...
public:
	...
	inline quaternion(real a) :
			array { a, a, a, a } {
	}
	inline quaternion(real _w, real _x, real _y, real _z) :
			array { _w, _x, _y, _z } {
	}
	inline quaternion(const real3& v, real w) :
			array { v.w, v.x, v.y, v.z } {
	}
	inline real operator[](unsigned int i) const {
		return array[i];
	}
	...
	inline real3 vect() const {
		return real3(x, y, z);
	}

#if defined(USE_AVX)
	inline quaternion(__m256d m) {
		_mm256_storeu_pd(&w, m);
	}
	inline operator __m256d() const {
		return _mm256_loadu_pd(&w);
	}
	...
	static inline __m256d Set(real w,real x, real y, real z) {
		return _mm256_setr_pd(w, x, y, z);
	}
#elif defined(USE_SSE)
	...
	static inline __m128 Set(real w, real x, real y, real z) {
		return _mm_setr_ps(w, x, y, z);
	}
#else
	static inline quaternion Set(real x) {
		return quaternion(x);
	}
	static inline quaternion Set(real w, real x, real y, real z) {
...
static inline real3 AbsRotate(const quaternion& q, const real3& v) {
	...
	result[0] = Abs((e0e0 + e1e1) * real(2.0) - real(1.0)) * v[0]
			+ Abs((e1e2 - e0e3) * real(2.0)) * v[1]
			+ Abs((e1e3 + e0e2) * real(2.0)) * v[2];
	result[1] = Abs((e1e2 + e0e3) * real(2.0)) * v[0]
			+ Abs((e0e0 + e2e2) * real(2.0) - real(1.0)) * v[1]
			+ Abs((e2e3 - e0e1) * real(2.0)) * v[2];
	result[2] = Abs((e1e3 - e0e2) * real(2.0)) * v[0]
			+ Abs((e2e3 + e0e1) * real(2.0)) * v[1]
