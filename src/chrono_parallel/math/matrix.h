...
CUDA_HOST_DEVICE Mat33 MultTranspose(const Mat33& M, const Mat33& N);  // M * N^T
CUDA_HOST_DEVICE Mat33 TransposeMult(const Mat33& M, const Mat33& N);  // M^T * N
CUDA_HOST_DEVICE real Trace(const Mat33& m);
CUDA_HOST_DEVICE Mat33 OuterProduct(const real3& a, const real3& b);  // Multiply a 3x1 by a 1x3 to get a 3x3
...
CUDA_HOST_DEVICE real Norm(const Mat33& A);  // double dot product of a matrix
