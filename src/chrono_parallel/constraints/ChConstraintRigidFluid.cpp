...
#pragma omp paralle for
    for (int index = 0; index < num_rigid_fluid_contacts; index++) {
        ...
        SetRow3(D_T, num_unilaterals + num_bilaterals + index * 3 + 0, num_rigid_bodies * 6 + num_shafts + fluid * 3,
                U);
        SetRow3(D_T, num_unilaterals + num_bilaterals + index * 3 + 1, num_rigid_bodies * 6 + num_shafts + fluid * 3,
                V);
        SetRow3(D_T, num_unilaterals + num_bilaterals + index * 3 + 2, num_rigid_bodies * 6 + num_shafts + fluid * 3,
                W);
    }
