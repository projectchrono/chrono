...
void ChConstraintRigidFluid::Project(real* gamma) {
    //    custom_vector<int>& bids = data_manager->host_data.bids_rigid_fluid;
...
void ChConstraintRigidFluid::Build_D() {
    ...
    custom_vector<int>& bids = data_manager->host_data.bids_rigid_fluid;
    custom_vector<uint8_t>& contact_counts = data_manager->host_data.c_counts_rigid_fluid;
    ...
    int index = 0;
    // cannot parallelize until we store the contact number...
    //#pragma omp parallel for
    for (int fluid = 0; fluid < num_fluid_bodies; fluid++) {
        for (int i = 0; i < contact_counts[fluid]; i++) {
            int rigid = bids[fluid * max_rigid_neighbors + i];
            int off = num_unilaterals + num_bilaterals;

            real3 U = norm[fluid * max_rigid_neighbors + i], V, W;
            Orthogonalize(U, V, W);
            real3 T1, T2, T3;
            Compute_Jacobian(rot_rigid[rigid], U, V, W, cpta[fluid * max_rigid_neighbors + i] - pos_rigid[rigid], T1,
                             T2, T3);
...
void ChConstraintRigidFluid::Build_b() {
    ...
    for (int p = 0; p < num_fluid_bodies; p++) {
        for (int i = 0; i < contact_counts[p]; i++) {
            ...
            real bi = std::max(real(1.0) / step_size * depth, -data_manager->settings.fluid.contact_recovery_speed);
...
void ChConstraintRigidFluid::GenerateSparsity() {
    ...
    for (int p = 0; p < num_fluid_bodies; p++) {
        for (int i = 0; i < contact_counts[p]; i++) {
            int rigid = bids[p * max_rigid_neighbors + i];
            int fluid = p;
            int off = num_unilaterals + num_bilaterals;
