...
bool Cone_generalized_rf(real& gamma_n, real& gamma_u, real& gamma_v,
		const real& mu) {
...
void ChConstraintRigidFluid::Project(real* gamma) {
	...
	for (int p = 0; p < num_fluid_bodies; p++) {
		for (int i = 0; i < contact_counts[p]; ++i) {
			...
			real cohesion = Max(
					(data_manager->host_data.cohesion_data[rigid] + coh) * .5,
					0.0);
			real friction =
					(rigid_fric == 0 || mu == 0) ? 0 : (rigid_fric + mu) * .5;

			real3 gam;
			gam.x = gamma[num_unilaterals + num_bilaterals + index];
			gam.y = gamma[num_unilaterals + num_bilaterals
					+ num_rigid_fluid_contacts + index * 2 + 0];
			gam.z = gamma[num_unilaterals + num_bilaterals
					+ num_rigid_fluid_contacts + index * 2 + 1];
			...
			if (mu == 0) {
				...
				gamma[num_unilaterals + num_bilaterals
						+ num_rigid_fluid_contacts + index * 2 + 0] = gam.y;
				gamma[num_unilaterals + num_bilaterals
						+ num_rigid_fluid_contacts + index * 2 + 1] = gam.z;
			...
			gamma[num_unilaterals + num_bilaterals + num_rigid_fluid_contacts
					+ index * 2 + 0] = gam.y;
			gamma[num_unilaterals + num_bilaterals + num_rigid_fluid_contacts
					+ index * 2 + 1] = gam.z;
			index++;
	...
	for (int p = 0; p < num_fluid_bodies; p++) {
		for (int i = 0; i < contact_counts[p]; i++) {
			...
			Compute_Jacobian(rot_rigid[rigid], U, V, W,
					cpta[p * max_rigid_neighbors + i] - pos_rigid[rigid], T1,
					T2, T3);
			...
			SetRow3Check(D_rfn_T, index + 0,
					num_rigid_bodies * 6 + num_shafts + fluid * 3, U);
			SetRow3Check(D_rft_T, index * 2 + 0,
					num_rigid_bodies * 6 + num_shafts + fluid * 3, V);
			SetRow3Check(D_rft_T, index * 2 + 1,
...
void ChConstraintRigidFluid::Build_b() {
	...
	for (int p = 0; p < num_fluid_bodies; p++) {
		for (int i = 0; i < contact_counts[p]; i++) {
			real bi = 0;
			real depth = data_manager->host_data.dpth_rigid_fluid[p
					* max_rigid_neighbors + i];

			bi = std::max(real(1.0) / step_size * depth,
					-data_manager->settings.fluid.contact_recovery_speed);
			//
			data_manager->host_data.b[num_unilaterals + num_bilaterals + index
					+ 0] = bi;
			data_manager->host_data.b[num_unilaterals + num_bilaterals
					+ num_rigid_fluid_contacts + index * 2 + 0] = 0;
			data_manager->host_data.b[num_unilaterals + num_bilaterals
					+ num_rigid_fluid_contacts + index * 2 + 1] = 0;
			index++;
		}
	}
}
...
#pragma omp parallel for
	for (int index = 0; index < num_rigid_fluid_contacts; index++) {
		...
		real h = data_manager->settings.fluid.kernel_radius;
		real compliance = 4.0 / (step_size * step_size)
				* (epsilon / (1.0 + 4.0 * tau / h));
...
void ChConstraintRigidFluid::GenerateSparsity() {
	...
	for (int p = 0; p < num_fluid_bodies; p++) {
		for (int i = 0; i < contact_counts[p]; i++) {
			...
			AppendRow6(D_T, off + num_rigid_fluid_contacts + index_t * 2 + 0,
					rigid * 6, 0);
			AppendRow3(D_T, off + num_rigid_fluid_contacts + index_t * 2 + 0,
					num_rigid_bodies * 6 + num_shafts + fluid * 3, 0);
			D_T.finalize(off + num_rigid_fluid_contacts + index_t * 2 + 0);

			AppendRow6(D_T, off + num_rigid_fluid_contacts + index_t * 2 + 1,
					rigid * 6, 0);
