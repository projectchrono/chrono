...
void ChConstraintFluidFluid::DetermineNeighbors() {
    ...
    host_vector<long long> temp_pairs(num_fluid_contacts);
    {
        int counter = 0;
        for (int p = 0; p < num_fluid_bodies; p++) {
            for (int i = 0; i < contact_counts[p]; i++) {
                ...
                int t_p = data_manager->host_data.particle_indices_fluid[p];
                int t_q = data_manager->host_data.particle_indices_fluid[q];

                if (t_p > t_q) {
                    tmp = ((long long)t_p << 32 | (long long)t_q);
                } else {
                    tmp = ((long long)t_q << 32 | (long long)t_p);
                }
                temp_pairs[counter] = tmp;
                counter++;
            }
        }
    }
    Thrust_Sort(temp_pairs);
    ...
    //            const int q = neighbor_fluid_fluid[p * max_neighbors + i];
    //
    //            fluid_contact_idA[counter + num_fluid_bodies] = p;
    //            fluid_contact_idA[counter + num_fluid_bodies + num_fluid_contacts] = q;
    //
    //            // fluid_contact_idB[counter + num_fluid_bodies] = q;
    //            // fluid_contact_idB[counter + num_fluid_bodies + num_fluid_contacts] = p;
    //            counter++;
    //        }
    //    }
