...
#pragma omp parallel for
	for (int body_a = 0; body_a < num_fluid_bodies; body_a++) {
		...
		real3 pos_p = sorted_pos[body_a];
		for (int i = 0;
				i < data_manager->host_data.c_counts_fluid_fluid[body_a]; i++) {
			int body_b = data_manager->host_data.neighbor_fluid_fluid[body_a
					* max_neighbors + i];
			if (body_a == body_b) {
...
void ChConstraintFluidFluid::Build_b() {
	...
	SubVectorType b_sub = blaze::subvector(data_manager->host_data.b,
			index_offset, num_fluid_bodies);
	custom_vector<real>& density = data_manager->host_data.den_fluid;
	DynamicVector<real> g(num_fluid_bodies);
	SubVectorType v_sub = blaze::subvector(data_manager->host_data.v,
			body_offset, num_fluid_bodies * 3);
	CompressedMatrix<real>& D_T = data_manager->host_data.D_T;
	SubMatrixType D_T_sub = submatrix(D_T, index_offset, body_offset,
			num_fluid_bodies, num_fluid_bodies * 3);
...
void ChConstraintFluidFluid::GenerateSparsity() {
	...
	if (data_manager->settings.fluid.enable_viscosity) {
		for (int body_a = 0; body_a < num_fluid_bodies; body_a++) {
			for (int i = 0;
					i < data_manager->host_data.c_counts_fluid_fluid[body_a];
					i++) {
				int body_b = data_manager->host_data.neighbor_fluid_fluid[body_a
						* max_neighbors + i];
				AppendRow3(D_T,
						index_offset + num_fluid_bodies + body_a * 3 + 0,
			...
			for (int i = 0;
				...
				int body_b = data_manager->host_data.neighbor_fluid_fluid[body_a
						* max_neighbors + i];
				AppendRow3(D_T,
						index_offset + num_fluid_bodies + body_a * 3 + 1,
						body_offset + body_b * 3, 0);
			}
			...
			for (int i = 0;
				...
				int body_b = data_manager->host_data.neighbor_fluid_fluid[body_a
						* max_neighbors + i];
				AppendRow3(D_T,
						index_offset + num_fluid_bodies + body_a * 3 + 2,
						body_offset + body_b * 3, 0);
			}
...
void ChConstraintFluidFluid::Dx(const DynamicVector<real>& x,
		DynamicVector<real>& output) {
...
void ChConstraintFluidFluid::D_Tx(const DynamicVector<real>& x,
		DynamicVector<real>& output) {
	custom_vector<int>& neighbor_fluid_fluid = data_manager->host_data.neighbor_fluid_fluid;
	custom_vector<int>& c_counts_fluid_fluid = data_manager->host_data.c_counts_fluid_fluid;
	custom_vector<int>& particle_indices_fluid = data_manager->host_data.particle_indices_fluid;
