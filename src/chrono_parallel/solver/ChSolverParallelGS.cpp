...
uint ChSolverParallelGS::Solve(ChShurProduct& ShurProduct,
	...
	for (int index = 0; index < data_manager->num_rigid_contacts; index++) {
		D[index] = Nshur(index, index) + Nshur(num_contacts + index * 2 + 0, num_contacts + index * 2 + 0) + Nshur(num_contacts + index * 2 + 1, num_contacts + index * 2 + 1);
		D[index] = 3.0 / D[index];
	...
	if (data_manager->num_fluid_bodies) {
		...
		for (size_t i = 0; i < data_manager->num_rigid_fluid_contacts; i++) {
			if (data_manager->node_container->contact_mu == 0) {
				...
				D[offset + i] = Nshur(offset + i, offset + i) + Nshur(offset + i * 2 + 0, offset + i * 2 + 0) + Nshur(offset + i * 2 + 1, offset + i * 2 + 1);
				D[offset + i] = 3.0 / D[offset + i];
	...
	for (current_iteration = 0; current_iteration < max_iter; current_iteration++) {
		...
		for (int i = 0; i < data_manager->num_rigid_contacts; i++) {
			ml[offset + i] = ml[offset + i] - omega * D[offset + i] * ((row(Nshur, offset + i * 1 + 0), blaze::subvector(ml, 0, 1 * data_manager->num_constraints)) - r[offset + i]);
			ml[nc + i * 2 + 0] = ml[nc + i * 2 + 0] - omega * D[nc + i * 2 + 0] * ((row(Nshur, nc + i * 2 + 0), blaze::subvector(ml, 0, 1 * data_manager->num_constraints)) - r[nc + i * 2 + 0]);
			ml[nc + i * 2 + 1] = ml[nc + i * 2 + 1] - omega * D[nc + i * 2 + 1] * ((row(Nshur, nc + i * 2 + 1), blaze::subvector(ml, 0, 1 * data_manager->num_constraints)) - r[nc + i * 2 + 1]);
		...
		if (data_manager->num_fluid_bodies) {
			...
			for (size_t i = 0; i < data_manager->num_rigid_fluid_contacts; i++) {
				ml[offset + i] = ml[offset + i] - omega * D[offset + i] * ((row(Nshur, offset + i * 1 + 0), blaze::subvector(ml, 0, 1 * data_manager->num_constraints)) - r[offset + i]);
				if (data_manager->node_container->contact_mu != 0) {
					ml[offset + nfc + i * 2 + 0] = ml[offset + nfc + i * 2 + 0] - omega * D[offset + nfc + i * 2 + 0] * ((row(Nshur, offset + nfc + i * 2 + 0), blaze::subvector(ml, 0, 1 * data_manager->num_constraints)) - r[offset + nfc + i * 2 + 0]);
					ml[offset + nfc + i * 2 + 1] = ml[offset + nfc + i * 2 + 1] - omega * D[offset + nfc + i * 2 + 1] * ((row(Nshur, offset + nfc + i * 2 + 1), blaze::subvector(ml, 0, 1 * data_manager->num_constraints)) - r[offset + nfc + i * 2 + 1]);
				}
