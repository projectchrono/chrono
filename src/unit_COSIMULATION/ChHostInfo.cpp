	...
	try 
	{
		if (type == NAME)
		{
			...
			if (hostPtr == NULL)
			{
				...
				#ifdef UNIX
					ChExceptionSocket* gethostbynameException = new ChExceptionSocket(0,"unix: error getting host by name");
					throw gethostbynameException;
				#endif
			}
        }
		...
		{
			ChExceptionSocket* unknownTypeException = new ChExceptionSocket(0,"unknown host type: host name/address has to be given ");
