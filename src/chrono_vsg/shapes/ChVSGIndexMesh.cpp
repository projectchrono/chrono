...
void ChVSGIndexMesh::genPhongSubgraph(vsg::ref_ptr<vsg::StateGroup> subgraph) {
    ...
    m_lightPosition[0] = 100.0;
    m_lightPosition[1] = 100.0;
    ...
    auto uniformValue1 = vsg::vec3Value::create(m_lightPosition);
    auto uniform1 = vsg::DescriptorBuffer::create(uniformValue1);

    auto uniformValue2 = vsg::vec3Value::create(vsg::vec3(1.0, 1.0, 1.0));
    auto uniform2 = vsg::DescriptorBuffer::create(uniformValue2);

    auto descriptorSet = vsg::DescriptorSet::create(descriptorSetLayout, vsg::Descriptors{uniform1, uniform2});
    auto bindDescriptorSets = vsg::BindDescriptorSets::create(VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline->layout,
                                                              0, vsg::DescriptorSets{descriptorSet});
    ...
    // Descriptors to decorate the whole graph auto scenegraph = vsg::StateGroup::create();
    subgraph->add(bindGraphicsPipeline);
    // subgraph->add(bindDescriptorSets); no effect, uniform values are nor propagated
    ...
    drawCommands->addChild(vsg::BindVertexBuffers::create(
        0, vsg::DataList{m_vertices, m_normals, m_ambientColors, m_diffuseColors, m_specularColors, m_shininess}));
    drawCommands->addChild(vsg::BindIndexBuffer::create(m_indices));
    drawCommands->addChild(vsg::DrawIndexed::create(m_indices->size(), 1, 0, 0, 0));
