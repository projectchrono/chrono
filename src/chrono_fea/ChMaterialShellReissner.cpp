...
void ChMaterialShellReissnerPreIntegrated::ComputeTangentC(ChMatrix<>& mC, 
...
void ChMaterialShellReissnerIntegrable::ComputeTangentC(ChMatrix<>& mC, 
                               const ChVector<>& eps_u, 
                               const ChVector<>& eps_v,
                               const ChVector<>& kur_u, 
                               const ChVector<>& kur_v, 
...
ChMaterialShellReissnerIsothropic::ChMaterialShellReissnerIsothropic(
...
void ChMaterialShellReissnerIsothropic::ComputeTangentC(ChMatrix<>& mC, 
    ...
    if (z_inf == - z_sup) {
        ...
        mC(0,0) = h1 * Q11;   mC(0,4) = h1 * Q12;   mC(0,7) = h2 * Q11;   mC(0,9) = h2 * Q12;
        mC(1,1) = h1 * Q33;   mC(1,6) = h2 * Q33;
        mC(2,2) = h1 * Qss; 
        mC(3,3) = h1 * Q44;   mC(3,10)= h2 * Q44;
        mC(4,0) = h1 * Q12;   mC(4,4) = h1 * Q22;   mC(4,7) = h2 * Q12;   mC(4,9) = h2 * Q22;
        mC(5,5) = h1 * Qss; 
        mC(6,1) = h2 * Q33;   mC(6,6) = h3 * Q33; 
        mC(7,0) = h2 * Q11;   mC(7,4) = h2 * Q12;   mC(7,7) = h3 * Q11;   mC(7,9) = h3 * Q12;
        mC(8,8) = h3 * Qdd;
        mC(9,0) = h2 * Q12;   mC(9,4) = h2 * Q22;   mC(9,7) = h3 * Q12;   mC(9,9) = h3 * Q22;
        mC(10,3)= h2 * Q44;   mC(10,10)=h3 * Q44; 
        mC(11,11)=h3 * Qdd;
    } 
}
...
void ChMaterialShellReissnerOrthotropic::ComputeTangentC(ChMatrix<>& mC, 
    ...
    Tm(0,0) = CC;  Tm(0,1) = SS;   Tm(0,2) = SC;   Tm(0,3) = SC;
    Tm(1,0) = SS;  Tm(1,1) = CC;   Tm(1,2) =-SC;   Tm(1,3) =-SC;
...
//GetLog() << "Qm=" << Qm << "\n";
    ...
    // upper left part
    double h=z_sup-z_inf;
    mC(0,0)=h*Qm(0,0);  mC(0,1)=h*Qm(0,2);      mC(0,3)=h*Qm(0,3);  mC(0,4)=h*Qm(0,1); 
    mC(1,0)=h*Qm(2,0);  mC(1,1)=h*Qm(2,2);      mC(1,3)=h*Qm(2,3);  mC(1,4)=h*Qm(2,1); 
    mC(3,0)=h*Qm(3,0);  mC(3,1)=h*Qm(3,2);      mC(3,3)=h*Qm(3,3);  mC(3,4)=h*Qm(3,1); 
    mC(4,0)=h*Qm(1,0);  mC(4,1)=h*Qm(1,2);      mC(4,3)=h*Qm(1,3);  mC(4,4)=h*Qm(1,1); 
    mC(2,2)=h*alpha*Qs(0,0);  mC(2,5)=h*alpha*Qs(0,1);
    mC(5,2)=h*alpha*Qs(1,0);  mC(5,5)=h*alpha*Qs(1,1);
    // lower right part
    double H=(1./3.)*(pow(z_sup,3)-pow(z_inf,3));
    mC(6, 6)=H*Qm(2,2);  mC(6,7)=H*Qm(2,0);      mC(6, 9)=H*Qm(2,1);  mC(6, 10)=H*Qm(2,3); 
    mC(7, 6)=H*Qm(0,2);  mC(7,7)=H*Qm(0,0);      mC(7, 9)=H*Qm(0,1);  mC(7, 10)=H*Qm(0,3); 
    mC(9, 6)=H*Qm(1,2);  mC(9,7)=H*Qm(1,0);      mC(9, 9)=H*Qm(1,1);  mC(9, 10)=H*Qm(1,3); 
    mC(10,6)=H*Qm(3,2); mC(10,7)=H*Qm(3,0);      mC(10,9)=H*Qm(3,1);  mC(10,10)=H*Qm(3,3); 
    mC(8, 8)=H*beta*Qs(0,0);  mC(8, 11)=H*beta*Qs(0,1);
    mC(11,8)=H*beta*Qs(1,0);  mC(11,11)=H*beta*Qs(1,1);
    // upper right part and lower right part
    double hh=(0.5)*(pow(z_sup,2)-pow(z_inf,2));
    mC(0,6)=hh*Qm(0,2);  mC(0,7)=hh*Qm(0,0);      mC(0,9)=hh*Qm(0,1);  mC(0,10)=hh*Qm(0,3); 
    mC(1,6)=hh*Qm(2,2);  mC(1,7)=hh*Qm(2,0);      mC(1,9)=hh*Qm(2,1);  mC(1,10)=hh*Qm(2,3); 
    mC(3,6)=hh*Qm(3,2);  mC(3,7)=hh*Qm(3,0);      mC(3,9)=hh*Qm(3,1);  mC(3,10)=hh*Qm(3,3); 
    mC(4,6)=hh*Qm(1,2);  mC(4,7)=hh*Qm(1,0);      mC(4,9)=hh*Qm(1,1);  mC(4,10)=hh*Qm(1,3);
    mC(6, 0)=hh*Qm(2,0);  mC(6, 1)=hh*Qm(2,2);      mC(6, 3)=hh*Qm(2,3);  mC(6, 4)=hh*Qm(2,1); 
    mC(7, 0)=hh*Qm(0,0);  mC(7, 1)=hh*Qm(0,2);      mC(7, 3)=hh*Qm(0,3);  mC(7, 4)=hh*Qm(0,1); 
    mC(9, 0)=hh*Qm(1,0);  mC(9, 1)=hh*Qm(1,2);      mC(9, 3)=hh*Qm(1,3);  mC(9, 4)=hh*Qm(1,1); 
    mC(10,0)=hh*Qm(3,0);  mC(10,1)=hh*Qm(3,2);      mC(10,3)=hh*Qm(3,3);  mC(10,4)=hh*Qm(3,1);
}
...
ChMaterialShellReissnerPreIntIsothropic::ChMaterialShellReissnerPreIntIsothropic(
...
void ChMaterialShellReissnerPreIntIsothropic::ComputeStress(ChVector<>& n_u, 
                               ChVector<>& n_v,
                               ChVector<>& m_u, 
                               ChVector<>& m_v,
                               const ChVector<>& eps_u, 
...
void ChMaterialShellReissnerPreIntIsothropic::ComputeTangentC(ChMatrix<>& mC, 
                               const ChVector<>& eps_u, 
                               const ChVector<>& eps_v,
                               const ChVector<>& kur_u, 
                               const ChVector<>& kur_v)  {
