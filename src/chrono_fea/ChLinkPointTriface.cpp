...
int ChLinkPointTriface::Initialize(std::shared_ptr<ChNodeFEAxyz> anodeA,
    ...
    this->d = collision::ChCollisionUtils::PointTriangleDistance(
        this->mnodeA->pos, 
        this->mtriangle.mnodeB1->pos,
        this->mtriangle.mnodeB2->pos,
        this->mtriangle.mnodeB3->pos,
        s2, s3, 
        is_into, 
        p_projected);
...
void ChLinkPointTriface::IntLoadConstraint_C(const unsigned int off_L,  // offset in Qc residual
    ...
    ChVector<> N = Vcross(mtriangle.mnodeB2->pos - mtriangle.mnodeB1->pos, 
                          mtriangle.mnodeB3->pos - mtriangle.mnodeB1->pos);
    N.Normalize();
    double s1 = 1 - s2 - s3;

    ChVector<> res = 
        mnodeA->GetPos() 
        - s1 * mtriangle.mnodeB1->pos
        - s2 * mtriangle.mnodeB2->pos
        - s3 * mtriangle.mnodeB3->pos
        - N * d;
...
//***OBSOLETE*** will be removed in favour of IntLoadConstraint_C
...
void ChLinkPointTriface::ConstraintsLoadJacobians() {
    ...
    if (d != 0) {
        ...
        Jxb1(0,0) = -s1 + (d*t17*t26*t35)/2;        Jxb1(0,1) = -t32 - (d*t17*t26*t37)/2;   Jxb1(0,2) = -(d*t17*t26*t40)/2 + d*t27*t28;
        Jxb1(1,0) = -(d*t11*t26*t35)/2 + t32;       Jxb1(1,1) = -s1 + (d*t11*t26*t37)/2;    Jxb1(1,2) =      -t58 + (d*t11*t26*t40)/2;
        Jxb1(2,0) = -d*t27*t28 + (d*t7*t26*t35)/2;  Jxb1(2,1) = -(d*t7*t26*t37)/2 + t58;    Jxb1(2,2) =        -s1 - (d*t7*t26*t40)/2;

        Jxb2(0,0) = -s2 - (d*t17*t26*t43)/2;        Jxb2(0,1) =  d*t8*t28 + (d*t17*t26*t45)/2; Jxb2(0,2) =  -t60 + (d*t17*t26*t48)/2;
        Jxb2(1,0) = -d*t8*t28 + (d*t11*t26*t43)/2,  Jxb2(1,1) = -s2 - (d*t11*t26*t45)/2;       Jxb2(1,2) =  -(d*t11*t26*t48)/2 + d*t6*t28;
        Jxb2(2,0) = -(d*t7*t26*t43)/2 + t60;        Jxb2(2,1) = -d*t6*t28 + (d*t7*t26*t45)/2;  Jxb2(2,2) =  -s2 + (d*t7*t26*t48)/2;

        Jxb3(0,0) = -s3 + (d*t17*t26*t52)/2;         Jxb3(0,1) = -t49 - (d*t17*t26*t54)/2;   Jxb3(0,2) = -(d*t17*t26*t57)/2 + d*t5*t28;
        Jxb3(1,0) = -(d*t11*t26*t52)/2 + t49;        Jxb3(1,1) =  -s3 + (d*t11*t26*t54)/2;   Jxb3(1,2) =      -t62 + (d*t11*t26*t57)/2;
        Jxb3(2,0) = -d*t5*t28 + (d*t7*t26*t52)/2;    Jxb3(2,1) =  -(d*t7*t26*t54)/2 + t62;   Jxb3(2,2) =   -s3 - (d*t7*t26*t57)/2;
    } else {
...
int ChLinkPointTrifaceRot::Initialize(std::shared_ptr<ChNodeFEAxyz> anodeA,
    ...
    this->d = collision::ChCollisionUtils::PointTriangleDistance(
        this->mnodeA->pos, 
        this->mtriangle.mnodeB1->coord.pos,
        this->mtriangle.mnodeB2->coord.pos,
        this->mtriangle.mnodeB3->coord.pos,
        s2, s3, 
        is_into, 
        p_projected);
...
void ChLinkPointTrifaceRot::IntLoadConstraint_C(const unsigned int off_L,  // offset in Qc residual
    ...
    ChVector<> res = 
        mnodeA->GetPos() 
        - s1 * mtriangle.mnodeB1->coord.pos
        - s2 * mtriangle.mnodeB2->coord.pos
        - s3 * mtriangle.mnodeB3->coord.pos
        - N * d;
