...
class ChElementShellANCF : public ChElementShell {
  ...
  public:
    ...
    virtual void SetNodes(ChSharedPtr<ChNodeFEAxyzD> nodeA,
        ...
        initialposD(0, 0) = pA(0);
        initialposD(1, 0) = pA(1);
        initialposD(2, 0) = pA(2);
        initialposD(3, 0) = dA(0);
        initialposD(4, 0) = dA(1);
        initialposD(5, 0) = dA(2);
        initialposD(6, 0) = pB(0);
        initialposD(7, 0) = pB(1);
        initialposD(8, 0) = pB(2);
        initialposD(9, 0) = dB(0);
        initialposD(10, 0) = dB(1);
        initialposD(11, 0) = dB(2);
        initialposD(12, 0) = pC(0);
        initialposD(13, 0) = pC(1);
        initialposD(14, 0) = pC(2);
        initialposD(15, 0) = dC(0);
        initialposD(16, 0) = dC(1);
        initialposD(17, 0) = dC(2);
        initialposD(18, 0) = pD(0);
        initialposD(19, 0) = pD(1);
        initialposD(20, 0) = pD(2);
        initialposD(21, 0) = dD(0);
        initialposD(22, 0) = dD(1);
        initialposD(23, 0) = dD(2);
    }
            ...
            class MyMass : public ChIntegrable3D<ChMatrixNM<double, 24, 24> > {
              public:
                ...
                virtual void Evaluate(ChMatrixNM<double, 24, 24>& result,
                    ...
                    result *= detJ0 * (element->GetLengthX() / 2) * (element->GetLengthY() / 2) *
                              (element->GetThickness() / 2);
                }
            };
    ...
    virtual void ComputeInternalForces(ChMatrixDynamic<>& Fi) {
        ...
        for (int kl = 0; kl < NumLayerPerElem; kl++) {
            ...
            double DELTA = 1.0 - (CCOM[3] * CCOM[3]) * CCOM[1] / CCOM[0] - (CCOM[4] * CCOM[4]) * CCOM[2] / CCOM[0] -
                           (CCOM[5] * CCOM[5]) * CCOM[2] / CCOM[1] -
                           2.0 * CCOM[3] * CCOM[4] * CCOM[5] * CCOM[2] / CCOM[0];
                ...
                class MyForce : public ChIntegrable3D<ChMatrixNM<double, 750, 1> > {
                  public:
                    ...
                    virtual void Evaluate(ChMatrixNM<double, 750, 1>& result,
                        ...
                        JAC11 =
                            (temp246 * strainD * (1.0 + DampCoefficient * (element->GetAlphaDamp()))) + temp249 * Gd;
                        JAC11 *= detJ0 * (element->GetLengthX() / 2.0) * (element->GetLengthY() / 2.0) *
                                 (element->GetThickness() / 2.0);
                        /// Internal force calculation ///
                        tempC.MatrTMultiply(strainD, *E_eps);
                        Fint.MatrMultiply(tempC, strain);
                        Fint *= detJ0 * (element->GetLengthX() / 2) * (element->GetLengthY() / 2) *
                                (element->GetThickness() / 2);
    ...
    virtual void AssumedNaturalStrain_BilinearShell(ChMatrixNM<double, 8, 3>& d,
                                                    ChMatrixNM<double, 8, 3>& d0,
                                                    ChMatrixNM<double, 8, 1>& strain_ans,
                                                    ChMatrixNM<double, 8, 24>& strainD_ans) {
    ...
    virtual void T0DetJElementCenterForEAS(ChMatrixNM<double, 8, 3>& d0,
                                           ChMatrixNM<double, 6, 6>& T0,
                                           double& detJ0C,
                                           double& theta) {
        double x = 0;
    ...
    // virtual void LUBKSB55(ChMatrixNM<double,5,5>&A,double N,double NP,ChMatrixNM<int,5,1>&
    // INDX,ChMatrixNM<double,5,1>& B){
    //		int II=0;
    //		for (int I=0;I<N;I++){
    //		  int LL=INDX(I);
    //		  double SUM=B(LL);
    ...
    //		}
    //}
    virtual void LUBKSB55(ChMatrixNM<double, 5, 5>& A,
                          int N,
                          int NP,
                          ChMatrixNM<int, 5, 1>& INDX,
    ...
    virtual void EvaluateSectionPoint(const double u, const double v, const ChMatrix<>& displ, ChVector<>& point) {
        ChVector<> u_displ;
