...
ChExtruderBeamIGA::ChExtruderBeamIGA(
 ...
 GetLog()<< "Create node n."<< beam_nodes.size() << " at x=" << nodeA->GetPos().x() << " x0=" << nodeA->GetX0().GetPos().x() <<  "\n";
...
void ChExtruderBeamIGA::SetContact(std::shared_ptr<ChMaterialSurfaceSMC> mcontact_material,
                                             double mcontact_radius) {
    this->contact_material = mcontact_material;
    this->contact_radius = mcontact_radius;
    this->contactcloud = std::make_shared<ChContactSurfaceNodeCloud>();
