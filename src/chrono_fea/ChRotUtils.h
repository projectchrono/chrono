...
void
RotCo(const int cid, const T1 &phi, const ChVector<> &p, T2 *const cf)
{	
	T2 phip[10];
	T2 phi2(phi^phi);
...
// Coefficients:            up to d     (COEFF_D)
template<class T1, class T2>
void CoeffCStar(const T1 &phi, const ChVector<> &p,
...
// Coefficients:            up to f     (COEFF_F)
template<class T1, class T2>
void CoeffEStar(const T1 &phi, const ChVector<> &p,
			T2 *const coeff, T2 *const coeffs) {
    RotCo(COEFF_F,phi,p,coeff);
    coeffs[0]=-coeff[3]/(coeff[1]*2.);
...
ChMatrix33<> Rot(const ChVector<> & phi){
    ...
    ChMatrix33<> Eye(1);
    ChMatrix33<> Phix; Phix.Set_X_matrix(phi*coeff[0]);
	ChMatrix33<> Phi(Eye + Phix);		/* I + c[0] * phi x */
    ChMatrix33<> pxpx; pxpx.Set_XY_matrix(phi, phi*coeff[1]);
...
static
 ChMatrix33<> DRot(const ChVector<> & phi){
    ...
    ChMatrix33<> Eye(1);
    ChMatrix33<> Phix; Phix.Set_X_matrix(phi*coeff[1]);
...
ChMatrix33<> DRot_IT(const ChVector<> & phi){
    ...
    ChMatrix33<> Eye(1);
    ChMatrix33<> Phix; Phix.Set_X_matrix(phi*0.5);
	ChMatrix33<> GaIT(Eye + Phix);		/* I +  0.5 phi x */
    ChMatrix33<> pxpx; pxpx.Set_XY_matrix(phi, phi*coeffs[0]);
    GaIT += pxpx;
...
void RotAndDRot_IT(const ChVector<> & phi, ChMatrix33<> & PhiIT, ChMatrix33<> & GaIT){
    ...
    ChMatrix33<> Phix; Phix.Set_X_matrix(phi*coeff[0]);
	PhiIT = Eye + Phix;
    ChMatrix33<> pxpx; pxpx.Set_XY_matrix(phi, phi*coeffs[1]);
	PhiIT += pxpx;
...
ChVector<> VecRot(const ChMatrix33<> & Phi){
	...
	if (cosphi > 0.) {
        ...
        ChMatrix33<> unitx; unitx.Set_X_matrix(unit);
...
ChMatrix33<> Elle(const ChVector<> & phi, const ChVector<> & a){
    ...
    ChMatrix33<> L; L.Set_X_matrix(a*(-coeff[1]));
    ChMatrix33<> mx;
