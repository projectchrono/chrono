...
ChMaterialShellEANSnew::ChMaterialShellEANSnew(
...
void ChMaterialShellEANSnew::ComputeStress(ChVector<>& n_u, 
                               ChVector<>& n_v,
                               ChVector<>& m_u, 
                               ChVector<>& m_v,
...
void ChMaterialShellEANSnew::ComputeTangentC(ChMatrix<>& mC, 
                               const ChVector<>& eps_u, 
                               const ChVector<>& eps_v,
                               const ChVector<>& kur_u, 
                               const ChVector<>& kur_v)  {
...
//--------------------------------------------------------------
// ChElementShellEANS4new 
//--------------------------------------------------------------


// Static integration tables
 
double ChElementShellEANS4new::xi_i[ChElementShellEANS4new::NUMIP][2] = {
...
double ChElementShellEANS4new::xi_n[ChElementShellEANS4new::NUMNODES][2] = {
	{ 1.,  1.},
	{-1.,  1.},
	{-1., -1.},
	{ 1., -1.}
...
double ChElementShellEANS4new::xi_0[2] = {0., 0.};

void ChElementShellEANS4new::UpdateNodalAndAveragePosAndOrientation()
{
	ChMatrix33<> T_avg;
...
void ChElementShellEANS4new::ComputeInitialNodeOrientation()
{
	for (int i = 0; i < NUMNODES; i++) {
		xa[i] = this->m_nodes[i]->GetPos();
	}
...
void ChElementShellEANS4new::InterpolateOrientation()
{
	ChMatrix33<> DRot_I_phi_tilde_n_MT_T_overline[NUMNODES];
	ChMatrix33<> Ri, Gammai;
	for (int n = 0; n < NUMNODES; n++) {
		DRot_I_phi_tilde_n_MT_T_overline[n].MatrMultiplyT(ChRotUtils::DRot_I(phi_tilde_n[n]),T_overline); //=ChRotUtils::DRot_I(phi_tilde_n[n]).MulMT(T_overline);
	}
	for (int i = 0; i < NUMIP; i++) {
...
void ChElementShellEANS4new::ComputeIPCurvature()
{
	ChMatrix33<> Gamma_I_n_MT_T_overline[NUMNODES];
	for (int n = 0; n < NUMNODES; n++) {
		Gamma_I_n_MT_T_overline[n].MatrMultiplyT(ChRotUtils::DRot_I(phi_tilde_n[n]), T_overline);//= ChRotUtils::DRot_I(phi_tilde_n[n]).MulMT(T_overline);
	}
	for (int i = 0; i < NUMIP; i++) {
		ChVector<> phi_tilde_1_i;
		ChVector<> phi_tilde_2_i;
		InterpDeriv(phi_tilde_n, L_alpha_beta_i[i], phi_tilde_1_i, phi_tilde_2_i);
		ChMatrix33<> T_overlineGamma_tilde_i(T_overline * ChRotUtils::DRot(phi_tilde_i[i]));
...
ChElementShellEANS4new::ChElementShellEANS4new() :  m_numLayers(0), m_thickness(0) {
    m_nodes.resize(4);
    m_Alpha = 0;
...
void ChElementShellEANS4new::SetNodes(std::shared_ptr<ChNodeFEAxyzrot> nodeA,
...
ChVector<> ChElementShellEANS4new::EvaluateGP(int igp) {
        return 
...
ChVector<> ChElementShellEANS4new::EvaluatePT(int ipt) {
        return 
...
void ChElementShellEANS4new::AddLayer(double thickness, double theta, std::shared_ptr<ChMaterialShellEANSnew> material) {
...
void ChElementShellEANS4new::SetAsNeutral() {
...
// Initial element setup.
void ChElementShellEANS4new::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementShellEANS4new::Update() {
...
void ChElementShellEANS4new::GetStateBlock(ChMatrixDynamic<>& mD) {
    mD.Reset(4*7, 1);
...
void ChElementShellEANS4new::ComputeKRMmatricesGlobal(ChMatrix<>& H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementShellEANS4new::ComputeMmatrixGlobal(ChMatrix<>& M) {
    M = m_MassMatrix;
}
...
void ChElementShellEANS4new::ComputeMassMatrix() {
    m_MassMatrix.Reset();
...
void ChElementShellEANS4new::ComputeInternalForces(ChMatrixDynamic<>& Fi) {
...
void ChElementShellEANS4new::ComputeInternalJacobians(double Kfactor, double Rfactor) {
...
void ChElementShellEANS4new::ShapeFunctions(ChMatrix<>& N, double x, double y, double z) {
    double xi[2];
    xi[0]=x, xi[1]=y;
...
void ChElementShellEANS4new::ShapeFunctionsDerivativeX(ChMatrix<>& Nx, double x, double y, double z) {
    double xi[2];
    xi[0]=x, xi[1]=y;
    Nx(0) = L1_1(xi);
    Nx(1) = L2_1(xi);
...
void ChElementShellEANS4new::ShapeFunctionsDerivativeY(ChMatrix<>& Ny, double x, double y, double z) {
    double xi[2];
    xi[0]=x, xi[1]=y;
    Ny(0) = L1_2(xi);
    Ny(1) = L2_2(xi);
...
void ChElementShellEANS4new::EvaluateSectionDisplacement(const double u,
                                                     const double v,
...
void ChElementShellEANS4new::EvaluateSectionFrame(const double u,
                                              const double v,
                                              const ChMatrix<>& displ,
                                              ChVector<>& point,
                                              ChQuaternion<>& rot) {
...
void ChElementShellEANS4new::EvaluateSectionPoint(const double u,
                                              const double v,
                                              const ChMatrix<>& displ,
                                              ChVector<>& point) {
    ChMatrixNM<double, 1, 4> N;
...
void ChElementShellEANS4new::LoadableGetStateBlock_x(int block_offset, ChVectorDynamic<>& mD) {
...
void ChElementShellEANS4new::LoadableGetStateBlock_w(int block_offset, ChVectorDynamic<>& mD) {
...
void ChElementShellEANS4new::EvaluateSectionVelNorm(double U, double V, ChVector<>& Result) {
...
void ChElementShellEANS4new::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellEANS4new::ComputeNF(
...
double ChElementShellEANS4new::GetDensity() {
...
ChVector<> ChElementShellEANS4new::ComputeNormal(const double U, const double V) {
...
ChElementShellEANS4new::Layer::Layer(ChElementShellEANS4new* element,
                                 double thickness,
                                 double theta,
                                 std::shared_ptr<ChMaterialShellEANSnew> material)
    : m_element(element), m_thickness(thickness), m_theta(theta), m_material(material) {}

// Initial setup for this layer:
void ChElementShellEANS4new::Layer::SetupInitial() {
