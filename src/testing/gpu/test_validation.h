...
//#define USEBULLET 0
#ifdef USEBULLET
#define BODYPTR ChModelBullet*
#define SHAREDPTR ChSharedBodyPtr
#define BODYTYPE ChBody
#else
#define BODYPTR ChCollisionModelGPU *
#define SHAREDPTR ChSharedBodyGPUPtr
#define BODYTYPE ChBodyGPU
#endif
class System{
public:
	...
	void DoTimeStep();
	void PrintStats();
	void MakeSphere		(SHAREDPTR &body,     double radius, double mass,ChVector<> pos,double sfric,double kfric,double restitution,bool collide);
	void MakeBox		(SHAREDPTR &body, ChVector<> radius, double mass,ChVector<> pos,ChQuaternion<> rot,double sfric,double kfric,double restitution,int family,int nocolwith,bool collide, bool fixed);
	void MakeEllipsoid	(SHAREDPTR &body, ChVector<> radius, double mass,ChVector<> pos,ChQuaternion<> rot,double sfric,double kfric,double restitution,bool collide);
...
void drawSphere(ChBody *abody){
	...
	float4 h=F4(0,0,0,1);
#else
	float4 h=((BODYPTR)(abody->GetCollisionModel()))->mData[0].A;
#endif
...
void drawEllipsoid(ChBody *abody){
	...
	glRotatef(angle*180.0/3.1415, axis.x, axis.y, axis.z);
#ifdef USEBULLET
	float4 h=F4(10,.01,10,0);
#else
	float4 h=((BODYPTR)(abody->GetCollisionModel()))->mData[0].B;
#endif
...
void processNormalKeys(unsigned char key, int x, int y) { 	
	if (key=='w'){camera_pos_delta+=dir*.05;}
	if (key=='s'){camera_pos_delta-=dir*.05;}
	if (key=='d'){camera_pos_delta+=cross(dir,camera_up)*.05;}
	if (key=='a'){camera_pos_delta-=cross(dir,camera_up)*.05;}
	if (key=='q'){camera_pos_delta+=camera_up*.05;}
	if (key=='e'){camera_pos_delta-=camera_up*.05;}
