...
void System::DoTimeStep() {
	if (mNumCurrentObjects < mNumObjects && mFrameNumber % 500 == 0) {
		float x = 18, y = 100, z = 30;
		float posX = 0, posY = -15, posZ = 0;
		...
		for (int xx = 0; xx < x; xx++) {
			for (int yy = 0; yy < y; yy++) {
				for (int zz = 0; zz < z; zz++) {
					...
					InitObject(mrigidBody, mass, mParticlePos*.01, quat, mu, mu, rest, true, false, 0, 1);
...
//	SaveByObject(Anchor.get_ptr(), ss.str());
	...
	if (mFrameNumber % (int(1.0 / mTimeStep / 10.0)) == 0) {
