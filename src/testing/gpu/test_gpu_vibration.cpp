...
void System::DoTimeStep(){
	...
	if(mFrameNumber%3==0&&saveData){
		ofstream ofile;
		stringstream ss;
		ss<<"data/vibration"<<mFileNumber<<".txt";
...
int main(int argc, char* argv[]){
	...
	GPUSystem=new System(&SysG,73500,"vibration.txt");
	GPUSystem->mMu=mMu;
...
#pragma omp section
		{
			ofstream output_body ("output_vibration.txt");
			while(true){
				GPUSystem->renderScene();
				ChVector<> pos=BTM->GetPos();
				ChVector<> rot=BTM->GetRot().Q_to_NasaAngles();
				ChVector<> vel=BTM->GetPos_dt();
				ChVector<> acc=BTM->GetPos_dtdt();
				ChVector<> trq=BTM->Get_gyro();
