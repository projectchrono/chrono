...
The GPU ray-cast backend does not reproduce the results of the default CPU path, which casts rays through Bullet. On `demo_ROBOT_Viper_SCM` the wheel sinks about 5.5 cm with this backend against about 4.3 cm through Bullet, consistently across grid resolutions and on both AMD and NVIDIA hardware.

This is a property of the formulation rather than of the GPU. `EnableRaycastGpuReference()`, which runs the same algorithm on the CPU, agrees with the kernels to within 0.09%; the contact-force backend accounts for about 2%, the collision margin for a few millimetres, and kernel precision for micrometres. Expect a systematic offset of this order when switching a model onto this backend, and validate against your own reference before relying on absolute sinkage.
