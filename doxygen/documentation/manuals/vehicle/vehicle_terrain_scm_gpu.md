...
The GPU and CPU ray-cast paths do not produce identical results. On `demo_ROBOT_Viper_SCM` the difference is roughly a centimetre of wheel sinkage, consistent across grid resolutions and reproducing on both AMD and NVIDIA, so expect a systematic offset of that order rather than agreement to solver tolerance. The contact-force backend is not the source: switching it alone changes sinkage by about 2%, switching the ray-cast backend alone reproduces the full difference. Both paths test the same triangles, since Bullet builds a per-triangle collision shape for a `ChTriangleMeshConnected`, and the collision margin is not the source either.
