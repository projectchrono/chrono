...
| `num_proximity_search_steps` | Frequency of neighbor-list rebuild. | Lower value (e.g., `1`) for rapidly changing flow (if you are doing CFD); higher values reduce computational cost but risk stale neighbors. With a solid coupled to the soil, values above 1 are safe only in versions that refresh the solid marker state on the skipped steps (see #823); in one plate probe a value of 8 then brought the bearing pressure back to within about 1 percent of the value at 1, while peak particle speeds still differed. Compare against `1` on your own case before relying on a higher value. | `SPHParameters::num_proximity_search_steps`, `SetNumProximitySearchSteps` |
