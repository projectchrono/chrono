...
int main(int argc, char** argv) {
    ...
    */
    ...
    for (unsigned int i = 0; i < my_mesh->GetNumNodes(); i++) {
        ...
        auto p = node->GetPos();
        if (abs(p.z()) < 0.1 && abs(p.x() - 75) < 2 && abs(p.y() - 25) < 2) {
            RP2 = node;
        }
    }
    ...
    for (unsigned int i = 0; i < my_mesh->GetNumNodes(); i++) {
        auto node = std::dynamic_pointer_cast<ChNodeFEAxyzrot>(my_mesh->GetNode(i));
        auto p = node->GetPos();
        if (abs(p.z() - 150) < 0.1 && abs(p.x() - 75) < 2 && abs(p.y() - 25) < 2) {
            RP1 = node;
        }
    }
    ...
    for (unsigned int i = 0; i < my_mesh->GetNumNodes(); i++) {
        auto node = std::dynamic_pointer_cast<ChNodeFEAxyzrot>(my_mesh->GetNode(i)); 
        auto p=node->GetPos();
        if (abs(p.z() - 150) < 0.001 && p!= RP1->GetPos()){
	...
	*/
	...
	my_motion_function1->SetCoefficients(std::vector<double>{0.0, 0.0, 500.});

    auto my_motion_function2 = chrono_types::make_shared<ChFunctionRamp>(0, 1.0);
