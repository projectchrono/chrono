...
int main(int argc, char** argv) {
    ...
    //std::string LDPM_data_path = "LDPMgeo000Cylinder000/";
    ...
    */
    ...
    for (unsigned int i = 0; i < my_mesh->GetNumNodes(); i++) {
        ...
        auto p = node->GetPos();
        if (abs(p.z()) < 0.1 && abs(p.x()) < 1 && abs(p.y()) < 1) {
            RP2 = node;
        }
    }
    ...
    for (unsigned int i = 0; i < my_mesh->GetNumNodes(); i++) {
        auto node = std::dynamic_pointer_cast<ChNodeFEAxyzrot>(my_mesh->GetNode(i));
        auto p = node->GetPos();
        if (abs(p.z() - 200) < 0.1 && abs(p.x()) < 1 && abs(p.y()) < 1) {
            RP1 = node;
        }
    }
    ...
    //auto constr_sur = chrono_types::make_shared<ChLinkMateGeneric>(true, true, false, false, false, false);
    //constr_sur->Initialize(sur_nodes, mtruss, sur_nodes->Frame());
    //sys.Add(constr_sur);
    ...
    */   
    ...
    vis->AddCamera(ChVector3d(60.0, 60.0, 100.));
	...
	*/
    ...
    auto my_motion_function2 = chrono_types::make_shared<ChFunctionRamp>(0, -50.0);
	...
	*/
    ...
    //while (vis->Run() & sys.GetChTime() <= 0.1) {
